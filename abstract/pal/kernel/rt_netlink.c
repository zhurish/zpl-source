...
static void set_ifindex(struct interface *ifp, ifindex_t ifi_index)
{
	...
	if (((oifp = if_lookup_by_kernel_index(ifi_index)) != NULL) && (oifp != ifp))
	{
...
static int netlink_recvbuf(struct nlsock *nl, uint32_t newsize)
{
	...
	if (ret < 0)
	{
		zlog_err(ZLOG_PAL, "Can't get %s receive buffer size: %s", nl->name,
				safe_strerror(errno));
		return -1;
	}
		...
		vrf_id_t vrf_id)
{
	...
	if (rtm->rtm_family == AF_INET)
	{
		...
		if (!tb[RTA_MULTIPATH])
			rib_add_ipv4(ZEBRA_ROUTE_KERNEL, flags, &p, gate, src, ifkernel2ifindex(index),
		...
		else
		{
			...
			for (;;)
			{
				...
				{
					if (index)
						rib_nexthop_ipv4_ifindex_add(rib, gate, src, ifkernel2ifindex(index));
					else
	...
	if (rtm->rtm_family == AF_INET6)
	{
		...
		rib_add_ipv6 (ZEBRA_ROUTE_KERNEL, flags, &p, gate, ifkernel2ifindex(index), vrf_id,
		...
		vrf_id_t vrf_id)
{
	...
	if (rtm->rtm_family == AF_INET)
	{
		...
		{
			if (!tb[RTA_MULTIPATH])
				rib_add_ipv4(ZEBRA_ROUTE_KERNEL, 0, &p, gate, src, ifkernel2ifindex(index),
						vrf_id, table, 0, mtu, 0, SAFI_UNICAST);
			else
			{
				/* This is a multipath route */
			...
			rib_delete_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, gate, ifkernel2ifindex(index),
					vrf_id, SAFI_UNICAST);
	}
	...
	if (rtm->rtm_family == AF_INET6)
	{
		...
		if (h->nlmsg_type == RTM_NEWROUTE)
		rib_add_ipv6 (ZEBRA_ROUTE_KERNEL, 0, &p, gate, ifkernel2ifindex(index), vrf_id, table,
				0, mtu, 0, SAFI_UNICAST);
		else
		rib_delete_ipv6 (ZEBRA_ROUTE_KERNEL, zebra_flags, &p, gate, ifkernel2ifindex(index), vrf_id,
				SAFI_UNICAST);
	}
#endif /* HAVE_IPV6 */
		...
		vrf_id_t vrf_id)
{
	...
	if (!(h->nlmsg_type == RTM_NEWLINK || h->nlmsg_type == RTM_DELLINK))
	{
		...
		zlog_warn(ZLOG_PAL,
				"netlink_link_change: wrong kernel message %d vrf %u\n",
				h->nlmsg_type, vrf_id);
		return 0;
	}
		...
		struct nlmsghdr *h, vrf_id_t vrf_id)
{
	...
	case RTM_DELADDR:
		...
		break;
	default:
		zlog_warn(ZLOG_PAL, "Unknown netlink nlmsg_type %d vrf %u\n",
				h->nlmsg_type, vrf_id);
		break;
	}
	return 0;
...
int interface_lookup_netlink(struct nsm_vrf *zvrf)
{
	int ret;
...
/* Routing table read function using netlink interface.  Only called
 bootstrap time. */
int netlink_route_read(struct nsm_vrf *zvrf)
{
	int ret;
...
void kernel_init(struct nsm_vrf *zvrf)
{
	...
	if (zvrf->netlink.sock > 0)
	{
		...
		if (fcntl(zvrf->netlink.sock, F_SETFL, O_NONBLOCK) < 0)
			zlog_err(ZLOG_PAL, "Can't set %s socket flags: %s", zvrf->netlink.name,
					safe_strerror(errno));
		...
		nl_rcvbuf.p = XMALLOC(MTYPE_STREAM, bufsize);
...
void kernel_terminate(struct nsm_vrf *zvrf)
{
...
void kernel_load(struct nsm_vrf *zvrf)
{
	char nl_name[64];
	/* Initialize netlink sockets */
	snprintf (nl_name, 64, "netlink-listen (vrf %u)", zvrf->vrf_id);
	zvrf->netlink.sock = -1;
	zvrf->netlink.name = XSTRDUP (MTYPE_NETLINK_NAME, nl_name);

	snprintf (nl_name, 64, "netlink-cmd (vrf %u)", zvrf->vrf_id);
	zvrf->netlink_cmd.sock = -1;
