...
03l,02may02,elr  Removed telnetdMutexSem in telnetdParserSet()  (SPR #76641)
                 ...
                 Removed superfluous telnetdInitialized = zpl_false
...
LOCAL zpl_bool telnetdTaskFlag = zpl_true;    /* Create tasks when client connects? */
...
LOCAL zpl_bool telnetdInitialized = zpl_false; 	/* Server initialized? */
LOCAL zpl_bool telnetdParserFlag = zpl_false; 	/* Parser access task registered? */
LOCAL zpl_bool telnetdStartFlag = zpl_false;    /* Server started? */

LOCAL zpl_bool myOpts [256];	/* current option settings - this side */
LOCAL zpl_bool remOpts [256];	/* current option settings - other side */

LOCAL zpl_bool raw;			/* zpl_true = raw mode enabled */
LOCAL zpl_bool echo;		/* zpl_true = echo enabled */

LOCAL FUNCPTR telnetdParserControl = NULL; /* Accesses command interpreter. */

LOCAL zpl_bool remoteInitFlag = zpl_false; 	/* No remote users have connected. */
...
LOCAL int tnInput (int state, int slaveFd, int clientSock, int inputFd, 
                   char *buf, int n);
LOCAL STATUS remDoOpt (int opt, int slaveFd, zpl_bool enable, int clientSock, 
                       zpl_bool remFlag);
LOCAL STATUS localDoOpt (int opt, int slaveFd, zpl_bool enable, int clientSock, 
                         zpl_bool remFlag);
LOCAL void setMode (int slaveFd, int telnetOption, zpl_bool enable);
...
* zpl_false causes the server to spawn a task pair and create the associated data
...
STATUS telnetdInit
    ...
    zpl_bool staticFlag /* zpl_true: create all tasks in advance of any clients */
    )
    {
    int count;
    int result;
    ...
    if (remoteInitFlag == zpl_false)
        ...
        remoteInitFlag = zpl_true;   /* Disable further device creation. */
        }
    ...
    for (count = 0; count < numClients; count++)
        ...
        telnetdTaskList[count].pSession->busyFlag       = zpl_false;
        telnetdTaskList[count].pSession->loggedIn       = zpl_false;
...
LOCAL STATUS telnetdIoTasksCreate
    ...
    sprintf (sessionTaskName, "_%x", (zpl_uint32)pSlot);
...
* to zpl_true causes this routine to create all of those tasks in advance of
...
LOCAL void telnetdTaskDelete
    ...
    zpl_bool telnetdTaskFlagSave; 

    telnetdTaskFlagSave = telnetdTaskFlag; /* Save original state */

    /* 
     * Make sure the flag is set to zpl_false so we delete objects instead of
     * restart objects with  telnetdSessionDisconnectFromShell ()
     */

    telnetdTaskFlag = zpl_false;
    ...
    for (count = 0; count < numTasks; count++)
        ...
        if (telnetdTaskList [count].pSession != NULL)
            {
            telnetdSessionDisconnect (telnetdTaskList [count].pSession, zpl_true);
...
LOCAL void telnetdSessionDisconnect 
     (
     TELNETD_SESSION_DATA *pSlot, 
     zpl_bool pSlotDelete /* For DYNAMIC mode,  should pSlot be free'd when done? */
     )
     {
     ...
     if (telnetdTaskFlag)  
        ...
        if ((pSlot->loggedIn == zpl_true) && 
            (pSlotDelete == zpl_true))
            free (pSlot);
        else
            pSlot->loggedIn = zpl_false; 
        }
...
LOCAL void telnetdSessionDisconnectFromShellJob (TELNETD_SESSION_DATA *pSlot)
    ...
    telnetdSessionDisconnect (pSlot, zpl_false);
...
LOCAL void telnetdSessionDisconnectFromRemote (TELNETD_SESSION_DATA *pSlot)
    ...
    if (pSlot->loggedIn == zpl_true)
        ...
        telnetdSessionDisconnect (pSlot, zpl_true);
        }
...
void telnetd (void)
    ...
    int optval;
    TELNETD_SESSION_DATA *pSlot;
    zpl_bool startFlag;  /* Command interpreter started successfully? */
    ...
    FOREVER
        ...
        (void)localDoOpt (pSlot->slaveFd, TELOPT_ECHO, zpl_true, newSock, zpl_false);
...
LOCAL int tnInput
    ...
    while (--n >= 0)
    {
        ...
        ci = (zpl_uint8) cc;   /* convert to int since many values
	...
	case TS_DATA:	/* just pass data */
		...
		(void)remDoOpt (slaveFd, ci, zpl_true, clientSock, zpl_true);
		state = TS_DATA;
		break;

	    case TS_WONT:		/* remote side said it wont do opt */
		(void)remDoOpt (slaveFd, ci, zpl_false, clientSock, zpl_true);
		state = TS_DATA;
		break;

	    case TS_DO:			/* remote wants us to do opt */
		(void)localDoOpt (slaveFd, ci, zpl_true, clientSock, zpl_true);
		state = TS_DATA;
		break;

	    case TS_DONT:		/* remote wants us to not do opt */
		(void)localDoOpt (slaveFd, ci, zpl_false, clientSock, zpl_true);
		state = TS_DATA;
		break;
...
* RETURNS  
*
* zpl_true, if all tasks are pre-spawned; zpl_false, if tasks are spawned at the 
...
*
*/
zpl_bool telnetdStaticTaskInitializationGet()
    {
