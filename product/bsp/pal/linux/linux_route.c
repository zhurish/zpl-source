 ...
 */
static void linux_netlink_route_build_nexthop(zpl_uint8 family, hal_nexthop_t *nexthop, 
	struct ipstack_nlmsghdr *nlmsg, struct ipstack_rtmsg *ipstack_rtmsg, size_t req_size)
{
	...
	{
		linux_netlink_addattr_l(nlmsg, req_size, IPSTACK_RTA_GATEWAY, &nexthop->gateway.ipv4, IPV4_MAX_BYTELEN);
		linux_netlink_addattr32(nlmsg, req_size, IPSTACK_RTA_OIF, nexthop->kifindex);
	}
#ifdef ZPL_BUILD_IPV6
	if (nexthop->kifindex && family == IPSTACK_AF_INET6)
	{
		linux_netlink_addattr_l (nlmsg, req_size, IPSTACK_RTA_GATEWAY,
 ...
 */
static void linux_netlink_route_build_more_nexthop(zpl_uint8 family, zpl_uint32 bytelen,
		hal_nexthop_t *nexthop, struct ipstack_rtattr *rta, struct ipstack_rtnexthop *rtnh)
{
	...
	if (nexthop->kifindex && family == IPSTACK_AF_INET)
	{
		linux_netlink_rta_addattr_l(rta, NL_PKT_BUF_SIZE, IPSTACK_RTA_GATEWAY, &nexthop->gateway.ipv4, bytelen);
	...
	if (nexthop->kifindex && family == IPSTACK_AF_INET6)
	{
		linux_netlink_rta_addattr_l (rta, NL_PKT_BUF_SIZE, IPSTACK_RTA_GATEWAY,
...
static int linux_netlink_route_multipath(zpl_uint32 cmd, hal_route_param_t *param)
{
	zpl_uint32 bytelen;
	...
	if(param->family == IPSTACK_AF_INET)
		linux_netlink_addattr_l(&req.n, sizeof req, IPSTACK_RTA_DST, &param->destination.ipv4, bytelen);
#ifdef ZPL_BUILD_IPV6
	else
		linux_netlink_addattr_l(&req.n, sizeof req, IPSTACK_RTA_DST, &param->destination.ipv6, bytelen);
#endif
	/* Metric. */
	linux_netlink_addattr32(&req.n, sizeof req, IPSTACK_RTA_PRIORITY, param->metric);
	...
	if (param->mtu)
	{
		...
		linux_netlink_rta_addattr_l(rta, NL_PKT_BUF_SIZE, IPSTACK_RTAX_MTU, &param->mtu, sizeof param->mtu);
		linux_netlink_addattr_l(&req.n, NL_PKT_BUF_SIZE, IPSTACK_RTA_METRICS, IPSTACK_RTA_DATA(rta),
	...
	{
		linux_netlink_route_debug( cmd, param->family, param->prefixlen, &param->nexthop[0], param->destination, param->vrf_id);

		linux_netlink_route_build_nexthop(param->family, &param->nexthop[0], &req.n, &req.r, sizeof req);
		if(param->family == IPSTACK_AF_INET && param->source.ipv4.s_addr)
			linux_netlink_addattr_l(&req.n, sizeof req, IPSTACK_RTA_PREFSRC, &param->source.ipv4, IPV4_MAX_BYTELEN);
#ifdef ZPL_BUILD_IPV6
		else if(param->family == IPSTACK_AF_INET6)
			linux_netlink_addattr_l(&req.n, sizeof req, IPSTACK_RTA_PREFSRC, &param->source.ipv6, IPV6_MAX_BYTELEN);
#endif
	}
	else if (param->nexthop_num  > 1)
	{
		...
		{
			linux_netlink_route_debug( cmd, param->family, param->prefixlen, &param->nexthop[nexthop_num], param->destination, param->vrf_id);
			linux_netlink_route_build_more_nexthop(param->family, bytelen, &param->nexthop[nexthop_num], rta, rtnh);
		...
		if (rta->rta_len > IPSTACK_RTA_LENGTH(0))
			linux_netlink_addattr_l(&req.n, NL_PKT_BUF_SIZE, IPSTACK_RTA_MULTIPATH, IPSTACK_RTA_DATA(rta),
					IPSTACK_RTA_PAYLOAD(rta));
	}
	...
	return linux_netlink_talk(&req.n, &netlink_cmd, param->vrf_id);
}

int linux_netlink_route_rib_add(void *p, hal_route_param_t *param)
{
	return linux_netlink_route_multipath(IPSTACK_RTM_NEWROUTE, param);
}

int linux_netlink_route_rib_del(void *p, hal_route_param_t *param)
{
	return linux_netlink_route_multipath(IPSTACK_RTM_DELROUTE, param);
}