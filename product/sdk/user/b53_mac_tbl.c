...
static int mac_address_cb(struct b53125_mac_arl_entry *entry, void *pp)
{
	...
	{
		halmactbl->mactbl = XMALLOC(MTYPE_SDK_DATA, sizeof(hal_mac_tbl_t));
		halmactbl->macnum = 0;
	}
	else
	{
		...
		{
			halmactbl->mactbl = XREALLOC(MTYPE_SDK_DATA, halmactbl->mactbl, sizeof(hal_mac_tbl_t)*(halmactbl->macnum+1));
		}
	}
	if(halmactbl->mactbl)
	{
