...
/*************************************************************************/
/****************************    DOS   ***********************************/
static int drv_dos_enable(sdk_driver_t *dev, zpl_bool enable, dos_type_en type)
{
	int ret = 0;
	if(dev && dev->b53_device)
	{
		...
		case TCP_SHORTHDR_DROP:
			ret = b53125_dos_tcp_zpl_int16hdr_drop(dev->b53_device, enable);
			break;
...
static int drv_dos_icmp_maxsize(sdk_driver_t *dev, zpl_bool ipv6, int size)
{
	if(dev && dev->b53_device)
	{
		if(ipv6)
...
static int drv_mstp_enable(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_mstp_bypass(sdk_driver_t *dev, int id, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
/******************************** trunk ***************************/
static int drv_trunk_enable(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
/******************************** mirror ***************************/
static int drv_mirror_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
...
static int drv_mirror_source_enable(sdk_driver_t *dev, zpl_bool enable, ifindex_t ifindex,
...
static int drv_mirror_source_filter(sdk_driver_t *dev, zpl_bool enable,
...
static int drv_jumbo_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_limit_egress_rate(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 limitkb, zpl_uint32 burst_size)
{
	//1000Mbps/((64B+8B+12B)×8bit)=1.488095pps
	int burst = 0, cnt = 0;
...
static int drv_port_limit_ingress_rate(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 limitkb, zpl_uint32 burst_size)
{
	//1000Mbps/((64B+8B+12B)×8bit)=1.488095pps
	int burst = 0, cnt = 0;
	int burst_kbyte = burst_size / 8;
    ...
    if(dev && dev->b53_device)
    {
		b53125_qos_buck_mode(dev->b53_device, 0, zpl_true);
		b53125_qos_buck_type(dev->b53_device, 0, 0x3f);
		b53125_qos_ingress_rate_mode(dev->b53_device, if_ifindex2phy(ifindex),  3, zpl_true);
...
static int drv_port_strom_rate(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 limitkb, zpl_uint32 burst_size)
{
	//1000Mbps/((64B+8B+12B)×8bit)=1.488095pps
	int burst = 0, cnt = 0;
	int burst_kbyte = burst_size / 8;
    ...
    if(dev && dev->b53_device)
    {
		b53125_qos_buck_mode(dev->b53_device, 1, zpl_true);
...
static int drv_port_strom_mode(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable, int mode)
{
	if(dev && dev->b53_device)
...
/******************************** qos ***************************/
static int drv_qos_ipg(sdk_driver_t *dev, zpl_bool tx, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_qos_enable(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
	{
		b53125_qos_base_port(dev->b53_device, zpl_false);
		return b53125_qos_layer_sel(dev->b53_device, 3);
	}
	return ERROR;
}

static int drv_qos_8021q_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_qos_diffserv_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_qos_class_scheduling(sdk_driver_t *dev, nsm_qos_class_e class, nsm_class_sched_t mode)
{
	...
	if(mode == NSM_CLASS_SCHED_STRICT)
	{
		...
		{
			return b53125_qos_class4_weight(dev->b53_device, zpl_true,  -1);
		}
	}
	else
		sched_mode = 0;
	...
	{
		return b53125_qos_class4_weight(dev->b53_device, zpl_false,  -1);
	}
	/*
...
static int drv_qos_class_weight(sdk_driver_t *dev, nsm_qos_class_e class, int weight)
{
	...
	{
		if(class == NSM_QOS_CLASS_4)
			return b53125_qos_class4_weight(dev->b53_device, zpl_false,  weight);
...
/******************************** vlan ***************************/
static int drv_vlan_enable(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
{
	if(dev && dev->b53_device)
	return b53125_add_vlan_port(dev->b53_device, vlan, if_ifindex2phy(ifindex), zpl_false);
	return ERROR;
}
...
{
	if(dev && dev->b53_device)
	return b53125_del_vlan_port(dev->b53_device, vlan, if_ifindex2phy(ifindex), zpl_false);
	return ERROR;
}
...
{
	if(dev && dev->b53_device)
	return b53125_add_vlan_port(dev->b53_device, vlan, if_ifindex2phy(ifindex), zpl_true);
	return ERROR;
}
...
{
	if(dev && dev->b53_device)
	return b53125_del_vlan_port(dev->b53_device, vlan, if_ifindex2phy(ifindex), zpl_true);
	return ERROR;
}
...
static int drv_port_add_allowed_tag_batch_vlan(sdk_driver_t *dev, ifindex_t ifindex, vlan_t start, vlan_t end)
{
	...
	for(i = start; i <= end; i++)
	{
		ret |= b53125_add_vlan_port(dev->b53_device, i, if_ifindex2phy(ifindex), zpl_true);
		if(ret != 0)
			break;
	}
	return ret;
...
static int drv_port_del_allowed_tag_batch_vlan(sdk_driver_t *dev, ifindex_t ifindex, vlan_t start, vlan_t end)
{
	...
	for(i = start; i <= end; i++)
	{
		ret |= b53125_del_vlan_port(dev->b53_device, i, if_ifindex2phy(ifindex), zpl_true);
		if(ret != 0)
			break;
	}
	return ret;
...
/*static int drv_port_vlan_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
/******************************** global ***************************/
static int drv_switch_manage(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
		return b53125_switch_manege(dev->b53_device, enable);
	return ERROR;
}
static int drv_switch_forwarding(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
		return b53125_switch_forwarding(dev->b53_device, enable);
	return ERROR;
}
static int drv_multicast_flood(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
		return b53125_multicast_flood(dev->b53_device, enable);
	return ERROR;
}
static int drv_unicast_flood(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_multicast_learning(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_enable_bpdu(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
/******************************** cpu port ***************************/
static int drv_global_imp_enable(sdk_driver_t *dev, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_imp_speed(sdk_driver_t *dev, zpl_uint32 speed)
{
	if(dev && dev->b53_device)
...
static int drv_imp_duplex(sdk_driver_t *dev, zpl_uint32 duplex)
{
	if(dev && dev->b53_device)
...
static int drv_imp_flow(sdk_driver_t *dev, zpl_bool rx, zpl_bool tx)
{
	if(dev && dev->b53_device)
...
/******************************** port ***************************/
static int drv_port_protected_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
/*
static int drv_port_pasue_transmit_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_pasue_receive_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_pasue_pass_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool rx)
{
	if(dev && dev->b53_device)
	{
		if(rx)
			return b53125_pause_pass_rx(dev->b53_device, if_ifindex2phy(ifindex), zpl_true);
		else
			return b53125_pause_pass_tx(dev->b53_device, if_ifindex2phy(ifindex), zpl_true);
	}
	return ERROR;
}

static int drv_port_pasue_pass_disable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool rx)
{
	if(dev && dev->b53_device)
	{
		if(rx)
			return b53125_pause_pass_rx(dev->b53_device, if_ifindex2phy(ifindex), zpl_false);
		else
			return b53125_pause_pass_tx(dev->b53_device, if_ifindex2phy(ifindex), zpl_false);
	}
	return ERROR;
}
*/
...
static int drv_port_wan_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_learning_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static int drv_port_software_learning_enable(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
...
static zpl_bool drv_port_get_link(sdk_driver_t *dev, ifindex_t ifindex)
{
	if(dev && dev->b53_device)
...
static zpl_uint32 drv_port_get_speed(sdk_driver_t *dev, ifindex_t ifindex)
{
	if(dev && dev->b53_device)
		return b53125_port_get_speed(dev->b53_device, if_ifindex2phy(ifindex));
	return zpl_false;
}
static zpl_uint32 drv_port_get_duplex(sdk_driver_t *dev, ifindex_t ifindex)
{
	if(dev && dev->b53_device)
...
static int drv_port_speed(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 value)
{
	if(dev && dev->b53_device)
...
static int drv_port_duplex(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 value)
{
	if(dev && dev->b53_device)
...
static int drv_port_flow(sdk_driver_t *dev, ifindex_t ifindex, zpl_uint32 value)
{
	if(dev && dev->b53_device)
...
static int drv_port_set_link(sdk_driver_t *dev, ifindex_t ifindex, zpl_bool enable)
{
	if(dev && dev->b53_device)
