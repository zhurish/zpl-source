/*
 * $Id: cint_operators.c,v 1.31 Broadcom SDK $
...
__cint_pointer_conversion(cint_variable_t** v)
{
    ...
    if(v && *v && (*v)->dt.desc.array != 0) {
        ...
        *(void**)rv->data = (*v)->data; 
        ...
        /* Pass the original array size along for bounds checking and debug */
...
static int
__cint_check_operand(cint_variable_t* v, uint32_t opflags)
{
    if(v) {
        uint32_t vflags = cint_atomic_flags(v); 
...
cint_eval_operator_internal(cint_ast_t* ast, cint_operator_t op, cint_variable_t* left, cint_variable_t* right)
{
    ...
    if( (left && left->dt.desc.pcount) ||
        ...
        uint32_t flags = cint_atomic_flags(right) | cint_atomic_flags(left); 
