/*
 * $Id: cint_builtins.c,v 1.33.4.2 Broadcom SDK $
...
unsigned long
cint_timer_start(void)
{
    return cint_time_usecs(); 
}

void
cint_timer_stop(unsigned long t0)
{
    unsigned long t = cint_time_usecs(); 
    unsigned long d = t - t0; 
    CINT_PRINTF("%ld usecs\n", d); 
}
...
CINT_FWRAPPER_CREATE_VP1(cint_timer_stop, 
                         unsigned long, unsigned long, start, 0, 0); 
                    ...
                    case 'o':
                        {
                            ...
                            uint32_t flags = cint_atomic_flags(v); 
