/*
 * $Id: cint_variables.c,v 1.9 Broadcom SDK $
 * 
...
__variable_find(cint_variable_t* vlist, const char* name)
{
    cint_variable_t* v; 
...
cint_variable_find(const char* name, int current_scope_only)
{
    ...
    if(_scopes && _scopes->lscopes) {
        if(current_scope_only) {
            rv = __variable_find(_scopes->lscopes->vlist, name); 
            if(!rv) rv = __variable_find(_scopes->lscopes->autolist, name); 
            return rv; 
        }
        else {
            for(s = _scopes->lscopes; s; s = s->next) {
                if((rv = __variable_find(s->vlist, name)) != NULL) {
                    break; 
                }
                if((rv = __variable_find(s->autolist, name)) != NULL) {
                    break; 
                }       
            }       
        }   
    ...
    if(rv == NULL) {
        rv = __variable_find(_global_scope.lscopes->vlist, name); 
        if(!rv) rv = __variable_find(_global_scope.lscopes->autolist, name); 
    }
