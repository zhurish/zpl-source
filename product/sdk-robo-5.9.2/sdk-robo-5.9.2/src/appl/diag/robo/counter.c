/*
 * $Id: counter.c,v 1.16 Broadcom SDK $
...
static struct {	        /* match enum from bcm/stat.h */
    bcm_stat_val_t  type;
    char  *name;
} custom_dbg_names[] = {
    { snmpBcmCustomReceive0,  "bcmcustom_receive0" },
    { snmpBcmCustomReceive1,  "bcmcustom_receive1" },
    { snmpBcmCustomReceive2,  "bcmcustom_receive2" },
    { snmpBcmCustomReceive3,  "bcmcustom_receive3" },
    { snmpBcmCustomReceive4,  "bcmcustom_receive4" },
    { snmpBcmCustomReceive5,  "bcmcustom_receive5" },
    { snmpBcmCustomReceive6,  "bcmcustom_receive6" },
    { snmpBcmCustomReceive7,  "bcmcustom_receive7" },
    { snmpBcmCustomReceive8,  "bcmcustom_receive8" },
    { snmpBcmCustomTransmit0,  "bcmcustom_transmit0" },
    { snmpBcmCustomTransmit1,  "bcmcustom_transmit1" },
    { snmpBcmCustomTransmit2,  "bcmcustom_transmit2" },
    { snmpBcmCustomTransmit3,  "bcmcustom_transmit3" },
    { snmpBcmCustomTransmit4,  "bcmcustom_transmit4" },
    { snmpBcmCustomTransmit5,  "bcmcustom_transmit5" },
    { snmpBcmCustomTransmit6,  "bcmcustom_transmit6" },
    { snmpBcmCustomTransmit7,  "bcmcustom_transmit7" },
    { snmpBcmCustomTransmit8,  "bcmcustom_transmit8" },
    { snmpBcmCustomTransmit9,  "bcmcustom_transmit9" },
    { snmpBcmCustomTransmit10,  "bcmcustom_transmit10" },
    { snmpBcmCustomTransmit11,  "bcmcustom_transmit11" },
    { snmpBcmCustomTransmit12,  "bcmcustom_transmit12" },
    { snmpBcmCustomTransmit13,  "bcmcustom_transmit13" },
    { snmpBcmCustomTransmit14,  "bcmcustom_transmit14" },
    { 0,  NULL }		/* LAST ENTRY */
};
...
cmd_robo_counter(int unit, args_t *a)
{
    ...
    if (ARG_CNT(a) > 0 && !sal_strcasecmp(_ARG_CUR(a), "debug")) {
        int i = 0, found = 0;
        char *subcmd = NULL, *type = NULL, *name = NULL;
        char cname[20] = {0};
        soc_robo_reg_t reg = INVALID_Rr;
        uint32 ctr_reg = 0;
        ...
        if (sal_strcasecmp(subcmd, "set") == 0) {
            ...
            if (isint(name)) {
                ...
                found = 0;
                if (sal_strcasecmp("INVALID", name) == 0) {
                    found = 1;
                    ctr_reg = 0;
                } else {
                    for (reg = 0; reg < NUM_SOC_ROBO_REG; reg++) {
                        ...
                        /* The input flags is not counter register */
                        if (!SOC_REG_IS_COUNTER(unit, reg)) {
                            continue;
                        }
    
                        if (sal_strcasecmp(SOC_ROBO_REG_NAME(unit, reg), name) == 0) {
                            found = 1;
                            ctr_reg = (uint32)reg;
                            reg = NUM_SOC_ROBO_REG;
                        }
                    }
                }

                if (!found) {
                    printk("No matching counter register found\n");
                    return CMD_FAIL;
                }
            }

            for (i = 0; custom_dbg_names[i].name != NULL; i++) {
                if (sal_strcasecmp(type, custom_dbg_names[i].name) == 0) {

                    r = bcm_stat_custom_set(unit, -1, custom_dbg_names[i].type, ctr_reg);
                
                    if (r < 0) {
                        printk("%s: Error: Could not set debug counter type: %s\n",
            ...
            if (custom_dbg_names[i].name == NULL){
                printk("\t Unknown Custom Debug Counter type at %s!\n", type);
                return CMD_FAIL;
            }
        } 
        ...
        if (sal_strcasecmp(subcmd, "get") == 0) {
            ...
            for (i = 0; custom_dbg_names[i].name != NULL; i++) {
                if (sal_strcasecmp(type, custom_dbg_names[i].name) == 0) {
                    ...
                    r = bcm_stat_custom_get(unit, -1, custom_dbg_names[i].type, &ctr_reg);
                
                    if (r < 0) {
                        printk("%s: Error: Could not get debug counter type: %s\n",
                               ARG_CMD(a), soc_errmsg(r));
                        return CMD_FAIL;
                    } else {
                        if (ctr_reg == 0) {
                            reg = INVALID_Rr;
                            sprintf(cname, "%s", "INVALID");
                        } else {
                            reg = (soc_robo_reg_t)ctr_reg;

                            if (strlen(SOC_ROBO_REG_NAME(unit, reg)) > 12) {
                                sal_memcpy(cname, SOC_ROBO_REG_NAME(unit, reg), 12);
                            } else {
                                sprintf(cname, "%s", SOC_ROBO_REG_NAME(unit, reg));
                            }
                        }

                        printk("%s  : %s\n", custom_dbg_names[i].name, cname);
                    }
