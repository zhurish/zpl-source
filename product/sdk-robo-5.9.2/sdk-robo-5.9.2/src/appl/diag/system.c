/*
 * $Id: system.c,v 1.162.6.2 Broadcom SDK $
...
diag_shell(void)
{
    ...
    if ((script = sal_boot_script()) != NULL) {
        ...
        no_rc_warning = 0;
    } else if (soc_ndev > 0) {    /* Robo units do not use rc.soc */
#if defined(BCM_ROBO_SUPPORT)
        for (i = 0; (i < soc_ndev) && SOC_IS_ROBO(i); i++) {
            int rv;
...
system_shutdown(int unit, int cleanup)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (soc_feature(unit, soc_feature_field_tcam_parity_check)){
            if (_robo_field_thread_stop(unit) < 0){
                soc_cm_debug(DK_ERR, 
                    "system_shutdown: could not stop field thread!\n");
            }
        }
#endif        
