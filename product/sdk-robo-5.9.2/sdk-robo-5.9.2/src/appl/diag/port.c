/*
 * $Id: port.c,v 1.172.6.7 Broadcom SDK $
...
#define WC_DSC_STATUS  1
#define WC_VEYE_MARGIN  2
#define WC_HEYE_R_MARGIN  3
#define WC_HEYE_L_MARGIN  4
/*
 * This function is only available for Trident Warpcore serdes device. It intends to
 * be used for debug purpose only.
 * In the future, we'll need to expose these diagnostic functions for each PHY device
 * through a well-defined phy driver interface if this facility is to be widely used. 
 */
STATIC cmd_result_t
...
#ifndef __KERNEL__
extern int wc40_eye_margin( int unit, int port,int type);
extern int wc40_uc_status_dump(int unit, soc_port_t port);
    bcm_port_t          port, dport;
    bcm_pbmp_t          pbmp;
    int                 rv, cmd;
    ...
    if (sal_strcasecmp(cmd_str, "dsc") == 0) {
        ...
        cmd = WC_HEYE_R_MARGIN;
    } else if (sal_strcasecmp(cmd_str, "heye_l") == 0) {
        cmd = WC_HEYE_L_MARGIN;
    } else {
        return CMD_USAGE;
    }

    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        if (cmd == WC_DSC_STATUS) {
            rv = wc40_uc_status_dump(unit, port);
        } else if (cmd == WC_VEYE_MARGIN) {
            rv = wc40_eye_margin(unit,port,0);
        } else if (cmd == WC_HEYE_R_MARGIN) {
            rv = wc40_eye_margin(unit,port,1);
        } else if (cmd == WC_HEYE_L_MARGIN) {
            rv = wc40_eye_margin(unit,port,2);
        } else {
            /* should not happen */
        }
    }
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
      ...
      if (!SOC_IS_SBX_QE2000(unit)) {
        ...
        if (cmd == _PHY_PRBS_SET_CMD || cmd == _PHY_PRBS_CLEAR_CMD) {
            ...
            /* 
             * Set rx/tx enable. If clear, enable == 0
             */                                      
            rv = bcm_port_control_set(unit, port,                       
...
void _print_timesync_egress_message_mode(char *message, bcm_port_timesync_event_message_egress_mode_t mode) 
{

    soc_cm_print("%s (no,uc,rc,ct) - ", message);

    switch (mode) {
    case bcmPortTimesyncEventMessageEgressModeNone:
        soc_cm_print("NOne\n");
        break;
    case bcmPortTimesyncEventMessageEgressModeUpdateCorrectionField:
        soc_cm_print("Update_Correctionfield\n");
        break;
    case bcmPortTimesyncEventMessageEgressModeReplaceCorrectionFieldOrigin:
        soc_cm_print("Replace_Correctionfield_origin\n");
        break;
    case bcmPortTimesyncEventMessageEgressModeCaptureTimestamp:
...
bcm_port_timesync_event_message_egress_mode_t _convert_timesync_egress_message_str(char *str, 
    bcm_port_timesync_event_message_egress_mode_t def) 
{
    int i;
    struct s_array {
        char *s;
        bcm_port_timesync_event_message_egress_mode_t value;
    } data[] = {
    {"no",bcmPortTimesyncEventMessageEgressModeNone},
    {"uc",bcmPortTimesyncEventMessageEgressModeUpdateCorrectionField},
    {"rc",bcmPortTimesyncEventMessageEgressModeReplaceCorrectionFieldOrigin},
    {"ct",bcmPortTimesyncEventMessageEgressModeCaptureTimestamp} };
...
void _print_timesync_ingress_message_mode(char *message, bcm_port_timesync_event_message_ingress_mode_t mode)
{

    soc_cm_print("%s (no,uc,it,id) - ", message);

    switch (mode) {
    case bcmPortTimesyncEventMessageIngressModeNone:
        soc_cm_print("NOne\n");
        break;
    case bcmPortTimesyncEventMessageIngressModeUpdateCorrectionField:
        soc_cm_print("Update_Correctionfield\n");
        break;
    case bcmPortTimesyncEventMessageIngressModeInsertTimestamp:
        soc_cm_print("Insert_Timestamp\n");
        break;
    case bcmPortTimesyncEventMessageIngressModeInsertDelaytime:
...
bcm_port_timesync_event_message_ingress_mode_t _convert_timesync_ingress_message_str(char *str, 
    bcm_port_timesync_event_message_ingress_mode_t def) 
{
    int i;
    struct s_array {
        char *s;
        bcm_port_timesync_event_message_ingress_mode_t value;
    } data[] = {
    {"no",bcmPortTimesyncEventMessageIngressModeNone},
    {"uc",bcmPortTimesyncEventMessageIngressModeUpdateCorrectionField},
    {"it",bcmPortTimesyncEventMessageIngressModeInsertTimestamp},
    {"id",bcmPortTimesyncEventMessageIngressModeInsertDelaytime} };
...
void _print_timesync_gmode(char *message, bcm_port_timesync_global_mode_t mode)
{

    soc_cm_print("%s (fr,si,cp) - ", message);

    switch (mode) {
    case bcmPortTimesyncModeFree:
        soc_cm_print("FRee\n");
        break;
    case bcmPortTimesyncModeSyncin:
        soc_cm_print("SyncIn\n");
        break;
    case bcmPortTimesyncModeCpu:
        soc_cm_print("CPu\n");
        break;
    default:
        soc_cm_print("\n");
...
bcm_port_timesync_global_mode_t _convert_timesync_gmode_str(char *str, 
    bcm_port_timesync_global_mode_t def) 
{
    int i;
    struct s_array {
        char *s;
        bcm_port_timesync_global_mode_t value;
    } data[] = {
    {"fr",bcmPortTimesyncModeFree},
    {"si",bcmPortTimesyncModeSyncin},
    {"cp",bcmPortTimesyncModeCpu} };
...
void _print_timesync_config(bcm_port_timesync_config_t *conf)
{

    printk("ENable (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_ENABLE ? "Yes" : "No");

    printk("CaptureTS (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_CAPTURE_TS_ENABLE ? "Yes" : "No");

    printk("HeartbeatTS (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_HEARTBEAT_TS_ENABLE ? "Yes" : "No");

    /*
    printk("OneStep (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_ONE_STEP ? "Yes" : "No");
     */

    printk("RxCrc (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_RX_CRC_ENABLE ? "Yes" : "No");

    printk("AS (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_8021AS_ENABLE ? "Yes" : "No");

    printk("L2 (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_L2_ENABLE ? "Yes" : "No");

    printk("IP4 (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_IP4_ENABLE ? "Yes" : "No");

    printk("IP6 (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_IP6_ENABLE ? "Yes" : "No");

    printk("ExtClock (Y or N) - %s\n", conf->flags & BCM_PORT_TIMESYNC_CLOCK_SRC_EXT ? "Yes" : "No");
...
if_esw_phy(int u, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "eee") == 0) {
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            if (flags & 0x1) {
                ...
                if (rv == BCM_E_NONE) {
                    printk("EEE mode set to %s EEE mode\n", mode_type);
                } else {
                    if (rv == BCM_E_UNAVAIL) {
                        printk("EEE %s mode not available\n", mode_type);
                    } else {
                        printk("EEE %s mode set unsuccessful\n", mode_type);
                    }
                }
            ...
            if (flags & 0x2) {
                ...
                if (rv == BCM_E_NONE) {
                    ...
                    printk("EEE Auto mode Latency = %s\n", str);
                }
            }
            ...
            if (flags & 0x4) {
                ...
                printk("EEE Auto mode IDLE Threshold = %d\n", idle);
            }
            ...
            if (sal_strcasecmp(stats, "get") == 0) {
                ...
                (void)bcm_port_phy_control_get(u,p,BCM_PORT_PHY_CONTROL_EEE_RECEIVE_DURATION,
                                           &rx_duration);
                printk("Tx events = %d TX Duration = %d RX events = %d RX "
                        "Duration = %d\n", tx_events, tx_duration, 
        ...
        parse_arg_eq_done(&pt);
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if ((eee_auto_mode_value == 1) && (eee_mode_value == 1)){
                    str = "auto";
                    rv = bcm_port_phy_control_get(u,p,
                            BCM_PORT_PHY_CONTROL_EEE_AUTO_FIXED_LATENCY,
                            &latency);
    ...
    if (c != NULL && sal_strcasecmp(c, "timesync") == 0) {
        bcm_port_timesync_config_t conf;
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            parse_table_t    pt;
            uint32 enable, capture_ts, heartbeat_ts, one_step, rx_crc, as, l2, ip4, ip6, ec, /* bool */
                   itpid, otpid, tx_offset, rx_offset, original_timecode_seconds, original_timecode_nanoseconds, flags; 
            ...
            parse_table_add(&pt, "HeartbeatTS", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 2 */
                            0, &heartbeat_ts, 0);
            parse_table_add(&pt, "OneStep", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 3 */
                            0, &one_step, 0);
            parse_table_add(&pt, "RxCrc", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 4 */
                            0, &rx_crc, 0);
            parse_table_add(&pt, "AS", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 5 */
                            0, &as, 0);
            parse_table_add(&pt, "L2", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 6 */
                            0, &l2, 0);
            parse_table_add(&pt, "IP4", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 7 */
                            0, &ip4, 0);
            parse_table_add(&pt, "IP6", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 8 */
                            0, &ip6, 0);
            parse_table_add(&pt, "ExtClock", PQ_BOOL | PQ_DFL | PQ_NO_EQ_OPT, /* index 9 */
                            0, &ec, 0);
            parse_table_add(&pt, "ITpid", PQ_DFL | PQ_INT, /* index 10 */
                            0, &itpid, 0);
            parse_table_add(&pt, "OTpid", PQ_DFL | PQ_INT, /* index 11 */
                            0, &otpid, 0);
            parse_table_add(&pt, "GMode", PQ_DFL | PQ_STRING, /* index 12 */
                            0, &gmode_str, 0);
            parse_table_add(&pt, "TxOffset", PQ_DFL | PQ_INT, /* index 13 */
                            0, &tx_offset, 0);
            parse_table_add(&pt, "RxOffset", PQ_DFL | PQ_INT, /* index 14 */
                            0, &rx_offset, 0);
            parse_table_add(&pt, "TxSync", PQ_DFL | PQ_STRING, /* index 15 */
                            0, &tx_sync_mode_str, 0);
            parse_table_add(&pt, "TxDelayReq", PQ_DFL | PQ_STRING, /* index 16 */
                            0, &tx_delay_request_mode_str, 0);
            parse_table_add(&pt, "TxPdelayReq", PQ_DFL | PQ_STRING, /* index 17 */
                            0, &tx_pdelay_request_mode_str, 0);
            parse_table_add(&pt, "TxPdelayreS", PQ_DFL | PQ_STRING, /* index 18 */
                            0, &tx_pdelay_response_mode_str, 0);
            parse_table_add(&pt, "RxSync", PQ_DFL | PQ_STRING, /* index 19 */
                            0, &rx_sync_mode_str, 0);
            parse_table_add(&pt, "RxDelayReq", PQ_DFL | PQ_STRING, /* index 20 */
                            0, &rx_delay_request_mode_str, 0);
            parse_table_add(&pt, "RxPdelayReq", PQ_DFL | PQ_STRING, /* index 21 */
                            0, &rx_pdelay_request_mode_str, 0);
            parse_table_add(&pt, "RxPdelayreS", PQ_DFL | PQ_STRING, /* index 22 */
                            0, &rx_pdelay_response_mode_str, 0);
            parse_table_add(&pt, "OriginalTimecodeSeconds", PQ_DFL | PQ_INT, /* index 23 */
                            0, &original_timecode_seconds, 0);
            parse_table_add(&pt, "OriginalTimecodeNanoseconds", PQ_DFL | PQ_INT, /* index 24 */
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                if ((rv = bcm_port_timesync_config_get(u,p,&conf)) == BCM_E_FAIL) {
                    printk("bcm_port_timesync_config_get() failed, u=%d, p=%d\n", u, p);
                    return BCM_E_FAIL;
                }

                if (flags & (1U << 0)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_ENABLE;
                    conf.flags |= enable ? BCM_PORT_TIMESYNC_ENABLE : 0;
                }

                if (flags & (1U << 1)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_CAPTURE_TS_ENABLE;
                    conf.flags |= capture_ts ? BCM_PORT_TIMESYNC_CAPTURE_TS_ENABLE : 0;
                }

                if (flags & (1U << 2)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_HEARTBEAT_TS_ENABLE;
                    conf.flags |= heartbeat_ts ? BCM_PORT_TIMESYNC_HEARTBEAT_TS_ENABLE : 0;
                }

                if (flags & (1U << 3)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_ONE_STEP;
                    conf.flags |= one_step ? BCM_PORT_TIMESYNC_ONE_STEP : 0;
                }

                if (flags & (1U << 4)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_RX_CRC_ENABLE;
                    conf.flags |= rx_crc ? BCM_PORT_TIMESYNC_RX_CRC_ENABLE : 0;
                }

                if (flags & (1U << 5)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_8021AS_ENABLE;
                    conf.flags |= as ? BCM_PORT_TIMESYNC_8021AS_ENABLE : 0;
                }

                if (flags & (1U << 6)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_L2_ENABLE;
                    conf.flags |= l2 ? BCM_PORT_TIMESYNC_L2_ENABLE : 0;
                }

                if (flags & (1U << 7)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_IP4_ENABLE;
                    conf.flags |= ip4 ? BCM_PORT_TIMESYNC_IP4_ENABLE : 0;
                }

                if (flags & (1U << 8)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_IP6_ENABLE;
                    conf.flags |= ip6 ? BCM_PORT_TIMESYNC_IP6_ENABLE : 0;
                }

                if (flags & (1U << 9)) {
                    conf.flags &= ~BCM_PORT_TIMESYNC_CLOCK_SRC_EXT;
                    conf.flags |= ec ? BCM_PORT_TIMESYNC_CLOCK_SRC_EXT : 0;
                }
                ...
                if ((rv = bcm_port_timesync_config_set(u,p,&conf)) == BCM_E_FAIL) {
                    printk("bcm_port_timesync_config_set() failed, u=%d, p=%d\n", u, p);
                    return BCM_E_FAIL;
                }
            }
