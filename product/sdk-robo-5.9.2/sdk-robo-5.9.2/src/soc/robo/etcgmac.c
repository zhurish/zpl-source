 ...
 * $Id: etcgmac.c,v 1.10.6.1 Broadcom SDK $
 */
...
static void chiprxreset(ch_t *ch);
static void chiprxinit(ch_t *ch);
...
chipattach(etc_soc_info_t *etc, void *dev, void *regsva)
{
    ...
    /* using GMAC 0 */
    if ((regs = (gmac0regs_t *)ai_soc_setcore(ch->sih, GMAC_CORE_ID, 0)) == NULL) {
    ...
    /* TX: TC_BK, RX: RX_Q0 */
    ch->di[0] = dma_soc_attach(etc->et, name, ch->dev, 1,
    ...
    /* TX: TC_BE, RX: RX_Q1 */
    ch->di[1] = dma_soc_attach(etc->et, name, ch->dev, 1,
    ...
    /* TX: TC_CL, RX: RX_Q2 */
    ch->di[2] = dma_soc_attach(etc->et, name, ch->dev, 1,
                           ...
                           etc->en_rxsephdr[2]);
    /* TX: TC_VO, RX: RX_Q3 */
    ch->di[3] = dma_soc_attach(etc->et, name, ch->dev, 1,
...
chipinreset:
    ...
    com_regs = ai_soc_setcoreidx(ch->sih, SI_GMAC_COMMON_IDX);
    ASSERT(regs != NULL);
...
chipphyrd(ch_t *ch, uint phyaddr, uint reg)
{
    ...
    origidx = ai_soc_coreidx(ch->sih);
    regs = ai_soc_setcoreidx(ch->sih, SI_GMAC_COMMON_IDX);
    ASSERT(regs != NULL);
