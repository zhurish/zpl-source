/*
 * $Id: port.c,v 1.89.4.1 Broadcom SDK $
...
int
_tb_phy_reset_callback(int unit, _shr_port_t port, void * user_data)
{
    phy_ctrl_t        *pc;
    ...
    if (pc->phy_rev != BCM53280_A0_REV_ID) {
        /* TB Bx RL WAR*/
    ...
    soc_cm_debug(DK_PORT, "_tb_phy_reset_callback port %d\n",port);    
    return (SOC_E_NONE);
}
...
_drv_port_probe(int unit, soc_port_t p, int *okay)
{
    ...
    *okay = FALSE;
    ...
    if (SOC_IS_TB(unit) && IS_FE_PORT(unit, p)) {
        soc_phy_reset_register(unit, p, 
            _tb_phy_reset_callback, NULL, FALSE);
    }
            ...
            if (SOC_IS_ROBO5324(unit)){
                ...
                if (reg_value & (1 << port)) {
                    ...
                    *val = DRV_PORT_STATUS_SPEED_10M;
                       ...
                       SOC_IS_TB(unit) || SOC_IS_ROBO53101(unit) ||
                       SOC_IS_ROBO53125(unit) || SOC_IS_ROBO53128(unit)){
