/*
 * $Id: mem.c,v 1.51.18.2 Broadcom SDK $
...
drv_bcm53280_mem_delete(int unit, uint32 mem, uint32 *entry, uint32 flags)
{
    ...
    if (fastaging_flags) {
        ...
        /* BCM_L2_REPLACE_PENDING means replace pending entries as well as 
         * non-pending entries 
         */
        if (!(flags & DRV_MEM_OP_PENDING)){
            /* if PENDING flag is set, remove both valid and pending l2 entry.
             * and if the PENDING flag is not set, remove only valid l2 entry.
             */
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0;i < count; i++ ) {
        ...
        for (j = 0; j < (mem_len + 1)/2; j++) {
            ...
            /* endian handler */
            if (table_mask & (1<< (2*j))){            
                *(entry+(2*k)) =  COMPILER_64_LO(temp_data);
            }
            if(mem_len > (2*j+1) ){
                if (table_mask & (1<< (2*j+1))){
                     *(entry+(2*k+1)) = COMPILER_64_HI(temp_data);
                        k++;
                }
           }
        }
