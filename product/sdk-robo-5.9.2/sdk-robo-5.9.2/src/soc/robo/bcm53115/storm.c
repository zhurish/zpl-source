/*
 * $Id: storm.c,v 1.10 Broadcom SDK $
 *
    ...
    uint32 type, uint32 enable)
{
    ...
    uint32  rev_mcast_fld_index = 0, vlan_pol_fld_index = 0;
    int		rv= SOC_E_NONE;
    ...
    if (SOC_IS_ROBO53101(unit)) {
        if (type & DRV_STORM_CONTROL_VLAN_POLICING) {
            /* VLAN based policing */
            vlan_pol_fld_index = EN_VLAN_POLICINGf;
        }
    }
    ...
    if (vlan_pol_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, vlan_pol_fld_index, &temp));
    }
...
_drv_bcm53115_storm_control_type_enable_get(int unit, uint32 port, uint32 *type)
{
    ...
    *type = 0;
    ...
    if (SOC_IS_ROBO53101(unit)) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_VLAN_POLICINGf, &temp));
        if (temp) {
