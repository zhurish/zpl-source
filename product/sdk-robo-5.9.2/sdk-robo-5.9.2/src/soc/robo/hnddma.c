...
hnddma_t *
dma_soc_attach(void *drv, char *name, void *dev, uint dma64, 
...
_dma_rxfill(dma_soc_info_t *di)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        */
        if (di->en_rxsephdr) {
            if ((p = ET_PKTGET(di->dev, di->rxsephdrsize,
                            FALSE)) == NULL) {
            ...
            *(uint32*)(ET_PKTDATA(di->dev, p)) = 0;
            ...
            if ((p = ET_PKTGET(di->dev, (di->rxbufsize + extra_offset - di->rxsephdrsize),
                            FALSE)) == NULL) {
                DMA_ERROR(("%s: dma_rxfill: out of rxbufs\n", di->name));
                di->hnddma.rxnobuf++;
                break;
            ...
            *(uint32*)(ET_PKTDATA(di->dev, p)) = 0;
            ...
            if ((p = ET_PKTGET(di->dev, di->rxbufsize + extra_offset,
                            FALSE)) == NULL) {
                DMA_ERROR(("%s: dma_rxfill: out of rxbufs\n", di->name));
                di->hnddma.rxnobuf++;
                break;
