/*
 * $Id: counter.c,v 1.87.6.1 Broadcom SDK $
...
typedef struct soc_robo_dbg_cntr_s {
    uint32  counter;
    soc_robo_reg_t  reg;
} soc_robo_dbg_cntr_t;


soc_robo_dbg_cntr_t soc_robo_dbg_cntr_rx[] = {
    { DRV_SNMP_BCM_CUSTOM_RCV0,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV1,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV2,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV3,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV4,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV5,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV6,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV7,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_RCV8,  INVALID_Rr  }
};

soc_robo_dbg_cntr_t soc_robo_dbg_cntr_tx[] = {
    { DRV_SNMP_BCM_CUSTOM_TRANS0,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS1,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS2,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS3,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS4,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS5,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS6,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS7,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS8,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS9,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS10,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS11,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS12,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS13,  INVALID_Rr  },
    { DRV_SNMP_BCM_CUSTOM_TRANS14,  INVALID_Rr  }
};

/*
 * Function:
 *      soc_robo_custom_dbg_cntr_set
 * Purpose:
 *      Set the debug counter to count certain packet types.
 */
int
soc_robo_custom_dbg_cntr_set(int unit, uint32 type, soc_robo_reg_t ctr_reg)
{
    int  i;

    /* INVALID_Rr is the default value : means to turn off the debug counter type */
    if (ctr_reg != INVALID_Rr) {
        /* The input counter register is invalid */
    ...
    for (i = 0; i < COUNTOF(soc_robo_dbg_cntr_rx); i++) {
        if (soc_robo_dbg_cntr_rx[i].counter == type) {
            soc_robo_dbg_cntr_rx[i].reg = ctr_reg;
            return SOC_E_NONE;
        }
    }

    for (i = 0; i < COUNTOF(soc_robo_dbg_cntr_tx); i++) {
        if (soc_robo_dbg_cntr_tx[i].counter == type) {
            soc_robo_dbg_cntr_tx[i].reg = ctr_reg;
            return SOC_E_NONE;
        }
    }
...
/*
 * Function:
 *      soc_robo_custom_dbg_cntr_get
 * Purpose:
 *      Get the debug counter by selected type.
 */
int
soc_robo_custom_dbg_cntr_get(int unit, uint32 type, soc_robo_reg_t *ctr_reg)
{
    int  i;
    ...
    *ctr_reg = INVALID_Rr;
    ...
    for (i = 0; i < COUNTOF(soc_robo_dbg_cntr_rx); i++) {
        if (soc_robo_dbg_cntr_rx[i].counter == type) {
            *ctr_reg = soc_robo_dbg_cntr_rx[i].reg;
            return SOC_E_NONE;
        }
    }
    ...
    for (i = 0; i < COUNTOF(soc_robo_dbg_cntr_tx); i++) {
        if (soc_robo_dbg_cntr_tx[i].counter == type) {
            *ctr_reg = soc_robo_dbg_cntr_tx[i].reg;
            return SOC_E_NONE;
        }
    }
    ...
    PBMP_ITER(bmp, port) {
        switch (counter_type) {
            ...
            case DRV_SNMP_DOT1D_PORT_IN_DISCARDS:
                if (SOC_IS_TB(unit)|| SOC_IS_ROBO53242(unit) ||
            ...
            /* IEEE 802.1bb */
            case DRV_SNMP_IEEE_8021P_FC_REQUESTS:
            case DRV_SNMP_IEEE_8021P_FC_INDICATIONS:
                /* not support for robo */
                return SOC_E_UNAVAIL;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_64_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
                    soc_robo_counter_set(unit, port, RXPKTS64OCTETSr, val);
                    break;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_65TO127_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
                    soc_robo_counter_set(unit, port, RXPKTS65TO127OCTETSr, val);
                    break;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_128TO255_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
                    soc_robo_counter_set(unit, port, RXPKTS128TO255OCTETSr, val);
                    break;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_256TO511_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
                    soc_robo_counter_set(unit, port, RXPKTS256TO511OCTETSr, val);
                    break;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_512TO1023_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
                    soc_robo_counter_set(unit, port, RXPKTS512TO1023OCTETSr, val);
                    break;
            ...
            case DRV_SNMP_BCM_RECEIVED_PKTS_1024TO1518_OCT:
            case DRV_SNMP_BCM_RECEIVED_PKTS_1519TO2047_OCT:
            case DRV_SNMP_BCM_RECEIVED_PKTS_2048TO4095_OCT:
            case DRV_SNMP_BCM_RECEIVED_PKTS_4095TO9216_OCT:
               return SOC_E_UNAVAIL;
    
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_64_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
            ...
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_65TO127_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
            ...
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_128TO255_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
            ...
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_256TO511_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
            ...
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_512TO1023_OCT:
                if (SOC_IS_TB(unit)) {
#ifdef BCM_TB_SUPPORT
            ...
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_1024TO1518_OCT:
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_1519TO2047_OCT:
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_2048TO4095_OCT:
            case DRV_SNMP_BCM_TRANSMITTED_PKTS_4095TO9216_OCT:
                return SOC_E_UNAVAIL;
            ...
            case DRV_SNMP_BCM_CUSTOM_TRANS14:
                SOC_IF_ERROR_RETURN(soc_robo_custom_dbg_cntr_get
                    (unit, counter_type, &ctr_reg));
                /* The register is invalid */
                if (SOC_COUNTER_INVALID(unit, ctr_reg)) {
                    return SOC_E_UNAVAIL;
                } else {
                    soc_robo_counter_set(unit, port, ctr_reg, val);
                }
                break;
        ...
        case DRV_SNMP_BCM_CUSTOM_TRANS14:
            ...
            if (SOC_COUNTER_INVALID(unit, ctr_reg)) {
                *val = 0;
                ...
                soc_robo_counter_get(unit, port, ctr_reg, &count);
            }
            break;
...
soc_robo_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    if (SOC_IS_ROBO5398(unit) || SOC_IS_ROBO5347(unit) ||
        ...
        SOC_IS_ROBO5395(unit) || SOC_IS_ROBO53242(unit) ||
        SOC_IS_ROBO53262(unit) || SOC_IS_ROBO53115(unit) ||
        SOC_IS_ROBO53118(unit) || SOC_IS_TB(unit) || 
        SOC_IS_ROBO53101(unit) || SOC_IS_ROBO53125(unit) ||
        SOC_IS_ROBO53128(unit)) {
        interval = 0;
    }
