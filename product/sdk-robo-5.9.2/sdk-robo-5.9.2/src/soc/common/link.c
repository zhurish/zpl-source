/*
 * $Id: link.c,v 1.33.6.2 Broadcom SDK $
...
_soc_link_scan_ports_write(int unit, pbmp_t hw_mii_pbm)
{
    ...
    if (SOC_IS_TUCANA(unit)) {
        ...
        if (((SOC_IS_TR_VL(unit) || SOC_IS_SIRIUS(unit)) 
            ...
            WRITE_CMIC_SCAN_PORTS_HIr(unit, SOC_PBMP_WORD_GET(tmp_pbmp, 1));
        }
#ifdef BCM_TRIDENT_SUPPORT
        if (SOC_REG_IS_VALID(unit, CMIC_SCAN_PORTS_HI_2r)) {
            WRITE_CMIC_SCAN_PORTS_HI_2r(unit, SOC_PBMP_WORD_GET(tmp_pbmp, 2));
        }
#endif
    }    
}
