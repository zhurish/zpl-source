/*
 * $Id: scache.c,v 1.26.6.6 Broadcom SDK $
...
#define SCACHE_HANDLE_P_FORMAT "%4d 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%04x"
...
soc_scache_dump_handle_state(int idx, scache_handle_state_t *sc)
{
    ...
    soc_cm_print(SCACHE_HANDLE_P_FORMAT, 
                 idx, sc->handle, sc->offset, sc->size, (uint32)sc->buf,
                 (uint32)sc->lock, sc->flags);
...
int
soc_scache_realloc(int unit, soc_scache_handle_t handle_id, uint32 incr_size)
{
    int rv;
    scache_handle_state_t *hs;
