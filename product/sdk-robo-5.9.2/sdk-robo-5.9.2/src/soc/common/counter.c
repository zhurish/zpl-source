/*
 * $Id: counter.c,v 1.51.6.3 Broadcom SDK $
...
soc_counter_attach(int unit)
{
    ...
    if (SOC_IS_SHADOW(unit)) {
        if (SOC_FAILURE(_soc_counter_shadow_non_dma_init(unit, nports, n_entries, 
                        &non_dma_entries))) {
            goto error;
        }
    } else
	...
	switch (blktype) {
		...
		break;
	    case SOC_BLK_GPIC:
	    case SOC_BLK_GPORT: /* a.k.a. SOC_BLK_QGPORT */
	    case SOC_BLK_SPORT:
		ctype = SOC_CTR_TYPE_GE;
		break;
	    case SOC_BLK_XGPORT: /* a.k.a. SOC_BLK_XQPORT */
	    ...
	    case SOC_BLK_XPIC:
	    case SOC_BLK_XPORT:
            case SOC_BLK_GXPORT: /* a.k.a. SOC_BLK_XLPORT */
		ctype = SOC_CTR_TYPE_XE;
		break;
	    case SOC_BLK_CMIC:
		if (soc_feature(unit, soc_feature_cpuport_stat_dma)) {
    ...
    for (index = index_min; index <= index_max; index++) {
        if (ctr_reg >= NUM_SOC_REG) {
            ...
            if (non_dma->mem != INVALIDm) {
                ...
                if (PBMP_MEMBER(soc->counter_pbmp, port)) {
                    uint64 regval64;
                    uint32 regval32;

                    rv = soc_reg_read(unit, 
                            non_dma->reg, 
                            soc_reg_addr(unit, non_dma->reg, port, index),
                            &regval64);
                    if (SOC_FAILURE(rv)) {
                        goto counter_set_done;
                    }
...
#define TRIDENT_LEDUP0_PORT_MAX        36
    ...
    if (SOC_IS_TD_TT(unit)) {
        ...
        if (phy_port > TRIDENT_LEDUP0_PORT_MAX) {
            ...
            dram_base = CMIC_LEDUP1_DATA_RAM_BASE;
        } else {
            dram_base = CMIC_LEDUP0_DATA_RAM_BASE;
        }
        ...
        for (i = 0; i < non_dma->num_entries; i++) {
            ...
            if (non_dma->mem != INVALIDm) {
                ...
                if (non_dma->entries_per_port == 1) {
                    ...
                    /* This non-dma counter is per cosq */
                    port_base_cosq_index = 0;
                    for (k = 0; k < SOC_MAX_NUM_PORTS; k++) {
                        if ((i >= port_base_cosq_index) && 
                            (i < port_base_cosq_index + num_cosq[unit][k])) {
                            port = k;
                            cosq = i - port_base_cosq_index;
                            break;
                        }  
                        port_base_cosq_index += num_cosq[unit][k];
                    }
                    if (k == SOC_MAX_NUM_PORTS) {
                        /* Cannot find port and cosq */
                        return;
                    }
                }
                if (PBMP_MEMBER(soc->counter_pbmp, port)) {
                    rv = soc_reg_read(unit, 
                                     non_dma->reg,
                                     soc_reg_addr(unit, non_dma->reg, port, cosq), 
                                     &ctr_new); 
                    if (SOC_FAILURE(rv)) {
                        return;
                    }
            ...
            COUNTER_ATOMIC_BEGIN(s);
            COMPILER_64_ADD_64(soc->counter_sw_val[base_index + i], ctr_diff);
