/*
 * $Id: wc40_config.c,v 1.1.10.3 Broadcom SDK $
...
phy_wc40_config_init(phy_ctrl_t *pc)
{
    ...
    if (SOC_IS_SHADOW(unit)) {
        ...
        /* warpcore intstance number and lane number */
        /*
         * In case of 2 Interlakens
         * Ports 9 thru 11 are assigned to IL0
         * port 12 thru 16 are assigned to IL1
         * WC2 assigned to Ports 9,10
         * WC3 assigned to Ports 11,12
         * WC4 assigned to Ports 13,14
         * WC5 assigned to Ports 15,16
         */
        if ((phy_port > 8) &&
                    ((pc->phy_mode == PHYCTRL_LANE_MODE_CUSTOM1) ||
                     (soc_property_get(unit, spn_BCM88732_2X40_2X40, 0)) ||
                     (soc_property_get(unit, spn_BCM88732_8X10_2X40, 0)) ||
                     (soc_property_get(unit, "IL3125", 0)) ) ) {
            pc->lane_num = ((phy_port - 1) % 2) * 2;
            pc->chip_num = 2 + (phy_port - 9) / 2;
        } else {
            pc->lane_num = (phy_port - 1) % 4;
            pc->chip_num = (phy_port - 1) / 4;
        }
