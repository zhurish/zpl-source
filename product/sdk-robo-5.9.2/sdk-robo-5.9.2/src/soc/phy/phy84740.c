...
#define PHY84740_USER_PRBS_INVERT      (1 << 4)
...
#define PHY84740_RXLOS_MOD_ABS_OVERRIDE_MASK   0xc8c8
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        ...
        if (PHY84740_SINGLE_PORT_MODE(pc)) {
            for (ix = 0; ix < 4; ix++) {
                ...
                SOC_IF_ERROR_RETURN
                    (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC8E4, 
                             PHY84740_RXLOS_MOD_ABS_OVERRIDE_MASK, 
                             PHY84740_RXLOS_MOD_ABS_OVERRIDE_MASK));
            }
        } else {
            /* XXX temp 0xc0c0: RXLOS override: 0x0808 MOD_ABS override */
...
phy_84740_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    for (ix = 0; ix < 4; ix++) {
        ...
        if (enable) {
            SOC_IF_ERROR_RETURN
                (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC8E4,&data));
            ...
            SOC_IF_ERROR_RETURN
                (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC800, &tx_ctrl));
            tx_ctrl &= (1 << 7);
            if (data & (1 << 4)) {
                SOC_IF_ERROR_RETURN
                    (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC800,
                         tx_ctrl? 0:(1 << 7),
                         (1 << 7)));
            }
        } 
        ...
        for (ix = start; ix < end; ix++) {
            ...
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit, pc, 0xca02, 
                          main_tap, PHY84740_PREEMPH_REG_MAIN_TAP_MASK));
        }
        break;
...
_phy_84740_control_edc_mode_set(int unit, soc_port_t port, uint32 value)
{
    ...
    if (PHY84740_SINGLE_PORT_MODE(pc)) {
        for (ix = 0; ix < 4; ix++) {
            ...
            /* XXX temp 0xc0c0: RXLOS override: 0x0808 MOD_ABS override */
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC8E4, 0,
                         PHY84740_RXLOS_MOD_ABS_OVERRIDE_MASK));
        }
    } else {
        /* XXX temp 0xc0c0: RXLOS override: 0x0808 MOD_ABS override */
    ...
    for (ix = 0; ix < 4; ix++) {
        ...
        /* EDC mode programming sequence*/     mask16 = 1 << 9;

        /* induce LOS condition: toggle register bit 0xc800.9 */
        SOC_IF_ERROR_RETURN         
            (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc, 0xC800,&data16));

        /* only change toggled bit 9 */
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit,pc, 0xC800,~data16,mask16));
...
_phy_84740_control_prbs_tx_invert_data_set(int unit, soc_port_t port, int invert)
{
    ...
    if (PHY84740_SINGLE_PORT_MODE(pc)) {
        ...
        data16 = invert? PHY84740_USER_PRBS_INVERT: 0;
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY84740_MMF_PMA_PMD_REG(unit, pc, 
                  PHY84740_USER_PRBS_CONTROL_0_REGISTER, data16,
                           PHY84740_USER_PRBS_INVERT));
    }
    return SOC_E_NONE;
}
...
_phy_84740_control_prbs_tx_invert_data_get(int unit, soc_port_t port, uint32 *value)
{
    ...
    if (PHY84740_SINGLE_PORT_MODE(pc)) {
        ...
        SOC_IF_ERROR_RETURN
            (READ_PHY84740_MMF_PMA_PMD_REG(unit, pc, 
                      PHY84740_USER_PRBS_CONTROL_0_REGISTER, &data16));
        if (data16 & PHY84740_USER_PRBS_INVERT) {
            *value = TRUE;
        } else {
            *value = FALSE;
        }
...
phy_84740_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    if (pif == SOC_PORT_IF_CR4 || pif == SOC_PORT_IF_XLAUI) {
        ...
        for (lane = 0; lane < 4; lane++) {
            ...
            SOC_IF_ERROR_RETURN
                (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC8E4,
                             &rxlos_reg[lane]));
        }
        ...
        for (lane = 0; lane < 4; lane++) {
            ...
            SOC_IF_ERROR_RETURN
                (WRITE_PHY84740_MMF_PMA_PMD_REG(unit,pc,0xC8E4,
                             rxlos_reg[lane]));
        }
