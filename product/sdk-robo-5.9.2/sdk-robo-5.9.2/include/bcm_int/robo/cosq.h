/*
 * $Id: cosq.h,v 1.3 Broadcom SDK $Copyright: Copyright 2011 Broadcom Corporation.
...
#define _BCM_COS_DEFAULT(unit) \
        (SOC_IS_TB(unit) ? BCM_COS_COUNT: BCM_COS_DEFAULT)

#define _BCM_PRIO_MAX(unit) \
        (SOC_IS_TB(unit) ? 15: BCM_PRIO_MAX)
                                          
#define _BCM_COSQ_PRIO_VALID(unit, prio) \
        (SOC_IS_TB(unit) ? ((prio) >= 0 && (prio < 16)) : \
                           ((prio) >= 0 && (prio < 8)))
