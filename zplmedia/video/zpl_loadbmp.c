...
static zpl_uint16 OSD_MAKECOLOR_U16(zpl_uint8 r, zpl_uint8 g, zpl_uint8 b, OSD_COMP_INFO compinfo)
{
    zpl_uint8 r1, g1, b1;
    zpl_uint16 pixel = 0;
    zpl_uint32 tmp = 15;
...
zpl_int32 GetBmpInfo(const char* filename,     OSD_BITMAPFILEHEADER*  pBmpFileHeader
...
static int LoadBMP(const char* filename, OSD_LOGO_T* pVideoLogo)
{
    ...
    pVideoLogo->width = (zpl_uint16)bmpInfo.bmiHeader.biWidth;
    pVideoLogo->height = (zpl_uint16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
    w = pVideoLogo->width;
    h = pVideoLogo->height;
...
#endif
    /* RGB8888 or RGB1555 */
    pOrigBMPBuf = (zpl_uint8*)malloc(h * stride);
    if (NULL == pOrigBMPBuf)
    {
            ...
            if (Bpp == 3)
            {
                switch (enFmt)
                {
                    ...
                    case OSD_COLOR_FMT_RGB4444:
                        ...
                        pDst = (zpl_uint16*)(pRGBBuf + i * pVideoLogo->stride + j * 2);
                        r = *(pStart);
                        g = *(pStart + 1);
                        b = *(pStart + 2);
...
static int LoadBMPCanvas(const char* filename, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    FILE* pFile;
    zpl_uint16  i, j;
    ...
    w = (zpl_uint16)bmpInfo.bmiHeader.biWidth;
    h = (zpl_uint16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));
...
static int LoadCanvasEx(const char* filename, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    char* ext = GetExtName((char*)filename);
