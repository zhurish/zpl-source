        ...
        case NALU_TYPE_SEI:
            if (extradata->fSEI == NULL)
            {
                ...
                if (extradata->fSEI)
                {
                    ...
                    memcpy(extradata->fSEI, nalu->pstart + nalu->hdr_len, nalu->len);
                }
            }
            break;
        case NALU_TYPE_SPS:
            if (extradata->fSPS == NULL)
            {
                ...
                if (extradata->fSPS)
                {
                    ...
                    memcpy(extradata->fSPS, nalu->pstart+nalu->hdr_len, nalu->len);
                    extradata->fSPSSize = nalu->len;

                    if(zpl_media_channel_get_profileLevelId(nalu->pstart+nalu->hdr_len, nalu->len,
                                                            tmp, sizeof(tmp)) >= 4)
        ...
        case NALU_TYPE_PPS:
            if (extradata->fPPS == NULL)
            {
                ...
                if (extradata->fPPS)
                {
                    ...
                    memcpy(extradata->fPPS, nalu->pstart + nalu->hdr_len, nalu->len);
                }
            }
            break;
        case NALU_TYPE_AUD:
