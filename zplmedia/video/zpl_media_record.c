...
static int media_buffer_data_record_handle(zpl_media_event_t *event)
{
    ...
    zpl_media_buffer_data_t *bufdata = zpl_media_buffer_get(record->buffer_queue);
    while(bufdata)
    {
        //memcpy(&record->record_file->packet.data, bufdata, sizeof(zpl_media_buffer_data_t));
        ...
        if(bufdata->buffer_type == ZPL_MEDIA_VIDEO)
        {
            ...
            record->record_file->tmphead.video_len = bufdata->buffer_len;
            record->record_file->tmphead.audio_len = 0;
        }
        else if(bufdata->buffer_type == ZPL_MEDIA_AUDIO)
        {
            record->record_file->tmphead.video = 0;
            record->record_file->tmphead.audio = 1;
            record->record_file->tmphead.video_len = 0;
            record->record_file->tmphead.audio_len = bufdata->buffer_len;
        } 
        zpl_media_file_write(record->record_file, bufdata);
        zpl_media_buffer_finsh(record->buffer_queue, bufdata);
        bufdata = zpl_media_buffer_get(record->buffer_queue);
    }
    return OK;
}
        ...
        const zpl_media_buffer_data_t *bufdata,  void *pVoidUser)
{
    ...
    {
        zpl_media_buffer_data_t *rdata = zpl_media_buffer_data_clone(record->buffer_queue, bufdata);
        if(rdata)
        {
            //zpl_media_buffer_data_copy(&record->record_file.packet.data, bufdata);
            zpl_media_buffer_put(record->buffer_queue, rdata);
...
int zpl_media_channel_record_enable(zpl_media_channel_t *mediachn, zpl_bool enable)
{
    ...
    if(enable == zpl_true)
    {
		...
		record->buffer_queue = zpl_media_buffer_create(ZPL_MEDIA_BUFFER_FRAME_CACHESIZE, zpl_false, NULL);
        ...
        {
            zpl_media_buffer_destroy(record->buffer_queue);
            free(record);
            return ERROR;
        }
