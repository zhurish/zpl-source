...
void md_stream_BctbxLogFunc(const char *domain, BctbxLogLevel lev, const char *fmt, va_list args)
{
	...
	vzlog(NULL, ZLOG_VOIP, priority, fmt, args);
}
...
int voip_mediastream_show_config(void * p, struct vty *vty)
{
	...
	if(mm)
	{
		...
		vty_out(vty, " voip stream el-force               : %d%s", mm->el_force, VTY_NEWLINE);
		vty_out(vty, " voip stream el-thres               : %d%s", mm->el_thres, VTY_NEWLINE);
		vty_out(vty, " voip stream el-transmit-thres      : %d%s", mm->el_transmit_thres, VTY_NEWLINE);
		...
		vty_out(vty, " voip stream ng-floorgain           : %d%s", mm->ng_floorgain, VTY_NEWLINE);
		vty_out(vty, " voip stream ng-threshold           : %d%s", mm->ng_threshold, VTY_NEWLINE);
	}
	return OK;
}
#endif /* PL_VOIP_MEDIASTREAM */
