 ...
 * mqtt 代理服务器用户名
 */
int mqtt_username_api(struct mqtt_app_config *cfg, const ospl_char * username);
/*
 * mqtt 代理服务器用户密码
 */
int mqtt_password_api(struct mqtt_app_config *cfg, const ospl_char * password);
...
int mqtt_version_api(struct mqtt_app_config *cfg,
		const ospl_uint32 protocol_version);
...
int mqtt_pub_repeat_count_delay_api(struct mqtt_app_config *cfg,
		const ospl_uint32 repeat_count, const ospl_float repeat_delay);
