...
int socks5__send(struct mosquitto *mosq)
{
	...
	struct ipstack_in_addr addr_ipv4;
	struct ipstack_in6_addr addr_ipv6;
	...
	if(state == mosq_cs_socks5_new){
		...
		ipv4_pton_result = ipstack_inet_pton(IPSTACK_AF_INET, mosq->host, &addr_ipv4);
		ipv6_pton_result = ipstack_inet_pton(IPSTACK_AF_INET6, mosq->host, &addr_ipv6);
