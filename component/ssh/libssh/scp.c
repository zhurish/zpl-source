...
int ssh_scp_pull_request(ssh_scp scp)
{
  ...
  if(p!=NULL)
	  *p='\0';
  SSH_LOG(SSH_LOG_PROTOCOL,"Received SCP request: '%s'",tmpbuf);
  switch(tmpbuf[0]){
    case 'C':
      /* File */
    case 'D':
    ...
    case 'T':
      /* Timestamp */
    default:
      ssh_set_error(scp->session,SSH_FATAL,"Unhandled message: (%d)%s",tmpbuf[0],tmpbuf);
      return SSH_ERROR;
  }
  ...
  SAFE_FREE(name);
  SAFE_FREE(mode);
  ssh_set_error(scp->session,SSH_FATAL,"Parsing error while parsing message: %s",tmpbuf);
  return SSH_ERROR;
}
...
int ssh_scp_deny_request(ssh_scp scp, const char *reason){
  ...
  snprintf(tmpbuf,sizeof(tmpbuf),"%c%s\n",2,reason);
  err=ssh_channel_write(scp->channel,tmpbuf,strlen(tmpbuf));
  if(err==SSH_ERROR) {
    return SSH_ERROR;
  }
  else {
...
int ssh_scp_accept_request(ssh_scp scp){
  ...
  err=ssh_channel_write(scp->channel,tmpbuf,1);
  if(err==SSH_ERROR) {
    return SSH_ERROR;
  }
