...
--------------------------------------------------------------------------------
*/
#pragma GCC diagnostic ignored "-Werror=shadow"
...
tcp_gethostbyname(struct soap *soap, const char *addr, struct hostent *hostent, struct in_addr *inaddr)
{
  ...
  if (inaddr)
  {
    soap_int32 iadd = -1;
#ifdef AS400
    iadd = ipstack_inet_addr((void*)addr);
#else
    iadd = ipstack_inet_addr((char*)addr);
#endif
    if (iadd != -1)
    {
...
{
#ifdef WITH_IPV6
  struct ipstack_addrinfo hints, *res, *ressave;
#endif
  SOAP_SOCKET sk;
  int err = 0;
#ifndef WITH_LEAN
...
{
#if defined(WITH_IPV6)
  struct ipstack_addrinfo *ipstack_addrinfo = NULL;
  struct ipstack_addrinfo hints;
  struct ipstack_addrinfo res;
  int err;
  int set = 1;
  int unset = 0;
  ...
  if (err || !ipstack_addrinfo)
  {
    (void)soap_set_receiver_error(soap, SOAP_GAI_STRERROR(err), "getaddrinfo failed in soap_bind()", SOAP_TCP_ERROR);
    if (ipstack_addrinfo)
      freeaddrinfo(ipstack_addrinfo);
    return SOAP_INVALID_SOCKET;
  }
  res = *ipstack_addrinfo;
  if (soap_memcpy((void*)&soap->peer.storage, sizeof(soap->peer.storage), (const void*)ipstack_addrinfo->ai_addr, ipstack_addrinfo->ai_addrlen))
  {
    freeaddrinfo(ipstack_addrinfo);
    soap->error = SOAP_EOM;
    return SOAP_INVALID_SOCKET;
  }
  soap->peerlen = ipstack_addrinfo->ai_addrlen;
...
#ifdef WITH_IPV6
      char port[16];
      struct ipstack_addrinfo *res = NULL;
      struct ipstack_addrinfo hints;
