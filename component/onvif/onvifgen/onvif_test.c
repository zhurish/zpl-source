...
#define IPV4_ADDRESS "192.168.182.129"
#define WSDL_URL "http://192.168.182.129/onvif/ver10/device/wsdl/devicemgmt.wsdl"
#define SERVICE_URL "http://192.168.182.129/onvif/services"
...
void _hw_mxml_load() 
{
	...
	fp = fopen("/home/zhurish/workspace/planform/source/component/onvif/onvifgen/xml/PTZBinding.GetNode.res.xml", "r");
	tree = mxmlLoadFile(NULL, fp, MXML_OPAQUE_CALLBACK);
	fclose(fp);	
	if(tree)
	{
		mxml_node_t *node = mxmlFindElement(tree, tree, "tptz:PTZNode", "xsi:type", NULL, MXML_DESCEND);
		if(node)
		{
			printf("tptz:PTZNode -> xsi:type:%s\n",mxmlElementGetAttr(node,"xsi:type"));
			printf("tptz:PTZNode -> token:%s\n",mxmlElementGetAttr(node,"token"));
			printf("tptz:PTZNode -> FixedHomePosition:%s\n",mxmlElementGetAttr(node,"FixedHomePosition"));
			printf("tptz:PTZNode -> GeoMove:%s\n",mxmlElementGetAttr(node,"GeoMove"));

			mxml_node_t *author = mxmlFindElement(node, tree, "tt:Name", "xsi:type",NULL,MXML_DESCEND);
			if(author)
			{
				printf("tt:Name -> xsi:type:%s\n",mxmlElementGetAttr(author, "xsi:type"));
			}
			author = mxmlFindElement(node, tree, "tt:SupportedPTZSpaces", "xsi:type",NULL,MXML_DESCEND);
			if(author)
			{
				printf("tt:SupportedPTZSpaces -> xsi:type:%s\n",mxmlElementGetAttr(author, "xsi:type"));
			}
		}
		mxmlDelete(tree);
	}
