...
typedef struct _modbus_mapping_t {
    int nb_bits;					//线圈状态寄存器数量
    int start_bits;					//线圈状态寄存器起始地址
    int nb_input_bits;				//离散输入寄存器数量
    int start_input_bits;			//离散输入寄存器起始地址
    int nb_input_registers;			//输入寄存器数量
    int start_input_registers;		//输入寄存器起始地址
    int nb_registers;				//保持寄存器数量
    int start_registers;			//保持寄存器起始地址
    uint8_t *tab_bits;				//指向线圈状态寄存器值
    uint8_t *tab_input_bits;		//指向离散输入寄存器值
    uint16_t *tab_input_registers;	//指向输入寄存器值
    uint16_t *tab_registers;		//指向保持寄存器值
} modbus_mapping_t;
...
MODBUS_API void modbus_version(char *ver);
