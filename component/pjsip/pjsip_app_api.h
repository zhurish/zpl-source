...
#define PJSIP_ENABLE_DEFAULT			ospl_true
#define PJSIP_PORT_DEFAULT				5060
#define PJSIP_PROXY_PORT_DEFAULT		5060
#define PJSIP_100_REL_DEFAULT			ospl_false
...
typedef struct pjsip_username_s
{
	...
	ospl_bool				is_default;
	ospl_uint8				id;
	ospl_bool				is_current;
}pjsip_username_t;
...
{
	ospl_bool				sip_enable;
	//just for app
	ospl_bool				sip_active_standby;
	ospl_bool				sip_multi_user ;

	ospl_bool				sip_proxy_enable;
	ospl_uint16				sip_server_cnt;
	ospl_uint16				sip_proxy_cnt;
	...
	//ospl_uint16				sip_reg_timeout;		//Optional registration interval (default %d) useing sip_expires
	ospl_uint16				sip_rereg_delay;		//Optional auto retry registration interval (default %d)
	...
	ospl_bool				sip_publish;			//Send presence PUBLISH for this account
	ospl_bool				sip_mwi;				//Subscribe to message summary/waiting indication
	ospl_bool				sip_ims_enable;			//Enable 3GPP/IMS related settings on this account
	...
	ospl_bool				sip_100_rel;
	pjsip_timer_t		sip_timer;				//Use SIP session timers? (default=1) 0:inactive, 1:optional, 2:mandatory, 3:always"
	ospl_uint16				sip_timer_sec;
	ospl_uint16				sip_outb_rid;			//Set SIP outbound reg-id (default:1)
	ospl_bool				sip_auto_update_nat;	//Where N is 0 or 1 to enable/disable SIP traversal behind symmetric NAT (default 1)
	ospl_bool				sip_stun_disable;		//Disable STUN for this account
	ospl_uint16				sip_expires;


	//Transport Options:
	ospl_bool				sip_ipv6_enable;
	ospl_bool				sip_set_qos;			//Enable QoS tagging for SIP and media.
	...
	//Audio Options:
	ospl_uint16				sip_clock_rate;
	ospl_uint16				sip_snd_clock_rate;
	ospl_bool				sip_stereo;
	ospl_bool				sip_audio_null;
	char				sip_play_file[PJSIP_FILE_MAX];
	char				sip_play_tone[PJSIP_DATA_MAX];
	ospl_bool				sip_auto_play;
	ospl_bool				sip_auto_loop;
	ospl_bool				sip_auto_conf;
	char				sip_rec_file[PJSIP_FILE_MAX];
	ospl_uint16				sip_quality;							//Specify media quality (0-10, default=2)
	ospl_uint16				sip_ptime;								//Override codec ptime to MSEC (default=specific)
	ospl_bool				sip_no_vad;								//Disable VAD/silence detector (default=vad enabled)
	ospl_uint16				sip_echo_tail;							//Set echo canceller tail length
	pjsip_echo_mode_t	sip_echo_mode;							//Select echo canceller algorithm (0=default, 1=speex, 2=suppressor, 3=WebRtc)
	ospl_uint16				sip_ilbc_mode;							//Set iLBC codec mode (20 or 30, default is 20)

	ospl_int32				sip_capture_dev;
	ospl_int32				sip_playback_dev;
	ospl_uint32				sip_capture_lat;						//Audio capture latency, in ms
	ospl_uint32				sip_playback_lat;						//Audio capture latency, in ms
	ospl_int32				sip_snd_auto_close;						//Auto close audio device when idle for N secs (default=1)
																//		Specify N=-1 to disable this feature. Specify N=0 for instant close when unused.
	ospl_bool				sip_notones;							//Disable audible tones
	ospl_int32				sip_jb_max_size;						//Specify jitter buffer maximum size, in frames (default=-1)");

#if PJSUA_HAS_VIDEO
	//Video Options:
	ospl_bool				sip_video;
	ospl_int32				sip_vcapture_dev;
	ospl_int32				sip_vrender_dev;
	char				sip_play_avi[PJSIP_FILE_MAX];
	ospl_bool				sip_auto_play_avi;
#endif
	//Media Transport Options:
	ospl_bool				sip_ice;				//Enable ICE (default:no)
	ospl_uint32				sip_ice_regular;		//Use ICE regular nomination (default: aggressive)
	ospl_uint16				sip_ice_max_host;		//Set maximum number of ICE host candidates
	ospl_bool				sip_ice_nortcp;			//Disable RTCP component in ICE (default: no)
	ospl_uint16				sip_rtp_port;
	ospl_uint16				sip_rx_drop_pct;		//Drop PCT percent of RX RTP (for pkt lost sim, default: 0)
	ospl_uint16				sip_tx_drop_pct;		//Drop PCT percent of TX RTP (for pkt lost sim, default: 0)
	...
	ospl_bool				sip_rtcp_mux;			//Enable RTP & RTCP multiplexing (default: no)
	...
	void				*buddy_list;
	//User Agent options:
	ospl_uint16				sip_auto_answer_code;	//Automatically answer incoming calls with code (e.g. 200)
	ospl_uint16				sip_max_calls;			//Maximum number of concurrent calls (default:4, max:255)
	ospl_uint16				sip_thread_max;			//Number of worker threads (default:1)
	ospl_uint32				sip_duration;			//Set maximum call duration (default:no limit)
	ospl_uint16				sip_norefersub;			//Suppress event subscription when transferring calls

	ospl_uint16				sip_use_compact_form;	//Minimize SIP message size
	ospl_uint16				sip_no_force_lr;		//Allow strict-route to be used (i.e. do not force lr)
	...
	ospl_uint32						debug_level;
	ospl_bool					debug_detail;
