...
#define PJSIP_100_REL_DEFAULT			FALSE
...
typedef struct pl_pjsip_s
{
	...
	u_int16				sip_server_cnt;
	u_int16				sip_proxy_cnt;
	...
	//u_int16				sip_reg_timeout;		//Optional registration interval (default %d) useing sip_expires
	u_int16				sip_rereg_delay;		//Optional auto retry registration interval (default %d)
	...
	BOOL				sip_publish;			//Send presence PUBLISH for this account
	BOOL				sip_mwi;				//Subscribe to message summary/waiting indication
	BOOL				sip_ims_enable;			//Enable 3GPP/IMS related settings on this account
	...
	u_int16				sip_timer_sec;
	u_int16				sip_outb_rid;			//Set SIP outbound reg-id (default:1)
	BOOL				sip_auto_update_nat;	//Where N is 0 or 1 to enable/disable SIP traversal behind symmetric NAT (default 1)
	BOOL				sip_stun_disable;		//Disable STUN for this account
	u_int16				sip_expires;


	//Transport Options:
	BOOL				sip_ipv6_enable;
	BOOL				sip_set_qos;			//Enable QoS tagging for SIP and media.
	...
	u_int32				sip_source_interface;	//Bind transports to this IP interface
	BOOL				sip_noudp;
	BOOL				sip_notcp;
	...
	u_int16				sip_quality;							//Specify media quality (0-10, default=2)
	u_int16				sip_ptime;								//Override codec ptime to MSEC (default=specific)
	BOOL				sip_no_vad;								//Disable VAD/silence detector (default=vad enabled)
	u_int16				sip_echo_tail;							//Set echo canceller tail length
	pjsip_echo_mode_t	sip_echo_mode;							//Select echo canceller algorithm (0=default, 1=speex, 2=suppressor, 3=WebRtc)
	u_int16				sip_ilbc_mode;							//Set iLBC codec mode (20 or 30, default is 20)

	s_int32				sip_capture_dev;
	s_int32				sip_playback_dev;
	u_int32				sip_capture_lat;						//Audio capture latency, in ms
	u_int32				sip_playback_lat;						//Audio capture latency, in ms
	s_int32				sip_snd_auto_close;						//Auto close audio device when idle for N secs (default=1)
																//		Specify N=-1 to disable this feature. Specify N=0 for instant close when unused.
	BOOL				sip_notones;							//Disable audible tones
	s_int32				sip_jb_max_size;						//Specify jitter buffer maximum size, in frames (default=-1)");

#if PJSUA_HAS_VIDEO
	//Video Options:
	BOOL				sip_video;
	s_int32				sip_vcapture_dev;
	s_int32				sip_vrender_dev;
	char				sip_play_avi[PJSIP_FILE_MAX];
	BOOL				sip_auto_play_avi;
#endif
	//Media Transport Options:
	BOOL				sip_ice;				//Enable ICE (default:no)
	u_int32				sip_ice_regular;		//Use ICE regular nomination (default: aggressive)
	u_int16				sip_ice_max_host;		//Set maximum number of ICE host candidates
	BOOL				sip_ice_nortcp;			//Disable RTCP component in ICE (default: no)
	u_int16				sip_rtp_port;
	u_int16				sip_rx_drop_pct;		//Drop PCT percent of RX RTP (for pkt lost sim, default: 0)
	u_int16				sip_tx_drop_pct;		//Drop PCT percent of TX RTP (for pkt lost sim, default: 0)
	...
	BOOL				sip_rtcp_mux;			//Enable RTP & RTCP multiplexing (default: no)
	...
	void				*buddy_list;
	//User Agent options:
	u_int16				sip_auto_answer_code;	//Automatically answer incoming calls with code (e.g. 200)
	u_int16				sip_max_calls;			//Maximum number of concurrent calls (default:4, max:255)
	u_int16				sip_thread_max;			//Number of worker threads (default:1)
	u_int32				sip_duration;			//Set maximum call duration (default:no limit)
	u_int16				sip_norefersub;			//Suppress event subscription when transferring calls

	u_int16				sip_use_compact_form;	//Minimize SIP message size
	u_int16				sip_no_force_lr;		//Allow strict-route to be used (i.e. do not force lr)
