 ...
 * 		    										开始检测（事件）
 * 													   |
 * 			模块插入，邦定到固定的 serial-profile   		   |		模块删除，解除serial-profile的邦定
 * 							|													|
 * 							|													|
 * 						模块插入事件			   								模块删除事件
 *
 ...
 *															 |
 *															 |
 *														开始事件调度： 若不是模块删除事件，确认是否打开AT Channel
 *																	 模块删除事件：关闭通道，清空serial邦定的 模块
 *															 |
 *															 |
 *												-------------|
 *												|
 *												|
 *												|
 *												|-----------模块插入： 更新 modem-profile，serial-profile邦定的
 *												|                       模块数据结构，确认AT通道是否打开(发送AT命令)，
 *												|                    查询模块信息：商家名称，模块名称，模块版本信息
 *												|                    发送 init 事件
 *												|
 *												|-----------模块初始化事件： 初始化模块，打开模块功能，确认模块功能打开
 *												|							获取模块序列号等信息，检测USIM卡是否插入
 *												|                     发送USIM卡插入事件
 *												|
 *												|
 *												|-----------USIM卡插入事件： 检测USIM卡是否插入，检测USIM卡是否准备好
 *												|                            USIM卡准备好，发送网络设置事件
 *												|                            发送 USIM卡插入事件
 *												|
 *												|
 *												|-----------网络设置事件：   设置网络PDP上下文，APN等信息，查询网络信息
 *												|                            查询信号等信息
 *												|						   发送网络上线事件（拨号）
 *												|
 *												|
 *												|-----------网络拨号事件：   确认DHCP，PPP拨号，
 *												|                          DHCP拨号： DHCP拨号，确认是否获取IP，激活网络接口
 *												|									 处罚DHCP获取IP地址
 *												|									发送定时事件
 *												|
 *												|                          PPP拨号： 设置更新PPP配置参数，生成脚步文件等信息
 *												|									 PPP拨号，激活网络
 *												|
 *												|-----------定时事件：	  确认定时时间是否到达，定时事件：查询网络信息，信号强度
 *												|													USIM卡信息
 *												|                         发送定时事件
 *												|
 *												|-----------模块删除事件：   关闭网络，解除serial-profile邦定的模块，关闭AT通道
 *												|
 *												|
 *												|-----------USIM卡拔除事件： 关闭网络，清除网络，USIM卡状态，发送USIM卡插入事件
 *												|
 *												|
 *												|-----------模块探测事件：	   检测是否有模块插入，删除，更新AT通道信息，发送相关事件
 *												|
 *												|
 *												|-------------|
 *
