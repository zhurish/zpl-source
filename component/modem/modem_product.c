...
{
	assert(driver);
	int i = modem_usb_driver_lookup(driver);
	if(i == ERROR)
		return -1;
	if(i >= 0)
	{
...
static int modem_usb_driver_ttyusb_sort(tty_devname *arr, int sz)
{
    int i = 0;
    int j = 0;
    tty_devname tmp;
...
//static modem_usb_driver driver_tmp;
...
static int usb_event_isok_string(char *buf, int len)
{
	/*
...
static int usb_vendor_product_read(modem_usb_driver *driver, char *buf, int len)
{
...
static int usb_event_key_detection(modem_usb_driver *driver, char *buf, int len)
{
	////add@/devices/platform/101c0000.ehci/usb2/2-1/2-1:1.0/ttyUSB0
	return modem_usb_key_detection(buf, driver->usbkey);
#if 0
	if(os_strstr(buf, "usb"))
	{
		int offset = 0;
		char usbkey[USB_SERIAL_NAME_MAX];
		char *brk = os_strstr(buf, "usb");
		if(!brk)
			return ERROR;

		offset = strchr_step(brk, '/', 2);//:usb2/2-1/2-1:1.0/ttyUSB0
		if(offset)
		{
			brk += (offset + 1);
			if(!brk)
		...
		if(offset)
		{
			os_memset(usbkey, 0, sizeof(usbkey));
			os_strncpy(usbkey, brk, offset); //2-1:1
			//usbkey[os_strlen(usbkey)-1] = '\0';
			os_strcpy(driver->usbkey, usbkey);
...
static int usb_event_add_detection(modem_usb_driver *driver, char *buf, int len)
{
...
static int usb_event_del_detection(char *buf, int len)
{
...
static int usb_event_handle_detection(char *buf, int len)
{
	if(os_strstr(buf, "add@/"))
	{
		modem_usb_driver driver;
...
#ifdef USB_KERNEL_UEVENT
	return usb_event_socket_handle(2);
#else
