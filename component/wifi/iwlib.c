...
int iw_get_range_info(int skfd, const char * ifname, iwrange * range)
{
	...
	else
	{
		...
		memcpy((char *) range + iwr_off(avg_qual), buffer + iwr15_off(avg_qual),
...
int iw_set_basic_config(int skfd, const char * ifname, wireless_config * info)
{
	...
	if (info->has_key)
	{
		zpl_uint32 flags = info->key_flags;
 ...
 * libm (math library).
 * In this case, please use enable ZPL_BUILD_NOLIBM in the makefile
 *
...
void iw_float2freq(double in, iwfreq * out)
{
	...
	out->e = (zpl_int16) (floor(log10(in)));
	if (out->e > 8)
	{
 ...
 * Output a frequency with proper scaling
 */
void iw_print_freq(char * buffer, int buflen, double freq, int channel,
		int freq_flags)
{
	...
	else
	{
		...
		if (channel >= 0)
			snprintf(buffer, buflen, "Frequency%c%s (Channel %d)", sep, vbuf,
					channel);
		else
 ...
 */
int iw_channel_to_freq(int channel, double * pfreq,
	...
	else
	{
		...
		while (fgets(buf, 255, f))
		{
					...
					&& bp[strlen(ifname)] == ':')
			{
				...
				bp = strtok(bp, " ");
				sscanf(bp, "%X", &t);
				stats->status = (zpl_uint16) t;
				/* -- link quality -- */
				bp = strtok(NULL, " ");
				if (strchr(bp, '.') != NULL)
					stats->qual.updated |= 1;
				sscanf(bp, "%d", &t);
				stats->qual.qual = (zpl_uint8) t;
				/* -- signal level -- */
				bp = strtok(NULL, " ");
				if (strchr(bp, '.') != NULL)
					stats->qual.updated |= 2;
				sscanf(bp, "%d", &t);
				stats->qual.level = (zpl_uint8) t;
				/* -- noise level -- */
				bp = strtok(NULL, " ");
				if (strchr(bp, '.') != NULL)
					stats->qual.updated += 4;
				sscanf(bp, "%d", &t);
				stats->qual.noise = (zpl_uint8) t;
				/* -- discarded packets -- */
				bp = strtok(NULL, " ");
 ...
 */
void iw_print_stats(char * buffer, int buflen, const iwqual * qual,
		const iwrange * range, int has_range)
{
	int len;
 ...
 */
void iw_print_key(char * buffer, int buflen, const zpl_uint8 * key, /* Must be unsigned */
 ...
 */
static int iw_pass_key(const char * input, zpl_uint8 * key)
{
	input = input;
	key = key;
 ...
 */
int iw_in_key(const char * input, zpl_uint8 * key)
{
	int keylen = 0;
	...
	else
	{
		...
		zpl_uint8 out[IW_ENCODING_TOKEN_MAX];
		...
		while (*p != '\0')
		{
			...
			if (count == 2)
				...
				templ = temph;
			out[keylen++] = (zpl_uint8) (templ & 0xFF);
 ...
 */
void iw_print_pm_value(char * buffer, int buflen, int value, int flags,
		int we_version)
{
	/* Check size */
	if (buflen < 25)
 ...
 */
void iw_print_retry_value(char * buffer, int buflen, int value, int flags,
		int we_version)
{
	/* Check buffer size */
	if (buflen < 20)
	...
	{
		strcpy(buffer, " zpl_int16"); /* Size checked */
		buffer += 6;
	}
 ...
 */
void iw_print_timeval(char * buffer, int buflen, const struct timeval * timev,
...
int iw_check_if_addr_type(int skfd, const char * ifname)
{
	...
	printf("Interface : %d - 0x%lX\n", ifr.ifr_addr.sa_family,
			*((zpl_ulong *) ifr.ifr_addr.sa_data));
#endif
 ...
 */
char *
iw_mac_ntop(const zpl_uint8 * mac, int maclen, char * buf, int buflen)
{
	int i;
...
{
	sprintf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", eth->ether_addr_octet[0],
 ...
 */
char *
iw_sawap_ntop(const struct sockaddr * sap, char * buf)
{
 ...
 * Return address size.
 */
int iw_mac_aton(const char * orig, zpl_uint8 * mac, int macmax)
{
	const char * p = orig;
	int maclen = 0;
	...
	while (*p != '\0')
	{
		...
		templ |= temph << 4;
		mac[maclen++] = (zpl_uint8) (templ & 0xFF);
...
int iw_in_addr(int skfd, const char * ifname, char * bufp, struct sockaddr *sap)
{
	...
	if (strchr(bufp, ':') == NULL)
	{
		...
		memcpy((char *) &(arp_query.arp_pa), (char *) &if_address,
		...
		memcpy((char *) sap, (char *) &(arp_query.arp_ha),
				sizeof(struct sockaddr));
...
int iw_get_priv_size(int args)
{
	...
	zpl_uint32 type = (args & IW_PRIV_TYPE_MASK) >> 12;
...
{ .header_type = IW_HEADER_TYPE_NULL, }, [SIOCGIWNAME - SIOCIWFIRST] =
{ .header_type = IW_HEADER_TYPE_CHAR, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWNWID - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, .flags = IW_DESCR_FLAG_EVENT, },
		[SIOCGIWNWID - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWFREQ - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_FREQ, .flags = IW_DESCR_FLAG_EVENT, },
		[SIOCGIWFREQ - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_FREQ, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWMODE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_UINT, .flags = IW_DESCR_FLAG_EVENT, },
		[SIOCGIWMODE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_UINT, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWSENS - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWSENS - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWRANGE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_NULL, }, [SIOCGIWRANGE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				sizeof(struct iw_range), .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWPRIV - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_NULL, }, [SIOCGIWPRIV - SIOCIWFIRST] =
		{ /* (handled directly by us) */
		.header_type = IW_HEADER_TYPE_NULL, }, [SIOCSIWSTATS - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_NULL, }, [SIOCGIWSTATS - SIOCIWFIRST] =
		{ /* (handled directly by us) */
		.header_type = IW_HEADER_TYPE_NULL, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWSPY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size =
				sizeof(struct sockaddr), .max_tokens = IW_MAX_SPY, },
		[SIOCGIWSPY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size =
				sizeof(struct sockaddr) + sizeof(struct iw_quality),
				.max_tokens = IW_MAX_SPY, }, [SIOCSIWTHRSPY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size =
				sizeof(struct iw_thrspy), .min_tokens = 1, .max_tokens = 1, },
		[SIOCGIWTHRSPY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size =
				sizeof(struct iw_thrspy), .min_tokens = 1, .max_tokens = 1, },
		[SIOCSIWAP - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_ADDR, }, [SIOCGIWAP - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_ADDR, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWMLME - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .min_tokens =
				sizeof(struct iw_mlme), .max_tokens = sizeof(struct iw_mlme), },
		[SIOCGIWAPLIST - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size =
				sizeof(struct sockaddr) + sizeof(struct iw_quality),
				.max_tokens = IW_MAX_AP, .flags = IW_DESCR_FLAG_NOMAX, },
		[SIOCSIWSCAN - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .min_tokens = 0,
				.max_tokens = sizeof(struct iw_scan_req), }, [SIOCGIWSCAN
				- SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_SCAN_MAX_DATA, .flags = IW_DESCR_FLAG_NOMAX, }, [SIOCSIWESSID
				- SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_ESSID_MAX_SIZE + 1, .flags = IW_DESCR_FLAG_EVENT, },
		[SIOCGIWESSID - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_ESSID_MAX_SIZE + 1, .flags = IW_DESCR_FLAG_DUMP, },
		[SIOCSIWNICKN - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_ESSID_MAX_SIZE + 1, }, [SIOCGIWNICKN - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_ESSID_MAX_SIZE + 1, }, [SIOCSIWRATE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWRATE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWRTS - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWRTS - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWFRAG - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWFRAG - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWTXPOW - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWTXPOW - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWRETRY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWRETRY - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWENCODE - SIOCIWFIRST
				] =
				{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1,
						.max_tokens = IW_ENCODING_TOKEN_MAX, .flags =
								IW_DESCR_FLAG_EVENT | IW_DESCR_FLAG_RESTRICT, },
		[SIOCGIWENCODE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_ENCODING_TOKEN_MAX, .flags = IW_DESCR_FLAG_DUMP
				| IW_DESCR_FLAG_RESTRICT, }, [SIOCSIWPOWER - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWPOWER - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWMODUL - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWMODUL - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWGENIE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_GENERIC_IE_MAX, }, [SIOCGIWGENIE - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
				IW_GENERIC_IE_MAX, }, [SIOCSIWAUTH - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCGIWAUTH - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_PARAM, }, [SIOCSIWENCODEEXT
				- SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .min_tokens =
				sizeof(struct iw_encode_ext), .max_tokens =
				sizeof(struct iw_encode_ext) +
				IW_ENCODING_TOKEN_MAX, }, [SIOCGIWENCODEEXT - SIOCIWFIRST] =
		{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .min_tokens =
				sizeof(struct iw_encode_ext), .max_tokens =
				sizeof(struct iw_encode_ext) +
				IW_ENCODING_TOKEN_MAX, }, [SIOCSIWPMKSA - SIOCIWFIRST
				] =
				{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1,
						.min_tokens = sizeof(struct iw_pmksa), .max_tokens =
								sizeof(struct iw_pmksa), }, };
static const zpl_uint32 standard_ioctl_num = (sizeof(standard_ioctl_descr)
...
{ .header_type = IW_HEADER_TYPE_ADDR, }, [IWEVQUAL - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_QUAL, }, [IWEVCUSTOM - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		IW_CUSTOM_MAX, }, [IWEVREGISTERED - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_ADDR, }, [IWEVEXPIRED - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_ADDR, }, [IWEVGENIE - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		IW_GENERIC_IE_MAX, }, [IWEVMICHAELMICFAILURE - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		sizeof(struct iw_michaelmicfailure), }, [IWEVASSOCREQIE - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		IW_GENERIC_IE_MAX, }, [IWEVASSOCRESPIE - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		IW_GENERIC_IE_MAX, }, [IWEVPMKIDCAND - IWEVFIRST] =
{ .header_type = IW_HEADER_TYPE_POINT, .token_size = 1, .max_tokens =
		sizeof(struct iw_pmkid_cand), }, };
static const zpl_uint32 standard_event_num = (sizeof(standard_event_descr)
		/ sizeof(struct iw_ioctl_description));
...
int we_version)
{
	...
	int event_type = 0;
	zpl_uint32 event_len = 1; /* Invalid */
	char * pointer;
	...
	if ((we_version > 18) && (event_type == IW_HEADER_TYPE_POINT))
		memcpy((char *) iwe + IW_EV_LCP_LEN + IW_EV_POINT_OFF, pointer,
				event_len);
	else
	...
	if (event_type == IW_HEADER_TYPE_POINT)
	{
		...
		zpl_uint32 extra_len = iwe->len - (event_len + IW_EV_LCP_PK_LEN);
		if (extra_len > 0)
		{
			...
			else
			{
				...
				zpl_uint32 token_len = iwe->u.data.length * descr->token_size;
				...
				if ((token_len != extra_len) && (extra_len >= 4))
				{
					...
					zpl_uint32 alt_token_len = alt_dlen * descr->token_size;
 ...
 * Jean II
 */
static int iw_split_gen_ie(zpl_uint8 * buffer, int buflen)
{
	zpl_uint32 offset = 0;
	int value[2] = {0, 0};
...
*/
			...
			//iw_split_ie_wpa(buffer + offset, buflen);
			break;
		default:
...
iw_process_scanning_token(struct iw_event * event, struct wireless_scan * wscan)
{
	...
	case IWEVGENIE:
		...
		if (event->u.data.pointer)
		{
			wscan->auth = iw_split_gen_ie(event->u.data.pointer, event->u.data.length);
			wscan->has_ie = 1;
		}
		break;
	default:
...
int iw_start_scan(char * ifname, wireless_scan_head *context)
{
	int skfd = 0, ret = 0;
	/* Create a channel to the NET kernel. */
	if ((skfd = iw_sockets_open()) < 0)
	...
	ret = iw_scan(skfd, ifname, WE_VERSION, context);
...
int iw_free_scan(wireless_scan_head *context)
{
...
int iw_is_connect(char *ifname, zpl_uint8 *bssid)
{
	int skfd; /* generic raw socket desc.	*/
	int goterr = -1;
	struct iwreq wrq;
	...
	if (iw_get_ext(skfd, ifname, SIOCGIWAP, &wrq) >= 0)
	{
		...
		const struct ether_addr * ether_wap =
				(const struct ether_addr *) &wrq.u.ap_addr.sa_data;
		...
		if (!iw_ether_cmp(ether_wap, &ether_zero))
			...
			goterr = 0;
		if(bssid)
			memcpy(&bssid, ether_wap->ether_addr_octet, 6);
		//memcpy(&bssid, &(wrq.u.ap_addr), sizeof(sockaddr));
		//goterr = 0;
	}
	/* Close the socket. */
	iw_sockets_close(skfd);
...
int iw_kernel_mode(char *ifname, int *mode)
{
	int skfd; /* generic raw socket desc.	*/
	int goterr = -1;
	struct iwreq wrq;
