...
static zpl_bool iwlist_detail = zpl_true;

int iwlist_detail_set(zpl_bool value)
{
	iwlist_detail = value;
	return 0;
}
...
{
	zpl_uint32 mask; /* bit mask for the value */
 ...
 */
static void iw_print_mask_name(zpl_uint32 mask,
		const struct iwmask_name names[], const zpl_uint32 num_names,
		const char * sep)
{
	zpl_uint32 i;
 ...
 */
static void iw_print_value_name(zpl_uint32 value, const char * names[],
		const zpl_uint32 num_names)
{
	if (value >= num_names)
 ...
 *
 */
static void iw_print_ie_unknown(zpl_uint8 * iebuf, int buflen)
{
	int ielen = iebuf[1] + 2;
	int i;
 ...
 *
 */
static inline void iw_print_ie_wpa(zpl_uint8 * iebuf, int buflen)
{
	int ielen = iebuf[1] + 2;
	zpl_uint32 offset = 2; /* Skip the IE id, and the length. */
	zpl_uint8 wpa1_oui[3] =
	{ 0x00, 0x50, 0xf2 };
	zpl_uint8 wpa2_oui[3] =
	{ 0x00, 0x0f, 0xac };
	zpl_uint8 * wpa_oui;
	int i;
	zpl_uint16 ver = 0;
	...
	if (ielen < (offset + 4))
	{
		/* We have a zpl_int16 IE.  So we should assume TKIP/TKIP. */
 ...
 */
static inline void iw_print_gen_ie(zpl_uint8 * buffer, int buflen)
{
	zpl_uint32 offset = 0;
...
int has_range)
{
	...
	case SIOCGIWMODUL:
	{
		zpl_uint32 modul = event->u.param.value;
		int i;
		int n = 0;
...
int iw_get_bitrate_info(char * ifname, iw_dev_t *iwdev)
{
	...
	else
	{
		...
		if ((range.num_bitrates > 0) && (range.num_bitrates <= IW_MAX_BITRATES))
		{
			...
			{
				iwdev->bitrates[k].active = zpl_true;
		...
		if (iw_get_ext(skfd, ifname, SIOCGIWRATE, &wrq) >= 0)
		{
			...
			iwdev->cu_bitrates.active = zpl_true;
		...
		if (range.bitrate_capa & IW_BITRATE_BROADCAST)
		{
			...
			if (iw_get_ext(skfd, ifname, SIOCGIWRATE, &wrq) >= 0)
			{
				...
				iwdev->broadcast_bitrates.active = zpl_true;
 ...
 */
static int get_pm_value(int skfd, char * ifname, struct iwreq * pwrq, zpl_uint32 flags,
		...
		int we_version_compiled)
{
	if (iwr_flags & mask)
	{
		zpl_uint32 flags = (iwr_flags & ~(IW_POWER_MIN | IW_POWER_MAX));
 ...
 */
static const zpl_uint32 pm_type_flags[] =
{
IW_POWER_PERIOD,
IW_POWER_TIMEOUT,
IW_POWER_SAVING, };
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	else
	{
		...
		{
			zpl_uint32 flags = wrq.u.power.flags;
			...
			{
				zpl_uint32 pm_type = 0;
				zpl_uint32 pm_mask = 0;
				zpl_uint32 remain_mask = range.pm_capa & IW_POWER_TYPE;
				int i = 0;
...
static int get_retry_value(int skfd, char * ifname, struct iwreq * pwrq,
		zpl_uint32 flags, char * buffer, int buflen, int we_version_compiled)
{
		...
		int we_version_compiled)
{
	if (iwr_flags & mask)
	{
		zpl_uint32 flags = (iwr_flags & ~(IW_RETRY_MIN | IW_RETRY_MAX));
		/* Display if auto or fixed */
		iw_printf("%s %s ; ", (iwr_flags & IW_POWER_MIN) ? "Auto " : "Fixed",
				name);
		/* Print the range */
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	else
	{
		...
		{
			zpl_uint32 flags = wrq.u.retry.flags;
			...
			{
				zpl_uint32 retry_type = 0;
				zpl_uint32 retry_mask = 0;
				zpl_uint32 remain_mask = range.retry_capa & IW_RETRY_TYPE;
				...
				while (1)
				{
					/* Deal with min/max/zpl_int16/long for the current value */
					retry_mask = 0;
					/* If we have been returned a MIN value, ask for the MAX */
					...
					/* Loop back for min/max/zpl_int16/long */
				}
			}
		}
		iw_printf("\n");
...
int count, iw_user_cb_t *cb) /* Args count */
{
	struct iwreq wrq;
	struct iw_range range;
	zpl_uint8 extbuf[IW_EXTKEY_SIZE];
...
{
	struct iwreq wrq;
	zpl_uint8 buf[IW_GENERIC_IE_MAX];
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	else
	{
		...
		else
		{
			...
			{
				zpl_uint32 modul = wrq.u.param.value;
				int n = 0;
