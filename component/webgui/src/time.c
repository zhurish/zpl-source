...
static int daysSinceEpoch(int iyear)
{
    int     idays;

    idays = 365 * (iyear - 1970);
    idays += ((iyear-1) / 4) - (1970 / 4);
    idays -= ((iyear-1) / 100) - (1970 / 100);
    idays += ((iyear-1) / 400) - (1970 / 400);
    return idays;
}
...
{
    int     ret = 0, idays, iyear, imonth;

    iyear = tp->tm_year + 1900 + tp->tm_mon / 12;
    imonth = tp->tm_mon % 12;
    if (imonth < 0) {
        imonth += 12;
        --iyear;
    }
    idays = daysSinceEpoch(iyear);
    idays += leapYear(iyear) ? leapMonthStart[imonth] : normalMonthStart[imonth];
    idays += tp->tm_mday - 1;
    ret = (idays * SEC_PER_DAY) + ((((((tp->tm_hour * 60)) + tp->tm_min) * 60) + tp->tm_sec));
    return (WebsTime)ret;
}
