...
static Js   **jsHandles = NULL;    /* List of js handles */
...
PUBLIC int jsLexOpenScript(Js *ep, cchar *script)
{
    ...
    web_assert(ip);
    web_assert(ip->putBackToken == NULL);
    web_assert(ip->putBackTokenId == 0);
...
PUBLIC void jsLexPutbackToken(Js *ep, int tid, cchar *pstring)
{
    JsInput *ip;
    ...
    ip->putBackToken = sclone(pstring);
}
