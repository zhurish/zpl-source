...
typedef ospl_uint32 leasetime_t;
...
typedef struct dyn_lease {
	 ...
	 */
	ospl_uint8 		lease_mac[ETHER_ADDR_LEN];
...
extern dyn_lease_t * dhcp_lease_lookup_by_lease_address(LIST *lst, lease_mode_t mode, ospl_uint32 lease_address);
extern dyn_lease_t * dhcp_lease_lookup_by_lease_mac(LIST *lst, lease_mode_t mode, ospl_uint8 *lease_mac);
extern dyn_lease_t * dhcp_lease_add(LIST *lst, dyn_lease_t *lease);
extern int dhcp_lease_update(LIST *lst, dyn_lease_t *lease);
extern int dhcp_lease_del_mac(LIST *lst, ospl_uint8 *lease_mac);
extern int dhcp_lease_del_address(LIST *lst, ospl_uint32 lease_address);
...
extern int dhcp_lease_show(struct vty *vty, char *poolname, ifindex_t ifindex, ospl_bool detail);
