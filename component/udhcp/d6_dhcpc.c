...
static void *d6_find_option(zpl_uint8 *option, zpl_uint8 *option_end, unsigned code)
{
	/* "length minus 4" */
...
static void option_to_env(zpl_uint8 *option, zpl_uint8 *option_end)
{
#if ENABLE_FEATURE_UDHCPC6_RFC3646
 ...
 */
		...
		case D6_OPT_DNS_SERVERS: {
			...
			*new_env() = dlist = malloc(4 + addrs * 40 - 1);
...
static zpl_uint32  random_xid(void)
{
	zpl_uint32  t = rand() & htonl(0x00ffffff);
	return t;
}

/* Initialize the packet with the proper defaults */
static zpl_uint8 *init_d6_packet(struct d6_packet *packet, char type, zpl_uint32  xid)
{
...
static zpl_uint8 *add_d6_client_options(zpl_uint8 *ptr)
{
	zpl_uint8 *start = ptr;
	unsigned option;
...
static int d6_mcast_from_client_config_ifindex(struct d6_packet *packet, zpl_uint8 *end)
{
	static const zpl_uint8 FF02__1_2[16] = {
...
static  int send_d6_discover(zpl_uint32  xid, struct in6_addr *requested_ipv6)
{
	...
	*(zpl_uint32 *)client6_data.ia_na->data = rand(); /* IAID */
	if (requested_ipv6) {
 ...
 */
/* NOINLINE: limit stack usage in caller */
static int send_d6_select(zpl_uint32  xid)
{
	struct d6_packet packet;
	zpl_uint8 *opt_ptr;
...
//	packet.udp.check = 0;
//	if (check && check != inet_cksum((zpl_uint16 *)&packet, bytes)) {
 ...
 */
				...
				if (client6_data.ia_na->len < (4 + 4 + 4) + (2 + 2 + 16 + 4 + 4)) {
					zlog_err(MODULE_DHCP,"IA_NA option is too zpl_int16:%d bytes", client6_data.ia_na->len);
					continue;
				}
				...
				if (iaaddr->len < (16 + 4 + 4)) {
					zlog_err(MODULE_DHCP,"IAADDR option is too zpl_int16:%d bytes", iaaddr->len);
					continue;
				}
...
/// TODO: check for 0 lease time?
				...
				zlog_err(MODULE_DHCP,"lease obtained, lease time %u",
					/*ipstack_inet_ntoa(temp_addr),*/ (unsigned)lease_seconds);
