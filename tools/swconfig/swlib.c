...
store_port_val(struct nl_msg *msg, struct nlattr *nla, struct switch_val *val)
{
	...
	nla_for_each_nested(p, nla, remaining) {
		struct nlattr *ptb[SWITCH_PORT_ATTR_MAX+1];
		struct switch_port *port;

		if (val->len >= ports)
			break;

		err = nla_parse_nested(ptb, SWITCH_PORT_ATTR_MAX, p, port_policy);
		if (err < 0)
			goto out;

		if (!ptb[SWITCH_PORT_ID])
			continue;

		port = &val->value.ports[val->len];
		port->id = nla_get_u32(ptb[SWITCH_PORT_ID]);
		port->flags = 0;
		if (ptb[SWITCH_PORT_FLAG_TAGGED])
...
{
	struct nlattr *ptb[SWITCH_LINK_ATTR_MAX + 1];
	...
	err = nla_parse_nested(ptb, SWITCH_LINK_ATTR_MAX, nla, link_policy);
	if (err < 0)
		goto out;

	link = val->value.link;
	link->link = !!ptb[SWITCH_LINK_FLAG_LINK];
	link->duplex = !!ptb[SWITCH_LINK_FLAG_DUPLEX];
	link->aneg = !!ptb[SWITCH_LINK_FLAG_ANEG];
	link->tx_flow = !!ptb[SWITCH_LINK_FLAG_TX_FLOW];
	link->rx_flow = !!ptb[SWITCH_LINK_FLAG_RX_FLOW];
	link->speed = nla_get_u32(ptb[SWITCH_LINK_SPEED]);
	link->eee = 0;
	if (ptb[SWITCH_LINK_FLAG_EEE_100BASET])
		link->eee |= SWLIB_LINK_FLAG_EEE_100BASET;
	if (ptb[SWITCH_LINK_FLAG_EEE_1000BASET])
...
add_port_map(struct switch_dev *dev, struct nlattr *nla)
{
	...
	nla_for_each_nested(p, nla, remaining) {
		struct nlattr *ptb[SWITCH_PORTMAP_MAX+1];

		if (idx >= dev->ports)
			continue;

		err = nla_parse_nested(ptb, SWITCH_PORTMAP_MAX, p, portmap_policy);
		if (err < 0)
			continue;


		if (ptb[SWITCH_PORTMAP_SEGMENT] && ptb[SWITCH_PORTMAP_VIRT]) {
			dev->maps[idx].segment = strdup(nla_get_string(ptb[SWITCH_PORTMAP_SEGMENT]));
			dev->maps[idx].virt = nla_get_u32(ptb[SWITCH_PORTMAP_VIRT]);
		}
		idx++;
	}
	return err;
