...
static int distribute_master_get (zpl_uint16 proto, struct distribute_master **dm)
{
	zpl_uint32 i = 0;
	for(i = 0; i < 3; i++)
	{
		if(d_master[i])
		{
			if(d_master[i]->protocol == proto)
			{
				//d_master[i]->distribute_add_hook = func;
				if(dm)
					*dm = d_master[i];
				return 1;
			}
...
distribute_free_if_empty(struct distribute_master *dm, struct distribute *dist)
{
  ...
  hash_release (dm->disthash, dist);
  distribute_free (dist);
}

/* Lookup interface's distribute list. */
struct distribute *
distribute_lookup (struct distribute_master *dm, const char *ifname)
{
  struct distribute key;
  struct distribute *dist;

  /* temporary reference */
  key.ifname = (zpl_char *)ifname;
...
void
distribute_list_add_hook (zpl_uint16 proto,void (*func) (struct distribute *))
{
	zpl_uint32 i = 0;
	for(i = 0; i < 3; i++)
	{
		...
		//RIPNG_NODE || node == BABEL_NODE
		//distribute_add_hook = func;
	}
}

void
distribute_list_delete_hook (zpl_uint16 proto, void (*func) (struct distribute *))
{
	zpl_uint32 i = 0;
	for(i = 0; i < 3; i++)
	{
		...
		{
			if(d_master[i]->protocol == proto)
				d_master[i]->distribute_delete_hook = func;
		}
		//RIPNG_NODE || node == BABEL_NODE
...
static struct distribute *
distribute_get (struct distribute_master *dm, const char *ifname)
{
  struct distribute key;
...
static struct distribute *
distribute_list_set (zpl_uint16 proto, const char *ifname, enum distribute_type type,
                     const char *alist_name)
{
  struct distribute *dist;
  struct distribute_master *dm = NULL;
  distribute_master_get ( proto, &dm);
  if(dm == NULL)
	  return NULL;
...
/* Unset distribute-list.  If matched distribute-list exist then
   return 1. */
static int
distribute_list_unset (zpl_uint16 proto, const char *ifname, enum distribute_type type,
		       const char *alist_name)
{
  struct distribute *dist;
  struct distribute_master *dm = NULL;
  ...
  /* Apply this distribute-list to the interface. */
  (*dm->distribute_delete_hook) (dist);

  /* If all dist are NULL, then free distribute list. */
  distribute_free_if_empty(dm, dist);
  return 1;
}

/* Set access-list name to the distribute list. */
static struct distribute *
distribute_list_prefix_set (zpl_uint16 proto, const char *ifname, enum distribute_type type,
...
/* Unset distribute-list.  If matched distribute-list exist then
   return 1. */
static int
distribute_list_prefix_unset (zpl_uint16 proto, const char *ifname, enum distribute_type type,
			      const char *plist_name)
{
  struct distribute *dist;
  struct distribute_master *dm = NULL;
       ...
       "Filter outgoing routing updates\n")
{
  ...
  distribute_list_set (vty->node, NULL, type, argv[0]);
       ...
       "Filter outgoing routing updates\n")
{
  ...
  ret = distribute_list_unset (vty->node, NULL, type, argv[0]);
  if (! ret)
    {
       ...
       "Interface name\n")
{
  ...
  /* Get interface name corresponding distribute list. */
  distribute_list_set (vty->node, argv[2], type, argv[0]);
       ...
       "Interface name\n")
{
  ...
  ret = distribute_list_unset (vty->node, argv[2], type, argv[0]);
  if (! ret)
    {
      vty_out (vty, "distribute list doesn't exist%s", VTY_NEWLINE);
      return CMD_WARNING;
       ...
       "Filter outgoing routing updates\n")
{
  ...
  /* Get interface name corresponding distribute list. */
  distribute_list_prefix_set (vty->node, NULL, type, argv[0]);
       ...
       "Filter outgoing routing updates\n")
{
  ...
  ret = distribute_list_prefix_unset (vty->node, NULL, type, argv[0]);
  if (! ret)
    {
      vty_out (vty, "distribute list doesn't exist%s", VTY_NEWLINE);
      return CMD_WARNING;
       ...
       "Interface name\n")
{
  ...
  /* Get interface name corresponding distribute list. */
  distribute_list_prefix_set (vty->node, argv[2], type, argv[0]);
       ...
       "Interface name\n")
{
  ...
  ret = distribute_list_prefix_unset (vty->node, argv[2], type, argv[0]);
  if (! ret)
    {
      vty_out (vty, "distribute list doesn't exist%s", VTY_NEWLINE);
      return CMD_WARNING;
...
static int
distribute_print (struct vty *vty, zpl_char *tab[], int is_prefix,
...
config_show_distribute (struct distribute_master *dm, struct vty *vty)
{
  ...
  for (i = 0; i < dm->disthash->size; i++)
    for (mp = dm->disthash->index[i]; mp; mp = mp->next)
      {
	dist = mp->data;
	if (dist->ifname)
          {
...
void
distribute_list_reset (zpl_uint16 proto)
{
	zpl_uint32 i = 0;
	for(i = 0; i < 3; i++)
	{
		if(d_master[i])
		{
			if(d_master[i]->protocol == proto)
			{
				  hash_clean (d_master[i]->disthash, (void (*) (void *)) distribute_free);
				  XFREE(MTYPE_DISTRIBUTE, d_master[i]);
			}
		}
	}
}

struct distribute_master * distribute_master_list_init(zpl_uint16 protocol)
{
	...
	dis_master->protocol = protocol;
	dis_master->disthash = hash_create (distribute_hash_make,
...
void
distribute_list_init (zpl_uint32 node)
{
  #ifdef ZPL_SHELL_MODULE
  ...
  if (node == RIP_NODE || node == BABEL_NODE) {
    install_element(node, CMD_CONFIG_LEVEL, &distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &distribute_list_prefix_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_distribute_list_prefix_cmd);
  }

  /* install v6 */
  if (node == RIPNG_NODE || node == BABEL_NODE) {
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_distribute_list_prefix_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_distribute_list_prefix_cmd);
  }

  /* install v4 syntax command for v6 only protocols. */
  if (node == RIPNG_NODE) {
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_as_v4_distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_as_v4_distribute_list_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_as_v4_distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_as_v4_distribute_list_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_as_v4_distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_as_v4_distribute_list_prefix_all_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &ipv6_as_v4_distribute_list_prefix_cmd);
    install_element(node, CMD_CONFIG_LEVEL, &no_ipv6_as_v4_distribute_list_prefix_cmd);
  }
  #endif
}
