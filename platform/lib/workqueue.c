...
static int work_queue_schedule(struct work_queue *wq, zpl_uint32  delay)
{
	...
	{
		//wq->thread = thread_add_timer_msec(wq->master, work_queue_run, wq,
		//			delay);
...
void work_queue_add(struct work_queue *wq, void *data)
{
	...
	{
		zlog_err(MODULE_DEFAULT, "%s: unable to get new queue item", __func__);
		return;
	}
		...
		"Work Queue information\n")
{
	...
	vty_out(vty, "%c %8s %5s %8s %21s %6s %5s%s", ' ', "List", "(ms) ",
			"Q. Runs", "Cycle Counts   ", " ", "Worst",
			VTY_NEWLINE);
	vty_out(vty, "%c %8s %5s %8s %7s %6s %6s %6s %5s %s%s", 'P', "Items",
			"Hold", "Total", "Best", "Worst", "Gran.", "Avg.", "Lat.", "Name",
			VTY_NEWLINE);
