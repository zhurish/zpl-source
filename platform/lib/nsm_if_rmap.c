...
#include "memory.h"
#include "if.h"
#include "nsm_if_rmap.h"

struct hash *ifrmaphash;

/* Hook functions. */
static void (*if_rmap_add_hook[2]) (struct if_rmap *) = {NULL, NULL};
static void (*if_rmap_delete_hook[2]) (struct if_rmap *) = {NULL, NULL};
...
void
if_rmap_hook_add (zpl_uint16 type, void (*func) (struct if_rmap *))
{
  if(type == RIP_NODE )
  if_rmap_add_hook[0] = func;
  else if(type == RIPNG_NODE )
  if_rmap_add_hook[1] = func;
}

void
if_rmap_hook_delete (zpl_uint16 type, void (*func) (struct if_rmap *))
{
	  if(type == RIP_NODE )
		  if_rmap_delete_hook[0] = func;
	  else if(type == RIPNG_NODE )
		  if_rmap_delete_hook[1] = func;
}
...
static struct if_rmap *
if_rmap_set (int node, const char *ifname, enum if_rmap_type type,
             const char *routemap_name)
{
  ...
  if (if_rmap_add_hook[node])
    (*if_rmap_add_hook[node]) (if_rmap);
/*  if (if_rmap_add_hook[1])
    (*if_rmap_add_hook[1]) (if_rmap);*/
  return if_rmap;
}

static int
if_rmap_unset (int node, const char *ifname, enum if_rmap_type type,
               const char *routemap_name)
{
  struct if_rmap *if_rmap;
  ...
  if (if_rmap_delete_hook[node])
    (*if_rmap_delete_hook[node]) (if_rmap);
/*  if (if_rmap_delete_hook[1])
    (*if_rmap_delete_hook[1]) (if_rmap);*/
       ...
       "Route map interface name\n")
{
  ...
  if_rmap_set (vty->node, argv[2], type, argv[0]);
       ...
       "Route map interface name\n")
{
  ...
  ret = if_rmap_unset (vty->node, argv[2], type, argv[0]);
  if (! ret)
    {
...
if_rmap_init (int node)
{
  ...
  if (node == RIPNG_NODE) {
    install_element (RIPNG_NODE, CMD_CONFIG_LEVEL, &if_ipv6_rmap_cmd);
    install_element (RIPNG_NODE, CMD_CONFIG_LEVEL, &no_if_ipv6_rmap_cmd);
  } else if (node == RIP_NODE) {
    install_element (RIP_NODE, CMD_CONFIG_LEVEL, &if_rmap_cmd);
    install_element (RIP_NODE, CMD_CONFIG_LEVEL, &no_if_rmap_cmd);
  }
}
