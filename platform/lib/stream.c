...
stream_read (struct stream *s, int fd, size_t size, int type)
{
  ...
  nbytes = readn (fd, s->data + s->endp, size, type);
...
stream_read_try(struct stream *s, int fd, size_t size, int type)
{
  ...
  if(type == IPCOM_STACK)
	  nbytes = ip_read(fd, s->data + s->endp, size);
  else
                 ...
                 struct sockaddr *from, socklen_t *fromlen, int type)
{
  ...
  if(type == IPCOM_STACK)
	  nbytes = ip_recvfrom (fd, s->data + s->endp, size,
	                            flags, from, fromlen);
  else
	  nbytes = recvfrom (fd, s->data + s->endp, size,
                            flags, from, fromlen);
  if (nbytes >= 0)
    {
      s->endp += nbytes;
                ...
                size_t size, int type)
{
  ...
  iov->iov_len = size;
  if(type == IPCOM_STACK)
	  nbytes = ip_recvmsg (fd, msgh, flags);
  else
...
stream_flush (struct stream *s, int fd, int type)
{
  ...
  STREAM_VERIFY_SANE(s);
  if(type == IPCOM_STACK)
	  nbytes = ip_write (fd, s->data + s->getp, s->endp - s->getp);
  else
