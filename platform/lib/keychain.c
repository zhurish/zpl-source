...
static struct key *
key_lookup (const struct keychain *keychain, zpl_uint32 index)
{
  struct listnode *node;
  struct key *key;
...
#endif
struct key *
key_lookup_for_accept (const struct keychain *keychain, zpl_uint32 index)
{
  struct listnode *node;
  struct key *key;
  zpl_time_t now;
...
#ifdef ZPL_SHELL_MODULE
static struct key *
key_get (const struct keychain *keychain, zpl_uint32 index)
{
  struct key *key;
...
/* Convert HH:MM:SS MON DAY YEAR to zpl_time_t value.  -1 is returned when
	      ...
	      const char *year_str)
{
  ...
  struct tm tm;
  zpl_time_t time;
  zpl_uint32  sec, min, hour;
  zpl_uint32  day, month, year;
...
static int
keychain_strftime (zpl_char *buf, int bufsiz, zpl_time_t *time)
{
  struct tm *tm = NULL;
  struct tm stm;
  zpl_size_t len;
...
keychain_init ()
{
  ...
  install_element (CONFIG_NODE, CMD_CONFIG_LEVEL, &key_chain_cmd);
  install_element (CONFIG_NODE, CMD_CONFIG_LEVEL, &no_key_chain_cmd);
  install_element (KEYCHAIN_NODE, CMD_CONFIG_LEVEL, &key_cmd);
  install_element (KEYCHAIN_NODE, CMD_CONFIG_LEVEL, &no_key_cmd);

  install_element (KEYCHAIN_NODE, CMD_CONFIG_LEVEL, &key_chain_cmd);
  install_element (KEYCHAIN_NODE, CMD_CONFIG_LEVEL, &no_key_chain_cmd);

  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &key_string_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &no_key_string_cmd);

  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &key_chain_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &no_key_chain_cmd);

  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &key_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &no_key_cmd);

  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_day_month_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_day_month_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_month_day_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_month_day_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_infinite_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_infinite_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_duration_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &accept_lifetime_duration_month_day_cmd);

  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_day_month_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_day_month_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_month_day_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_month_day_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_infinite_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_infinite_month_day_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_duration_day_month_cmd);
  install_element(KEYCHAIN_KEY_NODE, CMD_CONFIG_LEVEL, &send_lifetime_duration_month_day_cmd);
#endif  
}
