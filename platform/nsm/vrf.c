...
static struct vrf * vrf_new_one (vrf_id_t vrf_id, const char *name)
{
  ...
  zlog_info (ZLOG_NSM, "VRF %u is created.", vrf_id);
...
{
  zlog_info (ZLOG_NSM, "VRF %u is to be deleted.", vrf->vrf_id);
  if(vrfMutex)
	  os_mutex_lock(vrfMutex, OS_WAIT_FOREVER);
...
{
   zlog_info (ZLOG_NSM, "VRF %u is enabled.", vrf->vrf_id);
...
int nsm_vrf_delete (const char *name)
{
  ...
  if (!vrf)
    {
      zlog_err (ZLOG_NSM, "vrf_init: failed to create the default VRF!");
    }
  vrf_del_one (vrf);
  return 0;
}
