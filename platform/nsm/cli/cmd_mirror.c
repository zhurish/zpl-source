		...
		CMD_USP_STR_HELP)
{
	...
	if(!nsm_mirror_global_is_enable())
		ret = nsm_mirror_global_enable(zpl_true);
	...
	if (argv[0] && argv[1])
	{
		...
		if(ifindex)
		{
			...
			if(dir)
				ret = nsm_mirror_source_set_api(ifindex, zpl_true, dir);
		}
	}
		...
		CMD_USP_STR_HELP)
{
	...
	if (argv[0] && argv[1])
	{
		...
		if(ifindex)
		{
			...
			if(ret == OK && enable)
			{
				ret = nsm_mirror_source_set_api(ifindex, zpl_false, dir);
			}
			else
			{
		...
		CMD_MAC_STR_HELP)
{
	...
	ret = nsm_mirror_source_mac_filter_set_api(zpl_true, mac,  dst,  dir);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"specify MAC address\n")
{
	...
	ret = nsm_mirror_source_mac_filter_set_api(zpl_false, NULL,  dst,  dir);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
/*
		...
		CMD_MAC_STR_HELP)
{
	...
	ret = nsm_mirror_mode_set_api(zpl_true);
	VTY_IMAC_GET(argv[0], mac);
	...
	if (ret == OK)
	{
		...
		if(dir)
			ret = nsm_mirror_source_mac_set_api(zpl_true, mac, dir);
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"specify MAC address\n")
{
	...
	ret = nsm_mirror_mode_set_api(zpl_false);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
*/
	...
	if (argv[0] && argv[1])
	{
		...
		if(ifindex)
		{
			...
			if(!nsm_mirror_is_enable_api(ifindex))
				ret = nsm_mirror_destination_set_api(ifindex, zpl_true);
			else
			{
		...
		CMD_USP_STR_HELP)
{
	...
	if (argv[0] && argv[1])
	{
		...
		if(ifindex)
		{
			...
			if(nsm_mirror_is_enable_api(ifindex))
				ret = nsm_mirror_destination_set_api(ifindex, zpl_false);
			else
			{
...
void cmd_mirror_init(void)
{
	...
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_monitor_session_source_filter_cmd);

/*
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &monitor_session_source_mac_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &monitor_session_source_mac_both_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_monitor_session_source_mac_cmd);
*/
