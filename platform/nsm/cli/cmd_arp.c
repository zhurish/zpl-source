		...
		CMD_ARP_STR_HELP)
{
	...
	if(argc == 0)
	{
		ret = ip_arp_cleanup_api(ARP_DYNAMIC, zpl_false, 0);
	}
	else if(argc == 1)
	{
		//ret = ip_arp_cleanup_api(ARP_DYNAMIC, zpl_false, 0);
	...
	else if(argc > 2)
	{
		if(if_lookup_by_index(if_ifindex_make(argv[1], argv[2])))
			ret = ip_arp_cleanup_api(ARP_DYNAMIC, zpl_false, if_ifindex_make(argv[1], argv[2]));
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
...
/*	sprintf(mac, "%02x%02x-%02x%02x-%02x%02x",node->mac[0],node->mac[1],node->mac[2],
	...
	vty_out(vty, "%-8s %-16s %-8s %-16s %-16s %-8s %-16s %s",
			"Internet", ipstack_inet_ntoa(node->address.u.prefix4), timeout, mac, ifname, vlan, vrf, VTY_NEWLINE);
	return OK;
}
...
void cmd_arp_init(void)
{
	...
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_arp_add_interface_cmd);

	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &clear_ip_arp_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &clear_ip_arp_address_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &clear_ip_arp_interface_cmd);

	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &ip_arp_retry_interval_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_arp_retry_interval_cmd);

	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &ip_arp_timeout_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_arp_timeout_cmd);

	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &ip_arp_proxy_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_arp_proxy_cmd);

	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &ip_arp_proxy_local_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_arp_proxy_local_cmd);
