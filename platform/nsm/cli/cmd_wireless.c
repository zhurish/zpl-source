		...
		"Enable\n")
{
	...
	ret = nsm_iw_enable_api(ifp, zpl_true);
		...
		"Enable\n")
{
	int ret = ERROR;
	struct interface *ifp = vty->index;
	ret = nsm_iw_enable_api(ifp, zpl_false);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"Network name configure\n")
{
	...
	{
		if((nsm_iw_mode(ifp) == IW_MODE_MANAGE || nsm_iw_mode(ifp) == IW_MODE_CLIENT) && iw_client_lookup_api(ifp))
			ret = iw_client_db_del_api(iw_client_lookup_api(ifp), NULL, zpl_false);
		...
		"Password configure\n")
{
	...
	if(ifp/* && iw_ap_lookup_api(ifp)*/)
	{
		...
		{
			iw_client_t *client = iw_client_lookup_api(ifp);
			ret = iw_client_db_del_api(client, client->cu.SSID, zpl_true);
		}
		if(nsm_iw_mode(ifp) == IW_MODE_AP && iw_ap_lookup_api(ifp))
		...
		"enable configure\n")
{
	...
	{
		if(iw_ap_lookup_api(ifp))
			ret = iw_ap_isolate_set_api(iw_ap_lookup_api(ifp), zpl_true);
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"enable configure\n")
{
	...
	{
		if(iw_ap_lookup_api(ifp))
			ret = iw_ap_isolate_set_api(iw_ap_lookup_api(ifp), zpl_false);
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"wmm-enable configure\n")
{
	...
	{
		if(iw_ap_lookup_api(ifp))
			ret = iw_ap_wmm_set_api(iw_ap_lookup_api(ifp), zpl_true);
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"wmm-enable configure\n")
{
	...
	{
		if(iw_ap_lookup_api(ifp))
			ret = iw_ap_wmm_set_api(iw_ap_lookup_api(ifp), zpl_false);
	}
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		if (ifp && if_is_wireless(ifp))
		{
			...
			if (iw_client_lookup_api(ifp))
			{
				...
				if(strstr(cmd, "neighbor"))
					iw_client_neighbor_show(iw_client_lookup_api(ifp), vty, zpl_true);
...
//显示当前连接到AP的设备
static int iw_ap_show_cmd(struct vty *vty, int ifindex, zpl_bool detail)
{
	struct listnode *node = NULL;
	struct interface *ifp = NULL;
	struct list *if_list = NULL;
		...
		"client information\n")
{
	...
	{
		ifindex = if_ifindex_make("wireless", argv[0]);
		iw_ap_show_cmd(vty, ifindex, zpl_true);
	}
	else if(argc == 1)
	{
		...
		{
			ifindex = if_ifindex_make("wireless", argv[0]);
			iw_ap_show_cmd(vty, ifindex, zpl_false);
		}
	}
	else
		iw_ap_show_cmd(vty, ifindex, zpl_false);
...
{
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_neighbor_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_neighbor_interface_cmd);

	install_element(node, CMD_VIEW_LEVEL, &show_wireless_connect_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_connect_station_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_connect_interface_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_connect_station_interface_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &scan_wireless_neighbor_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &scan_wireless_neighbor_interface_cmd);


	install_element(node, CMD_VIEW_LEVEL, &show_wireless_info_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_info_detail_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_info_interface_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_info_interface_detail_cmd);

	install_element(node, CMD_VIEW_LEVEL, &show_wireless_client_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_client_detail_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_client_interface_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_wireless_client_interface_detail_cmd);
}

static void cmd_base_wireless_init(int node)
{

	install_element(node, CMD_CONFIG_LEVEL, &wireless_enable_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_wireless_enable_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &work_mode_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_work_mode_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &radio_type_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_radio_type_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &work_channel_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_work_channel_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &power_level_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_power_level_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &beacon_interval_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_beacon_interval_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &netwotrk_name_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_netwotrk_name_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &authentication_password_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_authentication_password_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_authentication_method_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_authentication_method_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_encryption_method_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_encryption_method_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_mac_acl_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_mac_acl_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &signal_level_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_signal_level_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_bitrate_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_bitrate_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_threshold_rts_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_threshold_rts_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_threshold_frame_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_threshold_frame_cmd);


	install_element(node, CMD_CONFIG_LEVEL, &ap_isolate_enable_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_isolate_enable_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_wmm_enable_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_wmm_enable_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_country_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_country_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &ap_accept_client_num_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_accept_client_num_cmd);

/*	install_element(node, CMD_CONFIG_LEVEL, &ap_name_pass_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &ap_name_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_ap_name_cmd);*/

	install_element(node, CMD_CONFIG_LEVEL, &scan_interval_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_scan_interval_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &scan_num_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_scan_num_cmd);
}
...
void cmd_wireless_init(void)
{
	...
	install_element(ENABLE_NODE, CMD_ENABLE_LEVEL, &debug_wireless_set_cmd);
	install_element(ENABLE_NODE, CMD_ENABLE_LEVEL, &debug_wireless_set_detail_cmd);

	install_element(ENABLE_NODE, CMD_ENABLE_LEVEL, &no_debug_wireless_set_cmd);
	install_element(ENABLE_NODE, CMD_ENABLE_LEVEL, &no_debug_wireless_set_detail_cmd);

	install_element(CONFIG_NODE, CMD_ENABLE_LEVEL, &debug_wireless_set_cmd);
	install_element(CONFIG_NODE, CMD_ENABLE_LEVEL, &debug_wireless_set_detail_cmd);

	install_element(CONFIG_NODE, CMD_ENABLE_LEVEL, &no_debug_wireless_set_cmd);
	install_element(CONFIG_NODE, CMD_ENABLE_LEVEL, &no_debug_wireless_set_detail_cmd);
}
#endif
