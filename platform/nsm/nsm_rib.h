...
extern int rib_add_ipv4 (zpl_uint32 type, zpl_uint32 flags, struct prefix_ipv4 *p, 
			 ...
			 ifindex_t ifindex, vrf_id_t vrf_id, zpl_uint32 table_id,
...
extern struct rib *rib_match_ipv4_safi (struct ipstack_in_addr addr, safi_t safi,
					zpl_bool skip_bgp, struct route_node **rn_out,
					vrf_id_t);
extern struct rib *rib_match_ipv4_multicast (struct ipstack_in_addr addr,
...
extern zpl_ulong rib_score_proto (zpl_uchar proto);

extern int
static_add_ipv4_safi (safi_t safi, struct prefix *p, struct ipstack_in_addr *gate,
		      const char *ifname, zpl_uchar flags, route_tag_t, 
		      zpl_uchar distance, vrf_id_t vrf_id);
extern int
static_delete_ipv4_safi (safi_t safi, struct prefix *p, struct ipstack_in_addr *gate,
			 const char *ifname, route_tag_t tag, zpl_uchar distance,
			 vrf_id_t vrf_id);

extern int
rib_add_ipv6 (zpl_uint32 type, zpl_uint32 flags, struct prefix_ipv6 *p,
	      struct ipstack_in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id,
	      zpl_uint32 table_id, zpl_uint32 metric, zpl_uint32 mtu,
	      zpl_uchar distance, safi_t safi);

extern int
rib_delete_ipv6 (zpl_uint32 type, zpl_uint32 flags, struct prefix_ipv6 *p,
		 struct ipstack_in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, safi_t safi);

extern struct rib *rib_lookup_ipv6 (struct ipstack_in6_addr *, vrf_id_t);
...
static_delete_ipv6 (struct prefix *p, zpl_uchar type, struct ipstack_in6_addr *gate,
		    const char *ifname, route_tag_t, zpl_uchar distance, 
		    vrf_id_t vrf_id);
 ...
 *
 * Returns zpl_true if this iterator has started iterating over the set of
 * tables.
 */
static inline int
