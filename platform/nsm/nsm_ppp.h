...
typedef struct pppd_options_s
{
    ...
    ospl_char crtscts[PPP_OPTIONS_MAX];
        //使用硬件流控制来控制串口的数据流向，如RTS/CTS
    ospl_bool xonxoff;
        //使用软件流控制来控制串口的数据流向，如RTS/CTS
    ospl_char escape[PPP_OPTIONS_MAX];
        //指定传输时，需要避免的某些字符
    ospl_bool local;
        //不使用调制解调器的控制线
    ospl_bool modem;
	...
	ospl_bool	debug;
	ospl_char	logfile[PPP_OPTIONS_MAX];
    ...
    ospl_char domain[PPP_OPTIONS_MAX];// <d>
        //附加域名到主机名，用于认证。如获取到的主机名为porsche，而完整的域名是porsche.qsa.com,则需要指定"domain qsa.com"

    ospl_char name[PPP_OPTIONS_MAX];// <n>
        //为认证设置本地系统的名字
    ospl_char usehostname[PPP_OPTIONS_MAX];
        //优先使用hostname作为本地系统的名字
	ospl_char remotename[PPP_OPTIONS_MAX];// <n>
        //为认证设置远程系统的名字
