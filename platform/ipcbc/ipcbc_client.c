...
static int
zebra_message_send (struct zclient *zclient, zpl_uint16 command)
{
  struct stream *s;
...
zclient_start (struct zclient *zclient)
{
  ...
  if (ipstack_set_nonblocking(zclient->sock) < 0)
    zlog_warn(MODULE_DEFAULT,"%s: set_nonblocking(%d) failed", __func__, zclient->sock._fd);
 ...
 */
int
zebra_redistribute_send (zpl_uint16 command, struct zclient *zclient, zpl_uint32 type)
{
  struct stream *s;
...
zebra_interface_add_read (struct stream *s)
{
  ...
  ifp = if_get_by_name_len (ifname_tmp, strnlen(ifname_tmp, IF_NAME_MAX));
...
struct connected *
zebra_interface_address_read (zpl_uint16 type, struct stream *s)
{
  zpl_uint32  ifindex;
  struct interface *ifp;
