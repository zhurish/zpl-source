...
/* Nexthop register */
static int
zserv_nexthop_register(struct zserv *client, int sock, zpl_ushort length, vrf_id_t vrf_id)
{
  struct rnh *rnh;
  struct stream *s;
  struct prefix p;
...
/* Nexthop register */
static int
zserv_nexthop_unregister(struct zserv *client, int sock, zpl_ushort length)
{
  struct rnh *rnh;
  struct stream *s;
  struct prefix p;
...
zebra_client_read(struct thread *thread)
{
  ...
  if ((already = stream_get_endp(client->ibuf)) < ZEBRA_HEADER_SIZE)
  {
    ...
    if (((nbyte = stream_read_try(client->ibuf, sock,
                                  ZEBRA_HEADER_SIZE - already, OS_STACK)) == 0) ||
        (nbyte == -1))
    {
...
zebra_serv()
{
  ...
  accept_sock = ipstack_socket(IPSTACK_AF_INET, IPSTACK_SOCK_STREAM, 0);
...
DEFUN (config_table, 
       ...
       "TABLE integer\n")
{
  zebrad.rtm_table_default = strtol (argv[0], (zpl_char**)0, 10);
  return CMD_SUCCESS;
}
