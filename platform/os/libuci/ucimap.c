...
ucimap_parse(struct uci_map *map, struct uci_package *pkg)
{
	...
	uci_foreach_element(&pkg->sections, e) {
		...
		for (i = 0; i < map->n_sections; i++) {
			...
			struct ucimap_section_data *psd;

			if (strcmp(s->type, map->sections[i]->type) != 0)
				continue;

			if (sm->alloc) {
				psd = sm->alloc(map, sm, s);
				memset(sd, 0, sizeof(struct ucimap_section_data));
			} else {
				psd = malloc(sm->alloc_len);
				memset(psd, 0, sm->alloc_len);
				psd = ucimap_ptr_section(sm, psd);
			}
			if (!psd)
				continue;

			ucimap_parse_section(map, sm, psd, s);
		}
	}
	if (!map->parsed) {
		map->parsed = true;
