...
static os_task_t * os_task_lookup(unit32 id, pthread_t pid)
{
	...
	while(node)
	{
		if(node)
		{
			...
			else if(((pthread_t)pid != 0) && (pthread_t)pid == (pthread_t)task->td_thread)
			{
				if(task_mutex)
					os_mutex_unlock(task_mutex);
				return task;
...
static int os_task_refresh_total_cpu(struct os_task_history *hist)
{
	...
	if(fp)
	{
		...
		if(ret)
		{
			...
			hist->total = (user + nice + system + idle + iowait + irq + softirq + stealstolen + guest);
			hist->total_realy = (user + nice + system + idle + iowait + irq + softirq + stealstolen + guest) - hist->total_realy;
	...
	if(fp)
	{
		...
		if(ret)
		{
			sscanf(buf,"%*d %*s %s %*d %*d %*d %*d %*d %*d %*d %*d %*d "
					"%d %d %d %d %d %d %*d %*d %d",
					sta,&utime,&stime,&cutime,&cstime,&priority,&nice,&start_time);
...
//			OS_DEBUG("thread total cpu(%d)\r\n",utime + stime + cutime + cstime);
			...
			task->hist.total_realy = (utime + stime + cutime + cstime) - task->hist.total_realy;
			task->hist.total = (utime + stime + cutime + cstime) - task->hist.total;
...
static int os_task_entry_start(os_task_t *task)
{
	...
	printf("\r\ncreate task:%s(%u->LWP=%u)\r\n",task->td_name,
			task->td_thread,(pid_t)syscall(SYS_gettid));
...
unit32 os_task_entry_create(char *name, int pri, int op,
                         task_entry entry, void *pVoid, char *func_name, int stacksize)
{
	os_task_t *  task = os_task_tcb_entry_create(name,  pri,  op,
	                         entry, pVoid, func_name,  stacksize);
	if(task == NULL)
		return ERROR;
	if(os_task_tcb_start(task) == OK)
	{
		...
		if(ret != OK)
			zlog_err (ZLOG_DEFAULT, "%s: could not lower privs, %s",
...
static int os_task_show_head(struct vty *vty, int detail)
{
	...
	vty_out(vty, "%s                   	   CPU (user+system): Real (wall-clock):%s",VTY_NEWLINE,VTY_NEWLINE);
//vty_out(vty, "Runtime(ms)   Invoked Avg uSec Max uSecs Avg uSec Max uSecs  Type  Thread%s",VTY_NEWLINE);

	if(detail)
		vty_out(vty, "  %-16s %-12s %-20s %-4s %-8s %-6s %-8s %-8s%s","taskNmae","taskId","entry","pri","tacksize",
				"state", "Real", "Total", VTY_NEWLINE);
	else
		vty_out(vty, "  %-16s %-12s %-20s %-4s %-8s %-6s %-8s %-8s%s","taskNmae","taskId","entry","pri","tacksize",
				"state", "Real", "Total",VTY_NEWLINE);
	return 0;
...
static int os_task_show_detail(struct vty *vty, os_task_t *task, int detail)
{
	...
	if(total_cpu.total_realy)
		sprintf(real,"%.2f%%",total_cpu.cpu * 100 * (float)((float)task->hist.total_realy/(float)total_cpu.total_realy));
	else
		sprintf(real,"%s","0.00%");

	if(total_cpu.total)
		sprintf(total,"%.2f%%",total_cpu.cpu * 100 * (float)((float)task->hist.total/(float)total_cpu.total));
	else
		sprintf(total,"%s","0.00%");

	vty_out(vty, "  %-16s %-12s %-20s %-4s %-8s %-6s %-8s %-8s%s",task->td_name,taskId, task->td_entry_name, pri,
			tacksize, state, real, total,VTY_NEWLINE);
	return 0;
}
