...
extern int zploptind;

/* Callers store zero here to inhibit the error message `getopt' prints
   for unrecognized options.  */

extern int zplopterr;

/* Set to an option character which was unrecognized.  */

extern int zploptopt;

/* Describe the long-named options requested by the application.
   The LONG_OPTIONS argument to getopt_long or getopt_long_only is a vector
   of `struct zploption' terminated by an element containing a name which is
   zero.
...
/* Names for the values of the `has_arg' field of `struct zploption'.  */
...
extern int zplgetopt_long (int argc, char *const *argv, const char *shortopts,
		        const struct zploption *longopts, int *longind);
extern int zplgetopt_long_only (int argc, char *const *argv,
			     const char *shortopts,
		             const struct zploption *longopts, int *longind);

/* Internal only.  Users should not call this directly.  */
extern int zpl_getopt_internal (int argc, char *const *argv,
			     const char *shortopts,
		             const struct zploption *longopts, int *longind,
			     int long_only);
#else /* not __STDC__ */
