...
{
	install_element(node, &cli_mqtt_client_version_cmd);
	install_element(node, &no_cli_mqtt_client_version_cmd);

	install_element(node, &cli_mqtt_bind_address_cmd);
	install_element(node, &no_cli_mqtt_bind_address_cmd);

	install_element(node, &cli_mqtt_port_cmd);
	install_element(node, &no_cli_mqtt_port_cmd);

	install_element(node, &cli_mqtt_server_cmd);
	install_element(node, &no_cli_mqtt_server_cmd);

	install_element(node, &cli_mqtt_username_cmd);
	install_element(node, &no_cli_mqtt_username_cmd);

	install_element(node, &cli_mqtt_password_cmd);
	install_element(node, &no_cli_mqtt_password_cmd);

	install_element(node, &cli_mqtt_client_id_cmd);
	install_element(node, &no_cli_mqtt_client_id_cmd);

	install_element(node, &cli_mqtt_client_id_prefix_cmd);
	install_element(node, &no_cli_mqtt_client_id_prefix_cmd);

	install_element(node, &cli_mqtt_client_keepalive_cmd);
	install_element(node, &no_cli_mqtt_client_keepalive_cmd);

	install_element(node, &cli_mqtt_client_max_inflight_cmd);
	install_element(node, &no_cli_mqtt_client_max_inflight_cmd);

	install_element(node, &cli_mqtt_client_qos_cmd);
	install_element(node, &no_cli_mqtt_client_qos_cmd);

	install_element(node, &cli_mqtt_client_topic_cmd);
	install_element(node, &no_cli_mqtt_client_topic_cmd);


	install_element(node, &cli_mqtt_client_retain_cmd);
	install_element(node, &cli_mqtt_client_clean_session_cmd);
	install_element(node, &cli_mqtt_client_session_expiry_interval_cmd);
	install_element(node, &cli_mqtt_client_will_retain_cmd);
	install_element(node, &cli_mqtt_client_will_qos_cmd);
	install_element(node, &cli_mqtt_client_remove_retain_cmd);

	install_element(node, &cli_mqtt_client_will_payload_cmd);
	install_element(node, &no_cli_mqtt_client_will_payload_cmd);

	install_element(node, &cli_mqtt_client_will_topic_cmd);
	install_element(node, &no_cli_mqtt_client_will_topic_cmd);

	install_element(node, &cli_mqtt_client_option_cmd);
	install_element(node, &no_cli_mqtt_client_option_cmd);

#ifdef WITH_TLS
	install_element(node, &cli_mqtt_client_tls_insecure_cmd);

	install_element(node, &cli_mqtt_client_tls_keyfile_cmd);
	install_element(node, &no_cli_mqtt_client_tls_keyfile_cmd);

	install_element(node, &cli_mqtt_client_tls_keywork_cmd);
	install_element(node, &no_cli_mqtt_client_tls_keywork_cmd);
#endif

#ifdef FINAL_WITH_TLS_PSK
	install_element(node, &cli_mqtt_client_tls_psk_cmd);
	install_element(node, &no_cli_mqtt_client_tls_psk_cmd);
#endif

	install_element(node, &cli_mqtt_client_commit_cmd);
}

static void cmd_show_mqtt_init(int node)
{
	install_element(node, &show_cli_mqtt_server_cmd);
	install_element(node, &show_debugging_mqtt_cmd);
}
...
void cmd_mqtt_init(void)
{
	...
	if(temp)
	{
		...
		install_element(CONFIG_NODE, &cli_mqtt_service_cmd);
		install_element(CONFIG_NODE, &no_cli_mqtt_service_cmd);
		...
		install_element(ENABLE_NODE, &debug_cli_mqtt_cmd);
		install_element(ENABLE_NODE, &no_debug_cli_mqtt_cmd);
		install_element(CONFIG_NODE, &debug_cli_mqtt_cmd);
		install_element(CONFIG_NODE, &no_debug_cli_mqtt_cmd);
	}
}
