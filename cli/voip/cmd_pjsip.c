...
*/
	...
	install_element(node, &ip_sip_register_delay_cmd);
	install_element(node, &no_ip_sip_register_delay_cmd);

	install_element(node, &ip_sip_register_proxy_cmd);
	install_element(node, &no_ip_sip_register_proxy_cmd);

	install_element(node, &ip_sip_option_set_cmd);
	install_element(node, &no_ip_sip_option_set_cmd);

	install_element(node, &ip_sip_srtp_mode_set_cmd);
	install_element(node, &no_ip_sip_srtp_mode_set_cmd);

	install_element(node, &ip_sip_srtp_secure_set_cmd);
	install_element(node, &no_ip_sip_srtp_secure_set_cmd);

	install_element(node, &ip_sip_srtp_keying_set_cmd);
	install_element(node, &no_ip_sip_srtp_keying_set_cmd);

	install_element(node, &ip_sip_session_timers_set_cmd);
	install_element(node, &no_ip_sip_session_timers_set_cmd);

	install_element(node, &ip_sip_session_timers_sec_set_cmd);
	install_element(node, &no_ip_sip_session_timers_sec_set_cmd);

	install_element(node, &ip_sip_auto_update_nat_set_cmd);
	install_element(node, &no_ip_sip_auto_update_nat_set_cmd);

	install_element(node, &ip_sip_stun_enable_set_cmd);
	install_element(node, &no_ip_sip_stun_enable_set_cmd);

	install_element(node, &ip_sip_ipv6_enable_set_cmd);
	install_element(node, &no_ip_sip_ipv6_enable_set_cmd);


	install_element(node, &ip_sip_tagging_qos_set_cmd);
	install_element(node, &no_ip_sip_tagging_qos_set_cmd);

	install_element(node, &ip_sip_transport_tcpudp_set_cmd);
	install_element(node, &no_ip_sip_transport_tcpudp_set_cmd);

	install_element(node, &ip_sip_nameserver_cmd);
	install_element(node, &no_ip_sip_nameserver_cmd);
	install_element(node, &ip_sip_nameserver_port_cmd);
	install_element(node, &no_ip_sip_nameserver_port_cmd);

	install_element(node, &ip_sip_outbound_cmd);
	install_element(node, &no_ip_sip_outbound_cmd);

	install_element(node, &ip_sip_outbound_port_cmd);
	install_element(node, &no_ip_sip_outbound_port_cmd);

	install_element(node, &ip_sip_stun_server_cmd);
	install_element(node, &no_ip_sip_stun_server_cmd);

	install_element(node, &ip_sip_stun_server_port_cmd);
	install_element(node, &no_ip_sip_stun_server_port_cmd);
#if defined(PJSIP_HAS_TLS_TRANSPORT) && (PJSIP_HAS_TLS_TRANSPORT != 0)
	install_element(node, &ip_sip_tls_enable_set_cmd);
	install_element(node, &no_ip_sip_tls_enable_set_cmd);

	install_element(node, &ip_sip_tls_ca_file_cmd);
	install_element(node, &no_ip_sip_tls_ca_file_cmd);

	install_element(node, &ip_sip_tls_cert_file_cmd);
	install_element(node, &no_ip_sip_tls_cert_file_cmd);

	install_element(node, &ip_sip_tls_private_file_cmd);
	install_element(node, &no_ip_sip_tls_private_file_cmd);

	install_element(node, &ip_sip_tls_password_cmd);
	install_element(node, &no_ip_sip_tls_password_cmd);

	install_element(node, &ip_sip_verify_server_cmd);
	install_element(node, &no_ip_sip_verify_server_cmd);

	install_element(node, &ip_sip_verify_server_port_cmd);
	install_element(node, &no_ip_sip_verify_server_port_cmd);

	install_element(node, &ip_sip_verify_client_cmd);
	install_element(node, &no_ip_sip_verify_client_cmd);

	install_element(node, &ip_sip_verify_client_port_cmd);
	install_element(node, &no_ip_sip_verify_client_port_cmd);

	install_element(node, &ip_sip_tls_cipher_cmd);
	install_element(node, &no_ip_sip_tls_cipher_cmd);
	...
	install_element(node, &ip_sip_snd_clock_rate_cmd);
	install_element(node, &no_ip_sip_snd_clock_rate_cmd);

	install_element(node, &ip_sip_auto_play_file_cmd);
	install_element(node, &no_ip_sip_auto_play_file_cmd);

	install_element(node, &ip_sip_auto_tone_file_cmd);
	install_element(node, &no_ip_sip_auto_tone_file_cmd);

	install_element(node, &ip_sip_rec_file_cmd);
	install_element(node, &no_ip_sip_rec_file_cmd);

	install_element(node, &ip_sip_auto_option_set_cmd);
	install_element(node, &no_ip_sip_auto_option_set_cmd);

	install_element(node, &ip_sip_media_quality_cmd);
	install_element(node, &no_ip_sip_media_quality_cmd);

	install_element(node, &ip_sip_codec_ptime_cmd);
	install_element(node, &no_ip_sip_codec_ptime_cmd);

	install_element(node, &ip_sip_vad_silence_cmd);
	install_element(node, &no_ip_sip_vad_silence_cmd);


	install_element(node, &ip_sip_echo_tail_len_cmd);
	install_element(node, &no_ip_sip_echo_tail_len_cmd);

	install_element(node, &ip_sip_echo_mode_cmd);
	install_element(node, &no_ip_sip_echo_mode_cmd);

	install_element(node, &ip_sip_ilbc_fps_cmd);
	install_element(node, &no_ip_sip_ilbc_fps_cmd);

	install_element(node, &ip_sip_ice_enable_cmd);
	install_element(node, &no_ip_sip_ice_enable_cmd);

	install_element(node, &ip_sip_ice_regular_cmd);
	install_element(node, &no_ip_sip_ice_regular_cmd);

	install_element(node, &ip_sip_ice_max_host_cmd);
	install_element(node, &no_ip_sip_ice_max_host_cmd);

	install_element(node, &ip_sip_ice_notcp_cmd);
	install_element(node, &no_ip_sip_ice_notcp_cmd);

	install_element(node, &ip_sip_rtp_port_cmd);
	install_element(node, &no_ip_sip_rtp_port_cmd);

	install_element(node, &ip_sip_drop_pct_cmd);
	install_element(node, &no_ip_sip_drop_pct_cmd);

	install_element(node, &ip_sip_turn_enable_cmd);
	install_element(node, &no_ip_sip_turn_enable_cmd);

	install_element(node, &ip_sip_turn_tcp_enable_cmd);
	install_element(node, &no_ip_sip_turn_tcp_enable_cmd);

	install_element(node, &ip_sip_turn_server_cmd);
	install_element(node, &no_ip_sip_turn_server_cmd);

	install_element(node, &ip_sip_turn_server_port_cmd);
	install_element(node, &no_ip_sip_turn_server_port_cmd);

	install_element(node, &ip_sip_turn_username_cmd);
	install_element(node, &no_ip_sip_turn_username_cmd);

	install_element(node, &ip_sip_turn_password_cmd);
	install_element(node, &no_ip_sip_turn_password_cmd);

	install_element(node, &ip_sip_auto_answer_code_cmd);
	install_element(node, &no_ip_sip_auto_answer_code_cmd);
	...
	install_element(node, &ip_sip_max_calls_cmd);
	install_element(node, &no_ip_sip_max_calls_cmd);

	install_element(node, &ip_sip_max_call_duration_cmd);
	install_element(node, &no_ip_sip_max_call_duration_cmd);

	install_element(node, &ip_sip_norefersub_enable_cmd);
	install_element(node, &no_ip_sip_norefersub_enable_cmd);
