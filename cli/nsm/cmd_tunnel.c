...
*/
				...
				if(tunnel)
				{
					...
					vty_out(vty, " tunnel source %s%s", inet_ntoa(tunnel->source.u.prefix4), VTY_NEWLINE);
					vty_out(vty, " tunnel destination %s%s", inet_ntoa(tunnel->remote.u.prefix4), VTY_NEWLINE);
...
{
	install_element(node, &nsm_tunnel_protocol_cmd);
	install_element(node, &no_nsm_tunnel_protocol_cmd);

	install_element(node, &nsm_tunnel_ttl_cmd);
	install_element(node, &no_nsm_tunnel_ttl_cmd);

	install_element(node, &nsm_tunnel_mtu_cmd);
	install_element(node, &no_nsm_tunnel_mtu_cmd);

	install_element(node, &nsm_tunnel_tos_cmd);
	install_element(node, &no_nsm_tunnel_tos_cmd);

	install_element(node, &nsm_tunnel_source_cmd);
	install_element(node, &no_nsm_tunnel_source_cmd);

	install_element(node, &nsm_tunnel_destination_cmd);
	install_element(node, &no_nsm_tunnel_destination_cmd);
}
