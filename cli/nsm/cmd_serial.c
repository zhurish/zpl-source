		...
		"8 bit\n")
{
	...
	struct interface *ifp = vty->index;
	if(ifp)
		ret = nsm_serial_interface_data(ifp, atoi(argv[0]));
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"data bit configure\n")
{
	...
	struct interface *ifp = vty->index;
	if(ifp)
		ret = nsm_serial_interface_data(ifp, NSM_SERIAL_DATA_DEFAULT);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"2 bit\n")
{
	...
	struct interface *ifp = vty->index;
	if(ifp)
		ret = nsm_serial_interface_stop(ifp, atoi(argv[0]));
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
		...
		"stop bit configure\n")
{
	...
	struct interface *ifp = vty->index;
	if(ifp)
		ret = nsm_serial_interface_stop(ifp, NSM_SERIAL_STOP_DEFAULT);
	return (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
...
{
	install_element(node, &serial_clock_rate_cmd);
	install_element(node, &no_serial_clock_rate_cmd);

	install_element(node, &serial_data_bit_cmd);
	install_element(node, &no_serial_data_bit_cmd);

	install_element(node, &serial_stop_bit_cmd);
	install_element(node, &no_serial_stop_bit_cmd);

	install_element(node, &serial_parity_cmd);
	install_element(node, &no_serial_parity_cmd);

	install_element(node, &serial_flow_cmd);
	install_element(node, &no_serial_flow_cmd);
}
