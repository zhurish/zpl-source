...
zpl_uint32 _sqldb_debug = V9_SQLDB_DEBUG_MSG | V9_SQLDB_DEBUG_DB
...
static char * v9_user_sqldb_file(zpl_uint32 id)
{
	zpl_uint32 iid = APP_BOARD_CALCU_1;
...
static sqlite3 * v9_user_sqldb_init(zpl_uint32 id)
{
	int ret = 0;
	sqlite3 *db = NULL;
...
static int v9_user_sqldb_sync(sqlite3 *db, zpl_uint32 id)
{
	if (db)
	{
...
static zpl_bool v9_user_sqldb_table_exist(sqlite3 *db, zpl_uint32 id)
{
	char sqlcmd[256];
	char *zErrMsg = NULL;
...
static int v9_user_sqldb_table_create(sqlite3 *db, zpl_uint32 id)
{
	char sqlcmd[512];
	char *zErrMsg = NULL;
...
sqlite3 * v9_user_sqldb_open(zpl_uint32 id)
{
	sqlite3 *db = NULL;
	if (db == NULL)
	...
	if (!db)
		return NULL;
	if (v9_user_sqldb_table_exist(db, id) != zpl_true)
	{
...
int v9_user_sqldb_close(sqlite3 *db, zpl_uint32 id)
{
...
int v9_user_sqldb_count(sqlite3 *db, zpl_uint32 id, int group, int *pValue)
{
	int value = 0;
	char sqlcmd[512];
	char *zErrMsg = NULL;
...
int v9_user_sqldb_add(sqlite3 *db, zpl_uint32 id, zpl_bool gender, int group,
...
int v9_user_sqldb_update(sqlite3 *db, zpl_uint32 id, zpl_bool gender, int group,
		char *user, char *user_id, char *pic, char *text)
{
	char sqlcmd[1024];
	char *zErrMsg = NULL;
...
int v9_user_sqldb_key_update(sqlite3 *db, zpl_uint32 id, char *user_id,
		sql_snapfea_key *key)
{
	char sqlcmd[512];
...
int v9_user_sqldb_key_select(sqlite3 *db, zpl_uint32 id, char *user_id,
		sql_snapfea_key *key)
{
	int ret = 0;
	char sqlcmd[512];
...
/*static int v9_user_sqldb_del_callback(void *pArgv, zpl_uint32 type, char const * name,
...
int v9_user_sqldb_cleanup(zpl_uint32 id)
{
/*	if(_user_mutex)
...
int v9_user_sqldb_del(sqlite3 *db, zpl_uint32 id, char *user_id)
{
	char sqlcmd[256];
	char *zErrMsg = NULL;
	memset(sqlcmd, 0, sizeof(sqlcmd));
...
int v9_user_sqldb_del_group(sqlite3 *db, zpl_uint32 id, int group)
{
	char sqlcmd[256];
	char *zErrMsg = NULL;
	memset(sqlcmd, 0, sizeof(sqlcmd));
...
int v9_user_sqldb_lookup_user(sqlite3 *db, zpl_uint32 id, char *user_id,
		void *pArgv)
{
	char sqlcmd[512];
	char *zErrMsg = NULL;
...
int v9_user_sqldb_foreach(sqlite3 *db, zpl_uint32 id, int groupid, char *user_id,
		void * cb, void *pVoid)
{
	char sqlcmd[512];
	char *zErrMsg = NULL;
...
int v9_user_sqldb_key_foreach(sqlite3 *db, zpl_uint32 id,
		sql_snapfea_key *key, char *user_id, zpl_float* p_fResult)
{
	int ret = 0;
	char sqlcmd[512];
	sqlite3_stmt *stmt = NULL;
		...
		if (key && len && pReadBolbData)
		{
			if (key->feature_memcmp)
			{
				ret = (key->feature_memcmp)(key->feature.feature_data, pReadBolbData,
						MIN(len / sizeof(zpl_float), key->feature_len), &fresult);
				if (ret == OK)
				{
					//if (fresult >= value)
...
#ifdef V9_SQLDB_TEST
int v9_user_sqldb_test(zpl_uint32 i, void *pArg)
{
	//struct vty *vty = pArg;
	int pValue = 0;
	...
	if (db)
	{
		if (v9_user_sqldb_table_exist(db, 2) != zpl_true)
		{
