...
int hal_vlan_batch_create(zpl_vlan_bitmap_t vlanbitmap, vlan_t start, vlan_t end)
{
	//zpl_uint32 i = 0;
	zpl_uint32 command = 0;
	struct hal_ipcmsg ipcmsg;
	...
	HAL_ENTER_FUNC();
	hal_ipcmsg_msg_init(&ipcmsg, buf, sizeof(buf));
	hal_ipcmsg_putw(&ipcmsg, start);
	hal_ipcmsg_putw(&ipcmsg, end);
	//for(i = 0; i < num; i++)
		hal_ipcmsg_put(&ipcmsg, vlanbitmap.bitmap, sizeof(zpl_vlan_bitmap_t));
	command = IPCCMD_SET(HAL_MODULE_VLAN, HAL_MODULE_CMD_REQ, HAL_VLAN_RANGE_CREATE);
	return hal_ipcmsg_send_message(IF_UNIT_ALL,
		command, buf, hal_ipcmsg_msglen_get(&ipcmsg));
}

int hal_vlan_batch_destroy(zpl_vlan_bitmap_t vlanbitmap, vlan_t start, vlan_t end)
{
	//zpl_uint32 i = 0;
	zpl_uint32 command = 0;
	struct hal_ipcmsg ipcmsg;
...
int hal_vlan_test(int cmd, ifindex_t phyport, vlan_t vlan)
{
	...
	hal_ipcmsg_msg_init(&ipcmsg, buf, sizeof(buf));
	//port
    hal_ipcmsg_putc(&ipcmsg, 0); 
    hal_ipcmsg_putc(&ipcmsg, 0);
    hal_ipcmsg_putc(&ipcmsg, 0);
    hal_ipcmsg_putl(&ipcmsg, 0);
    hal_ipcmsg_putl(&ipcmsg, phyport);
    hal_ipcmsg_putl(&ipcmsg, 0);
    hal_ipcmsg_putl(&ipcmsg, 0);

	hal_ipcmsg_putl(&ipcmsg, cmd);
	hal_ipcmsg_putw(&ipcmsg, vlan);

	command = IPCCMD_SET(HAL_MODULE_VLAN, HAL_MODULE_CMD_REQ, HAL_VLAN_TEST);
	return hal_ipcmsg_send_message(0, 
		command, buf, hal_ipcmsg_msglen_get(&ipcmsg));
}
#if 0
#if 0
