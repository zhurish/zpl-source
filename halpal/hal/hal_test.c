...
{
	int ret = 0;
	ret = hal_mirror_source_enable(if_ifindex_make(argv[0], argv[1]), zpl_true, MIRROR_SOURCE_PORT);
	return  (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
...
{
	int ret = 0;
	ret = hal_mirror_source_enable(if_ifindex_make(argv[0], argv[1]), zpl_false, MIRROR_SOURCE_PORT);
	return  (ret == OK)? CMD_SUCCESS:CMD_WARNING;
}
#endif
/*
