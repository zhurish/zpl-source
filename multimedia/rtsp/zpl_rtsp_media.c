                          ...
                          const zpl_media_buffer_data_t *bufdata,  void *pVoidUser)
{
    ...
    ret = rtsp_media_adap_rtp_sendto(bufdata->buffer_codec, pVoidUser, bufdata->buffer_type,
                                     bufdata->buffer_data, bufdata->buffer_len);
...
int rtsp_media_rtp_recv(rtsp_session_t* session, bool bvideo, zpl_media_buffer_data_t *bufdata)
{
    ...
    while (havemore)
    {
        ...
        if (ret>0){
            ...
            //zpl_media_buffer_data_append(bufdata, pbuffer + 4, ret);
        }
        if (havemore)
        {
    ...
    if(bvideo && bufdata->buffer_len)
    {
        bufdata->buffer_type = ZPL_MEDIA_VIDEO;        //音频视频
        bufdata->buffer_codec = session->video_session.payload;       //编码类型
    ...
    else if(!bvideo && bufdata->buffer_len)
    {
        bufdata->buffer_type = ZPL_MEDIA_AUDIO;        //音频视频
        bufdata->buffer_codec = session->audio_session.payload;       //编码类型
        //bufdata->buffer_key = 0;         //帧类型
        //bufdata->buffer_priv = 0;
        //bufdata->buffer_timetick = 0;    //时间戳 毫秒
        //bufdata->buffer_seq = 0;         //序列号 底层序列号
    ...
    if(bufdata->buffer_type == ZPL_MEDIA_AUDIO)
        fprintf(stdout, "===============rtsp_media_rtp_recv audio %d byte timestamp=%d(ret=%d)\n", bufdata->buffer_len, session->audio_session.user_timestamp, ret);
    else if(bufdata->buffer_type == ZPL_MEDIA_VIDEO)
        fprintf(stdout, "===============rtsp_media_rtp_recv video %d byte timestamp=%d(ret=%d)\n", bufdata->buffer_len, session->video_session.user_timestamp, ret);
    fflush(stdout);
