...
int vpxDecoder::videoDecoderSetup(const int width, const int height, const int fmt, const int fps)
{
    ...
    */
    ...
    this->dec_buf = new unsigned char [this->dec_buf_size];
    return 0;
}

#if 0
int vpxDecoder::vpx_unpacketize(const unsigned char *buf,
                                size_t packet_size,
                                unsigned *p_desc_len)
{
    unsigned desc_len = 1;
...
int vpxDecoder::vpx_decode_framed(const unsigned char *packets, const  int in_size,
                                  unsigned char *output, unsigned out_size)
{
    ...
    /* Decode */
    res = vpx_codec_decode(&dec, (const uint8_t *)packets, in_size, 0, VPX_DL_REALTIME);
    if (res)
    {
...
int vpxDecoder::videoDecoderInput(const  unsigned char *frame, const int len)
{
    return vpx_decode_framed((unsigned char *)frame, len,
                                  dec_buf, dec_buf_size);
}

int vpxDecoder::videoDecoderOutput(unsigned char *frame, const int len)
{
...
unsigned char *vpxDecoder::videoDecoderOutput()
{
    if(dec_buf != nullptr && m_out_size > 0)
        return (unsigned char *)dec_buf;
    return nullptr;
}
