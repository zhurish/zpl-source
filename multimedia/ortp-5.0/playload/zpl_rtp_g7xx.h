 ...
 * 多个nalu组成一个包
 */
RTSP_API int _g7xx_data_packet(zpl_buffer_data_t *nalu, uint8_t *out);
/*
 * 发送一帧数据
 */
RTSP_API int _rtp_send_g7xx(rtsp_session_t *session, int type, const uint8_t *buffer, uint32_t len);
RTSP_API int _rtp_unpacket_g7xx(zpl_buffer_data_t *packet, uint8_t *payload, size_t payload_len);
/*
 * 封装g7xx媒体的SDP
 */
