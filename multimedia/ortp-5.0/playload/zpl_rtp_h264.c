...
static int _h264_file_read_more_data(zpl_media_file_t *file)
{
    ...
    if(feof(file->fp) != 0)
    {
        if(file->tmppacket.buffer_len <= 0)
            return -1;
    }
    else
    {
        memset(file->tmppacket.buffer_data + file->tmppacket.buffer_len, 0, file->tmppacket.buffer_maxsize - file->tmppacket.buffer_len);
        ret = fread(file->tmppacket.buffer_data + file->tmppacket.buffer_len, 1, file->tmppacket.buffer_maxsize - file->tmppacket.buffer_len, file->fp);
        if(ret <= 0)
        {
            if(file->tmppacket.buffer_len <= 0)
                return -1;
        }
        file->tmppacket.buffer_len += ret;
        return file->tmppacket.buffer_len;
    }
    return 0;
}
...
{
    file->tmppacket.buffer_len = file->tmppacket.buffer_len - packetsize;
    if(file->tmppacket.buffer_len && file->tmppacket.buffer_len < file->tmppacket.buffer_maxsize)
        memmove(file->tmppacket.buffer_data, file->tmppacket.buffer_data + packetsize, file->tmppacket.buffer_len);
    return 0;
}

static int _h264_file_read_data(zpl_media_file_t *file, zpl_buffer_data_t *outpacket)
{
    int finsh = 0;
    ...
    while(1)
    {
        if(_h264_file_read_more_data(file) == -1)
            return -1;
        p = file->tmppacket.buffer_data;

        if(packetsize > file->tmppacket.buffer_len)
            packetsize = file->tmppacket.buffer_len;
        else
        ...
        if(nalulen)
        {
            if(file->flags & NALU_START_FLAG)
            {
                ...
                {
                    naluhdr.len += (nalulen);
                    zpl_media_buffer_data_append(outpacket, file->tmppacket.buffer_data, nalulen);
                    _h264_file_data_append_finsh(file, nalulen);
                    file->flags = 0;
                    //zpl_media_channel_nalu_show(&naluhdr);
                    return outpacket->buffer_len;
                }
                else
                {
                    ...
                    zpl_media_buffer_data_append(outpacket, file->tmppacket.buffer_data, nalulen + naluhdr.hdr_len);
        ...
        else
        {
            if(packetsize < MAX_RTP_PAYLOAD_LENGTH)
            {
                if((file->flags & NALU_MAX_FLAG))
                {
                    naluhdr.len += file->tmppacket.buffer_len;
                    packetsize = file->tmppacket.buffer_len;
                    finsh = 1;
                }
                else
            ...
            zpl_media_buffer_data_append(outpacket, file->tmppacket.buffer_data, packetsize);
 ...
 * 多个nalu组成一个包
 */
int _rtp_packet_h264(zpl_buffer_data_t *nalu, uint8_t *out)
{
    uint16_t packetlen = 0;
    uint16_t *nalulen = NULL;
    if(nalu->buffer_data && nalu->buffer_len)
    {
        nalulen = (uint16_t *)out;
        if (is_nalu4_start(nalu->buffer_data))
        {
            packetlen = nalu->buffer_len + 2 - 4;
            *nalulen = htons(nalu->buffer_len-4);
            memcpy(out, nalu->buffer_data + 4, nalu->buffer_len - 4);
            nalu->buffer_len = 0;
            return packetlen;
        }
        else if (is_nalu3_start(nalu->buffer_data))
        {
            packetlen = nalu->buffer_len + 2 - 3;
            *nalulen = htons(nalu->buffer_len-3);
            memcpy(out, nalu->buffer_data + 3, nalu->buffer_len - 3);
            nalu->buffer_len = 0;
            return packetlen;
        }
        nalu->buffer_len = 0;
...
static int h264_nalu_unpacket_stap_a(uint8_t *payload,
                                     size_t payload_len, zpl_buffer_data_t *packet)
{
    ...
    while(len)
    {
        ...
        p += 2;
        len -= 2;
        if(packet->buffer_data == NULL)
        {
            packet->buffer_data = malloc(nalusize + 4);
            packet->buffer_len = packet->buffer_maxsize = nalusize + 4;
        }
        else
        {
            if(packet->buffer_maxsize < (packet->buffer_len + nalusize + 4))
            {
                packet->buffer_data = realloc(packet->buffer_data, nalusize + 4);
                packet->buffer_len += nalusize + 4;
                packet->buffer_maxsize = packet->buffer_len;
            }
        }
        if(packet->buffer_data)
        {
            if(dst == NULL)
                dst = packet->buffer_data;
            memcpy(dst, nal_start, 4);
            dst += 4;
            memcpy(dst, p, nalusize);
            packet->buffer_len += nalusize;
            dst += nalusize;
            p += nalusize;
            len -= nalusize;
        }
    }
    return packet->buffer_len;
}
                                   ...
                                   size_t payload_len, zpl_buffer_data_t *packet)
{
    ...
    if(hdr->FU_HDR_S)
    {
        ...
        zpl_media_buffer_data_append(packet, nal_start, 4);
        zpl_media_buffer_data_append(packet, &nalu, 1);
    }
    else
    {
        //payload[1] = nalu;
        zpl_media_buffer_data_append(packet, payload + 2, payload_len - 2);
    }
    if(hdr->FU_HDR_E)
    {
        return packet->buffer_len;
...
int _rtp_unpacket_h264(zpl_buffer_data_t *packet, uint8_t *payload,
                       size_t payload_len)
{
    int ret = 0;
    uint8_t nal_type = 0;
    ...
    default:
        ...
        {
            uint8_t nal_start[4] = {0, 0, 0, 1};
            ret = zpl_media_buffer_data_append(packet, nal_start, 4);
            ret = zpl_media_buffer_data_append(packet, payload, payload_len);
        }
        break;
    }
    return ret;
...
zpl_media_file_open(rfile);
        ...
        while(1)
        {
            ...
            if(ret > 0)
            {
                ret = zpl_media_file_write_data(wfile, packet.buffer_data, packet.buffer_len);
