...
/*	
	char *scanner_curptr = st->scanner.curptr;
	if(scanner_curptr[1] == ']')
	{
		pj_json_elem *child = NULL;
		st->scanner.curptr = scanner_curptr + 1;
	...
	else if(scanner_curptr[2] == ']')
	{
		pj_json_elem *child = NULL;
		st->scanner.curptr = scanner_curptr + 2;
		pj_scan_get_char(&st->scanner);
