...
static void capture(char *filename, void *param);
static void playbackv(char **filenames, unsigned int count);
static void capturev(char **filenames, unsigned int count, void *param);
...
int open_capture_param (void *am, char *name)
{
	int err = 0;
	//struct alsa_stream* pjstream = m;
	pjmedia_aud_param *pjparam = am;
	...
	chunk_size = 1024;
	//pjstream->ca_pcm = handle;
	//pjparam->bits_per_sample = bits_per_sample;
	//pjparam->channel_count = 1;
    /* Set clock rate */
	//pjparam->clock_rate = 8000;

	rhwparams.channels = pjparam->channel_count;
	rhwparams.format = SND_PCM_FORMAT_S16_LE;
	rhwparams.rate = pjparam->clock_rate;
	...
	err = snd_pcm_open(&handle, name, stream, open_mode);
	if (err < 0) {
...
snd_pcm_t *open_capture_get_pcm ()
{
	return handle;
}
...
static void device_list(void)
{
	...
	while (card >= 0) {
		...
		if ((err = snd_ctl_open(&devhandle, name, 0)) < 0) {
			error("control open (%i): %s", card, snd_strerror(err));
			goto next_card;
		}
		if ((err = snd_ctl_card_info(devhandle, info)) < 0) {
			error("control hardware info (%i): %s", card, snd_strerror(err));
			snd_ctl_close(devhandle);
			goto next_card;
		}
		dev = -1;
		while (1) {
			unsigned int count;
			if (snd_ctl_pcm_next_device(devhandle, &dev)<0)
			...
			if ((err = snd_ctl_pcm_info(devhandle, pcminfo)) < 0) {
				if (err != -ENOENT)
...
int aamain(int argc, char *argv[], void *param)
{
	...
	if (interleaved) {
		...
		if (stream == SND_PCM_STREAM_PLAYBACK)
			...
			capturev(&argv[optind], argc - optind, param);
	}
	if (verbose==2)
		putchar('\n');
	snd_pcm_close(handle);
...
static void done_stdin(void)
{
	...
	if (!interactive)
		return;
	if (gfd == fileno(stdin) || term_c_lflag == -1)
		return;
	tcgetattr(fileno(stdin), &term);
...
static int read_header(int *loaded, int header_size)
{
	...
	if (*loaded < header_size) {
		header_size -= *loaded;
		ret = safe_read(gfd, audiobuf + *loaded, header_size);
		if (ret != header_size) {
			error(_("read error"));
			prg_exit(EXIT_FAILURE);
		}
...
static int playback_au(char *name, int *loaded)
{
	...
	if (test_au(gfd, audiobuf) < 0)
		return -1;

	rhwparams.format = hwparams.format;
	pbrec_count = calc_count();
	playback_go(gfd, *loaded - sizeof(AuHeader), pbrec_count, FORMAT_AU, name);
...
static int playback_wave(char *name, int *loaded)
{
	...
	if ((dtawave = test_wavefile(gfd, audiobuf, *loaded)) < 0)
		return -1;

	pbrec_count = calc_count();
	playback_go(gfd, dtawave, pbrec_count, FORMAT_WAVE, name);
...
static int playback_raw(char *name, int *loaded)
{
	init_raw_data();
	pbrec_count = calc_count();
	playback_go(gfd, *loaded, pbrec_count, FORMAT_RAW, name);
...
static void capture(char *orig_name, void *param)
{
	...
	do {
		...
		if (fmt_rec_table[file_type].start)
			fmt_rec_table[file_type].start(gfd, rest);
		...
		if (fmt_rec_table[file_type].end && !tostdout) {
			fmt_rec_table[file_type].end(gfd);
			gfd = -1;
		}
...
static void capturev(char **names, unsigned int count, void *param)
{
	int ret = 0;
	unsigned int channel;
	unsigned int channels = rhwparams.channels;
