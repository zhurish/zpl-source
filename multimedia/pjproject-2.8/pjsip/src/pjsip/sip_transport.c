...
static void transport_send_callback(pjsip_transport *pjtransport,
				    void *token,
				    pj_ssize_t size)
{
    pjsip_tx_data *tdata = (pjsip_tx_data*) token;

    PJ_UNUSED_ARG(pjtransport);
...
/* send_raw() callback */
static void send_raw_callback(pjsip_transport *pjtransport,
			      void *token,
			      pj_ssize_t size)
{
    pjsip_tx_data *tdata = (pjsip_tx_data*) token;
    ...
    pjsip_transport_dec_ref(pjtransport);
}
...
PJ_DEF(pj_status_t) pjsip_tpmgr_destroy( pjsip_tpmgr *mgr )
{
	...
	pjsip_transport *pjtransport;
	
	pjtransport = (pjsip_transport*) pj_hash_this(mgr->table, itr);

	next = pj_hash_next(mgr->table, itr);

	destroy_transport(mgr, pjtransport);
						   ...
						   pjsip_transport **tp)
{
	...
	*tp = seltp;
	...
	/* If transport is found and listener is specified, verify listener */
		 ...
		 sel->u.listener && pjtransport->factory != sel->u.listener)
	{
		pjtransport = NULL;
	...
	if (pjtransport!=NULL && !pjtransport->is_shutdown) {
	    /*
	     * Transport found!
	     */
	    pjsip_transport_add_ref(pjtransport);
	    pj_lock_release(mgr->lock);
	    *tp = pjtransport;

	    TRACE_((THIS_FILE, "Transport %s acquired", pjtransport->obj_name));
	    return PJ_SUCCESS;
	}
