...
ARGBBlendRow GetARGBBlend() {
  void (*ARGBBlendRowCb)(const uint8* src_argb, const uint8* src_argb1,
                       uint8* dst_argb, int width) = ARGBBlendRow_C;
#if defined(HAS_ARGBBLENDROW_SSSE3)
  if (TestCpuFlag(kCpuHasSSSE3)) {
	  ARGBBlendRowCb = ARGBBlendRow_SSSE3;
    return ARGBBlendRowCb;
  }
#endif
#if defined(HAS_ARGBBLENDROW_NEON)
  if (TestCpuFlag(kCpuHasNEON)) {
	  ARGBBlendRowCb = ARGBBlendRow_NEON;
  }
#endif
#if defined(HAS_ARGBBLENDROW_MSA)
  if (TestCpuFlag(kCpuHasMSA)) {
	  ARGBBlendRowCb = ARGBBlendRow_MSA;
  }
#endif
  return ARGBBlendRowCb;
}
...
int ARGBBlend(const uint8* src_argb0,
  ...
  for (y = 0; y < height; ++y) {
	  ARGBBlendRowCb(src_argb0, src_argb1, dst_argb, width);
