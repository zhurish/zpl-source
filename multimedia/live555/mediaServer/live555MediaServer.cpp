...
extern lst_data_queue_t *m_queue;
...
int srvmain() {
  ...
  *env << "Play streams from this server using the URL\n";
  ...
  /*BasicQueueServerMediaSubsession *q_session =
          BasicQueueServerMediaSubsession::createNew(*env,
                             m_queue, BasicFormatType::BASIC_FORMAT_TYPE_H264);
  */
  H264VideoFileServerMediaSubsession *q_session =
            H264VideoFileServerMediaSubsession::createNew(*env,
                               "out.h264", False);
  ServerMediaSession *sms = ServerMediaSession::createNew(*env, "cam", "camtest","camstream");
  if (sms)
  {
      sms->addSubsession(q_session);
