...
::lookupByName(UsageEnvironment& env, char const* mediumName) {
  Medium* medium;
  if (!Medium::lookupByName(env, mediumName, medium)) return NULL;
  ...
  return (ServerMediaSession*)medium;
}
...
this->envir() << "====================ServerMediaSession:ServerMediaSession:stream=" << fStreamName <<"\n";
