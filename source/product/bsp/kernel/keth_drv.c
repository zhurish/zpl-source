 ...
 */
static int ethkernel_rx(struct net_device *dev, char *data, int RxLen)
{
  struct sk_buff *skb;
...
struct eth_interface *ethkernel_create(char *name, ifindex_t ifindex, char *mac)
{
  ...
  struct eth_interface *db = NULL;
  int ret = 0;

  /* Init network device */
  if (strstr(name, "loop"))
    ndev = alloc_netdev(sizeof(struct eth_interface), name, NET_NAME_UNKNOWN, kloopback_setup);
  else
