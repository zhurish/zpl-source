...
static int b53125_dos_icmpv6_longping_drop(sdk_driver_t *dev, zpl_bool enable)
{
	int ret = 0;
	u32 regval = 0;
	ret |= b53125_read32(dev->sdk_device, B53_DOS_PAGE, B53_DOS_CTL, &regval);
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_ICMP6_LONGPING_DROP_EN;
	else
		regval &= ~B53_ICMP6_LONGPING_DROP_EN;

	ret |= b53125_write32(dev->sdk_device, B53_DOS_PAGE, B53_DOS_CTL, regval);
	sdk_debug_detail(dev, "write %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
...
static int b53125_dos_icmpv4_longping_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_ICMP4_LONGPING_DROP_EN;
	else
		regval &= ~B53_ICMP4_LONGPING_DROP_EN;
...
static int b53125_dos_icmpv6_fragment_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_ICMP6_FRAGMENT_DROP_EN;
	else
		regval &= ~B53_ICMP6_FRAGMENT_DROP_EN;
...
static int b53125_dos_icmpv4_fragment_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_ICMP4_FRAGMENT_DROP_EN;
	else
		regval &= ~B53_ICMP4_FRAGMENT_DROP_EN;
...
static int b53125_dos_tcp_fragerror_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_FRAGERROR_DROP_EN;
	else
		regval &= ~B53_TCP_FRAGERROR_DROP_EN;
...
static int b53125_dos_tcp_shorthdr_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_SHORTHDR_DROP_EN;
	else
		regval &= ~B53_TCP_SHORTHDR_DROP_EN;
...
static int b53125_dos_tcp_synerror_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_SYNERROR_DROP_EN;
	else
		regval &= ~B53_TCP_SYNERROR_DROP_EN;
...
static int b53125_dos_tcp_synfinscan_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_SYNFINSCAN_DROP_EN;
	else
		regval &= ~B53_TCP_SYNFINSCAN_DROP_EN;
...
static int b53125_dos_tcp_nullscan_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_NULLSCAN_DROP_EN;
	else
		regval &= ~B53_TCP_NULLSCAN_DROP_EN;
...
static int b53125_dos_tcp_xmassscan_drop(sdk_driver_t *dev, zpl_bool enable)
{
	...
	sdk_debug_detail(dev, "read %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_CTL, regval);
	if(enable)
		regval |= B53_TCP_XMASSCAN_DROP_EN;
	else
		regval &= ~B53_TCP_XMASSCAN_DROP_EN;
...
static int b53125_dos_tcphdr_minsize(sdk_driver_t *dev, int minsize)
{
	...
	u8 regval = 0;
	regval = minsize;
	ret |= b53125_write8(dev->sdk_device, B53_DOS_PAGE, B53_MIN_TCPHDR_SIZE_CTL, regval);
	sdk_debug_detail(dev, "write %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_MIN_TCPHDR_SIZE_CTL, regval);
	sdk_handle_return(ret);
	return ret;
}
/*************************************************************************/
static int b53125_dos_icmp6_maxsize(sdk_driver_t *dev, int minsize)
{
	...
	u32 regval = 0;
	regval = minsize;
	ret |= b53125_write32(dev->sdk_device, B53_DOS_PAGE, B53_MAX_ICMPV6_SIZE_CTL, regval);
	sdk_debug_detail(dev, "write %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_MAX_ICMPV6_SIZE_CTL, regval);
	sdk_handle_return(ret);
	return ret;
}
/*************************************************************************/
static int b53125_dos_icmp4_maxsize(sdk_driver_t *dev, int minsize)
{
	...
	u32 regval = 0;
	regval = minsize;
	ret |= b53125_write32(dev->sdk_device, B53_DOS_PAGE, B53_MAX_ICMPV4_SIZE_CTL, regval);
	sdk_debug_detail(dev, "write %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_MAX_ICMPV4_SIZE_CTL, regval);
	sdk_handle_return(ret);
	return ret;
}
/*************************************************************************/
int b53125_dos_disable_lean(sdk_driver_t *dev, zpl_bool enable)
{
	...
	if(enable)
		...
		regval &= ~BIT(0);
	ret |= b53125_write8(dev->sdk_device, B53_DOS_PAGE, B53_DOS_DIS_LEARN_CTL, regval);
	sdk_debug_detail(dev, "write %s(ret=%d) page=0x%x reg=0x%x val=0x%x", __func__, ret, B53_DOS_PAGE, B53_DOS_DIS_LEARN_CTL, regval);
	sdk_handle_return(ret);
	return ret;
}
...
DEFUN (b53125_sdk_dos_test,
		...
		"dos (iplandrip|tcpblatdrop|udpblatdrop|tcpnull|tcpxma|tcpsynfinsh|tcpsynerr|"
...
{
	#if defined( _SDK_CLI_DEBUG_EN)	
	install_element(SDK_NODE, CMD_CONFIG_LEVEL, &b53125_sdk_dos_test_cmd);
	#endif
