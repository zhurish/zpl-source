...
static int netlink_parse_info(
		...
		struct nlsock *nl, struct nsm_ipvrf *zvrf)
{
	int status;
	int ret = 0;
	int error;
...
static int netlink_route_multipath(zpl_uint32 cmd, struct prefix *p, struct rib *rib)
{
	...
	struct nsm_ipvrf *zvrf = ip_vrf_info_lookup(rib->vrf_id);
	...
	if(rib->type <= ZPL_ROUTE_PROTO_STATIC)
...
/* Exported interface function.  This function simply calls
 netlink_socket (). */
void kernel_init(struct nsm_ipvrf *zvrf)
{
...
/*
 void
 kernel_terminate (struct nsm_ipvrf *zvrf)
 {
