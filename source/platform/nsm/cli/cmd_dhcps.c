...
DEFUN (show_dhcps_lease,
		...
		"Dhcp Pool name\n")
{
	nsm_dhcps_lease_show(vty, NULL, (argc == 1)? argv[0]:NULL, zpl_true);
	return CMD_SUCCESS;
}
		...
		CMD_USP_STR_HELP)
{
	...
	if (argv[0] && argv[1])
	{
		...
		if(ifp)
		{
			nsm_dhcps_lease_show(vty, ifp, NULL, zpl_true);
			return CMD_SUCCESS;
		}
	}
...
DEFUN (show_dhcps_pool,
		...
		"Dhcp Pool\n")
{
	nsm_dhcps_pool_show(vty, zpl_true);
	return CMD_SUCCESS;
}
...
{
	install_element(node, CMD_CONFIG_LEVEL, &nsm_interface_ip_dhcp_server_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_nsm_interface_ip_dhcp_server_cmd);
}


static void cmd_base_dhcps_init(int node)
{
	install_element(node, CMD_CONFIG_LEVEL, &dhcp_network_cmd);
	//install_element(node, CMD_CONFIG_LEVEL, &dhcp_network_range_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_network_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &dhcp_address_range_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_address_range_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &dhcp_default_router_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &dhcp_default_router_sec_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_default_router_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_default_router_sec_cmd);


	install_element(node, CMD_CONFIG_LEVEL, &dhcp_netbios_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &dhcp_netbios_sec_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_netbios_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_netbios_sec_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &dhcp_dns_server_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &dhcp_dns_server_sec_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_dns_server_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_dns_server_sec_cmd);


	install_element(node, CMD_CONFIG_LEVEL, &dhcp_tftp_server_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_tftp_server_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &dhcp_domain_name_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_domain_name_cmd);

	install_element(node, CMD_CONFIG_LEVEL, &dhcp_lease_time_cmd);
	install_element(node, CMD_CONFIG_LEVEL, &no_dhcp_lease_time_cmd);
}

static void cmd_show_dhcps_init(int node)
{
	install_element(node, CMD_VIEW_LEVEL, &show_dhcps_lease_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_dhcps_lease_interface_cmd);
	install_element(node, CMD_VIEW_LEVEL, &show_dhcps_pool_cmd);
...
void cmd_dhcps_init(void)
{
	...
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &ip_dhcp_pool_cmd);
	install_element(CONFIG_NODE, CMD_CONFIG_LEVEL, &no_ip_dhcp_pool_cmd);
