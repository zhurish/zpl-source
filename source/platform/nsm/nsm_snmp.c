           ...
           int exact, zpl_size_t *val_len, WriteMethod **write_method)
{
  ...
  if (snmp_header_generic(v, objid, objid_len, exact, val_len, write_method) == MATCH_FAILED)
    return NULL;
  ...
  for (np2 = route_top(table); np2; np2 = route_next(np2))
  {
    ...
    { /* have to look at each rib individually */
      RNODE_FOREACH_RIB(np2, rib2)
      {
        ...
        if ((policy < policy2) || ((policy == policy2) && (proto < proto2)) || ((policy == policy2) && (proto == proto2) && (in_addr_cmp((zpl_uchar *)&rib2->nexthop->gate.ipv4, (zpl_uchar *)&nexthop_addr) >= 0)))
          check_replace(np2, rib2, np, rib);
      }
    }
  }
          ...
          int exact, zpl_size_t *val_len, WriteMethod **write_method)
{
  ...
  if (snmp_header_table(v, objid, objid_len, exact, val_len, write_method) == MATCH_FAILED)
    return NULL;
...
{
  snmp_agentx_init(nsm_srv->master);
