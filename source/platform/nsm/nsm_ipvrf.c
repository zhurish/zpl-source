...
static int ipvrf_nsm_create_hook(struct ip_vrf *ip_vrf, void *p)
{
  int ret = 0;
  if (ip_vrf)
  {
    ip_vrf->info = nsm_vrf_create(ip_vrf->name, ip_vrf->vrf_id);
    if (ip_vrf->info)
    {
...
static int ipvrf_nsm_delete_hook(struct ip_vrf *ip_vrf, void *p)
{
  int ret = 0;
  if (!ip_vrf)
  {
  ...
  if (ret == OK)
  {
    return nsm_vrf_destroy(ip_vrf->info);
  }
  zlog_err(MODULE_NSM, "vrf_init: failed to create the default VRF!");
  return ERROR;
}

static int ipvrf_nsm_set_vrfid_hook(struct ip_vrf *ip_vrf, struct prefix *p)
{
  if (!ip_vrf)
  {
    zlog_err(MODULE_NSM, "vrf_init: failed to create the default VRF!");
  ...
  router_id_set(p, ip_vrf->vrf_id);
  return OK;
}

int ipvrf_nsm_init(void)
{
  ip_vrf_add_hook(VRF_NEW_HOOK, ipvrf_nsm_create_hook);
  ip_vrf_add_hook(VRF_DELETE_HOOK, ipvrf_nsm_delete_hook);
  ip_vrf_add_hook(VRF_SET_VRFID_HOOK, ipvrf_nsm_set_vrfid_hook);
  return OK;
}
