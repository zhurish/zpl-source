			   ...
			   int recursive)
{
  ...
  nhi.nhif_index = nexthop->ifindex;
  ...
  if (!nhi.gateway && nhi.nhif_index == 0)
    return 0;
			 ...
			 rib_dest_t *dest, struct rib *rib)
{
  ...
  if (!rib)
    {
      zlog_err(MODULE_NSM, "netlink_route_info_fill RTM_ADDROUTE called without rib info");
      return 0;
    }
			   ...
			   size_t in_buf_len)
{
  ...
  librtnl_addattr_l (&req->n, in_buf_len, RTA_DST, &ri->prefix->u.prefix, bytelen);

  req->r.rtm_type = ri->rtm_type;

  /* Metric. */
  if (ri->metric)
    librtnl_addattr32 (&req->n, in_buf_len, RTA_PRIORITY, *ri->metric);
  ...
  if (ri->num_nhs == 1)
    {
      ...
      if (nhi->gateway)
	{
	  librtnl_addattr_l (&req->n, in_buf_len, RTA_GATEWAY, nhi->gateway,
		     bytelen);
	}

      if (nhi->nhif_index)
	{
	  librtnl_addattr32 (&req->n, in_buf_len, RTA_OIF, nhi->nhif_index);
	}
  ...
  for (nexthop_num = 0; nexthop_num < ri->num_nhs; nexthop_num++)
    {
      ...
      if (nhi->gateway)
	{
	  librtnl_rta_addattr_l (rta, sizeof (buf), RTA_GATEWAY, nhi->gateway, bytelen);
	  rtnh->rtnh_len += sizeof (struct rtattr) + bytelen;
	}

      if (nhi->nhif_index)
	{
	  rtnh->rtnh_ifindex = nhi->nhif_index;
	}
  ...
  librtnl_addattr_l (&req->n, in_buf_len, RTA_MULTIPATH, RTA_DATA (rta),
	     RTA_PAYLOAD (rta));
  ...
  if (ri->pref_src)
    {
      librtnl_addattr_l (&req->n, in_buf_len, RTA_PREFSRC, &ri->pref_src, bytelen);
    }
...
zfpm_log_route_info (netlink_route_info_t *ri, const char *label)
{
  ...
  zfpm_debug ("%s : %s %s/%d, Proto: %s, Metric: %u", label,
	      librtnl_msg_type_to_str (ri->nlmsg_type),
	      prefix_addr_to_a (ri->prefix), ri->prefix->prefixlen,
	      librtnl_rtproto_to_str (ri->rtm_protocol),
  ...
  for (i = 0; i < ri->num_nhs; i++)
    {
      ...
      zfpm_debug("  Intf: %u, Gateway: %s, Recursive: %s, Type: %s",
		 nhi->nhif_index, addr_to_a (ri->af, nhi->gateway),
