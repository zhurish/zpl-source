...
zfpm_connect_cb (struct thread *t)
{
  ...
  zlog_info (MODULE_NSM, "can't connect to FPM %d: %s", sock, ipstack_strerror (errno));
  ipstack_close (sock);
	   ...
	   const char *format)
{
  ...
  install_element (ENABLE_NODE, 0, &show_zebra_fpm_stats_cmd);
  install_element (ENABLE_NODE, 0, &clear_zebra_fpm_stats_cmd);
  install_element (CONFIG_NODE, 0, &fpm_remote_ip_cmd);
  install_element (CONFIG_NODE, 0, &no_fpm_remote_ip_cmd);
