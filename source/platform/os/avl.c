 ...
 * otherwise next node
 */
//avl_walk(avl_tree_t *tree, void	*oldnode, int left) /* before */
void *
...
avl_rotation(avl_tree_t *tree, avl_node_t *node, int balance)
{
	...
	return (1);	/* the new tree is always zpl_int16er */
}
...
avl_insert(avl_tree_t *tree, void *new_data, avl_index_t where)
{
	...
	assert(tree);
#ifdef _LP64
	assert(((avl_uintptr_t)new_data & 0x7) == 0);
#endif
...
avl_remove(avl_tree_t *tree, void *data)
{
	...
	if((delete == NULL)||(tree->avl_numnodes == 0))
	{
		return;
	}
	/*
	...
	/*
	 * Since the subtree is now zpl_int16er, begin adjusting parent balances
...
avl_destroy_nodes(avl_tree_t *tree, void **cookie)
{
	 ...
	 */
	parent = (avl_node_t *)((avl_uintptr_t)(*cookie) & ~CHILDBIT);
	if (parent == NULL) {
	 ...
	 */
	child = (avl_uintptr_t)(*cookie) & CHILDBIT;
...
check_right_side:
	...
	if (parent == NULL) {
		...
		*cookie = (void *)((avl_uintptr_t)parent | AVL_XCHILD(node));
	}
