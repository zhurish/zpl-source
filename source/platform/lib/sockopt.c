...
int sockopt_intval(zpl_socket_t fd, int level, int optname, int optval)
{
	return ipstack_setsockopt(fd, level, optname, &optval, sizeof(int));
}

static int sockopt_SOL_SOCKET_intval(zpl_socket_t fd, int optname, int optval)
{
	return sockopt_intval(fd, IPSTACK_SOL_SOCKET, optname, optval);
}

static int sockopt_SOL_SOCKET_1(zpl_socket_t fd, int optname)
{
	return sockopt_SOL_SOCKET_intval(fd, optname, 1);
}
