...
static void
route_map_index_delete (struct route_map_index *, zpl_uint32);
...
static void
route_map_index_delete (struct route_map_index *index, zpl_uint32 notify)
{
  struct route_map_rule *rule;
  ...
  /* Free 'zpl_char *nextrm' if not NULL */
  if (index->nextrm)
                  ...
                  case RMAP_GOTO:
                    {
                      ...
                      zpl_uint32 nextpref = index->nextpref;
...
route_map_init_vty (void)
{
  ...
  install_element (CONFIG_NODE, CMD_CONFIG_LEVEL, &route_map_cmd);
  install_element (CONFIG_NODE, CMD_CONFIG_LEVEL, &no_route_map_cmd);
  install_element (CONFIG_NODE, CMD_CONFIG_LEVEL, &no_route_map_all_cmd);

  /* Install the on-match stuff */
  install_element (RMAP_NODE,CMD_CONFIG_LEVEL,  &route_map_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_onmatch_next_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &no_rmap_onmatch_next_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_onmatch_goto_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &no_rmap_onmatch_goto_cmd);
  
  /* Install the continue stuff (ALIAS of on-match). */
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_continue_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &no_rmap_continue_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_continue_index_cmd);
  
  /* Install the call stuff. */
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_call_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &no_rmap_call_cmd);

  /* Install description commands. */
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &rmap_description_cmd);
  install_element (RMAP_NODE, CMD_CONFIG_LEVEL, &no_rmap_description_cmd);
   
  /* Install show command */
  install_element (ENABLE_NODE, CMD_CONFIG_LEVEL, &rmap_show_name_cmd);
}
