...
zclient_new()
{
  ...
  zclient->ibuf = stream_new(ZCLIENT_MAX_PACKET_SIZ);
  zclient->obuf = stream_new(ZCLIENT_MAX_PACKET_SIZ);
...
void zclient_init(struct zclient *zclient, zpl_uint32 redist_default)
{
  zpl_uint32 i = 0;
  ...
  ipstack_bzero(zclient->sock);

  /* Clear redistribution flags. */
  for (i = 0; i < ZPL_ROUTE_PROTO_MAX; i++)
    zclient->redist[i] = ip_vrf_bitmap_init();
  ...
  zclient->default_information = ip_vrf_bitmap_init();

  /* Schedule first zclient connection. */
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient start scheduled");
...
void zclient_stop(struct zclient *zclient)
{
  int i = 0;
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient stopped");
  ...
  for (i = 0; i < ZPL_ROUTE_PROTO_MAX; i++)
  {
    ip_vrf_bitmap_free(zclient->redist[i]);
    zclient->redist[i] = IP_VRF_BITMAP_NULL;
  }
  ip_vrf_bitmap_free(zclient->default_information);
  zclient->default_information = IP_VRF_BITMAP_NULL;
}
...
zclient_flush_data(struct thread *thread)
{
  ...
  {
  case BUFFER_ERROR:
    zlog_warn(MODULE_LIB, "%s: buffer_flush_available failed on zclient fd %d, closing",
...
int zclient_send_message(struct zclient *zclient)
{
  ...
  {
  case BUFFER_ERROR:
    zlog_warn(MODULE_LIB, "%s: buffer_write failed to zclient fd %d, closing",
              __func__, ipstack_fd(zclient->sock));
    return zclient_failed(zclient);
    break;
  case BUFFER_EMPTY:
...
void zclient_create_header(struct stream *s, zpl_uint16 command, vrf_id_t vrf_id)
{
...
static int
zclient_message_send(struct zclient *zclient, zpl_uint16 command, vrf_id_t vrf_id)
{
  struct stream *s;
...
static int
zclient_hello_send(struct zclient *zclient)
{
  struct stream *s;
  ...
  if (zclient->redist_default)
  {
    ...
    zclient_create_header(s, NSM_EVENT_HELLO, 0);
...
void zclient_send_requests(struct zclient *zclient, vrf_id_t vrf_id)
{
  ...
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "%s: send messages for VRF %u", __func__, vrf_id);

  /* We need router-id information. */
  zclient_message_send(zclient, NSM_EVENT_ROUTER_ID_ADD, vrf_id);

  /* We need interface information. */
  zclient_message_send(zclient, NSM_EVENT_INTERFACE_ADD, vrf_id);

  /* Set unwanted redistribute route. */
  ip_vrf_bitmap_set(zclient->redist[zclient->redist_default], vrf_id);

  /* Flush all redistribute request. */
  for (i = 0; i < ZPL_ROUTE_PROTO_MAX; i++)
    if (i != zclient->redist_default &&
        ip_vrf_bitmap_check(zclient->redist[i], vrf_id))
      zclient_redistribute_send(NSM_EVENT_REDISTRIBUTE_ADD, zclient, i, vrf_id);

  /* If default information is needed. */
  if (ip_vrf_bitmap_check(zclient->default_information, 0))
    zclient_message_send(zclient, NSM_EVENT_REDISTRIBUTE_DEFAULT_ADD, vrf_id);
}
...
int zclient_start(struct zclient *zclient)
{
  ...
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient_start is called");
  ...
  if (ipstack_invalid(zclient_socket_connect(zclient)))
  {
    if (zclient_debug)
      zlog_debug(MODULE_LIB, "zclient connection fail");
    zclient->fail++;
    zclient_event(ZCLIENT_CONNECT, zclient);
    return -1;
  }
  ...
  if (ipstack_set_nonblocking(zclient->sock) < 0)
    zlog_warn(MODULE_LIB, "%s: set_nonblocking(%d) failed", __func__, ipstack_fd(zclient->sock));

  /* Clear fail count. */
  zclient->fail = 0;
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient connect success with socket [%d]", ipstack_fd(zclient->sock));

  /* Create read thread. */
  zclient_event(ZCLIENT_READ, zclient);

  zclient_hello_send(zclient);

  /* We need router-id information. */
  zclient_message_send(zclient, NSM_EVENT_ROUTER_ID_ADD, 0);

  /* We need interface information. */
  zclient_message_send(zclient, NSM_EVENT_INTERFACE_ADD, 0);

  /* Flush all redistribute request. */
  for (i = 0; i < ZPL_ROUTE_PROTO_MAX; i++)
    if (i != zclient->redist_default && zclient->redist[i])
      zclient_redistribute_send(NSM_EVENT_REDISTRIBUTE_ADD, zclient, i, 0);

  /* If default information is needed. */
  if (zclient->default_information)
    zclient_message_send(zclient, NSM_EVENT_REDISTRIBUTE_DEFAULT_ADD, 0);
...
zclient_connect(struct thread *t)
{
  ...
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient_connect is called");
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * | MSG Flags   | Message Flags | Prefix length |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 ...
 *
 * +-+-+-+-+-+-+-+-+
 * | Nexthop Type  |  Set to one of NSM_NEXTHOP_*
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |       IPv4 Nexthop address or Interface Index number          |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
 * Alternatively, if the flags field has NSM_RIB_FLAG_BLACKHOLE or
 * NSM_RIB_FLAG_REJECT is set then Nexthop count is set to 1, then _no_
 ...
 */
int zapi_ipv4_route(zpl_uint16 cmd, struct zclient *zclient, struct prefix_ipv4 *p,
                    struct zapi_ipv4 *api, vrf_id_t vrfid)
{
  zpl_uint32 i = 0;
  int psize;
  struct stream *s;
  ...
  zclient_create_header(s, cmd, vrfid);
  ...
  stream_write(s, (zpl_uchar *)&p->prefix, psize);

  /* Nexthop, ifindex, distance and metric information. */
  if (CHECK_FLAG(api->message, ZAPI_MESSAGE_NEXTHOP))
  {
    if (CHECK_FLAG(api->flags, NSM_RIB_FLAG_BLACKHOLE))
    {
      stream_putc(s, 1);
      stream_putc(s, NSM_NEXTHOP_BLACKHOLE);
    ...
    {
      stream_putc(s, NSM_NEXTHOP_IPV4);
      stream_put_in_addr(s, api->nexthop[i]);
    }
    for (i = 0; i < api->ifindex_num; i++)
    {
      stream_putc(s, NSM_NEXTHOP_IFINDEX);
...
int zapi_ipv6_route(zpl_uint16 cmd, struct zclient *zclient, struct prefix_ipv6 *p,
                    struct zapi_ipv6 *api, vrf_id_t vrfid)
{
  zpl_uint32 i = 0;
  int psize;
  struct stream *s;
  ...
  if (CHECK_FLAG(api->message, ZAPI_MESSAGE_NEXTHOP))
  {
    ...
    for (i = 0; i < api->nexthop_num; i++)
    {
      stream_putc(s, NSM_NEXTHOP_IPV6);
      stream_write(s, (zpl_uchar *)api->nexthop[i], 16);
    }
    for (i = 0; i < api->ifindex_num; i++)
    {
      stream_putc(s, NSM_NEXTHOP_IFINDEX);
...
/*
 * send a NSM_EVENT_REDISTRIBUTE_ADD or NSM_EVENT_REDISTRIBUTE_DELETE
 * for the route type (ZPL_ROUTE_PROTO_KERNEL etc.). The zebra server will
 * then set/unset redist[type] in the client handle (a struct zserv) for the
 * sending client
 */
int zclient_redistribute_send(zpl_uint16 command, struct zclient *zclient, zpl_uint32 type, vrf_id_t vrfid)
{
  struct stream *s;
  ...
  zclient_create_header(s, command, vrfid);
  stream_putc(s, type);
...
void zclient_router_id_update_read(struct stream *s, struct prefix *rid)
{
...
/*
 * The format of the message sent with type NSM_EVENT_INTERFACE_ADD or
 * NSM_EVENT_INTERFACE_DELETE from zebra to the client is:
 *     0                   1                   2                   3
...
struct interface *
zclient_interface_add_read(struct stream *s)
{
  ...
  zclient_interface_if_set_value(s, ifp);

  return ifp;
}

/*
 * Read interface up/down msg (NSM_EVENT_INTERFACE_UP/NSM_EVENT_INTERFACE_DOWN)
 * from zebra server.  The format of this message is the same as
 * that sent for NSM_EVENT_INTERFACE_ADD/NSM_EVENT_INTERFACE_DELETE (see
 * comments for zclient_interface_add_read), except that no ipstack_sockaddr_dl
 * is sent at the tail of the message.
 */
struct interface *
zclient_interface_state_read(struct stream *s)
{
  struct interface *ifp;
  zpl_char ifname_tmp[IF_NAME_MAX];
 ...
 *  0 1 2 3 4 5 6 7
 * +-+-+-+-+-+-+-+-+
 * |   type        |  NSM_EVENT_INTERFACE_ADDRESS_ADD or
 * +-+-+-+-+-+-+-+-+  NSM_EVENT_INTERFACE_ADDRES_DELETE
 * |               |
 * +               +
 * |   ifindex     |
 * +               +
...
void zclient_interface_if_set_value(struct stream *s, struct interface *ifp)
{
...
static int
memconstant(const void *s, int c, zpl_size_t n)
{
  const zpl_uchar *p = s;
...
struct connected *
zclient_interface_address_read(zpl_uint16 type, struct stream *s)
{
  zpl_uint32 ifindex;
  struct interface *ifp;
  ...
  if (ifp == NULL)
  {
    zlog_warn(MODULE_LIB, "zclient_interface_address_read(%s): "
                              "Can't find interface by ifindex: %d ",
              (type == NSM_EVENT_INTERFACE_ADDRESS_ADD ? "ADD" : "DELETE"),
              ifindex);
    return NULL;
  }
  ...
  if (type == NSM_EVENT_INTERFACE_ADDRESS_ADD)
  {
    /* N.B. NULL destination pointers are encoded as all zeroes */
    ...
    if (ifc != NULL)
    {
      ...
      else if (CHECK_FLAG(ifc->flags, IF_IFA_PEER))
      {
        /* carp interfaces on OpenBSD with 0.0.0.0/0 as "peer" */
        zpl_char buf[BUFSIZ];
        prefix2str(ifc->address, buf, sizeof(buf));
        zlog_warn(MODULE_LIB, "warning: interface %s address %s "
  ...
  {
    assert(type == NSM_EVENT_INTERFACE_ADDRESS_DELETE);
...
zclient_read(struct thread *thread)
{
  ...
  if ((already = stream_get_endp(zclient->ibuf)) < ZCLIENT_HEADER_SIZE)
  {
    ssize_t nbyte;
    ...
    {
      if (zclient_debug)
        zlog_debug(MODULE_LIB, "zclient connection closed socket [%d].", ipstack_fd(zclient->sock));
      return zclient_failed(zclient);
    }
  ...
  {
    zlog_err(MODULE_LIB, "%s: socket %d message length %u is less than %d ",
  ...
  if (zclient_debug)
    zlog_debug(MODULE_LIB, "zclient 0x%p command 0x%x \n", zclient, command);

  switch (command)
  {
  case NSM_EVENT_ROUTER_ID_UPDATE:
  ...
  case NSM_EVENT_IPV6_ROUTE_DELETE:
    ...
    break;
  case NSM_EVENT_NEXTHOP_UPDATE:
    if (zclient->nexthop_update)
      (*zclient->nexthop_update)(command, zclient, length, vrf_id);
    break;
  case NSM_EVENT_INTERFACE_VRF_BIND:
    if (zclient->interface_vrf_bind)
      (*zclient->interface_vrf_bind)(command, zclient, length, vrf_id);
    break;
  case NSM_EVENT_INTERFACE_VRF_UNBIND:
    if (zclient->interface_vrf_unbind)
      (*zclient->interface_vrf_unbind)(command, zclient, length, vrf_id);
    break;
...
void zclient_redistribute(zpl_uint16 command, struct zclient *zclient, zpl_uint32 type, vrf_id_t vrfid)
{

  if (command == NSM_EVENT_REDISTRIBUTE_ADD)
  {
    if (ip_vrf_bitmap_check(zclient->redist[type], vrfid))
      return;
    ip_vrf_bitmap_set(zclient->redist[type], vrfid);
  }
  else
  {
    if (!ip_vrf_bitmap_check(zclient->redist[type], vrfid))
      return;
    ip_vrf_bitmap_unset(zclient->redist[type], vrfid);
  }

  if (!ipstack_invalid(zclient->sock))
    zclient_redistribute_send(command, zclient, type, vrfid);
}

void zclient_redistribute_default(zpl_uint16 command, struct zclient *zclient, vrf_id_t vrfid)
{
  if (command == NSM_EVENT_REDISTRIBUTE_DEFAULT_ADD)
  {
    if (ip_vrf_bitmap_check(zclient->default_information, vrfid))
      return;
    ip_vrf_bitmap_set(zclient->default_information, vrfid);
  }
  else
  {
    if (!ip_vrf_bitmap_check(zclient->default_information, vrfid))
      return;
    ip_vrf_bitmap_unset(zclient->default_information, vrfid);
  }

  if (!ipstack_invalid(zclient->sock))
    zclient_message_send(zclient, command, vrfid);
}
...
zclient_event(enum event event, struct zclient *zclient)
{
  ...
  case ZCLIENT_CONNECT:
    if (zclient->fail >= 10)
      return;
    if (zclient_debug)
      zlog_debug(MODULE_LIB, "zclient connect schedule interval is %d",
