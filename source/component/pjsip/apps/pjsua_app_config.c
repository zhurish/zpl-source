                              ...
                              pj_str_t *uri_to_call)
{
    int c;
    int option_index;
    pjsua_app_config *cfg = &_global_config.app_config;
    ...
    while((c=pj_getopt_long(argc,argv, "", long_options,&option_index))!=-1) {
...
#if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
        case OPT_USE_SRTP:
            _global_config.app_config.cfg.use_srtp = my_atoi(pj_optarg);
            if (!pj_isdigit(*pj_optarg) || _global_config.app_config.cfg.use_srtp > 3) {
                PJ_LOG(1,(THIS_FILE, "Invalid value for --use-srtp option"));
                return -1;
            }
            if ((int)_global_config.app_config.cfg.use_srtp == 3) {
                /* SRTP optional mode with duplicated media offer */
                _global_config.app_config.cfg.use_srtp = PJMEDIA_SRTP_OPTIONAL;
                _global_config.app_config.cfg.srtp_optional_dup_offer = PJ_TRUE;
                cur_acc->srtp_optional_dup_offer = PJ_TRUE;
            }
            cur_acc->use_srtp = _global_config.app_config.cfg.use_srtp;
            break;
        case OPT_SRTP_SECURE:
            _global_config.app_config.cfg.srtp_secure_signaling = my_atoi(pj_optarg);
            if (!pj_isdigit(*pj_optarg) ||
                _global_config.app_config.cfg.srtp_secure_signaling > 2)
            {
                PJ_LOG(1,(THIS_FILE, "Invalid value for --srtp-secure option"));
                return -1;
            }
            cur_acc->srtp_secure_signaling = _global_config.app_config.cfg.srtp_secure_signaling;
            break;
        case OPT_SRTP_KEYING:
            _global_config.app_config.srtp_keying = my_atoi(pj_optarg);
            if (!pj_isdigit(*pj_optarg) || _global_config.app_config.srtp_keying < 0 ||
                _global_config.app_config.srtp_keying > 1)
            {
                PJ_LOG(1,(THIS_FILE, "Invalid value for --srtp-keying option"));
                return -1;
            }
            /* Set SRTP keying to use DTLS over SDES */
            if (_global_config.app_config.srtp_keying == 1) {
                pjsua_srtp_opt *srtp_opt = &_global_config.app_config.cfg.srtp_opt;
        ...
        */
        ...
        case OPT_STDOUT_REFRESH:
            _global_config.stdout_refresh = atoi(pj_optarg);
            break;
        ...
        case OPT_PLAY_AVI:
            if (_global_config.app_config.avi_cnt >= PJSUA_APP_MAX_AVI) {
                PJ_LOG(1,(THIS_FILE, "Too many AVIs"));
                return -1;
            }
            _global_config.app_config.avi[_global_config.app_config.avi_cnt++].path = pj_str(pj_optarg);
            break;

        case OPT_AUTO_PLAY_AVI:
            _global_config.app_config.avi_auto_play = PJ_TRUE;
            break;
...
static void default_config()
{
    ...
    pj_strdup2_with_null(_global_config.app_config.pool, &cfg->cfg.user_agent, tmp);
...
static void write_account_settings(int acc_index, pj_str_t *result)
{
    unsigned i;
    char line[128];
    pjsua_acc_config *acc_cfg = &_global_config.app_config.acc_cfg[acc_index];
...
int write_settings(pjsua_app_config *config, char *buf, pj_size_t max)
{
    ...
    /* SRTP */
#if PJMEDIA_HAS_SRTP
    if (_global_config.app_config.cfg.use_srtp != PJSUA_DEFAULT_USE_SRTP) {
        int use_srtp = (int)_global_config.app_config.cfg.use_srtp;
        if (use_srtp == PJMEDIA_SRTP_OPTIONAL &&
            _global_config.app_config.cfg.srtp_optional_dup_offer)
        {
            use_srtp = 3;
        }
    ...
    if (_global_config.app_config.cfg.srtp_secure_signaling !=
        PJSUA_DEFAULT_SRTP_SECURE_SIGNALING)
    {
        pj_ansi_sprintf(line, "--srtp-secure %d\n",
                        _global_config.app_config.cfg.srtp_secure_signaling);
        pj_strcat2(&cfg, line);
    }
    if (_global_config.app_config.srtp_keying >= 0 && _global_config.app_config.srtp_keying <= 1)
    {
        pj_ansi_sprintf(line, "--srtp-keying %d\n",
                        _global_config.app_config.srtp_keying);
        pj_strcat2(&cfg, line);
    }
#endif
