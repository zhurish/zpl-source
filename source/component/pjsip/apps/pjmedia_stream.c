 ...
 * Register all codecs. 
 */
static pj_status_t pjmedia_stream_codecs_init(pj_pool_factory *pf)
{
    pj_status_t status;
    ...
    PJ_UNUSED_ARG(pf);
#if defined(PJMEDIA_HAS_HW_CODECS) && PJMEDIA_HAS_HW_CODECS!=0
    status = pjmedia_codec_pjmedia_hwcode_vid_init(NULL, pf);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
#endif  /* PJMEDIA_HAS_HW_CODECS */
#if PJMEDIA_HAS_VIDEO && PJMEDIA_HAS_PASSTHROUGH_CODECS
    status = pjmedia_codec_video_passthrough_vid_init(NULL, pf);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
#endif
 ...
 * Register all codecs. 
 */
static void pjmedia_stream_codecs_deinit(void)
{
#if defined(PJMEDIA_HAS_FFMPEG_VID_CODEC) && PJMEDIA_HAS_FFMPEG_VID_CODEC != 0
...
#endif
#if PJMEDIA_HAS_VIDEO && PJMEDIA_HAS_PASSTHROUGH_CODECS
    pjmedia_codec_video_passthrough_vid_deinit();
#endif
}
 ...
 */
pj_status_t pjapp_media_stream_create(pj_pool_t *pool, pjapp_media_stream_cfg_t *cfg)
{
	...
	if (cfg->type == PJMEDIA_TYPE_VIDEO)
	{
		...
		vinfo.type = cfg->type; // PJMEDIA_TYPE_VIDEO;
		vinfo.dir = cfg->dir;
		vinfo.codec_info = *cfg->video_codec_info;
		vinfo.tx_pt = (cfg->tx_pt == -1) ? cfg->video_codec_info->pt : cfg->tx_pt;
		vinfo.rx_pt = (cfg->rx_pt == -1) ? cfg->video_codec_info->pt : cfg->rx_pt;
		vinfo.ssrc = pj_rand();
	...
	/* Copy remote address */
	pj_memcpy(&info.rem_addr, cfg->rem_addr, sizeof(pj_sockaddr_in));
	...
	if (cfg->mcast)
	{
		...
		status = pjmedia_transport_udp_attach(cfg->med_endpt, NULL, &si,
	...
	else
	{
		...
		status = pjmedia_transport_udp_create(cfg->med_endpt, NULL, cfg->local_port,
											  0, &transport);
		if (status != PJ_SUCCESS)
			return status;
	}
	...
	if (cfg->use_srtp)
	{
		status = pjmedia_transport_srtp_create(cfg->med_endpt, transport,
											   NULL, &srtp_tp);
		if (status != PJ_SUCCESS)
			return status;
...
static pj_status_t pjmedia_stream_create_file_player( pj_pool_t *pool,
...
static void pjmedia_file_clock_cb(const pj_timestamp *ts, void *user_data)
{
    pjmedia_file_data *play_file = (pjmedia_file_data*)user_data;
...
int pjapp_media_stream_factory_create(pj_factory *factory)
{
    //pj_status_t status; 	
    /* Must create a pool factory before we can allocate any memory. */
    pj_caching_pool_init(&factory->cach_pool, &pj_pool_factory_default_policy, 0);
    /* Create memory pool for application purpose */
    factory->pool = pj_pool_create( &factory->cach_pool.factory,     /* pool factory         */
                           "app",           /* pool name.           */
                           4000,            /* init size            */
                           4000,            /* increment size       */
...
int pjapp_media_stream_factory_endpt(pj_factory *factory, pjmedia_endpt *med_endpt)
{
	pj_status_t status; 
    status = pjmedia_endpt_create(&factory->cach_pool.factory, NULL, 1, &med_endpt);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);	

    /* Init video format manager */
    pjmedia_video_format_mgr_create(factory->pool, 64, 0, NULL);

    /* Init video converter manager */
    pjmedia_converter_mgr_create(factory->pool, NULL);

    /* Init event manager */
    pjmedia_event_mgr_create(factory->pool, 0, NULL);

    /* Init video codec manager */
    pjmedia_vid_codec_mgr_create(factory->pool, NULL);

    /* Init video subsystem */
    pjmedia_vid_dev_subsys_init(&factory->cach_pool.factory);

    /* Register all supported codecs */
    status = pjmedia_stream_codecs_init(&factory->cach_pool.factory);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, 1);
	return status; 
}

int pjapp_media_stream_codecparam_set(pjapp_media_stream_cfg_t *cfg, char *codec_id, pjmedia_rect_size vidsize)
{
	pj_status_t status;
	if (cfg->type == PJMEDIA_TYPE_VIDEO)
	{
...
int pjapp_media_stream_io_set(pj_factory *factory, pjapp_media_stream_cfg_t *cfg, char *file_name, pjmedia_port *play_port)
{
	pj_status_t status;
	pjmedia_file_data play_file;
	if (file_name)
	{
		...
		/* Create file player */
		status = pjmedia_stream_create_file_player(factory->pool, file_name, &play_port);
		if (status != PJ_SUCCESS)
			return status;
		...
		play_file.read_buf = pj_pool_zalloc(factory->pool, play_file.read_buf_size);

		/* Create decoder, if the file and the stream uses different codec */

		/* Create player clock */
		clock_param.usec_interval = PJMEDIA_PTIME(&file_vfd->fps);
		clock_param.clock_rate = cfg->video_codec_info->clock_rate;
		status = pjmedia_clock_create2(factory->pool, &clock_param,
									   PJMEDIA_CLOCK_NO_HIGHEST_PRIO,
									   &pjmedia_file_clock_cb, &play_file, &play_file.play_clock);
		if (status != PJ_SUCCESS)
			return status;

		/* Override stream codec param for encoding direction */
		cfg->video_codec_param->enc_fmt.det.vid.size = file_vfd->size;
		cfg->video_codec_param->enc_fmt.det.vid.fps = file_vfd->fps;
	}
	return status;
}

int pjapp_media_stream_iostart(pj_factory *factory, pjmedia_file_data *play_file, pjmedia_port *play_port, pjapp_media_stream_cfg_t *cfg)
{
	pj_status_t status;
	...
	status = pjapp_media_stream_create(factory->pool, cfg);
	...
	printf("Stream is active, dir is send-only, sending to %s:%d\n",
		   pj_inet_ntop2(pj_AF_INET(), &cfg->rem_addr->sin_addr, addr,
						 sizeof(addr)),
		   pj_ntohs(cfg->rem_addr->sin_port));
