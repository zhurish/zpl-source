...
static int          websBackground = 0;             /* Run as a daemon */
static int          websDebug = 0;                  /* Run in debug mode and defeat timeouts */
static int          defaultHttpPort = 0;            /* Default port number for http */
static int          defaultSslPort = 0;             /* Default port number for https */
static int          listens[WEBS_MAX_LISTEN];   /* Listen endpoints */;
static int          listenMax = 0;                  /* Max entry in listens */
static Webs         **webs;                     /* Open connection list head */
static WebsHash     websMime;                   /* Set of mime types */
static int          websMax = 0;                    /* List size */
...
#if ME_GOAHEAD_ACCESS_LOG && (ME_ROM==0)
...
static int      pruneId = 0;                            /* Callback ID */
...
static void initWebs(Webs *wp, int flags, int reuse)
{
     ...
     */
    web_assert(ME_GOAHEAD_LIMIT_BUFFER >= 1024);
...
PUBLIC int websAlloc(int sid)
{
    ...
    web_assert(wp);
#ifdef WEBS_PRIVATE_MAX
	if(websAppPrivateAlloc(wid) != 0)
	{
	    websMax = wfreeHandle(&webs, wid);
	    wfree(wp);
		return -1;
	}
#endif
...
PUBLIC void websFree(Webs *wp)
{
    ...
    termWebs(wp, 0);
#ifdef WEBS_PRIVATE_MAX
    websAppPrivateFree(wp->wid);
#endif
    websMax = wfreeHandle(&webs, wp->wid);
    wfree(wp);
    web_assert(websMax >= 0);
}
...
PUBLIC void websDone(Webs *wp)
{
    ...
    web_assert(WEBS_BEGIN <= wp->state && wp->state <= WEBS_COMPLETE);
#if DEPRECATED || 1
...
static int complete(Webs *wp, int reuse)
{
    web_assert(wp);
    web_assert(websValid(wp));
    web_assert(wp->state == WEBS_BEGIN || wp->state == WEBS_COMPLETE);
...
PUBLIC int websAccept(int sid, cchar *ipaddr, int port, int listenSid)
{
    ...
    web_assert(sid >= 0);
    web_assert(ipaddr && *ipaddr);
    web_assert(listenSid >= 0);
    web_assert(port >= 0);
...
PUBLIC void websConsumeInput(Webs *wp, ssize nbytes)
{
    ...
    web_assert(bufLen(&wp->input) >= nbytes);
    if (nbytes <= 0) {
        return;
    }
...
char *websParseCookie(Webs *wp, char *name)
{
    cchar   *cookie;
    char    *cp, *value;
    ssize   nlen;
...
//static cchar *websGetIpaddr(Webs *wp) { return wp->ipaddr; }
