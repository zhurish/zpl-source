...
PUBLIC int websOpenAuth(int minimal)
{
    ...
    web_assert(minimal == 0 || minimal == 1);
...
static WebsUser *createUser(cchar *username, cchar *password, cchar *proles)
{
    WebsUser    *user;
...
WebsUser *websAddUser(cchar *username, cchar *password, cchar *proles)
{
    ...
    if ((user = createUser(username, password, proles)) == 0) {
        return 0;
    }
...
PUBLIC int websSetUserRoles(cchar *username, cchar *proles)
{
    WebsUser    *user;
...
static void computeUserAbilities(WebsUser *user)
{
    char    *ability, *proles, *tok;

    web_assert(user);
    if ((user->abilities = hashCreate(-1)) == 0) {
        return;
    }
    proles = sclone(user->roles);
    for (ability = stok(proles, " \t,", &tok); ability; ability = stok(NULL, " \t,", &tok)) {
...
PUBLIC bool websVerifyPasswordFromPam(Webs *wp)
{
    ...
    web_assert(wp);
    web_assert(wp->username && wp->username);
    web_assert(wp->password);
    web_assert(!wp->encoded);
