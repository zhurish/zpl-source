...
static void *jrpc_get_in_addr(struct sockaddr *sa) {
...
static int jrpc_server_send_response(struct jrpc_connection * conn, char *response) {
	int fd = conn->fd;
...
static int web_jsonrpc_server_send_error(struct jrpc_connection * conn, int code, char* message,
	...
	return_value = jrpc_server_send_response(conn, str_result);
	free(str_result);
...
static int jrpc_server_send_result(struct jrpc_connection * conn, cJSON * result,
		cJSON * id) {
	int return_value = 0;
	cJSON *result_root = cJSON_CreateObject();
	if (result)
...
static int jrpc_server_invoke_procedure(struct jrpc_server *server,
	...
	if (!procedure_found)
		return jrpc_server_send_error(conn, JRPC_METHOD_NOT_FOUND,
				strdup("Method not found."), id);
	else {
		if (ctx.error_code)
			return jrpc_server_send_error(conn, ctx.error_code, ctx.error_message, id);
		else
			return jrpc_server_send_result(conn, returned, id);
	}
}

static int jrpc_server_eval_request(struct jrpc_server *server,
	...
	if (method != NULL && method->type == cJSON_String) {
		...
		|| params->type == cJSON_Object) {
			...
			//We have to copy ID because using it on the reply and deleting the response Object will also delete ID
				...
				return jrpc_server_invoke_procedure(server, conn, method->valuestring,
						params, id_copy);
			}
		}
	}
	jrpc_server_send_error(conn, JRPC_INVALID_REQUEST,
	...
	if (!bytes_read) {
		...
		if ((root = cJSON_Parse_Stream(conn->buffer, &end_ptr)) != NULL) {
			...
			if (root->type == cJSON_Object) {
				jrpc_server_eval_request(server, conn, root);
			}
			...
			if (end_ptr != (conn->buffer + conn->pos)) {
				...
				jrpc_server_send_error(conn, JRPC_PARSE_ERROR,
						strdup(
