...
static void processUploadHeader(Webs *wp, char *line)
{
	...
	if (scaselesscmp(headerTok, "Content-Disposition") == 0)
	{
		...
		while (key && stok(key, ";\r\n", &nextPair))
		{
			...
			else if (scaselesscmp(key, "filename") == 0)
			{
						...
						if (p)
						{
							...
							else
							{
								websError(wp, HTTP_CODE_INTERNAL_SERVER_ERROR,
										"Bad upload client filename:%s", value);
								wfree(value);
								return;
							}
						}
				...
				{
					websError(wp, HTTP_CODE_INTERNAL_SERVER_ERROR,
							"Cannot create upload temp file %s. Check upload temp dir %s",
							wp->uploadTmp, uploadDir);
					return;
				}
				web_trace(WEBS_DEBUG, "File upload of: %s stored as %s",
						wp->clientFilename, wp->uploadTmp);

				if ((wp->upfd = open(wp->uploadTmp,
						O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600)) < 0)
				{
					websError(wp, HTTP_CODE_INTERNAL_SERVER_ERROR,
							"Cannot open upload temp file %s", wp->uploadTmp);
					return;
				}
				/*
				 Create the files[id]
	...
	else if (scaselesscmp(headerTok, "Content-Type") == 0)
	{
		...
		{
			web_trace(WEBS_DEBUG, "Set files[%s][CONTENT_TYPE] = %s",
					wp->uploadVar, rest);
