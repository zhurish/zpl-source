...
{ if (soap)\
  { \
      ...
      fprintf(stdout, "%02d%02d%02d %02d:%02d:%02d.%03d|", (int)_localTime.wYear%100, (int)_localTime.wMonth, (int)_localTime.wDay, (int)_localTime.wHour, (int)_localTime.wMinute, (int)_localTime.wSecond, (int)_localTime.wMilliseconds);\
      CMD;\
      fflush(stdout);\
  }\
}
...
{ if (soap)\
  { \
	FILE *fdebug = stdout; \
    fwrite((void*)(MSG), 1, (size_t)(LEN), fdebug);\
    fflush(fdebug);\
  }\
}
#   else
...
{ if (soap)\
  { \
      ...
      gettimeofday(&_tv, NULL);\
      localtime_r(&_tv.tv_sec, &_tm);\
      fprintf(stdout, "%02d%02d%02d %02d:%02d:%02d.%03d|", (int)_tm.tm_year%100, (int)_tm.tm_mon+1, (int)_tm.tm_mday, (int)_tm.tm_hour, (int)_tm.tm_min, (int)_tm.tm_sec, (long)_tv.tv_usec);\
      CMD;\
      fflush(stdout);\
  }\
}
