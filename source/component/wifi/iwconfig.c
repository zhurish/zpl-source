...
static int get_info(int skfd, char * ifname, struct wireless_info * info)
{
	...
	/* Get stats */
	if (iw_get_stats(skfd, ifname, &(info->stats), &info->range,
			info->has_range) >= 0)
	{
		info->has_stats = 1;
	}
...
static void display_detail_info(struct wireless_info * info, char * ifname, iw_user_cb_t *cb)
{
	...
	/* Display device name and wireless name (name of the protocol used) */
	//iw_printf("%-8.16s  %s  ", ifname, info->b.name);
	/* Display ESSID (extended network), if any */
	if (info->b.has_essid)
	{
		if (info->b.essid_on)
...
static int print_info(int skfd, char * ifname, char * args[], int count, iw_user_cb_t *cb)
{
	...
	default:
		fprintf(stderr, "%-8.16s  %s\n\n", kname2ifname(ifname), strerror(-rc));
	}
	return (rc);
}
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	else
	{
		...
		do
		{
			...
			if (i < count)
			{
				keylen = iw_in_key_full(skfd, ifname, args[i], key,
						&wrq.u.data.flags);
				if (keylen > 0)
				{
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	else
	{
		...
		/* Parse modifiers */
		i = parse_modifiers(args, count, &wrq.u.power.flags, iwmod_power,
				IWMOD_POWER_NUM);
		if (i < 0)
			return (i);
...
int count, iw_user_cb_t *cb) /* Args count */
{
	...
	/* Parse modifiers */
	i = parse_modifiers(args, count, &wrq.u.retry.flags, iwmod_retry,
			IWMOD_RETRY_NUM);
	if (i < 0)
		return (i);
