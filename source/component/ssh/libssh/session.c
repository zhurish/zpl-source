...
void * ssh_get_session_private(ssh_session session) {
  if (session == NULL) {
    return NULL;
  }
...
void * ssh_get_session_client_address(ssh_session session) {
  if (session == NULL) {
    return NULL;
  }
...
int ssh_set_session_timeout_cb(ssh_session session, int (*func)(ssh_session, void *, int))
{
  if (session == NULL) {
    return -1;
  }
