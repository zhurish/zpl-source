...
#   define AV_RB(s, p)    av_bswap##s(AV_RN(32, p))



int av_base64_decode(uint8_t *out, const char *in_str, uint32_t out_size)
{
    uint8_t *dst = out;
...
char *av_base64_encode(char *out, uint32_t out_size, const uint8_t *in, uint32_t in_size)
{
