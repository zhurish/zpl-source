...
int sdp_attr_fmtp_h264(const char* fmtp, int *format, struct sdp_attr_fmtp_h264_t *h264)
{
    size_t nc, vc;
    const char *p1, *p2;
    const char *p = fmtp;
    ...
    while(*p)
    {
        ...
        case 's':
            ...
            if(0 == strncasecmp("sprop-parameter-sets", p, nc))
            {
                ...
                h264->sprop_parameter_sets_len = vc;
            }
...
int sdp_attr_fmtp_h265(const char* fmtp, int *format, struct sdp_attr_fmtp_h265_t *h265)
{
    size_t nc, vc;
    const char *p1, *p2;
    const char *p = fmtp;
...
int sdp_attr_fmtp_mpeg4(const char* fmtp, int *format, struct sdp_attr_fmtp_mpeg4_t *mpeg4)
{
    size_t nc, vc;
    const char *p1, *p2;
    const char *p = fmtp;
