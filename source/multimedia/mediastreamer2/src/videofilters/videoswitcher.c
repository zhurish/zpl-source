...
static bool_t is_h264_key_frame(ORTP_UNUSED(mblk_t *frame)){
	/*TODO*/
	return FALSE;
}

static bool_t is_key_frame_dummy(ORTP_UNUSED(mblk_t *frame)){
	return TRUE;
}
...
static void switcher_postprocess(ORTP_UNUSED(MSFilter *f)){
}
...
static int switcher_set_fmt(MSFilter *f, void *data){
	...
	if (fmt){
		if (strcasecmp(fmt->encoding,"VP8")==0){
			s->is_key_frame=NULL;//is_vp8_key_frame;
