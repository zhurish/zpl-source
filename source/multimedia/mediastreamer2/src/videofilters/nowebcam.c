...
mblk_t *ms_load_nowebcam(MSFactory *factory, MSVideoSize *reqsize, int idx) {
	...
	if (idx < 0)
		tmp = ortp_strdup_printf("%s/%s.jpg", image_resources_dir, NOWEBCAM_JPG);
	else
		tmp = ortp_strdup_printf("%s/%s%i.jpg", image_resources_dir, NOWEBCAM_JPG, idx);
...
int static_image_get_pix_fmt(ORTP_UNUSED(MSFilter *f), void *data) {
