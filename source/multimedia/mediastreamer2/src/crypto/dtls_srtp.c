...
#ifdef ZPL_MEDIASTREAM_DTLS_SRTP
#include "ortp/crypto.h"

typedef struct _DtlsBcToolBoxContexts {
	ortp_x509_certificate_t *crt;
	ortp_ssl_config_t *ssl_config;
	ortp_ssl_context_t *ssl;
	ortp_rng_context_t *rng;
	ortp_signing_key_t *pkey;
...
DtlsBcToolBoxContext *ms_dtls_srtp_ortp_context_new(void) {
	// allocate the memory
	DtlsBcToolBoxContext *ctx = ms_new0(DtlsBcToolBoxContext,1);

	// create and initialise the requested fields
	ctx->rng = ortp_rng_context_new();
	ctx->pkey = ortp_signing_key_new();
	ctx->crt = ortp_x509_certificate_new();
	ctx->ssl_config = ortp_ssl_config_new();
	ctx->ssl = ortp_ssl_context_new();
	ms_mutex_init(&(ctx->ssl_context_mutex), NULL);

	return ctx;
}

void ms_dtls_srtp_ortp_context_free(DtlsBcToolBoxContext *ctx) {
	if (ctx != NULL) {
		ortp_rng_context_free(ctx->rng);
		ortp_signing_key_free(ctx->pkey);
		ortp_x509_certificate_free(ctx->crt);
		ortp_ssl_context_free(ctx->ssl);
		ortp_ssl_config_free(ctx->ssl_config);
...
static uint8_t ms_dtls_srtp_check_certificate_fingerprint(const ortp_x509_certificate_t *certificate, const char *peer_fingerprint) {
	...
	ortp_x509_certificate_get_signature_hash_function(certificate, &certificate_signature_hash_function);
	...
	ret = ortp_x509_certificate_get_fingerprint(certificate, fingerprint, 255, hash_function);
	if (ret <= 0) {
 ...
 */
static MSCryptoSuite ms_dtls_srtp_ortp_protection_profile_to_ms_crypto_suite(ortp_dtls_srtp_profile_t dtls_srtp_protection_profile) {
	switch(dtls_srtp_protection_profile) {
		...
		case ORTP_SRTP_NULL_HMAC_SHA1_32: /* this profile is defined in DTLS-SRTP rfc but not implemented by libsrtp */
...
static void schedule_rtp(struct _RtpTransportModifier *t) {
	...
	if (ctx->rtp_time_reference>0) { /* only when retransmission timer is armed */
		if (current_time - ctx->rtp_time_reference > DTLS_REPETITION_TIMER_POLL) {
			...
			ortp_ssl_handshake(ctx->rtp_dtls_context->ssl);
...
static void schedule_rtcp(struct _RtpTransportModifier *t) {
	...
	if (ctx->rtcp_time_reference>0) { /* only when retransmission timer is armed */
		if (current_time - ctx->rtcp_time_reference > DTLS_REPETITION_TIMER_POLL) {
			...
			ortp_ssl_handshake(ctx->rtcp_dtls_context->ssl);
...
static bool_t ms_dtls_srtp_process_dtls_packet(mblk_t *msg, MSDtlsSrtpContext *ctx, int *ret, bool_t is_rtp) {
	...
	ortp_ssl_context_t *ssl = (is_rtp == TRUE)?ctx->rtp_dtls_context->ssl:ctx->rtcp_dtls_context->ssl;
	...
	if ((*(msg->b_rptr)>19) && (*(msg->b_rptr)<64)) {
		...
		if (channel_status < DTLS_STATUS_HANDSHAKE_OVER) {
			...
			*ret = ortp_ssl_handshake(ssl);
			...
			ms_mutex_lock(mutex);
			*ret = ortp_ssl_read(ssl, buf, msgLength);
		...
		if (*ret<0 && *ret != ORTP_ERROR_NET_WANT_READ) {
			char err_str[512];
			err_str[0]='\0';
			ortp_strerror(*ret, err_str, 512);
...
static int ms_dtls_srtp_rtp_DTLSread (void *ctx, unsigned char *buf, ORTP_UNUSED(size_t len)) {
	MSDtlsSrtpContext *context = (MSDtlsSrtpContext *)ctx;
...
static int ms_dtls_srtp_rtcp_DTLSread (void *ctx, unsigned char *buf, ORTP_UNUSED(size_t len)) {
	MSDtlsSrtpContext *context = (MSDtlsSrtpContext *)ctx;
...
static int ms_dtls_srtp_rtp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg){
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, TRUE) == TRUE){
		if ((ret==0) && (ctx->rtp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING)) { /* handshake is over, give the keys to srtp : 128 bits client write - 128 bits server write - 112 bits client salt - 112 server salt */
			...
			ctx->rtp_agreed_srtp_protection_profile = ms_dtls_srtp_ortp_protection_profile_to_ms_crypto_suite(ortp_ssl_get_dtls_srtp_protection_profile(ctx->rtp_dtls_context->ssl));
			if ( ctx->rtp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				...
				ret = ortp_ssl_get_dtls_srtp_key_material(ctx->rtp_dtls_context->ssl_config, ctx->rtp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
				...
				if (ms_dtls_srtp_check_certificate_fingerprint(ortp_ssl_get_peer_certificate(ctx->rtp_dtls_context->ssl), (const char *)(ctx->peer_fingerprint)) == 1) {
...
static int ms_dtls_srtp_rtcp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg)  {
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, FALSE) == TRUE){
		...
		if ((ret==0) && (ctx->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING)) { /* rtcp handshake is over, give the keys to srtp : 128 bits client write - 128 bits server write - 112 bits client salt - 112 server salt */
			...
			/* check the srtp profile get selected during handshake */
			ctx->rtcp_agreed_srtp_protection_profile = ms_dtls_srtp_ortp_protection_profile_to_ms_crypto_suite(ortp_ssl_get_dtls_srtp_protection_profile(ctx->rtcp_dtls_context->ssl));
			if ( ctx->rtcp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				...
				ret = ortp_ssl_get_dtls_srtp_key_material(ctx->rtcp_dtls_context->ssl_config, ctx->rtcp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
				...
				if (ms_dtls_srtp_check_certificate_fingerprint(ortp_ssl_get_peer_certificate(ctx->rtcp_dtls_context->ssl), (const char *)(ctx->peer_fingerprint)) == 1) {
...
static int ms_dtls_srtp_rtp_process_on_send(ORTP_UNUSED(struct _RtpTransportModifier *t), mblk_t *msg){
	return (int)msgdsize(msg);
}
static int ms_dtls_srtp_rtcp_process_on_send(ORTP_UNUSED(struct _RtpTransportModifier *t), mblk_t *msg)  {
	return (int)msgdsize(msg);
}
...
static int ms_dtls_srtp_initialise_ortp_dtls_context(DtlsBcToolBoxContext *dtlsContext, MSDtlsSrtpParams *params, ORTP_UNUSED(RtpSession *s)){
	int ret;
	ortp_dtls_srtp_profile_t dtls_srtp_protection_profiles[2] = {ORTP_SRTP_AES128_CM_HMAC_SHA1_80, ORTP_SRTP_AES128_CM_HMAC_SHA1_32};

	/* initialise certificate */
	ret = ortp_x509_certificate_parse( dtlsContext->crt, (const char *) params->pem_certificate, strlen( params->pem_certificate )+1 );
	if( ret < 0 ) {
		return ret;
	}

	ret =  ortp_signing_key_parse( dtlsContext->pkey, (const char *) params->pem_pkey, strlen( params->pem_pkey )+1, NULL, 0 );
	if( ret != 0 ) {
		return ret;
	}
	...
	if (params->role == MSDtlsSrtpRoleIsClient) {
		ortp_ssl_config_defaults(dtlsContext->ssl_config, ORTP_SSL_IS_CLIENT, ORTP_SSL_TRANSPORT_DATAGRAM);
		...
		ortp_ssl_config_defaults(dtlsContext->ssl_config, ORTP_SSL_IS_SERVER, ORTP_SSL_TRANSPORT_DATAGRAM);
	}

	ortp_ssl_config_set_dtls_srtp_protection_profiles(  dtlsContext->ssl_config, dtls_srtp_protection_profiles, 2 ); /* TODO: get param from caller to select available profiles */

	ortp_ssl_config_set_rng(dtlsContext->ssl_config, (int (*)(void *, unsigned char *, size_t))ortp_rng_get, dtlsContext->rng);
	...
	ortp_ssl_config_set_authmode(dtlsContext->ssl_config, ORTP_SSL_VERIFY_OPTIONAL);
	ortp_ssl_config_set_own_cert( dtlsContext->ssl_config, dtlsContext->crt, dtlsContext->pkey );
	/* This is useless as peer would certainly be a self signed certificate and we won't verify it but avoid runtime warnings */
	ortp_ssl_config_set_ca_chain(dtlsContext->ssl_config, dtlsContext->crt);

	/* we are not ready yet to actually start the ssl context, this will be done by calling ortp_ssl_context_setup when stream starts */
	return 0;
...
bool_t ms_dtls_srtp_available(){
	return ms_srtp_supported()  && ortp_dtls_srtp_supported();
}
...
void ms_dtls_srtp_reset_context(MSDtlsSrtpContext *context) {
	if (context) {
		...
		if ((context->rtp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING ) || (context->rtp_channel_status == DTLS_STATUS_HANDSHAKE_OVER )) {
			ortp_ssl_session_reset( context->rtp_dtls_context->ssl );
		}
		...
		if ((context->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING ) || (context->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_OVER )) {
			ortp_ssl_session_reset( context->rtcp_dtls_context->ssl );
		}
...
void ms_dtls_srtp_set_role(MSDtlsSrtpContext *context, MSDtlsSrtpRole role) {
	if (context) {
		...
		if (((context->role == MSDtlsSrtpRoleIsClient) || (context->role == MSDtlsSrtpRoleUnset)) && (role == MSDtlsSrtpRoleIsServer)) {
			ortp_ssl_config_set_endpoint(context->rtp_dtls_context->ssl_config, ORTP_SSL_IS_SERVER);

			ortp_ssl_config_set_endpoint(context->rtcp_dtls_context->ssl_config, ORTP_SSL_IS_SERVER);
		}
...
MSDtlsSrtpContext* ms_dtls_srtp_context_new(MSMediaStreamSessions *sessions, MSDtlsSrtpParams *params) {
	...
	DtlsBcToolBoxContext *rtp_dtls_context = ms_dtls_srtp_ortp_context_new();
	DtlsBcToolBoxContext *rtcp_dtls_context = ms_dtls_srtp_ortp_context_new();
	...
	ret = ms_dtls_srtp_initialise_ortp_dtls_context(rtp_dtls_context, params, s);
	if (ret!=0) {
		ms_error("DTLS init error : rtp bctoolbox context init returned -0x%0x on stream session [%p]", -ret, sessions);
		return NULL;
	}
	ret = ms_dtls_srtp_initialise_ortp_dtls_context(rtcp_dtls_context, params, s);
	if (ret!=0) {
	...
	ortp_ssl_set_io_callbacks( rtp_dtls_context->ssl, userData, ms_dtls_srtp_rtp_sendData, ms_dtls_srtp_rtp_DTLSread);
	ortp_ssl_set_io_callbacks( rtcp_dtls_context->ssl, userData, ms_dtls_srtp_rtcp_sendData, ms_dtls_srtp_rtcp_DTLSread);
...
void ms_dtls_srtp_start(MSDtlsSrtpContext* context) {
	...
	if (context->role == MSDtlsSrtpRoleIsClient) {
		ms_mutex_lock(&context->rtp_dtls_context->ssl_context_mutex);
		ortp_ssl_config_set_endpoint(context->rtp_dtls_context->ssl_config, ORTP_SSL_IS_CLIENT);
		/* complete ssl setup*/
		ortp_ssl_context_setup(context->rtp_dtls_context->ssl, context->rtp_dtls_context->ssl_config);
		ortp_ssl_set_mtu(context->rtp_dtls_context->ssl, context->mtu);
		/* and start the handshake */
		ortp_ssl_handshake(context->rtp_dtls_context->ssl);
		...
		if (!rtp_session_rtcp_mux_enabled(context->stream_sessions->rtp_session)) {
			ms_mutex_lock(&context->rtcp_dtls_context->ssl_context_mutex);
			ortp_ssl_config_set_endpoint(context->rtcp_dtls_context->ssl_config, ORTP_SSL_IS_CLIENT);
			/* complete ssl setup*/
			ortp_ssl_context_setup(context->rtcp_dtls_context->ssl, context->rtcp_dtls_context->ssl_config);
			ortp_ssl_set_mtu(context->rtcp_dtls_context->ssl, context->mtu);
			/* and start the handshake */
			ortp_ssl_handshake(context->rtcp_dtls_context->ssl);
...
void ms_dtls_srtp_context_destroy(MSDtlsSrtpContext *ctx) {
	...
	ms_dtls_srtp_ortp_context_free(ctx->rtp_dtls_context);
	ms_dtls_srtp_ortp_context_free(ctx->rtcp_dtls_context);
...
bool_t ms_dtls_srtp_available(void)
{
	return 0;
}
 ...
 */
MSDtlsSrtpContext* ms_dtls_srtp_context_new(struct _MSMediaStreamSessions *sessions, MSDtlsSrtpParams *params)
{
	return NULL;
}
