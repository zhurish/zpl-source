...
struct _MSZrtpContext{
	...
	ortp_mutex_t *cacheDBMutex; /**< pointer to a mutex used to lock cache access */
...
static int ms_zrtp_addExportedKeysInZidCache(void *clientData, int zuid, uint8_t role) {
	...
	if (userData->limeKeyTimeSpan == 0) { /* time span is 0: key is forever valid, just put 0 in cache */
		...
		ortp_get_utc_cur_time(&currentTime);
		ortp_timespec_add(&currentTime, (int64_t)(userData->limeKeyTimeSpan));
...
static int ms_zrtp_rtp_process_on_send(ORTP_UNUSED(RtpTransportModifier *t), mblk_t *msg){
	return (int)msgdsize(msg);
}
static int ms_zrtp_rtcp_process_on_send(ORTP_UNUSED(RtpTransportModifier *t), mblk_t *msg)  {
	return (int)msgdsize(msg);
}
...
static void ms_zrtp_rtp_on_schedule(RtpTransportModifier *t){
	...
	bzrtp_iterate(zrtpContext, userData->self_ssrc, ortp_get_cur_time_ms());
}
...
static int ms_zrtp_rtcp_process_on_receive(ORTP_UNUSED(RtpTransportModifier *t), mblk_t *msg)  {
	return (int)msgdsize(msg);
}
...
MSZrtpPeerStatus ms_zrtp_get_peer_status(void *db, const char *peerUri, ortp_mutex_t *dbMutex){
 ...
 */
int ms_zrtp_initCache(void *db, ortp_mutex_t *dbMutex) {
...
void ms_zrtp_sas_verified(MSZrtpContext* ctx){}
void ms_zrtp_sas_reset_verified(MSZrtpContext* ctx){}
MSZrtpPeerStatus ms_zrtp_get_peer_status(void *db, const char *peerUri, ortp_mutex_t *dbMutex) {return MS_ZRTP_PEER_STATUS_UNKNOWN;}
void ms_zrtp_context_destroy(MSZrtpContext *ctx){}
void ms_zrtp_reset_transmition_timer(MSZrtpContext* ctx) {};
...
int ms_zrtp_initCache(void *db, ortp_mutex_t *dbMutex){return 0;}
