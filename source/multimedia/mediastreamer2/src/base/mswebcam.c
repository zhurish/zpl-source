...
void ms_web_cam_manager_destroy(MSWebCamManager* scm){
	if (scm!=NULL){
		ortp_list_for_each(scm->cams,(void (*)(void*))ms_web_cam_destroy);
		ortp_list_free(scm->cams);
...
const ortp_list_t * ms_web_cam_manager_get_list(MSWebCamManager *m){
	if (!m) {
		return NULL;
	}
	return m->cams;
...
void ms_web_cam_manager_add_cam(MSWebCamManager *m, MSWebCam *c){
	...
	m->cams=ortp_list_append(m->cams,c);
}
...
void ms_web_cam_manager_prepend_cam(MSWebCamManager *m, MSWebCam *c){
	...
	m->cams=ortp_list_prepend(m->cams,c);
}
...
void ms_web_cam_manager_reload(MSWebCamManager *m){
	ortp_list_t *elem;
	ortp_list_for_each(m->cams,(void (*)(void*))ms_web_cam_destroy);
	ortp_list_free(m->cams);
	m->cams=NULL;
