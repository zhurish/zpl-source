...
MSSndCardManager * ms_snd_card_manager_new(void){
	...
	obj->cards=NULL;
	obj->descs=NULL;
	obj->paramString=ortp_strdup_printf("FAST=false;NOVOICEPROC=false;TESTER=false;RINGER=false");
	return obj;
}
...
void ms_snd_card_manager_destroy(MSSndCardManager* scm){
	if (scm!=NULL && !bypass_sndcard_detection){
		...
		ortp_list_for_each(scm->cards,(void (*)(void*))ms_snd_card_unref);
		ortp_list_free(scm->cards);
		ortp_list_free(scm->descs);
	}
	if (scm!=NULL && scm->paramString!= NULL) {
		ortp_free(scm->paramString);
	}
	ms_free(scm);
}
...
void ms_snd_card_manager_set_param_string(MSSndCardManager *m, const char *paramString){
	if (m->paramString!=NULL) {
		ortp_free(m->paramString);
	}
	m->paramString = ortp_strdup(paramString);
}
...
// Regex search on card id that will contains at least "Filter : Model"
		if( ortp_is_matching_regex_log(card_id, id, FALSE)){
			return card;
		}
	}
...
ortp_list_t * ms_snd_card_manager_get_all_cards_with_name(MSSndCardManager *m, const char *name){
	ortp_list_t *cards = NULL;
	ortp_list_t *elem;
	for (elem = m->cards; elem != NULL; elem = elem->next) {
		MSSndCard *card = (MSSndCard*)elem->data;
		if (strcmp(ms_snd_card_get_name(card),name) == 0) {
			cards = ortp_list_append(cards, ms_snd_card_ref(card));
		}
	}
	return cards;
}
...
const ortp_list_t * ms_snd_card_manager_get_list(MSSndCardManager *m){
	return m->cards;
}
...
void ms_snd_card_manager_add_card(MSSndCardManager *m, MSSndCard *c){
	...
	m->cards=ortp_list_append(m->cards, ms_snd_card_ref(c));
}
...
void ms_snd_card_manager_prepend_card(MSSndCardManager *m, MSSndCard *c){
	...
	m->cards=ortp_list_prepend(m->cards, ms_snd_card_ref(c));
}
...
bool_t ms_snd_card_manager_swap_cards(MSSndCardManager *m, MSSndCard *card0, MSSndCard *card1){
	...
	ortp_list_t *elem = NULL;
	ortp_list_t *ltmp = NULL;
...
void ms_snd_card_manager_prepend_cards(MSSndCardManager *m, ortp_list_t *l) {
	ortp_list_t *elem;
	ortp_list_t *lcopy = ortp_list_copy(l);
	if (m->cards != NULL) m->cards = ortp_list_concat(lcopy, m->cards);
	else m->cards = lcopy;
...
void ms_snd_card_manager_register_desc(MSSndCardManager *m, MSSndCardDesc *desc){
	if (ortp_list_find(m->descs, desc) == NULL) {
		m->descs = ortp_list_append(m->descs, desc);
		card_detect(m, desc);
	}
#ifdef __ANDROID__
...
void ms_snd_card_manager_unregister_desc(MSSndCardManager *m, MSSndCardDesc *desc){
	if (ortp_list_find(m->descs, desc) != NULL) {
		m->descs = ortp_list_remove(m->descs, desc);
	}
}
...
void ms_snd_card_manager_reload(MSSndCardManager *m){
	ortp_list_t *elem;
	ortp_list_t *cardsToKeep = NULL;
	ortp_list_t *oldCardsIt, *newCardsIt;
	...
	for (oldCardsIt = m->cards; oldCardsIt != NULL; oldCardsIt = oldCardsIt->next) {
		...
		cardsToKeep = ortp_list_append(cardsToKeep, ms_snd_card_ref(sndCard));
	}
	ortp_list_free_with_data(m->cards, (void (*)(void*))ms_snd_card_unref);
	m->cards = NULL;
	for (elem = m->descs; elem != NULL; elem = elem->next) {
	...
	ortp_list_free_with_data(cardsToKeep, (void (*)(void*))ms_snd_card_unref);
...
MSSndCard * ms_alsa_card_new_custom(ORTP_UNUSED(const char *pcmdev), ORTP_UNUSED(const char *mixdev)){
	ms_warning("Alsa support not available in this build of mediastreamer2");
	return NULL;
}

void ms_alsa_card_set_forced_sample_rate(ORTP_UNUSED(int samplerate)){
	ms_warning("Alsa support not available in this build of mediastreamer2");
}
#endif
#endif
...
MSSndCard* ms_snd_card_get_card_duplicate(MSSndCardManager *m, MSSndCard * card, bool_t checkCapabilities) {
	ortp_list_t* cards = ms_snd_card_manager_get_all_cards_with_name(m, card->name);
	...
	ortp_list_free_with_data(cards, (ortp_list_free_func)ms_snd_card_unref);
