...
typedef struct _MSVideoPreset {
	char *name;
	ortp_list_t *configs; /**< List of MSVideoPresetConfiguration objects */
} MSVideoPreset;

struct _MSVideoPresetsManager {
	MSFactory *factory;
	ortp_list_t *presets; /**< List of MSVideoPreset objects */
};


static void free_preset_config(MSVideoPresetConfiguration *vpc) {
	ortp_list_for_each(vpc->tags, ms_free);
	ortp_list_free(vpc->tags);
	ms_free(vpc);
}

static void free_preset(MSVideoPreset *vp) {
	ms_free(vp->name);
	ortp_list_for_each(vp->configs, (MSIterateFunc)free_preset_config);
	ortp_list_free(vp->configs);
	ms_free(vp);
}
...
static ortp_list_t * parse_tags(const char *tags) {
	ortp_list_t *tags_list = NULL;
	char *t;
	char *p;
	...
	while (p != NULL) {
		...
		tags_list = ortp_list_append(tags_list, ms_strdup(p));
		p = next;
	}
	ms_free(t);
	return tags_list;
...
static void add_video_preset_configuration(MSVideoPreset *preset, const char *tags, MSVideoConfiguration *config) {
	...
	vpc->config = config;
	preset->configs = ortp_list_append(preset->configs, vpc);
}

static int video_preset_configuration_match(MSVideoPresetConfiguration *vpc, ortp_list_t *platform_tags, ortp_list_t *codec_tags) {
	ortp_list_t *elem = vpc->tags;
	int nb = 0;
	while (elem != NULL) {
...
void ms_video_presets_manager_destroy(MSVideoPresetsManager *manager) {
	if (manager != NULL) {
		ortp_list_for_each(manager->presets, (MSIterateFunc)free_preset);
		ortp_list_free(manager->presets);
		ms_free(manager);
	}
}
...
MSVideoPresetConfiguration * ms_video_presets_manager_find_preset_configuration(MSVideoPresetsManager *manager,
	const char *name, ortp_list_t *codec_tags) {
