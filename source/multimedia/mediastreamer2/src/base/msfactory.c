...
void ms_factory_add_platform_tag(MSFactory *obj, const char *tag) {
	...
	if (ortp_list_find_custom(obj->platform_tags, (ortp_compare_func)strcasecmp, tag) == NULL) {
		obj->platform_tags = ortp_list_append(obj->platform_tags, ms_strdup(tag));
	}
}

ortp_list_t * ms_factory_get_platform_tags(MSFactory *obj) {
...
static MSFilterStats *find_or_create_stats(MSFactory *factory, MSFilterDesc *desc){
	ortp_list_t *elem=ortp_list_find_custom(factory->stats_list,(ortp_compare_func)compare_stats_with_name,desc->name);
	MSFilterStats *ret=NULL;
	if (elem==NULL){
		ret=ms_new0(MSFilterStats,1);
		ret->name=desc->name;
		factory->stats_list=ortp_list_append(factory->stats_list,ret);
...
void ms_factory_init(MSFactory *obj){
	...
	if (debug_log_enabled!=NULL && (strcmp("1",debug_log_enabled)==0) ){
		;//ortp_set_log_level(BCTBX_LOG_DOMAIN, BCTBX_LOG_MESSAGE);
	}
...
#warning "There is no code that detects the number of CPU for this platform."
	...
	obj->image_resources_dir = ortp_strdup_printf("%s/images", PACKAGE_DATA_DIR);
}
...
void ms_factory_register_filter(MSFactory* factory, MSFilterDesc* desc ) {
	...
	factory->desc_list=ortp_list_prepend(factory->desc_list,desc);
}
...
MSFilterDesc * ms_factory_get_encoding_capturer(MSFactory* factory, const char *mime) {
	...
	for (elem = factory->desc_list; elem != NULL; elem = ortp_list_next(elem)) {
...
ortp_list_t *ms_factory_lookup_filter_by_interface(MSFactory* factory, MSFilterInterfaceId id){
	ortp_list_t *ret=NULL;
	ortp_list_t *elem;
	for(elem=factory->desc_list;elem!=NULL;elem=elem->next){
		MSFilterDesc *desc=(MSFilterDesc*)elem->data;
		if (ms_filter_desc_implements_interface(desc,id))
			ret=ortp_list_append(ret,desc);
	}
	return ret;
}
...
const ortp_list_t * ms_factory_get_statistics(MSFactory* obj){
...
void ms_factory_log_statistics(MSFactory *obj) {
	ortp_list_t *sorted = NULL;
	ortp_list_t *elem;
	double total = 0.0;
	for (elem = obj->stats_list; elem != NULL; elem = elem->next) {
		MSFilterStats *stats = (MSFilterStats *)elem->data;
		sorted = ortp_list_insert_sorted(sorted, stats, (ortp_compare_func)usage_compare);
...
int ms_factory_load_plugins_from_list(MSFactory *factory, const ortp_list_t *plugins_list, const char *optionnal_plugins_path) {
	size_t num = 0;
	size_t plugins_list_size = 0;
	const ortp_list_t *it = NULL;

	if (plugins_list == NULL || ortp_list_size(plugins_list) == 0) {
		ms_error("Couldn't load plugins from empty list");
		return -1;
	}

	plugins_list_size = ortp_list_size(plugins_list);

	for (it = plugins_list; it != NULL; it = ortp_list_next(it)) {
		const char *plugin_name = (const char*)ortp_list_get_data(it);
#if defined(HAVE_DLOPEN)
...
int ms_factory_load_plugins(MSFactory *factory, const char *dir){
		...
		if (os_handle==NULL)
			...
			initroutine = (init_func_t) GetProcAddress (os_handle, szMethodName);
				if (initroutine!=NULL){
					...
					factory->ms_plugins_loaded_list=ortp_list_append(factory->ms_plugins_loaded_list,os_handle);
					num++;
				}else{
	...
	char plugin_name[64];
	DIR *ds;
	ortp_list_t *loaded_plugins = NULL;
	struct dirent *de;
	char *ext;
	ds=opendir(dir);
	if (ds==NULL){
	...
	while( (de=readdir(ds))!=NULL){
		if (
			...
			if (ortp_list_find_custom(loaded_plugins, (ortp_compare_func)strcmp, plugin_name) != NULL) continue;
			loaded_plugins = ortp_list_append(loaded_plugins, ms_strdup(plugin_name));
			if (ms_factory_dlopen_plugin(factory, dir, de->d_name)) {
				num++;
			}
		}
	}
	ortp_list_for_each(loaded_plugins, ms_free);
	ortp_list_free(loaded_plugins);
	closedir(ds);
#else
...
void ms_factory_uninit_plugins(MSFactory *factory){
	...
	factory->ms_plugins_loaded_list = ortp_list_free(factory->ms_plugins_loaded_list);
#endif
}
#ifndef _MSC_VER
...
void ms_factory_init_plugins(MSFactory *obj) {
	...
	ortp_list_t *plugins_list = ms_get_android_plugins_list();
...
const MSFmtDescriptor *ms_factory_get_format(MSFactory *obj, const MSFmtDescriptor *ref){
	MSFmtDescriptor *ret;
	ortp_list_t *found;
	if ((found=ortp_list_find_custom(obj->formats,(int (*)(const void*, const void*))compare_fmt, ref))==NULL){
		obj->formats=ortp_list_append(obj->formats,ret=ms_fmt_descriptor_new_copy(ref));
	}else{
...
void ms_factory_register_offer_answer_provider(MSFactory *f, MSOfferAnswerProvider *offer_answer_prov){
	if (ortp_list_find(f->offer_answer_provider_list, offer_answer_prov)) return; /*avoid registering several time the same pointer*/
	f->offer_answer_provider_list = ortp_list_prepend(f->offer_answer_provider_list, offer_answer_prov);
}
...
void ms_factory_set_image_resources_dir(MSFactory *f, const char *path) {
	if (f->image_resources_dir) {
		ortp_free(f->image_resources_dir);
		f->image_resources_dir = NULL;
	}
	if (path)
		f->image_resources_dir = ortp_strdup(path);
}
...
const char * ms_factory_get_default_video_renderer(ORTP_UNUSED(MSFactory *f)) {
...
JNIEXPORT jint JNICALL Java_org_linphone_mediastream_Factory_enableFilterFromName(JNIEnv* env,  ORTP_UNUSED(jobject obj), jlong factoryPtr, jstring jname, jboolean enable) {
...
JNIEXPORT jboolean JNICALL Java_org_linphone_mediastream_Factory_filterFromNameEnabled(JNIEnv* env, ORTP_UNUSED(jobject obj), jlong factoryPtr, jstring jname) {
	const char *name = jname ? (*env)->GetStringUTFChars(env, jname, NULL) : NULL;
	MSFactory *factory = (MSFactory *) factoryPtr;
...
JNIEXPORT jstring JNICALL Java_org_linphone_mediastream_Factory_getEncoderText(JNIEnv* env, ORTP_UNUSED(jobject obj),
...
JNIEXPORT jstring JNICALL Java_org_linphone_mediastream_Factory_getDecoderText(JNIEnv* env, ORTP_UNUSED(jobject obj),
    jlong factoryPtr, jstring jmime) {
	MSFactory *factory = (MSFactory*)factoryPtr;
	const char *mime = (*env)->GetStringUTFChars(env, jmime, NULL);
	jstring jtext = NULL;
...
JNIEXPORT jint JNICALL Java_org_linphone_mediastream_MediastreamerAndroidContext_enableFilterFromNameImpl(JNIEnv* env, ORTP_UNUSED(jobject obj), jstring jname, jboolean enable) {
	const char *mime;
	int result;
...
JNIEXPORT jboolean JNICALL Java_org_linphone_mediastream_MediastreamerAndroidContext_filterFromNameEnabledImpl(JNIEnv* env, ORTP_UNUSED(jobject obj), jstring jname) {
	const char *mime;
	jboolean result;
	if (ms_factory_get_fallback() == NULL) {
...
void ms_factory_destroy(MSFactory *factory) {
	...
	factory->formats = ortp_list_free_with_data(factory->formats, (void(*)(void*))ms_fmt_descriptor_destroy);
	factory->desc_list = ortp_list_free(factory->desc_list);
	ortp_list_for_each(factory->stats_list, ms_free);
	factory->stats_list = ortp_list_free(factory->stats_list);
	factory->offer_answer_provider_list = ortp_list_free(factory->offer_answer_provider_list);
	ortp_list_for_each(factory->platform_tags, ms_free);
	factory->platform_tags = ortp_list_free(factory->platform_tags);
