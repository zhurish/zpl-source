...
void ms_filter_add_notify_callback(MSFilter *f, MSFilterNotifyFunc fn, void *ud, bool_t synchronous){
	f->notify_callbacks=ortp_list_append(f->notify_callbacks,ms_notify_context_new(fn,ud,synchronous));
}

void ms_filter_remove_notify_callback(MSFilter *f, MSFilterNotifyFunc fn, void *ud){
	ortp_list_t *elem;
	ortp_list_t *found=NULL;
	...
	if (found){
		...
		f->notify_callbacks=ortp_list_erase_link(f->notify_callbacks,found);
...
void ms_filter_clear_notify_callback(MSFilter *f){
	f->notify_callbacks=ortp_list_free_with_data(f->notify_callbacks,(void (*)(void*))ms_notify_context_destroy);
}
