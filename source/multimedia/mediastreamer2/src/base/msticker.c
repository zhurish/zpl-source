...
static ortp_list_t *get_sources(ortp_list_t *filters){
	ortp_list_t *sources=NULL;
	MSFilter *f;
	for(;filters!=NULL;filters=filters->next){
		f=(MSFilter*)filters->data;
		if (f->desc->ninputs==0){
			sources=ortp_list_append(sources,f);
		}
	}
	return sources;
}
...
{
	ortp_list_t *sources=NULL;
	ortp_list_t *filters=NULL;
	ortp_list_t *it;
	ortp_list_t *total_sources=NULL;
	va_list l;
	...
	do{
		if (f->ticker==NULL) {
			...
			ortp_list_free(filters);
			total_sources=ortp_list_concat(total_sources,sources);
		}else{
			if (f->ticker == ticker){
	...
	if (total_sources){
		...
		ticker->execution_list=ortp_list_concat(ticker->execution_list,total_sources);
...
int ms_ticker_detach(MSTicker *ticker, MSFilter *f){
	...
	for(it=sources;it!=NULL;it=ortp_list_next(it)){
		ticker->execution_list=ortp_list_remove(ticker->execution_list,it->data);
	}
	ms_mutex_unlock(&ticker->lock);
	ortp_list_for_each(filters,(void (*)(void*))call_postprocess);
	ortp_list_free(filters);
	ortp_list_free(sources);
	return 0;
}
...
static void run_graph(MSFilter *f, MSTicker *s, ortp_list_t **unschedulable, bool_t force_schedule){
	int i;
	MSQueue *l;
	if (f->last_tick!=s->ticks ){
		if (filter_can_process(f,s->ticks) || force_schedule) {
			...
			*unschedulable=ortp_list_prepend(*unschedulable,f);
		}
	}
}

static void run_graphs(MSTicker *s, ortp_list_t *execution_list, bool_t force_schedule){
	ortp_list_t *it;
	ortp_list_t *unschedulable=NULL;
	...
	if (unschedulable!=NULL) {
		...
		ortp_list_free(unschedulable);
	}
}

static void run_tasks(MSTicker *ticker){
	ortp_list_t *elem,*prevelem=NULL;
...
static void remove_tasks_for_filter(MSTicker *ticker, MSFilter *f){
	ortp_list_t *elem,*nextelem;
	for (elem=ticker->task_list;elem!=NULL;elem=nextelem){
		MSFilterTask *t=(MSFilterTask*)elem->data;
		nextelem=elem->next;
		if (t->f==f){
			ticker->task_list=ortp_list_erase_link(ticker->task_list,elem);
			ms_free(t);
		}
	}
}

static uint64_t get_cur_time_ms(ORTP_UNUSED(void * unused)){
...
static int wait_next_tick(void *data, ORTP_UNUSED(uint64_t virt_ticker_time)){
...
void * ms_ticker_run(void *arg)
{
	...
	ms_mutex_lock(&s->lock);
	//ortp_set_self_thread_name(s->name);
...
static void print_graph(MSFilter *f, MSTicker *s, ortp_list_t **unschedulable, bool_t force_schedule){
	int i;
	MSQueue *l;
	if (f->last_tick!=s->ticks ){
		if (filter_can_process(f,s->ticks) || force_schedule) {
...
static void print_graphs(MSTicker *s, ortp_list_t *execution_list, bool_t force_schedule){
	ortp_list_t *it;
	ortp_list_t *unschedulable=NULL;
	for(it=execution_list;it!=NULL;it=it->next){
