...
static int sender_telephone_event_supported(MSFilter *f, ORTP_UNUSED(void *arg)) {
	SenderData *d = (SenderData *)f->data;
...
static int sender_mute(MSFilter * f, ORTP_UNUSED(void *arg))
{
	SenderData *d = (SenderData *) f->data;
	ms_filter_lock(f);
	d->mute=TRUE;
...
static int sender_unmute(MSFilter * f, ORTP_UNUSED(void *arg))
{
	SenderData *d = (SenderData *) f->data;
	ms_filter_lock(f);
	d->mute=FALSE;
...
static mblk_t *create_packet_with_volume_data_at_intervals(MSFilter *f, ORTP_UNUSED(size_t header_size), const uint8_t *payload, size_t payload_size) {
	SenderData *d = (SenderData *) f->data;
	RtpSession *s = d->session;
...
static int receiver_reset_jitter_buffer(MSFilter *f, ORTP_UNUSED(void *arg)) {
	ReceiverData *d = (ReceiverData *)f->data;
	d->reset_jb = TRUE;
	return 0;
}
