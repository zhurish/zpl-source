...
int ms_bits_writer_n_bits(MSBitsWriter *writer, int count, unsigned int value, ORTP_UNUSED(const char* symbol_name)) {
	uint8_t swap[4];
	int i;
	int byte_index;
	int bits_left;
...
int ms_bits_writer_ue(MSBitsWriter *writer, unsigned int value, ORTP_UNUSED(const char* symbol_name)) {
	int size_in_bits = 0;
	int tmp_val = value + 1;
