...
void media_stream_remove_tmmbr_handler(MediaStream *stream, void (*on_tmmbr_received)(const OrtpEventData *evd, void *), ORTP_UNUSED(void * user_data)){
...
RtpSession * ms_create_duplex_rtp_session(const char* local_ip, int loc_rtp_port, int loc_rtcp_port, int mtu) {
	...
	rtp_session_set_send_ts_offset(rtpr, (uint32_t)ortp_random());
...
void video_stream_open_player(ORTP_UNUSED(VideoStream *stream), ORTP_UNUSED(MSFilter *sink)){
}

void video_stream_close_player(ORTP_UNUSED(VideoStream *stream)){
}

void video_stream_enable_recording(ORTP_UNUSED(VideoStream *stream), ORTP_UNUSED(bool_t enabled)) {}
