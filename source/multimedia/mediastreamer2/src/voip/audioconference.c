...
struct _MSAudioConference{
	...
	ortp_list_t *members; /* list of MSAudioEndpoint */
	int nmembers;
...
static int request_volumes(ORTP_UNUSED(MSFilter *filter), rtp_audio_level_t **audio_levels, void *user_data) {
...
void ms_audio_conference_add_member(MSAudioConference *obj, MSAudioEndpoint *ep){
	...
	obj->members = ortp_list_append(obj->members, ep);
	obj->nmembers++;
...
void ms_audio_conference_remove_member(MSAudioConference *obj, MSAudioEndpoint *ep){
	...
	ep->conference=NULL;
	obj->nmembers--;
	obj->members = ortp_list_remove(obj->members, ep);
	if (obj->nmembers>0) ms_ticker_attach(obj->ticker,obj->mixer);
}

void ms_audio_conference_mute_member(ORTP_UNUSED(MSAudioConference *obj), MSAudioEndpoint *ep, bool_t muted){
