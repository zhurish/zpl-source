...
class SslContext {
  ...
  public:
	SslContext(ortp_socket_t socket, std::string rootCertificatePath, std::string cn, ortp_rng_context_t *rng);
	~SslContext();
  ...
  private:
	ortp_ssl_context_t *mContext;
	ortp_ssl_config_t *mConfig;
	ortp_x509_certificate_t *mRootCertificate;
