#include "mediastreamer2/msmediasend.h"
...
static void _create_decoders(MSMediaSend *obj);
static void _create_sinks(MSMediaSend *obj);
static void _destroy_graph(MSMediaSend *obj);
static bool_t _link_all(MSMediaSend *obj);
static void _unlink_all(MSMediaSend *obj);
...
MSMediaSend *ms_media_send_new(MSFactory* factory, MSSndCard *snd_card, const char *video_display_name, void *window_id) {
	MSMediaSend *obj = (MSMediaSend *)ms_new0(MSMediaSend, 1);
...
void ms_media_send_free(MSMediaSend *obj) {
	ms_media_send_close(obj);
	ms_ticker_destroy(obj->ticker);
	ms_free_if_not_null(obj->video_display);
	ms_free(obj);
}

void * ms_media_send_get_window_id(const MSMediaSend *obj) {
	return obj->window_id;
}

bool_t ms_media_send_open(MSMediaSend *obj, const char *filepath) {
...
void ms_media_send_close(MSMediaSend *obj) {
	if(obj->is_open) {
...
bool_t ms_media_send_start(MSMediaSend *obj) {
	if(!obj->is_open) {
...
void ms_media_send_stop(MSMediaSend *obj) {
	int seek_pos = 0;
	if(obj->is_open) {
...
void ms_media_send_pause(MSMediaSend *obj) {
	if(obj->is_open) {
		ms_filter_call_method_noarg(obj->player, MS_PLAYER_PAUSE);
	}
}

bool_t ms_media_send_seek(MSMediaSend *obj, int seek_pos_ms) {
	if(obj->is_open) {
...
MSPlayerState ms_media_send_get_state(MSMediaSend *obj) {
	if(obj->is_open) {
		MSPlayerState state;
...
int ms_media_send_get_duration(MSMediaSend *obj) {
	int duration;
	if(!obj->is_open) {
...
int ms_media_send_get_current_position(MSMediaSend *obj) {
	int position;
	if(!obj->is_open) {
...
void ms_media_send_set_eof_callback(MSMediaSend *obj, MSMediaSendEofCallback cb, void *user_data) {
...
void ms_media_send_set_loop(MSMediaSend *obj, int loop_interval_ms) {
...
bool_t ms_media_send_matroska_supported(void) {
#ifdef HAVE_MATROSKA
	return TRUE;
#else
	return FALSE;
...
MSFileFormat ms_media_send_get_file_format(const MSMediaSend *obj) {
	return obj->format;
}
...
static void _eof_filter_notify_cb(void *userdata, struct _MSFilter *f, unsigned int id, void *arg) {
	MSMediaSend *obj = (MSMediaSend *)userdata;
	ms_mutex_lock(&obj->cb_access);
