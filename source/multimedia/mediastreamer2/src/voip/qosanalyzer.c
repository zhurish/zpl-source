...
static bool_t stateful_analyzer_process_rtcp(MSQosAnalyzer *objbase, mblk_t *rtcp){
	...
	if (rb && report_block_get_ssrc(rb)==rtp_session_get_send_ssrc(obj->session)){
		if (ortp_loss_rate_estimator_process_report_block(objbase->lre,obj->session,rb)){
			...
			obj->rtcpstatspoint=ortp_list_insert_sorted(obj->rtcpstatspoint,
				obj->latest, (ortp_compare_func)sort_by_bandwidth);

			/*if the measure was 0% loss, reset to 0% every measures below it*/
			if (obj->latest->loss_percent < 1e-5){
				ortp_list_t *it=obj->rtcpstatspoint;
				ortp_list_t *latest_pos=ortp_list_find(obj->rtcpstatspoint,obj->latest);
			...
			if (ortp_list_size(obj->rtcpstatspoint) > ESTIM_HISTORY){
				size_t prev_size = ortp_list_size(obj->rtcpstatspoint);

				/*clean everything which occurred 60 sec or more ago*/
				time_t clear_time = ms_time(0) - 60;
				obj->rtcpstatspoint = ortp_list_remove_custom(obj->rtcpstatspoint,
					(ortp_compare_func)earlier_than, &clear_time);
				ms_message("MSStatefulQosAnalyzer[%p]: reached list maximum capacity "
					"(count=%u) --> Cleaned list (count=%u)",
					obj, (unsigned int)prev_size, (unsigned int)ortp_list_size(obj->rtcpstatspoint));
			}
			return TRUE;
		}
	}
...
static ortp_list_t *find_first_with_loss(ortp_list_t *list){
...
static void smooth_values(MSStatefulQosAnalyzer *obj){
	ortp_list_t *first_loss = find_first_with_loss(obj->rtcpstatspoint);
	ortp_list_t *it = obj->rtcpstatspoint;
...
static double compute_available_bw(MSStatefulQosAnalyzer *obj){
	...
	double mean_bw = 0.;
	ortp_list_t *current = obj->rtcpstatspoint;
	ortp_list_t *last = current;
	size_t size = ortp_list_size(obj->rtcpstatspoint);
	if (current == NULL){
	...
	for (it = obj->rtcpstatspoint; it != NULL; it=it->next){
		...
		ms_message("MSStatefulQosAnalyzer[%p]:\t\tsorted values %d: %f %f",
			obj, ortp_list_position(obj->rtcpstatspoint, it), point->bandwidth, point->loss_percent);
	}
#endif
	...
	if (size == 1){
		...
		while (current!=NULL && ((rtcpstatspoint_t*)current->data)->loss_percent<3+constant_network_loss){
			ms_message("MSStatefulQosAnalyzer[%p]:\t%d is stable", obj, ortp_list_position(obj->rtcpstatspoint, current));
			...
			for (it=last;it!=current;it=it->prev){
				if (((rtcpstatspoint_t *)it->data)->loss_percent <= 3 + ((rtcpstatspoint_t*)current->data)->loss_percent){
					ms_message("MSStatefulQosAnalyzer[%p]:\t%d is less than %d",
						obj, ortp_list_position(obj->rtcpstatspoint, it), ortp_list_position(obj->rtcpstatspoint, current));
					current = it;
					break;
				}
			}
		...
		ms_message("MSStatefulQosAnalyzer[%p]: [0->%d] last stable is %d(%f;%f)"
			, obj
			, ortp_list_position(obj->rtcpstatspoint, last)
			, ortp_list_position(obj->rtcpstatspoint, (current ? current->prev : last))
		...
		if (current!=NULL){
			ms_message("MSStatefulQosAnalyzer[%p]: , first unstable is %d(%f;%f)"
				, obj
				, ortp_list_position(obj->rtcpstatspoint, current)
...
static void stateful_analyzer_suggest_action(MSQosAnalyzer *objbase, MSRateControlAction *action){
	...
	*/
	if (obj->curindex==1){
		...
		greatest_pt = ortp_list_size(obj->rtcpstatspoint) ?
			(rtcpstatspoint_t*)ortp_list_nth_data(obj->rtcpstatspoint, (int)ortp_list_size(obj->rtcpstatspoint)-1)
			: NULL;
...
static bool_t stateful_analyzer_has_improved(ORTP_UNUSED(MSQosAnalyzer *objbase)){
...
static void stateful_analyzer_update(MSQosAnalyzer *objbase){
	...
	if (obj->burst_duration_ms>0){
		...
		case MSStatefulQosAnalyzerBurstEnable:{
			...
			ORTP_NO_BREAK; /*intentionally no break*/
...
static void stateful_analyzer_uninit(MSQosAnalyzer *objbase){
	MSStatefulQosAnalyzer *obj=(MSStatefulQosAnalyzer*)objbase;
	ortp_list_for_each(obj->rtcpstatspoint, ms_free);
	ortp_list_free(obj->rtcpstatspoint);
}
