...
static void source_event_cb(void *ud, MSFilter* f, unsigned int event, ORTP_UNUSED(void *eventdata)){
...
static void preview_source_event_cb(void* ud, MSFilter* f, unsigned int event, ORTP_UNUSED(void* eventdata)) {
	VideoStream* st = (VideoStream*)ud;
	MSVideoSize size;
	switch (event) {// Allow a source to reinitialize all tree formats
	case MS_FILTER_OUTPUT_FMT_CHANGED:
...
static void internal_event_cb(void *ud, ORTP_UNUSED(MSFilter *f), unsigned int event, void *eventdata) {
...
static void video_stream_process_encoder_control(VideoStream *stream, unsigned int method_id, void *arg, ORTP_UNUSED(void * user_data)){
...
static void ext_display_cb(void *ud, ORTP_UNUSED(MSFilter* f), ORTP_UNUSED(unsigned int event), void *eventdata){
...
void video_recorder_handle_event(void *userdata, ORTP_UNUSED(MSFilter * f), unsigned int event, ORTP_UNUSED(void * event_args)){
	VideoStream *stream = (VideoStream*) userdata;
	switch (event){
...
static void apply_video_preset(VideoStream *stream, PayloadType *pt) {
	...
	ortp_list_t *codec_tags = NULL;
	bool_t hardware_accelerated = FALSE;
	if (stream->preset != NULL) {
		codec_tags = ortp_list_append(codec_tags, ms_strdup(payload_type_get_mime(pt)));
		codec_tags = ortp_list_append(codec_tags, ms_strdup(stream->ms.encoder->desc->name));
		...
		if (hardware_accelerated == TRUE) {
			codec_tags = ortp_list_append(codec_tags, ms_strdup("hardware"));
		}
		vpc = ms_video_presets_manager_find_preset_configuration(vpm, stream->preset, codec_tags);
		ortp_list_for_each(codec_tags, ms_free);
		ortp_list_free(codec_tags);
		if (vpc != NULL) {
...
static void csrc_event_cb(void *ud, ORTP_UNUSED(MSFilter * f), unsigned int event, void *eventdata) {
	VideoStream *stream = (VideoStream *)ud;
...
bool_t video_preview_qrcode_enabled(ORTP_UNUSED(VideoPreview *stream)) {
#ifdef QRCODE_ENABLED
