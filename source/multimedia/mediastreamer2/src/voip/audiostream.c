...
static void on_dtmf_received(ORTP_UNUSED(RtpSession *s), uint32_t dtmf, void *user_data)
{
...
static void audio_stream_process_rtcp(ORTP_UNUSED(MediaStream *media_stream), ORTP_UNUSED(mblk_t *m)){
...
static void write_callback(void *ud, MSFilter *f, unsigned int id, ORTP_UNUSED(void *arg)){
...
static void read_callback(void *ud, MSFilter *f, unsigned int id, ORTP_UNUSED(void *arg)){
	AudioStream *stream=(AudioStream *)ud;
	switch(id){
		case MS_FILTER_OUTPUT_FMT_CHANGED:
...
static void _audio_stream_unprepare_sound(AudioStream *stream, ORTP_UNUSED(bool_t keep_sound_resources)){
...
static void player_callback(void *ud, MSFilter *f, unsigned int id, ORTP_UNUSED(void *arg)){
	AudioStream *stream=(AudioStream *)ud;
	int sr=0;
	int channels=0;
	switch(id){
...
static void video_input_updated(void *stream, ORTP_UNUSED(MSFilter *f), unsigned int event_id, ORTP_UNUSED(void *arg)){
...
static void on_silence_detected(void *data, ORTP_UNUSED(MSFilter *f), unsigned int event_id, void *event_arg){
...
static void on_cn_received(void *data, ORTP_UNUSED(MSFilter *f), unsigned int event_id, void *event_arg){
	AudioStream *as=(AudioStream*)data;
...
static void ms_audio_flow_control_event_handler(void *user_data, ORTP_UNUSED(MSFilter *source), unsigned int event, void *eventdata) {
...
static void on_volumes_received(void *data, ORTP_UNUSED(MSFilter *f), unsigned int event_id, void *event_arg) {
	AudioStream *as=(AudioStream*)data;
...
static int request_stream_volume(ORTP_UNUSED(MSFilter *filter), void* user_data) {
...
static void on_audio_route_changed_received(void *data, ORTP_UNUSED(MSFilter *f), unsigned int event_id, void *event_arg) {
	AudioStream *as=(AudioStream*)data;
...

	{
		...
		if (device && device->hacks) {
			...
			if (gains && stream->mic_equalizer) {
				ortp_list_t *gains_list = ms_parse_equalizer_string(gains);
				if (gains_list) {
					ortp_list_t *it;
					...
					ortp_list_free_with_data(gains_list, ms_free);
				}
			}
