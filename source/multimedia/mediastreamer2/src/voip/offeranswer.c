 ...
 */
static PayloadType * h264_match(ORTP_UNUSED(MSOfferAnswerContext *ctx), const ortp_list_t *local_payloads, const PayloadType *refpt, const ortp_list_t *remote_payloads, ORTP_UNUSED(bool_t reading_response)){
	PayloadType *pt = NULL;
	const ortp_list_t *it;
	PayloadType *local_h264_with_packetization_mode_1_pt = NULL;
	ortp_list_t *local_h264_list = NULL;
	...
	for (it=local_payloads;it!=NULL;it=it->next){
		pt=(PayloadType*)it->data;
		if (strcasecmp(pt->mime_type, "h264")==0) {
			local_h264_list = ortp_list_append(local_h264_list, pt);
	...
	if (ortp_list_size(local_h264_list) < 1) {
		ms_message("No H264 payload configured locally");
		goto end;
	}
	//taking first one by default
	matching_pt = ortp_list_get_data(local_h264_list);
	...
	if (local_h264_list) {
		ortp_list_free(local_h264_list);
	}
