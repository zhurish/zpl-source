...
static bool_t ice_has_componentID(const ortp_list_t *list, uint16_t componentID);
#if 0
...
static void ice_free_candidate_pair(IceCandidatePair *pair, IceCheckList *cl) {
	ortp_list_t *elem;
	while ((elem = ortp_list_find(cl->check_list, pair)) != NULL) {
		cl->check_list = ortp_list_remove(cl->check_list, pair);
	}
	while ((elem = ortp_list_find_custom(cl->valid_list, (ortp_compare_func)ice_find_pair_in_valid_list, pair)) !=
	       NULL) {
		ice_free_valid_pair(elem->data);
		cl->valid_list = ortp_list_erase_link(cl->valid_list, elem);
	}
	ms_free(pair);
}
...
void ice_check_list_destroy(IceCheckList *cl) {
	...
	ortp_list_for_each(cl->stun_server_requests, (void (*)(void *))ice_stun_server_request_free);
	ortp_list_for_each(cl->transaction_list, (void (*)(void *))ice_free_transaction);
	ortp_list_for_each(cl->foundations, (void (*)(void *))ice_free_pair_foundation);
	ortp_list_for_each2(cl->pairs, (void (*)(void *, void *))ice_free_candidate_pair, cl);
	ortp_list_for_each(cl->valid_list, (void (*)(void *))ice_free_valid_pair);
	ortp_list_for_each(cl->remote_candidates, (void (*)(void *))ice_free_candidate);
	ortp_list_for_each(cl->local_candidates, (void (*)(void *))ice_free_candidate);
	ortp_list_free(cl->stun_server_requests);
	ortp_list_free(cl->transaction_list);
	ortp_list_free(cl->foundations);
	ortp_list_free(cl->local_componentIDs);
	ortp_list_free(cl->remote_componentIDs);
	ortp_list_free(cl->valid_list);
	ortp_list_free(cl->check_list);
	ortp_list_free(cl->triggered_checks_queue);
	ortp_list_free(cl->losing_pairs);
	ortp_list_free(cl->pairs);
	ortp_list_free(cl->remote_candidates);
	ortp_list_free(cl->local_candidates);
...
bool_t ice_check_list_default_local_candidate(const IceCheckList *cl,
	...
	if (rtp_candidate != NULL) {
		...
		elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_default_local_candidate,
		                              &componentID);
...
bool_t ice_check_list_selected_valid_local_candidate(const IceCheckList *cl,
	...
	if (rtp_candidate != NULL) {
		componentID = ICE_RTP_COMPONENT_ID;
		elem = ortp_list_find_custom(cl->valid_list, (ortp_compare_func)ice_find_selected_valid_pair_from_componentID,
		                              &componentID);
		if (elem == NULL) {
	...
	if (rtcp_candidate != NULL) {
		if (ice_has_componentID(cl->local_componentIDs, ICE_RTCP_COMPONENT_ID)) {
			componentID = ICE_RTCP_COMPONENT_ID;
			elem = ortp_list_find_custom(
			    cl->valid_list, (ortp_compare_func)ice_find_selected_valid_pair_from_componentID, &componentID);
			if (elem == NULL) return FALSE;
			valid_pair = (IceValidCandidatePair *)elem->data;
...
IceCandidateType ice_check_list_selected_valid_candidate_type(const IceCheckList *cl) {
	...
	if ((type == ICT_ServerReflexiveCandidate) || (type == ICT_PeerReflexiveCandidate)) {
		elem =
		    ortp_list_find_custom(cl->pairs, (ortp_compare_func)ice_find_host_pair_identical_to_reflexive_pair, pair);
		if (elem != NULL) {
...
void ice_check_list_check_completed(IceCheckList *cl) {
	...
	if (cl->state != ICL_Completed) {
		cb.cl = cl;
		cb.result = TRUE;
		ortp_list_for_each2(cl->local_componentIDs,
...
static void ice_check_list_queue_triggered_check(IceCheckList *cl, IceCandidatePair *pair) {
	ortp_list_t *elem = ortp_list_find(cl->triggered_checks_queue, pair);
	if (elem != NULL) {
		/* The pair is already in the triggered checks queue, do not add it again. */
	} else {
		cl->triggered_checks_queue = ortp_list_append(cl->triggered_checks_queue, pair);
	}
}
...
static IceCandidatePair *ice_check_list_pop_triggered_check(IceCheckList *cl) {
	...
	if (ortp_list_size(cl->triggered_checks_queue) == 0) return NULL;
	pair = ortp_list_nth_data(cl->triggered_checks_queue, 0);
	if (pair != NULL) {
		/* Remove the first element in the triggered checks queue. */
		cl->triggered_checks_queue = ortp_list_erase_link(cl->triggered_checks_queue, cl->triggered_checks_queue);
	}
	return pair;
}

static int ice_find_non_frozen_pair(const IceCandidatePair *pair, ORTP_UNUSED(const void *dummy)) {
	return (pair->state == ICP_Frozen);
}

static bool_t ice_check_list_is_frozen(const IceCheckList *cl) {
	ortp_list_t *elem = ortp_list_find_custom(cl->check_list, (ortp_compare_func)ice_find_non_frozen_pair, NULL);
	return (elem == NULL);
}
...
static void ice_check_list_compute_pair_priorities(IceCheckList *cl) {
	ortp_list_for_each2(cl->pairs, (void (*)(void *, void *))ice_compute_pair_priority, &cl->session->role);
}
...
static void ice_find_default_remote_candidate_for_componentID(const uint16_t *componentID, IceCheckList *cl) {
	ortp_list_t *elem = ortp_list_find_custom(
	    cl->remote_candidates, (ortp_compare_func)ice_find_default_candidate_from_componentID, componentID);
	if (elem == NULL) {
...
static void ice_check_list_check_mismatch(IceCheckList *cl) {
	ortp_list_for_each2(cl->remote_componentIDs,
...
static void ice_check_list_add_stun_server_request(IceCheckList *cl, IceStunServerRequest *request) {
	cl->stun_server_requests = ortp_list_append(cl->stun_server_requests, request);
}
...
static bool_t ice_check_list_gather_candidates(IceCheckList *cl, Session_Index *si) {
	...
	if ((cl->rtp_session != NULL) && (cl->gathering_candidates == FALSE) && (cl->state != ICL_Completed) &&
		...
		if (rtptp) {
			...
			ortp_sockaddr_to_address(sa, cl->rtp_session->rtp.gs.loc_addrlen, source_addr_str,
		...
		if (!rtp_session_rtcp_mux_enabled(cl->rtp_session) && rtptp) {
			...
			memset(source_addr_str, 0, sizeof(source_addr_str));
			ortp_sockaddr_to_address(sa, cl->rtp_session->rtcp.gs.loc_addrlen, source_addr_str,
			                             sizeof(source_addr_str), &source_port);
			request = ice_stun_server_request_new(
...
static void ice_check_list_deallocate_turn_candidate(IceCheckList *cl,
	...
	if ((turn_context != NULL) &&
		...
		if (rtptp) {
			...
			memset(source_addr_str, 0, sizeof(source_addr_str));
			ortp_sockaddr_to_address(sa, stream->loc_addrlen, source_addr_str, sizeof(source_addr_str),
			                             &source_port);
...
static void ice_stun_server_check_sum_gathering_round_trip_time(const IceStunServerRequest *request,
	...
	if (request->gathering == TRUE) {
		ortp_list_for_each2(request->transactions,
...
static void ice_check_list_sum_gathering_round_trip_times(IceCheckList *cl) {
	ortp_list_for_each2(cl->stun_server_requests,
...
static void ice_check_list_select_candidates(IceCheckList *cl) {
	...
	ortp_list_for_each(cl->valid_list, (void (*)(void *))ice_unselect_valid_pair);
	for (componentID = 1; componentID <= 2; componentID++) {
		elem = ortp_list_find_custom(cl->valid_list,
		                              (ortp_compare_func)ice_find_nominated_valid_pair_from_componentID, &componentID);
...
static IceTransaction *ice_create_transaction(IceCheckList *cl, IceCandidatePair *pair, const UInt96 tr_id) {
	...
	cl->transaction_list = ortp_list_prepend(cl->transaction_list, transaction);
	return transaction;
}
...
static IceTransaction *ice_find_transaction(const IceCheckList *cl, const IceCandidatePair *pair) {
	ortp_list_t *elem =
	    ortp_list_find_custom(cl->transaction_list, (ortp_compare_func)ice_find_transaction_from_pair, pair);
...
static IceStunServerRequest *ice_stun_server_request_new(IceCheckList *cl,
	...
	request->rtptp = rtptp;
	request->salen = sizeof(struct sockaddr);
	ortp_address_to_sockaddr(family, srcaddr, srcport, &request->source_ai, &request->salen);
	/*request->source_ai = ortp_ip_address_to_addrinfo(family, SOCK_DGRAM, srcaddr, srcport);
...
static void ice_stun_server_request_add_transaction(IceStunServerRequest *request,
                                                    IceStunServerRequestTransaction *transaction) {
	if (transaction != NULL) {
		request->transactions = ortp_list_append(request->transactions, transaction);
	}
}
...
static void ice_stun_server_request_free(IceStunServerRequest *request) {
	ortp_list_for_each(request->transactions, (void (*)(void *))ice_stun_server_request_transaction_free);
	ortp_list_free(request->transactions);
	//if (request->source_ai != NULL) ortp_freeaddrinfo(request->source_ai);
	ms_free(request);
}
...
static int ice_send_message_to_socket(RtpTransport *rtptp,
	...
	if ((rtptp->session->rtp.gs.sockfamily == AF_INET6) && (to->sa_family == AF_INET)) {
		char to_addr_str[64];
		int to_port = 0;
		socklen_t to_salen = 0;
		memset(to_addr_str, 0, sizeof(to_addr_str));
	
		ortp_sockaddr_to_address(to, tolen, to_addr_str, sizeof(to_addr_str), &to_port);
		//ortp_sockaddr_to_ip_address(to, tolen, to_addr_str, sizeof(to_addr_str), &to_port);
		to_salen = sizeof(struct sockaddr);
		ortp_address_to_sockaddr(AF_INET6, to_addr_str, to_port, to, &to_salen);
		//v6ai = ortp_ip_address_to_addrinfo(AF_INET6, SOCK_DGRAM, to_addr_str, to_port);
		//to = v6ai->ai_addr;
		tolen = (socklen_t)to_salen;
	}
	err = meta_rtp_transport_modifier_inject_packet_to_send_to(rtptp, NULL, m, 0, to, tolen);
	freemsg(m);
	//if (v6ai) ortp_freeaddrinfo(v6ai);
	return err;
}
...
static IceStunServerRequestTransaction *ice_send_stun_request(RtpTransport *rtptp,
	...
	if (len > 0) {
		...
		ortp_sockaddr_ipv6_to_ipv4(server, (struct sockaddr *)&server_addr, &server_addrlen);
		memset(source_addr_str, 0, sizeof(source_addr_str));
		memset(dest_addr_str, 0, sizeof(dest_addr_str));
		ortp_sockaddr_to_print_address((struct sockaddr *)source, sourcelen, source_addr_str,
		                                       sizeof(source_addr_str));
		ortp_sockaddr_to_print_address((struct sockaddr *)&server_addr, server_addrlen, dest_addr_str,
...
ice_send_stun_server_binding_request(IceStunServerRequest *request, const struct sockaddr *server, socklen_t addrlen) {
	...
	transaction =
	    ice_send_stun_request(request->rtptp, &request->source_ai, (socklen_t)request->salen,
...
ice_send_turn_server_refresh_request(IceStunServerRequest *request, const struct sockaddr *server, socklen_t addrlen) {
	...
	if (ms_turn_context_get_state(request->turn_context) != MS_TURN_CONTEXT_STATE_IDLE) {
		...
		stun_message_fill_authentication_from_turn_context(msg, request->turn_context);
		request->stun_method = ms_stun_message_get_method(msg);
		transaction = ice_send_stun_request(request->rtptp, &request->source_ai,
		                                    (socklen_t)request->salen, server, addrlen, msg,
...
static void ice_send_binding_response(IceCheckList *cl,
	...
	if (len > 0) {
		...
		ms_stun_address_to_sockaddr(dest, (struct sockaddr *)&dest_addr, &dest_addrlen);
		ortp_sockaddr_to_print_address((struct sockaddr *)&dest_addr, dest_addrlen, dest_addr_str,
		                                       sizeof(dest_addr_str));
		ortp_recvaddr_to_sockaddr(&evt_data->packet->recv_addr, (struct sockaddr *)&source_addr, &source_addrlen);
		ortp_sockaddr_ipv6_to_ipv4((struct sockaddr *)&source_addr, (struct sockaddr *)&source_addr, &source_addrlen);
		ortp_sockaddr_to_print_address((struct sockaddr *)&source_addr, source_addrlen, source_addr_str,
		                                       sizeof(source_addr_str));
...
static void ice_send_keepalive_packet_for_componentID(const uint16_t *componentID, const CheckList_RtpSession *cr) {
	ortp_list_t *elem = ortp_list_find_custom(
	    cr->cl->valid_list, (ortp_compare_func)ice_find_selected_valid_pair_from_componentID, componentID);
	if (elem != NULL) {
...
ice_fill_transport_address_from_sockaddr(IceTransportAddress *taddr, struct sockaddr *addr, socklen_t addrlen) {
	ortp_sockaddr_to_address(addr, addrlen, taddr->ip, sizeof(taddr->ip), &taddr->port);
...
static void ice_transport_address_to_printable_ip_address(const IceTransportAddress *taddr,
	...
	if (taddr == NULL) {
		*printable_ip = '\0';
		...
		ortp_address_to_sockaddr(taddr->family, taddr->ip, taddr->port, &sa, &salen);
		ortp_sockaddr_to_print_address(&sa, salen,  printable_ip, printable_ip_size);
		//ai = ortp_ip_address_to_addrinfo(taddr->family, SOCK_DGRAM, taddr->ip, taddr->port);
		//if (ai) {
			//ortp_addrinfo_to_printable_ip_address(ai, printable_ip, printable_ip_size);
			//ortp_freeaddrinfo(ai);
		//} else {
		//	*printable_ip = '\0';
		//}
	}
...
static void ice_generate_arbitrary_foundation(char *foundation, int len, ortp_list_t *list) {
	uint64_t r;
	ortp_list_t *elem;
	...
	do {
		...
		elem = ortp_list_find_custom(list, (ortp_compare_func)ice_find_candidate_from_foundation, foundation);
	} while (elem != NULL);
}
...
static IceCandidate *ice_learn_peer_reflexive_candidate(IceCheckList *cl,
	...
	elem = ortp_list_find_custom(cl->remote_candidates,
	                              (ortp_compare_func)ice_find_candidate_from_transport_address_and_componentID, &taci);
	if (elem == NULL) {
...
static IceCandidatePair *ice_trigger_connectivity_check_on_binding_request(IceCheckList *cl,
                                                                           ORTP_UNUSED(const RtpSession *rtp_session),
                                                                           const OrtpEventData *evt_data,
	...
	ortp_sockaddr_ipv6_to_ipv4((struct sockaddr *)&recv_addr, (struct sockaddr *)&recv_addr, &recv_addrlen);
	ice_fill_transport_address_from_sockaddr(&local_taddr, (struct sockaddr *)&recv_addr, recv_addrlen);
	elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_candidate_from_transport_address,
	                              &local_taddr);
	if (elem == NULL) {
	...
	if (prflx_candidate != NULL) {
		...
		elem = ortp_list_find_custom(cl->remote_candidates,
		                              (ortp_compare_func)ice_find_candidate_from_transport_address_and_componentID,
		                              &taci);
		if (elem == NULL) {
	...
	elem = ortp_list_find_custom(cl->check_list, (ortp_compare_func)ice_find_pair_from_candidates, &candidates);
	if (elem == NULL) {
		...
		elem = ortp_list_find_custom(cl->pairs, (ortp_compare_func)ice_find_pair_from_candidates, &candidates);
		if (elem == NULL) {
			pair = ice_pair_new(cl, candidates.local, candidates.remote);
			cl->pairs = ortp_list_append(cl->pairs, pair);
		} else {
			pair = (IceCandidatePair *)elem->data;
		}
		elem = ortp_list_find(cl->check_list, pair);
		if (elem == NULL) {
			cl->check_list =
			    ortp_list_insert_sorted(cl->check_list, pair, (ortp_compare_func)ice_compare_pair_priorities);
		}
...
static int ice_check_received_binding_response_addresses(ORTP_UNUSED(const RtpSession *rtp_session),
                                                         const OrtpEventData *evt_data,
                                                         IceCandidatePair *pair,
...
static int ice_check_received_binding_response_attributes(const MSStunMessage *msg,
                                                          ORTP_UNUSED(const MSStunAddress *remote_addr),
...
ice_discover_peer_reflexive_candidate(IceCheckList *cl, const IceCandidatePair *pair, const MSStunMessage *msg) {
	...
	taci.ta = &taddr;
	elem = ortp_list_find_custom(cl->local_candidates,
	                              (ortp_compare_func)ice_find_candidate_from_transport_address_and_componentID, &taci);
	if (elem == NULL) {
...
static IceCandidatePair *ice_construct_valid_pair(IceCheckList *cl,
                                                  RtpSession *rtp_session,
                                                  ORTP_UNUSED(const OrtpEventData *evt_data),
	...
	ice_transport_address_to_printable_ip_address(&pair->local->taddr, local_addr_str, sizeof(local_addr_str));
	ice_transport_address_to_printable_ip_address(&pair->remote->taddr, remote_addr_str, sizeof(remote_addr_str));
	elem = ortp_list_find_custom(cl->valid_list, (ortp_compare_func)ice_find_valid_pair, valid_pair);
	if (elem == NULL) {
		if (pair->is_default) {
			OrtpEvent *ev;
		...
		cl->valid_list =
		    ortp_list_insert_sorted(cl->valid_list, valid_pair, (ortp_compare_func)ice_compare_valid_pair_priorities);
		ms_message("ice: Added pair %p to the valid list: %s:%s --> %s:%s", pair, local_addr_str,
		           candidate_type_values[pair->local->type], remote_addr_str,
		           candidate_type_values[pair->remote->type]);
		// ice_dump_valid_list(cl);
		elem = ortp_list_find_custom(cl->losing_pairs, (ortp_compare_func)ice_find_pair_from_candidates, &candidates);
		if (elem != NULL) {
			cl->losing_pairs = ortp_list_erase_link(cl->losing_pairs, elem);
...
static void ice_update_pair_states_on_binding_response(IceCheckList *cl, IceCandidatePair *pair) {
	...
	ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_change_state_of_frozen_pairs_to_waiting, pair);
}
...
static void ice_schedule_turn_allocation_refresh(IceCheckList *cl, int componentID, uint32_t lifetime) {
	...
	memset(source_addr_str, 0, sizeof(source_addr_str));
	ortp_sockaddr_to_address(sa, stream->loc_addrlen, source_addr_str, sizeof(source_addr_str), &source_port);
	request = ice_stun_server_request_new(cl, turn_context, rtptp, sa->sa_family, source_addr_str, source_port,
	                                      MS_TURN_METHOD_REFRESH);
	if (!request) return;
...
static bool_t ice_handle_received_turn_allocate_success_response(IceCheckList *cl,
	...
	ortp_sockaddr_ipv6_to_ipv4(servaddr, servaddr, &servaddrlen);
	...
	if (!ms_compare_stun_addresses(remote_addr, &serv_stun_addr)) {
		...
		if (request != NULL) {
			...
			if ((componentID > 0) && (ice_parse_stun_server_response(msg, &srflx_addr, &relay_addr) >= 0)) {
				...
				ortp_sockaddr_ipv6_to_ipv4((struct sockaddr *)&recv_addr, (struct sockaddr *)&recv_addr,
				                            &recv_addr_len);
				ice_fill_transport_address_from_sockaddr(&taddr, (struct sockaddr *)&recv_addr, recv_addr_len);
				taci.componentID = componentID;
				taci.ta = &taddr;
				base_elem = ortp_list_find_custom(
				    cl->local_candidates, (ortp_compare_func)ice_find_candidate_from_transport_address_and_componentID,
				    &taci);
				...
				if (cl->rtp_turn_context &&
					...
					base_elem =
					    ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_host_candidate, &cf);
					if ((base_elem == NULL) && (cf.family == AF_INET)) {
						 ...
						 * STUN is IPv4. */
						cf.family = AF_INET6;
						base_elem = ortp_list_find_custom(cl->local_candidates,
						                                   (ortp_compare_func)ice_find_host_candidate, &cf);
					}
				}
		...
		if (ortp_list_find_custom(cl->stun_server_requests,
		                           (ortp_compare_func)ice_find_non_responded_gathering_stun_server_request,
		                           NULL) == NULL) {
...
static void ice_handle_received_create_permission_success_response(IceCheckList *cl,
                                                                   ORTP_UNUSED(RtpSession *rtp_session),
                                                                   const OrtpEventData *evt_data,
                                                                   const MSStunMessage *msg,
                                                                   ORTP_UNUSED(const MSStunAddress *remote_addr)) {
...
static void ice_handle_received_binding_response(IceCheckList *cl,
	...
	elem = ortp_list_find_custom(cl->transaction_list, (ortp_compare_func)ice_find_pair_from_transactionID, &tr_id);
	if (elem == NULL) {
...
static void ice_handle_stun_server_error_response(IceCheckList *cl,
	...
	ice_get_transport_from_rtp_session(rtp_session, evt_data, &rtptp);
	elem = ortp_list_find_custom(cl->stun_server_requests, (ortp_compare_func)ice_find_stun_server_request, rtptp);
	if (elem != NULL) {
...
static void ice_handle_received_error_response(IceCheckList *cl,
	...
	if (cl->gathering_candidates == TRUE) {
		...
		UInt96 tr_id = ms_stun_message_get_tr_id(msg);
		ortp_list_t *elem =
		    ortp_list_find_custom(cl->transaction_list, (ortp_compare_func)ice_find_pair_from_transactionID, &tr_id);
		if (elem == NULL) {
...
static int ice_find_stun_server_request_transaction(IceStunServerRequest *request, UInt96 *tr_id) {
	return (ortp_list_find_custom(request->transactions, (ortp_compare_func)ice_compare_transactionIDs, tr_id) ==
	        NULL);
}
...
static void ice_check_list_remove_stun_server_request(IceCheckList *cl, UInt96 *tr_id) {
	ortp_list_t *elem = cl->stun_server_requests;
	while (elem != NULL) {
		elem = ortp_list_find_custom(cl->stun_server_requests,
		                              (ortp_compare_func)ice_find_stun_server_request_transaction, tr_id);
		if (elem != NULL) {
			IceStunServerRequest *request = (IceStunServerRequest *)elem->data;
			ice_stun_server_request_free(request);
			cl->stun_server_requests = ortp_list_erase_link(cl->stun_server_requests, elem);
		}
	}
}

static IceStunServerRequest *ice_check_list_get_stun_server_request(IceCheckList *cl, UInt96 *tr_id) {
	ortp_list_t *elem = ortp_list_find_custom(cl->stun_server_requests,
	                                            (ortp_compare_func)ice_find_stun_server_request_transaction, tr_id);
	if (elem == NULL) return NULL;
...
static void ice_set_transaction_response_time(IceCheckList *cl, UInt96 *tr_id, MSTimeSpec response_time) {
	...
	elem = ortp_list_find_custom(request->transactions, (ortp_compare_func)ice_compare_transactionIDs, tr_id);
	if (elem == NULL) return;
...
void ice_handle_stun_packet(IceCheckList *cl, RtpSession *rtp_session, const OrtpEventData *evt_data) {
	...
	ortp_recvaddr_to_sockaddr(&evt_data->packet->recv_addr, (struct sockaddr *)&recv_addr, &recv_addrlen);
	ortp_sockaddr_ipv6_to_ipv4((struct sockaddr *)&recv_addr, (struct sockaddr *)&recv_addr, &recv_addrlen);
	ortp_sockaddr_to_print_address((struct sockaddr *)&recv_addr, recv_addrlen, recv_addr_str,
	                                       sizeof(recv_addr_str));
	ortp_sockaddr_ipv6_to_ipv4((struct sockaddr *)&evt_data->source_addr, (struct sockaddr *)&evt_data->source_addr,
	                            &recv_addrlen);
...
static void ice_add_componentID(ortp_list_t **list, uint16_t *componentID) {
	ortp_list_t *elem = ortp_list_find_custom(*list, (ortp_compare_func)ice_find_componentID, componentID);
	if (elem == NULL) {
		*list = ortp_list_append(*list, componentID);
	}
}

static void ice_remove_componentID(ortp_list_t **list, uint16_t componentID) {
	*list = ortp_list_remove_custom(*list, (ortp_compare_func)ice_find_componentID, &componentID);
}

static bool_t ice_has_componentID(const ortp_list_t *list, uint16_t componentID) {
	const ortp_list_t *elem = ortp_list_find_custom(list, (ortp_compare_func)ice_find_componentID, &componentID);
	return elem != NULL;
}
...
IceCandidate *ice_add_local_candidate(IceCheckList *cl,
	...
	if (ortp_list_size(cl->local_candidates) >= ICE_MAX_NB_CANDIDATES) {
	...
	elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_compare_candidates, candidate);
	if (elem != NULL) {
	...
	cl->local_candidates = ortp_list_append(cl->local_candidates, candidate);

	return candidate;
}

static void ice_create_turn_permissions(IceCheckList *cl) {
	ortp_list_t *remote_candidate_it;
	...
	for (remote_candidate_it = cl->remote_candidates; remote_candidate_it != NULL;
		...
		ortp_list_t *elem = ortp_list_find_custom(
		    cl->local_candidates, (ortp_compare_func)ice_find_candidate_from_type_family_and_componentID, &tfci);
		if (elem != NULL) {
			IceStunServerRequest *request;
			IceStunServerRequestTransaction *transaction;
...
IceCandidate *ice_add_remote_candidate(IceCheckList *cl,
	...
	elem = ortp_list_find_custom(cl->remote_candidates, (ortp_compare_func)ice_compare_candidates, candidate);
	if (elem != NULL) {
		/* This candidate is already in the list, do not add it again. */
		ms_free(candidate);
		return NULL;
	...
	cl->remote_candidates = ortp_list_append(cl->remote_candidates, candidate);
...
void ice_add_losing_pair(IceCheckList *cl,
	...
	if (elem == NULL) {
		...
		elem = ortp_list_find_custom(cl->remote_candidates, (ortp_compare_func)ice_find_candidate_from_ip_address,
		                              local_addr);
		if (elem != NULL) {
			tc.componentID = componentID;
			tc.type = ICT_ServerReflexiveCandidate;
			srflx_elem = ortp_list_find_custom(cl->remote_candidates,
			                                    (ortp_compare_func)ice_find_candidate_from_type_and_componentID, &tc);
		}
		ice_transport_address_to_printable_ip_address(&taddr, taddr_str, sizeof(taddr_str));
		if (srflx_elem != NULL) {
	...
	elem = ortp_list_find_custom(cl->pairs, (ortp_compare_func)ice_find_pair_from_candidates, &lr);
	if (elem == NULL) {
	...
	elem = ortp_list_find_custom(cl->valid_list, (ortp_compare_func)ice_find_pair_in_valid_list, pair);
	if (elem == NULL) {
		LosingRemoteCandidate_InProgress_Failed lif;
		...
		ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_check_if_losing_pair_should_cause_restart,
		                     &lif);
		if ((lif.in_progress_candidates == FALSE) && (lif.failed_candidates == TRUE)) {
			...
			elem = ortp_list_find(cl->losing_pairs, pair);
			if (elem == NULL) {
				cl->losing_pairs = ortp_list_append(cl->losing_pairs, pair);
			}
		}
	} else {
		valid_pair = (IceValidCandidatePair *)elem->data;
...
static int ice_check_list_nb_losing_pairs(const IceCheckList *cl) {
	return (int)ortp_list_size(cl->losing_pairs);
}
...
static void ice_compute_candidate_foundation(IceCandidate *candidate, IceCheckList *cl) {
	ortp_list_t *l = ortp_list_find_custom(cl->local_candidates,
	                                         (ortp_compare_func)ice_find_candidate_with_same_foundation, candidate);
	if (l != NULL) {
...
static void ice_check_list_compute_candidates_foundations(IceCheckList *cl) {
	if (cl->state == ICL_Running) {
		ortp_list_for_each2(cl->local_candidates, (void (*)(void *, void *))ice_compute_candidate_foundation, cl);
	}
}
...
static void ice_check_list_eliminate_redundant_candidates(IceCheckList *cl) {
	...
	if (cl->state == ICL_Running) {
		do {
			elem_removed = FALSE;
			/* Do not use ortp_list_for_each2() here, we may remove list elements. */
			for (elem = cl->local_candidates; elem != NULL; elem = elem->next) {
				candidate = (IceCandidate *)elem->data;
				other_elem = ortp_list_find_custom(cl->local_candidates,
				                                    (ortp_compare_func)ice_find_redundant_candidate, candidate);
				if (other_elem != NULL) {
					...
					if (other_candidate->priority < candidate->priority) {
						...
						cl->local_candidates = ortp_list_erase_link(cl->local_candidates, other_elem);
					} else {
						ice_free_candidate(candidate);
						cl->local_candidates = ortp_list_erase_link(cl->local_candidates, elem);
					}
					elem_removed = TRUE;
					break;
				}
...
static void ice_choose_local_or_remote_default_candidates(IceCheckList *cl, ortp_list_t *list) {
	...
	for (i = ICE_MIN_COMPONENTID; i <= ICE_MAX_COMPONENTID; i++) {
		...
		for (k = 0; k < ICT_CandidateTypeMax && cl->session->default_types[k] != ICT_CandidateInvalid; ++k) {
			...
			tc.family = AF_INET;
			l = ortp_list_find_custom(list, (ortp_compare_func)ice_find_candidate_from_type_family_and_componentID,
			                           &tc);
...
static void ice_form_candidate_pairs(IceCheckList *cl) {
	ortp_list_t *local_list = cl->local_candidates;
	ortp_list_t *remote_list;
	IceCandidatePair *pair;
	...
	while (local_list != NULL) {
		...
		while (remote_list != NULL) {
			...
			remote_list = ortp_list_next(remote_list);
		}
		local_list = ortp_list_next(local_list);
	}
}
...
static int ice_prune_duplicate_pair(IceCandidatePair *pair, ortp_list_t **pairs, IceCheckList *cl) {
	ortp_list_t *other_pair = ortp_list_find_custom(*pairs, (ortp_compare_func)ice_compare_pairs, pair);
	if (other_pair != NULL) {
		...
		if (other_candidate_pair->priority > pair->priority) {
			...
			*pairs = ortp_list_remove(*pairs, pair);
...
static void ice_create_check_list(IceCandidatePair *pair, IceCheckList *cl) {
	cl->check_list = ortp_list_insert_sorted(cl->check_list, pair, (ortp_compare_func)ice_compare_pair_priorities);
}
...
static void ice_prune_candidate_pairs(IceCheckList *cl) {
	...
	ortp_list_for_each(cl->pairs, (void (*)(void *))ice_replace_srflx_by_base_in_pair);
	/* Do not use ortp_list_for_each2() here, because ice_prune_duplicate_pair() can remove list elements. */
	...
	ortp_list_free(cl->check_list);
	cl->check_list = NULL;
	ortp_list_for_each2(cl->pairs, (void (*)(void *, void *))ice_create_check_list, cl);

	/* Limit the number of connectivity checks. */
	nb_pairs = (int)ortp_list_size(cl->check_list);
	if (nb_pairs > cl->session->max_connectivity_checks) {
		...
		for (i = 0; i < (nb_pairs - 1); i++)
			list = ortp_list_next(list);
		for (i = 0; i < nb_pairs_to_remove; i++) {
			cl->pairs = ortp_list_remove(cl->pairs, list->data);
			prev = list->prev;
...
static void ice_generate_pair_foundations_list(const IceCandidatePair *pair, ortp_list_t **list) {
	...
	elem = ortp_list_find_custom(*list, (ortp_compare_func)ice_find_pair_foundation, &foundation);
	if (elem == NULL) {
		dyn_foundation = ms_new0(IcePairFoundation, 1);
		memcpy(dyn_foundation, &foundation, sizeof(foundation));
		*list = ortp_list_append(*list, dyn_foundation);
	}
}
...
static void ice_set_lowest_componentid_pair_with_foundation_to_waiting_state(const IcePairFoundation *foundation,
	...
	fc.componentID = ICE_INVALID_COMPONENTID;
	fc.priority = 0;
	ortp_list_for_each2(cl->check_list,
...
static void ice_compute_pairs_states(IceCheckList *cl) {
	ortp_list_for_each2(cl->foundations,
...
static void ice_check_list_pair_candidates(IceCheckList *cl) {
	if (cl->state == ICL_Running) {
		...
		ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_generate_pair_foundations_list,
		                     &cl->foundations);
	}
}
...
static ortp_list_t *ice_get_valid_pairs_for_componentID(IceCheckList *cl, uint16_t componentID) {
	const ortp_list_t *it;
	...
	for (it = cl->valid_list; it != NULL; it = it->next) {
		...
		if (valid_pair->valid->local->componentID == componentID) {
			ret = ortp_list_insert_sorted(ret, valid_pair, (ortp_compare_func)valid_pair_compare);
		}
	}
	return ret;
}
...
static void ice_pair_stop_retransmissions(IceCandidatePair *pair, IceCheckList *cl) {
	...
	if (pair->state == ICP_InProgress) {
		ice_pair_set_state(pair, ICP_Failed);
		elem = ortp_list_find(cl->triggered_checks_queue, pair);
		if (elem != NULL) {
			cl->triggered_checks_queue = ortp_list_erase_link(cl->triggered_checks_queue, elem);
		}
	}
}

static void ice_check_list_stop_retransmissions(IceCheckList *cl) {
	ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_pair_stop_retransmissions, cl);
}

static void ice_check_list_nominate(IceCheckList *cl, const ortp_list_t *best_valid_list) {
	const ortp_list_t *it;
...
static void ice_check_list_perform_nominations(IceCheckList *cl, bool_t nomination_delay_expired) {
	const ortp_list_t *comp_it;
	ortp_list_t *best_valid_list = NULL;
	...
	for (comp_it = cl->local_componentIDs; comp_it != NULL; comp_it = comp_it->next) {
		IceValidCandidatePair *valid_pair;
		ortp_list_t *valid_it = NULL;
		ortp_list_t *valid_list = NULL;
		...
		if (valid_it) {
			...
			best_valid_list = ortp_list_append(best_valid_list, valid_pair);
	...
	ortp_list_free(best_valid_list);
}

static void ice_remove_waiting_and_frozen_pairs_from_list(ortp_list_t **list, uint16_t componentID) {
	IceCandidatePair *pair;
	ortp_list_t *elem;
	ortp_list_t *next;
...
static void ice_find_nominated_valid_pair_for_componentID(const uint16_t *componentID, CheckList_Bool *cb) {
	ortp_list_t *elem = ortp_list_find_custom(
	    cb->cl->valid_list, (ortp_compare_func)ice_find_nominated_valid_pair_from_componentID, componentID);
	if (elem == NULL) {
...
static void ice_find_selected_valid_pair_for_componentID(const uint16_t *componentID, CheckList_Bool *cb) {
	ortp_list_t *elem = ortp_list_find_custom(
	    cb->cl->valid_list, (ortp_compare_func)ice_find_selected_valid_pair_from_componentID, componentID);
	if (elem == NULL) {
		/* This component ID is not present in the valid list. */
		cb->result = FALSE;
	}
...
{
	ortp_list_t *elem = ortp_list_find_custom(cb->cl->check_list, (ortp_compare_func)ice_find_not_failed_or_succeeded_pair, NULL);
	if (elem != NULL) {
		cb->result = FALSE;
	}
}
...
static void ice_notify_session_processing_finished(IceCheckList *cl, ORTP_UNUSED(RtpSession *rtp_session)) {
...
static void ice_check_list_create_turn_channel(IceCheckList *cl,
	...
	ortp_sockaddr_to_address(local_addr, local_addrlen, local_ip, sizeof(local_ip), &local_port);
...
static void ice_conclude_processing(IceCheckList *cl, RtpSession *rtp_session, bool_t nomination_delay_expired) {
	...
	if (cl->state == ICL_Running) {
		...
		ortp_list_for_each2(cl->valid_list, (void (*)(void *, void *))ice_conclude_waiting_frozen_and_inprogress_pairs,
		                     cl);
...
void ice_session_reset(IceSession *session, IceRole role) {
	...
	for (i = 0; i < ICE_SESSION_MAX_CHECK_LISTS; i++) {
		...
		if (cl != NULL) {
			cl->local_candidates =
			    ortp_list_free_with_data(cl->local_candidates, (ortp_list_free_func)ice_free_candidate);
			ortp_list_free(cl->local_componentIDs);
...
static void ice_remove_gathering_stun_server_requests(IceCheckList *cl) {
	...
	while (elem != NULL) {
		elem = ortp_list_find_custom(cl->stun_server_requests,
		                              (ortp_compare_func)ice_find_gathering_stun_server_request, NULL);
...
static void ice_send_stun_server_requests(IceStunServerRequest *request, IceCheckList *cl) {
	...
	if ((request->responded == FALSE) && (ice_compare_time(curtime, request->next_transmission_time) >= 0)) {
		if (ortp_list_size(request->transactions) < ICE_MAX_STUN_REQUEST_RETRANSMISSIONS) {
			request->next_transmission_time = ice_add_ms(curtime, ICE_DEFAULT_RTO_DURATION);
			transaction =
			    ice_send_stun_server_request(request, (struct sockaddr *)&cl->session->ss, cl->session->ss_len);
			if (transaction != NULL) {
...
ice_check_list_retransmit_connectivity_checks(IceCheckList *cl, RtpSession *rtp_session, MSTimeSpec curtime) {
	...
	params.time = curtime;
	ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_handle_connectivity_check_retransmission,
	                     &params);
}
...
void ice_check_list_process(IceCheckList *cl, RtpSession *rtp_session) {
	...
	ortp_list_for_each2(cl->stun_server_requests, (void (*)(void *, void *))ice_send_stun_server_requests, cl);

	cl->stun_server_requests = ortp_list_remove_custom(
	    cl->stun_server_requests, (ortp_compare_func)ice_compare_stun_server_requests_to_remove, NULL);
	...
	switch (cl->state) {
		...
		case ICL_Running:
			...
			if (ice_check_list_is_frozen(cl)) {
				 ...
				 * if there is one. */
				state = ICP_Waiting;
				elem = ortp_list_find_custom(cl->check_list, (ortp_compare_func)ice_find_pair_from_state, &state);
				if (elem != NULL) {
					pair = (IceCandidatePair *)elem->data;
					ice_send_binding_request(cl, pair, rtp_session);
					return;
				...
				ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_check_retransmissions_pending,
...
static void ice_set_base_for_srflx_candidate_with_componentID(uint16_t *componentID, IceCheckList *cl) {
	IceCandidate *base;
	ComponentID_Family cf = {*componentID, AF_INET};
	ortp_list_t *elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_host_candidate, &cf);
	if (elem != NULL) {
		base = (IceCandidate *)elem->data;
		ortp_list_for_each2(cl->local_candidates, (void (*)(void *, void *))ice_set_base_for_srflx_candidate,
		                     (void *)base);
	}
	cf.componentID = *componentID;
	cf.family = AF_INET6;
	elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_host_candidate, &cf);
	if (elem != NULL) {
		base = (IceCandidate *)elem->data;
		ortp_list_for_each2(cl->local_candidates, (void (*)(void *, void *))ice_set_base_for_srflx_candidate,
		                     (void *)base);
...
/* same as ortp_list_remove() but without warning if the element is not found */
static ortp_list_t *remove_from_list(ortp_list_t *l, void *data) {
	ortp_list_t *elem = ortp_list_find(l, data);
	return elem ? ortp_list_erase_link(l, elem) : l;
}

static void ice_remove_transaction_with_pair(IceCheckList *cl, IceCandidatePair *pair) {
	ortp_list_t *elem;
	ortp_list_t *next_elem;
	...
	for (elem = cl->transaction_list; elem != NULL;) {
		...
		if (tr->pair == pair) {
			...
			cl->transaction_list = ortp_list_erase_link(cl->transaction_list, elem);
		}
		elem = next_elem;
	}
}
...
static void ice_clean_rtcp_candidate_pairs(IceCheckList *cl) {
	...
	for (elem = cl->pairs; elem != NULL;) {
		...
		if (pair->local->componentID == ICE_RTCP_COMPONENT_ID) {
			 ...
			 */
			ice_free_candidate_pair(pair, cl);
			cl->pairs = ortp_list_erase_link(cl->pairs, elem);
		}
		elem = next_elem;
	}
}
...
void ice_check_list_remove_rtcp_candidates(IceCheckList *cl) {
	...
	while ((elem = ortp_list_find_custom(cl->local_candidates, (ortp_compare_func)ice_find_candidate_with_componentID,
	                                      &rtcp_componentID)) != NULL) {
		IceCandidate *candidate = (IceCandidate *)elem->data;
		cl->local_candidates = ortp_list_remove(cl->local_candidates, candidate);
		ice_free_candidate(candidate);
	}
	ice_remove_componentID(&cl->remote_componentIDs, rtcp_componentID);
	while (
	    (elem = ortp_list_find_custom(cl->remote_candidates, (ortp_compare_func)ice_find_candidate_with_componentID,
	                                   &rtcp_componentID)) != NULL) {
		IceCandidate *candidate = (IceCandidate *)elem->data;
		cl->remote_candidates = ortp_list_remove(cl->remote_candidates, candidate);
		ice_free_candidate(candidate);
	}
}
...
static void ice_get_valid_pair_for_componentID(const uint16_t *componentID, CheckList_MSListPtr *cm) {
	ortp_list_t *elem = ortp_list_find_custom(
	    cm->cl->valid_list, (ortp_compare_func)ice_find_nominated_valid_pair_from_componentID, componentID);
	if (elem != NULL) {
		IceValidCandidatePair *valid_pair = (IceValidCandidatePair *)elem->data;
		*cm->list = ortp_list_append(*cm->list, valid_pair->valid);
	}
}

static ortp_list_t *ice_get_valid_pairs(const IceCheckList *cl) {
	CheckList_MSListPtr cm;
	ortp_list_t *valid_pairs = NULL;

	cm.cl = cl;
	cm.list = &valid_pairs;
	ortp_list_for_each2(cl->local_componentIDs, (void (*)(void *, void *))ice_get_valid_pair_for_componentID, &cm);
	return valid_pairs;
}
...
void ice_get_remote_transport_addresses_from_valid_pairs(const IceCheckList *cl,
                                                         ...
                                                         IceTransportAddress **rtcp_taddr) {
	TransportAddresses taddrs;
	ortp_list_t *ice_pairs = ice_get_valid_pairs(cl);
	taddrs.rtp_taddr = rtp_taddr;
	taddrs.rtcp_taddr = rtcp_taddr;
	ortp_list_for_each2(ice_pairs, (void (*)(void *, void *))ice_get_remote_transport_addresses_from_valid_pair,
	                     &taddrs);
	ortp_list_free(ice_pairs);
}
...
static void ice_get_local_transport_addresses_from_valid_pairs(const IceCheckList *cl,
	...
	taddrs.rtp_taddr = rtp_taddr;
	taddrs.rtcp_taddr = rtcp_taddr;
	ortp_list_for_each2(ice_pairs, (void (*)(void *, void *))ice_get_local_transport_address_from_valid_pair, &taddrs);
	ortp_list_free(ice_pairs);
}
...
void ice_dump_candidates(const IceCheckList *cl) {
	...
	ortp_list_for_each2(cl->local_candidates, (void (*)(void *, void *))ice_dump_candidate, "\t");
	ms_message("Remote candidates:");
	ortp_list_for_each2(cl->remote_candidates, (void (*)(void *, void *))ice_dump_candidate, "\t");
}
...
void ice_dump_candidate_pairs(const IceCheckList *cl) {
	...
	ortp_list_for_each2(cl->pairs, (void (*)(void *, void *))ice_dump_candidate_pair, (void *)&i);
}
...
void ice_dump_candidate_pairs_foundations(const IceCheckList *cl) {
	...
	ortp_list_for_each(cl->foundations, (void (*)(void *))ice_dump_candidate_pair_foundation);
}
...
void ice_dump_valid_list(const IceCheckList *cl) {
	...
	ortp_list_for_each2(cl->valid_list, (void (*)(void *, void *))ice_dump_valid_pair, &i);
}
...
void ice_dump_check_list(const IceCheckList *cl) {
	...
	ortp_list_for_each2(cl->check_list, (void (*)(void *, void *))ice_dump_candidate_pair, (void *)&i);
}
...
void ice_dump_triggered_checks_queue(const IceCheckList *cl) {
	...
	ortp_list_for_each2(cl->triggered_checks_queue, (void (*)(void *, void *))ice_dump_candidate_pair, (void *)&i);
}
...
void ice_dump_componentIDs(const IceCheckList *cl) {
	...
	ortp_list_for_each(cl->local_componentIDs, (void (*)(void *))ice_dump_componentID);
}
