...
int VideoConferenceAllToAll::findSourcePin(const std::string &participant) {
	...
	for (const ortp_list_t *elem = getMembers(); elem != nullptr; elem = elem->next){
...
void VideoConferenceAllToAll::addMember(VideoEndpoint *ep) {
	...
	if (ep->mIsRemote && dir == MediaStreamSendOnly && content != MSVideoContentSpeaker) {
		...
		ms_ticker_attach(mTicker,mMixer);
		connectEndpoint(ep);
		mEndpoints = ortp_list_append(mEndpoints, ep);
		return;
	}
...
void VideoConferenceAllToAll::removeMember(VideoEndpoint *ep) {
	...
	if (ortp_list_find(mMembers,ep) != NULL) {
		...
		mMembers=ortp_list_remove(mMembers,ep);
		...
		ortp_list_for_each2(mEndpoints, (void (*)(void*,void*))unconfigureEndpoint, (void*)&ep->mPin);
	} else if (ortp_list_find(mEndpoints,ep) != NULL) {
		ms_message("[VideoConferenceAllToAll] conference %p remove endpoint %s with output pin %d", this, ep->mName.c_str(), ep->mOutPin);
		mEndpoints=ortp_list_remove(mEndpoints,ep);
...
void VideoConferenceAllToAll::chooseNewFocus(){
	...
	size_t size = ortp_list_size(mMembers);
	int newFocusIndex = ortp_random() % (int)size;
	VideoEndpoint *ep = (VideoEndpoint*) ortp_list_nth_data(mMembers, newFocusIndex);
	if (ep){
		setFocus(ep);
	}else{
