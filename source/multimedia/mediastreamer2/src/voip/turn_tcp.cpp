...
static int tls_callback_certificate_verify(void *data, ortp_x509_certificate_t *cert, int depth, uint32_t *flags) {
	const int tmp_size = 2048, flags_str_size = 256;
	char *tmp = (char *)malloc(tmp_size);
	char *flags_str = (char *)malloc(flags_str_size);

	ortp_x509_certificate_get_info_string(tmp, tmp_size - 1, "", cert);
	ortp_x509_certificate_flags_to_string(flags_str, flags_str_size - 1, *flags);
...
static int random_generator(void *ctx, unsigned char *ptr, size_t size) {
	ortp_rng_context_t *rng = (ortp_rng_context_t *)ctx;
	ortp_rng_get(rng, ptr, size);
...
static int tls_callback_read(void *ctx, unsigned char *buf, size_t len) {
	...
	if (ret < 0) {
		...
		if (ret == TURN_EWOULDBLOCK || ret == TURN_EINPROGRESS || ret == TURN_EINTR)
			return ORTP_ERROR_NET_WANT_READ;
		return ORTP_ERROR_NET_CONN_RESET;
	}
	return ret;
}
...
static int tls_callback_write(void *ctx, const unsigned char *buf, size_t len) {
	...
	if (ret < 0) {
		ret = -ret;
		if (ret == TURN_EWOULDBLOCK || ret == TURN_EINPROGRESS || ret == TURN_EINTR)
			return ORTP_ERROR_NET_WANT_WRITE;
		return ORTP_ERROR_NET_CONN_RESET;
	}
	return ret;
}

SslContext::SslContext(ortp_socket_t socket, std::string rootCertificatePath, std::string cn, ortp_rng_context_t *rng)
	: mSocket(socket) {
	mContext = ortp_ssl_context_new();
	mConfig = ortp_ssl_config_new();

	ortp_ssl_config_defaults(mConfig, ORTP_SSL_IS_CLIENT, ORTP_SSL_TRANSPORT_STREAM);

	if (!rootCertificatePath.empty()) {
		struct stat statbuf;
		if (stat(rootCertificatePath.c_str(), &statbuf) == 0) {
			mRootCertificate = ortp_x509_certificate_new();

			if (statbuf.st_mode & S_IFDIR) {
				if (ortp_x509_certificate_parse_path(mRootCertificate, rootCertificatePath.c_str()) < 0) {
					ms_error("SslContext [%p]: Failed to load ca from directory: %s", this,
							 rootCertificatePath.c_str());
					ortp_x509_certificate_free(mRootCertificate);
					mRootCertificate = NULL;
				}
			} else {
				if (ortp_x509_certificate_parse_file(mRootCertificate, rootCertificatePath.c_str()) < 0) {
		...
		ortp_ssl_config_set_ca_chain(mConfig, mRootCertificate);
		ortp_ssl_config_set_authmode(mConfig, ORTP_SSL_VERIFY_REQUIRED);
		ortp_ssl_config_set_callback_verify(mConfig, tls_callback_certificate_verify, this);
	} else {
		ortp_ssl_config_set_authmode(mConfig, ORTP_SSL_VERIFY_NONE);
		mRootCertificate = NULL;
	}

	ortp_ssl_config_set_rng(mConfig, random_generator, rng);
	ortp_ssl_set_io_callbacks(mContext, &mSocket, tls_callback_write, tls_callback_read);
	ortp_ssl_context_setup(mContext, mConfig);

	if (!cn.empty()) {
		ortp_ssl_set_hostname(mContext, cn.c_str());
	}
}
...
SslContext::~SslContext() {
	...
	ortp_ssl_context_free(mContext);
	ortp_ssl_config_free(mConfig);
	ortp_x509_certificate_free(mRootCertificate);
}

int SslContext::connect() {
	int error = ortp_ssl_handshake(mContext);
	if (error < 0) {
		char errbuf[1024] = {0};
		ortp_strerror(error, errbuf, sizeof(errbuf) - 1);
...
int SslContext::close() {
	return ortp_ssl_close_notify(mContext);
}

int SslContext::read(unsigned char *buffer, size_t length) {
	return ortp_ssl_read(mContext, buffer, length);
}

int SslContext::write(const unsigned char *buffer, size_t length) {
	return ortp_ssl_write(mContext, buffer, length);
}
...
int TurnSocket::connect() {
	struct addrinfo *ai =
		ortp_name_to_addrinfo(AF_UNSPEC, SOCK_STREAM, mClient->mTurnServerIp.c_str(), mClient->mTurnServerPort);
	if (!ai) {
	...
	if (error != 0) {
		...
		ms_error("TurnSocket [%p]: failed to connect to server (%d): %s", this, optVal, strerror(optVal));
		close();
		return -1;
	}
	...
	if (turnPoll(mSocket, 5, false) == 1) {
		...
		if (bytes < 0) {
			if (getSocketErrorCode() != TURN_EWOULDBLOCK) {
				if (mSsl) {
					if (bytes == ORTP_ERROR_SSL_PEER_CLOSE_NOTIFY) {
...
TurnClient::TurnClient(MSTurnContext *context, bool useSsl, std::string rootCertificate)
	...
	char ip[64] = {0};
	ortp_sockaddr_to_address((struct ipstack_sockaddr *)&context->turn_server_addr, context->turn_server_addrlen, ip,
								 sizeof(ip), &mTurnServerPort);
	mTurnServerIp = std::string(ip);

	mTurnConnection = nullptr;

	mRng = ortp_rng_context_new();
}

TurnClient::~TurnClient() {
	if (mRng)
		ortp_rng_context_free(mRng);
}
...
int TurnClient::recvfrom(mblk_t *msg, ORTP_UNUSED(int flags), struct sockaddr *from, socklen_t *fromlen) {
...
int TurnClient::sendto(mblk_t *msg, ORTP_UNUSED(int flags), const struct sockaddr *, socklen_t) {
