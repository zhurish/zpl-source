...
extern "C" const ortp_list_t* ms_video_conference_get_members(const MSVideoConference *obj) {
...
int VideoConferenceGeneric::getSize() const {
	return (int)ortp_list_size(mMembers);
}

const ortp_list_t* VideoConferenceGeneric::getMembers() const {
	return mMembers;
}
...
void on_filter_event(void *data, ORTP_UNUSED(MSFilter *f), unsigned int event_id, void *event_data) {
...
void ms_video_conference_process_encoder_control(ORTP_UNUSED(VideoStream *vs), unsigned int method_id, void *, void *user_data) {
