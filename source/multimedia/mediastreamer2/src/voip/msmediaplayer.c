...
bool_t ms_media_player_open(MSMediaPlayer *obj, const char *filepath) {
	...
	case MS_FILE_FORMAT_WAVE:
		fp = ortp_file_open2(ortp_vfs_get_default(), filepath, O_RDONLY);
		if(fp == NULL) {
...
static bool_t _get_format(const char *filepath, MSFileFormat *format) {
	FourCC four_cc;
	ortp_vfs_file_t *fp = ortp_file_open(ortp_vfs_get_default(), filepath, "rb");
	if(fp == NULL) {
		ms_error("Could not open %s: %s", filepath, strerror(errno));
		goto err;
	}
	if (ortp_file_read2(fp, four_cc, sizeof(FourCC)) == ORTP_VFS_ERROR){
...
static void _eof_filter_notify_cb(void *userdata, struct _MSFilter *f, unsigned int id, ORTP_UNUSED(void *arg)) {
