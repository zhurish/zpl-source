...
audio_io_handle_t AudioSystem::getInput(ORTP_UNUSED(audio_source_t inputSource),
                                    ORTP_UNUSED(uint32_t samplingRate),
                                    ORTP_UNUSED(audio_format_t format),
                                    ORTP_UNUSED(uint32_t channels),
                                    ORTP_UNUSED(audio_in_acoustics_t acoustics),
                                    ORTP_UNUSED(int sessionId)){
...
void RefBase::incStrong(ORTP_UNUSED(const void* id)) const{
	mCnt++;
	if (isRefCounted()) {
...
void RefBase::decStrong(ORTP_UNUSED(const void* id)) const{
	if (isRefCounted()) {
