...
static void android_native_snd_card_init(ORTP_UNUSED(MSSndCard *card)) {
...
static void android_snd_read_cb(int event, void* user, void *p_info){
	...
	if (ad->mTickerSynchronizer==NULL){
		 ...
		 */
		int loglevel=ortp_get_log_level_mask(BCTBX_LOG_DOMAIN);
		ortp_set_log_level_mask(NULL, BCTBX_LOG_ERROR|BCTBX_LOG_FATAL);
		ad->mTickerSynchronizer = ms_ticker_synchronizer_new();
		ms_ticker_set_synchronizer(obj->ticker, ad->mTickerSynchronizer);
		ortp_set_log_level_mask(BCTBX_LOG_DOMAIN, loglevel);
	}
