...
static int video_capture_set_autofocus(MSFilter *f, ORTP_UNUSED(void* data)){
...
static int video_capture_get_pix_fmt(ORTP_UNUSED(MSFilter *f), void *data){
...
static void video_capture_cam_init(ORTP_UNUSED(MSWebCam *cam)){
...
void android_video_capture_detect_cameras_legacy(MSWebCamManager *obj) {
	...
	for (int i = 0; i < count; i++) {
		...
		char *idstring = ortp_strdup_printf("%sFacingCamera%d", facing, c->id);
...
JNIEXPORT void JNICALL Java_org_linphone_mediastream_video_capture_AndroidVideoApi5JniWrapper_putImage(JNIEnv*  env,
		ORTP_UNUSED(jclass  thiz),jlong nativePtr,jbyteArray frame) {
