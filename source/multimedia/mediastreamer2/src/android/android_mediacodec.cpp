...
media_status_t AMediaCodec_configure(AMediaCodec *codec, const AMediaFormat* format, ORTP_UNUSED(ANativeWindow* surface), ORTP_UNUSED(AMediaCrypto *crypto), uint32_t flags) {
	JNIEnv *env = ms_get_jni_env();
...
media_status_t AMediaCodec_releaseOutputBuffer(AMediaCodec *codec, size_t idx, ORTP_UNUSED(bool render)) {
	JNIEnv *env = ms_get_jni_env();
