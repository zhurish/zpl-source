...
int ms_read_wav_header_from_fp(wave_header_t *header,ortp_vfs_file_t *fp) {
	int count;
	int skip;
	int hsize=0;
	...
	ssize_t len = ortp_file_read2(fp, (char*)riff_chunk, sizeof(riff_t));
	...
	len = ortp_file_read2(fp, (char*)format_chunk, sizeof(format_t));
	...
	{
		ortp_file_seek(fp, skip, SEEK_CUR);
	}
	hsize=sizeof(wave_header_t)-0x10+le_uint32(format_chunk->len);
	
	count=0;
	do{
		len = ortp_file_read2(fp, data_chunk, sizeof(data_t));
		...
		if (strncmp(data_chunk->data, "data", 4)!=0){
			...
			ortp_file_seek(fp, le_uint32(data_chunk->len), SEEK_CUR);
			count++;
...
static int read_wav_header(PlayerData *d){
	...
	not_a_wav:
		/*rewind*/
		ortp_file_seek(d->fp, 0, SEEK_SET);
		d->hsize=0;
		d->is_raw=TRUE;
		return -1;
}
...
static int player_open(MSFilter *f, void *arg){
	...
	if ((fp=ortp_file_open2(ortp_vfs_get_default(),file,O_RDONLY|O_BINARY))==NULL){
	...
	if ((fsize=ortp_file_size(fp))!=ORTP_VFS_ERROR){
...
static int player_start(MSFilter *f, ORTP_UNUSED(void *arg)){
	PlayerData *d=(PlayerData*)f->data;
...
static int player_stop(MSFilter *f, ORTP_UNUSED(void *arg)){
	PlayerData *d=(PlayerData*)f->data;
	ms_filter_lock(f);
...
static int player_pause(MSFilter *f, ORTP_UNUSED(void *arg)){
	PlayerData *d=(PlayerData*)f->data;
	ms_filter_lock(f);
...
static int player_close(MSFilter *f, ORTP_UNUSED(void *arg)){
	PlayerData *d=(PlayerData*)f->data;
	player_stop(f,NULL);
#ifdef HAVE_PCAP
