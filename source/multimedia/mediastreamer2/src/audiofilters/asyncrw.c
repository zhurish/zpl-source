...
MSAsyncReader *ms_async_reader_new(ortp_vfs_file_t *fp){
...
static void async_reader_fill(void *data){
	...
	int err = (int)ortp_file_read2(obj->fp, m->b_wptr, obj->blocksize);
...
static void async_reader_seek(void *data){
	MSAsyncReader *obj = (MSAsyncReader*) data;
	ms_mutex_lock(&obj->mutex);
	if (ortp_file_seek(obj->fp, obj->seekoff, SEEK_SET) == ORTP_VFS_ERROR){
...
MSAsyncWriter *ms_async_writer_new(ortp_vfs_file_t *fp){
...
static void async_writer_write(void *data){
	...
	ms_mutex_unlock(&obj->mutex);
	if (ok){
		if (ortp_file_write2(obj->fp, obj->wbuf, size) == ORTP_VFS_ERROR){
