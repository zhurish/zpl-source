...
static void context_state_notify_cb(pa_context *ctx, ORTP_UNUSED(void *userdata)){
	const char *sname="";
...
static void pulse_card_unload(ORTP_UNUSED(MSSndCardManager *m)) {
...
void pa_sinklist_cb(ORTP_UNUSED(pa_context *c), const pa_sink_info *l, int eol, void *userdata) {
	ortp_list_t **pa_devicelist = userdata;
	...
	*pa_devicelist = ortp_list_append(*pa_devicelist, pa_device);
end:
	pa_threaded_mainloop_signal(the_pa_loop, FALSE);
}

void pa_sourcelist_cb(ORTP_UNUSED(pa_context *c), const pa_source_info *l, int eol, void *userdata) {
	ortp_list_t **pa_devicelist = userdata;
	pa_device_t *pa_device;
...
static void pulse_card_merge_lists(pa_device_t *pa_device, ortp_list_t **pa_source_list) {
	ortp_list_t *sourceCard = ortp_list_find_custom(*pa_source_list, (ortp_compare_func)pulse_card_compare, pa_device);
	if (sourceCard!= NULL) {
		...
		*pa_source_list = ortp_list_remove(*pa_source_list, sourceCard->data);
...
static void pulse_card_detect(MSSndCardManager *m){
	pa_operation *pa_op;
	ortp_list_t *pa_sink_list=NULL;
	ortp_list_t *pa_source_list=NULL;
	...
	ortp_list_for_each2(pa_sink_list, (MSIterate2Func)pulse_card_merge_lists, &pa_source_list);

	/* create sink and souce cards */
	ortp_list_for_each2(pa_sink_list, (MSIterate2Func)pulse_card_sink_create, m);
	ortp_list_for_each2(pa_source_list, (MSIterate2Func)pulse_card_source_create, m);

	ortp_list_free_with_data(pa_sink_list, ms_free);
	ortp_list_free_with_data(pa_source_list, ms_free);
}
...
static void stream_write_request_cb(ORTP_UNUSED(pa_stream *p), size_t nbytes, void *user_data) {
	Stream *s = (Stream *)user_data;
	stream_play(s, nbytes);
}

static void stream_buffer_overflow_notification(ORTP_UNUSED(pa_stream *p), void *user_data) {
	Stream *st = (Stream*)user_data;
	st->overflow_notifs++;
}

static void stream_buffer_underflow_notification(ORTP_UNUSED(pa_stream *p), void *user_data) {
	Stream *st = (Stream*)user_data;
	st->underflow_notifs++;
}
...
static void stream_set_volume_cb(ORTP_UNUSED(pa_context *c), int success, void *user_data) {
...
static void stream_get_source_volume_cb(ORTP_UNUSED(pa_context *c), const pa_source_output_info *i, ORTP_UNUSED(int eol), void *user_data) {
	if(i) {
...
static void stream_get_sink_volume_cb(ORTP_UNUSED(pa_context *c), const pa_sink_input_info *i, ORTP_UNUSED(int eol), void *user_data) {
	if(i) {
		*(double *)user_data = volume_to_scale(pa_cvolume_avg(&i->volume));
	}
	pa_threaded_mainloop_signal(the_pa_loop, FALSE);
