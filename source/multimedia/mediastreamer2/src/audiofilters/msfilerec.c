...
static void write_wav_header(ortp_vfs_file_t *fp, int rate, int nchannels, int size);
...
static int rec_get_length(const char *file, int *length){
	...
	ortp_vfs_file_t *fp = ortp_file_open2(ortp_vfs_get_default(), file, O_RDONLY|O_BINARY);
...
static int rec_open(MSFilter *f, void *arg){
	...
	s->fp = ortp_file_open2(ortp_vfs_get_default(), filename, flags);
	if (s->fp == NULL) {
	...
	if (s->size>0){
		if ((fsize=ortp_file_size(s->fp))!=ORTP_VFS_ERROR){
			if (ortp_file_seek(s->fp,(off_t)fsize,SEEK_SET) == ORTP_VFS_ERROR){
				int err = errno;
...
static int rec_start(MSFilter *f, ORTP_UNUSED(void *arg)){
	RecState *s=(RecState*)f->data;
...
static int rec_stop(MSFilter *f, ORTP_UNUSED(void *arg)){
	RecState *s=(RecState*)f->data;
	ms_mutex_lock(&f->lock);
	s->state=MSRecorderPaused;
	ms_mutex_unlock(&f->lock);
...
static void write_wav_header(ortp_vfs_file_t *fp, int rate, int nchannels, int size){
	...
	if (ortp_file_write2(fp, &header, sizeof(header))!=sizeof(header)){
...
static void _rec_close(RecState *s){
	...
	if (s->fp){
		...
		ortp_file_close(s->fp);
		s->fp=NULL;
	}
}

static int rec_close(MSFilter *f, ORTP_UNUSED(void *arg)){
	RecState *s=(RecState*)f->data;
	ms_mutex_lock(&f->lock);
	_rec_close(s);
	ms_mutex_unlock(&f->lock);
