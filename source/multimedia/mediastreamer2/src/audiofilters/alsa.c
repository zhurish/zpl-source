...
static int alsa_set_params(snd_pcm_t *pcm_handle, int rw, ORTP_UNUSED(int bits), int stereo, int rate)
{
...
void alsa_error_log_handler(const char *file, int line, ORTP_UNUSED(const char *function), ORTP_UNUSED(int err), const char *fmt, ...) {
	char * format = ms_strdup_printf("alsa error in %s:%d - %s", file, line, fmt);
	va_list args;
	va_start (args, fmt);
	//ortp_logv(BCTBX_LOG_DOMAIN, BCTBX_LOG_MESSAGE, format, args);
	va_end (args);
	ms_free(format);
}
