...
void ms_h264_bitstream_to_nalus(const uint8_t *bitstream, size_t size, os_queue_t *nalus) {
	H26xUtils::byteStreamToNalus(bitstream, size, nalus);
}

uint8_t ms_h264_nalu_get_nri(const os_qdata_t *nalu) {
	return ((*nalu->b_rptr) >> 5) & 0x3;
}

MSH264NaluType ms_h264_nalu_get_type(const os_qdata_t *nalu) {
	return (*nalu->b_rptr) & ((1 << 5) - 1);
}

unsigned int _ms_h264_get_id(const os_qdata_t *parameter_set, size_t offset, const char *symbol_name) {
	unsigned int id;
	MSBitsReader reader;
...
unsigned int ms_h264_sps_get_id(const os_qdata_t *sps) {
	return _ms_h264_get_id(sps, 4, "seq_parameter_set_id");
}

unsigned int ms_h264_pps_get_id(const os_qdata_t *pps) {
	return _ms_h264_get_id(pps, 1, "pic_parameter_set_id");
}

void ms_h264_stream_to_nalus(const uint8_t *frame, size_t size, os_queue_t *nalus, int *idr_count) {
...
MSVideoSize ms_h264_sps_get_video_size(const os_qdata_t *sps) {
...
os_qdata_t *H264NaluHeader::forge() const {
	uint8_t h = _fBit ? 1 : 0;
	h <<= 2;
	h |= _nri;
	h <<= 5;
...
H264FrameAnalyser::Info H264FrameAnalyser::analyse(const os_queue_t *frame) {
	Info info;
	for (const os_qdata_t *nalu = qbegin(&frame->q); !qend(&frame->q, nalu); nalu = qnext(&frame->q, nalu)) {
...
bool H264FrameAnalyser::updateParameterSet(const os_qdata_t *new_parameter_set) {
	os_qdata_t *&last_parameter_set = ms_h264_nalu_get_type(new_parameter_set) == MSH264NaluTypePPS ? _lastPps : _lastSps;
...
os_qdata_t *H264Tools::prependFuIndicatorAndHeader(os_qdata_t *m, uint8_t indicator, bool_t start, bool_t end, uint8_t type) {
	os_qdata_t *h = allocb(2, 0);
...
void H264ParameterSetsInserter::process(os_queue_t *in, os_queue_t *out) {
	H264NaluHeader header;
	bool isKeyFrame = false;
	while (os_qdata_t *m = ms_queue_get(in)) {
	...
	if (isKeyFrame) {
		os_qdata_t *insPoint = ms_queue_peek_first(out);
