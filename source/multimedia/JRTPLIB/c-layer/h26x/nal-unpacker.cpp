...
NalUnpacker::Status NalUnpacker::unpack(os_qdata_t *im, os_queue_t *out) {
	...
	switch (type) {
		...
		case PacketType::FragmentationUnit: {
			try {
				...
				os_qdata_t *o = _fuAggregator->feed(im);
				if (o) storeNal(o);
...
NalUnpacker::Status NalUnpacker::outputFrame(os_queue_t *out, const Status &flags) {
	Status res = _status;
...
void NalUnpacker::storeNal(os_qdata_t *nal) {
	ms_queue_put(&_q, nal);
}
