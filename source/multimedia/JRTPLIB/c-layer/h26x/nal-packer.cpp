...
void NalPacker::pack(os_queue_t *naluq, os_queue_t *rtpq, uint32_t ts) {
	switch (_packMode) {
...
// Private methods
void NalPacker::packInSingleNalUnitMode(os_queue_t *naluq, os_queue_t *rtpq, uint32_t ts) {
	while (os_qdata_t *m = ms_queue_get(naluq)) {
...
void NalPacker::packInNonInterleavedMode(os_queue_t *naluq, os_queue_t *rtpq, uint32_t ts) {
	while (os_qdata_t *m = ms_queue_get(naluq)) {
		bool end = !!ms_queue_empty(naluq);
		size_t sz = msgdsize(m);
		if (_aggregationEnabled) {
			if (_naluAggregator->isAggregating()) {
				os_qdata_t *stapPacket = _naluAggregator->feed(m);
				if (stapPacket) {
...
void NalPacker::fragNaluAndSend(os_queue_t *rtpq, uint32_t ts, os_qdata_t *nalu, bool_t marker) {
	_naluSpliter->feed(nalu);
	os_queue_t *nalus = _naluSpliter->getPackets();
	while (os_qdata_t *m = ms_queue_get(nalus)) {
		sendPacket(rtpq, ts, m, ms_queue_empty(nalus) ? marker : false);
	}
}

void NalPacker::sendPacket(os_queue_t *rtpq, uint32_t ts, os_qdata_t *m, bool_t marker) {
