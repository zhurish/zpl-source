...
// =======================
os_qdata_t *H264FuaAggregator::feed(os_qdata_t *im) {
	os_qdata_t *om = nullptr;
	uint8_t fu_header;
	uint8_t nri, type;
	bool_t start, end;
...
os_qdata_t *H264FuaAggregator::completeAggregation() {
	os_qdata_t *res = _m;
	_m = nullptr;
	return res;
}
...
void H264StapaSpliter::feed(os_qdata_t *im) {
	uint16_t sz;
	for (uint8_t *p = im->b_rptr + 1; p < im->b_wptr;) {
		memcpy(&sz, p, 2);
		sz = ntohs(sz);
		os_qdata_t *nal = dupb(im);
		p += 2;
		nal->b_rptr = p;
		p += sz;
		nal->b_wptr = p;
...
void H264NalUnpacker::setOutOfBandSpsPps(os_qdata_t *sps, os_qdata_t *pps) {
...
// Private methods
// ---------------
NalUnpacker::PacketType H264NalUnpacker::getNaluType(const os_qdata_t *nalu) const {
...
H264NalUnpacker::Status H264NalUnpacker::outputFrame(os_queue_t *out, const Status &flags) {
