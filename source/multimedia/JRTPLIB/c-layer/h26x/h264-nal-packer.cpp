...
os_qdata_t *H264NalPacker::NaluAggregator::feed(os_qdata_t *nalu) {
	size_t size = OS_QUEUE_DATA_LEN(nalu);
	if (_stap == nullptr) {
		_stap = nalu;
...
void H264NalPacker::NaluAggregator::reset() {
	if (_stap) os_qdata_destroy(_stap);
	_size = 0;
}

os_qdata_t *H264NalPacker::NaluAggregator::completeAggregation() {
	os_qdata_t *res = _stap;
	_stap = nullptr;
	reset();
	return res;
}
...
os_qdata_t *H264NalPacker::NaluAggregator::concatNalus(os_qdata_t *m1, os_qdata_t *m2) {
	os_qdata_t *l = allocb(2, 0);
...
os_qdata_t *H264NalPacker::NaluAggregator::prependStapA(os_qdata_t *m) {
	os_qdata_t *hm = allocb(3, 0);
...
void H264NalPacker::NaluAggregator::putNalSize(os_qdata_t *m, size_t sz) {
...
void H264NalPacker::NaluSpliter::feed(os_qdata_t *nalu) {
	os_qdata_t *m;
