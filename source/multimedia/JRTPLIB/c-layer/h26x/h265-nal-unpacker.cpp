...
os_qdata_t *H265NalUnpacker::FuAggregator::feed(os_qdata_t *packet) {
...
os_qdata_t *H265NalUnpacker::FuAggregator::completeAggregation() {
...
void H265NalUnpacker::ApSpliter::feed(os_qdata_t *packet) {
	ms_queue_flush(&_q);
	...
	for (it = packet->b_rptr+2; it < packet->b_wptr;) {
		...
		os_qdata_t *m = allocb(naluSize, 0);
...
NalUnpacker::PacketType H265NalUnpacker::getNaluType(const os_qdata_t *nalu) const {
