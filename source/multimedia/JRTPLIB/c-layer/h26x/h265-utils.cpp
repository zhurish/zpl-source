...
os_qdata_t *H265NaluHeader::forge() const {
	...
	os_qdata_t *newHeader = allocb(2, 0);
...
os_qdata_t *H265FuHeader::forge() const {
	...
	os_qdata_t *newHeader = allocb(1, 0);
	*newHeader->b_wptr++ = header;
	return newHeader;
}

void H265ParameterSetsInserter::process(os_queue_t *in, os_queue_t *out) {
