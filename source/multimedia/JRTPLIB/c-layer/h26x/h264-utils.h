 ...
 *
 * Nal units are stored in freshly allocated os_qdata_t buffers which are pushed into
 ...
 * @param sps A os_qdata_t holding the SPS.
...
public:
	...
	os_qdata_t *forge() const override;
...
public:
	...
	~H264FrameAnalyser();
	Info analyse(const os_queue_t *frame);

private:
	bool updateParameterSet(const os_qdata_t *new_parameter_set);

	os_qdata_t *_lastSps = nullptr;
	os_qdata_t *_lastPps = nullptr;
};

class H264Tools {
public:
	static void nalHeaderInit(uint8_t *h, uint8_t nri, uint8_t type) {*h=((nri&0x3)<<5) | (type & ((1<<5)-1));}
	static os_qdata_t *prependFuIndicatorAndHeader(os_qdata_t *m, uint8_t indicator, bool_t start, bool_t end, uint8_t type);
};

class H264ParameterSetsInserter: public H26xParameterSetsInserter {
public:
	~H264ParameterSetsInserter();
	void process(os_queue_t *in, os_queue_t *out) override;
	void flush() override;
