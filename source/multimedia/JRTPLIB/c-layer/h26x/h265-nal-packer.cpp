...
os_qdata_t *H265NalPacker::NaluAggregator::feed(os_qdata_t *nalu) {
	H265NaluHeader header(nalu->b_rptr);
	if (_ap == nullptr) {
		placeFirstNalu(nalu);
	} else {
		if (H265NaluHeader::length + _size + 2 + msgdsize(nalu) > _maxSize) {
			os_qdata_t *m = completeAggregation();
			placeFirstNalu(nalu);
			return m;
		} else {
			aggregate(nalu);
...
os_qdata_t *H265NalPacker::NaluAggregator::completeAggregation() {
	os_qdata_t *m;
...
void H265NalPacker::NaluAggregator::placeFirstNalu(os_qdata_t *nalu) {
	H265NaluHeader header(nalu->b_rptr);
	_ap = nalu;
	_apHeader = header;
	_size = msgdsize(nalu);
...
void H265NalPacker::NaluAggregator::aggregate(os_qdata_t *nalu) {
	H265NaluHeader header(nalu->b_rptr);
...
void H265NalPacker::NaluSpliter::feed(os_qdata_t *nalu) {
...
os_qdata_t *H265NalPacker::NaluSpliter::makeFu(const H265NaluHeader &naluHeader, const H265FuHeader &fuHeader, const uint8_t *payload, size_t length) {
	os_qdata_t *naluHeaderBuf = naluHeader.forge();
	os_qdata_t *fuHeaderBuf = fuHeader.forge();
	os_qdata_t *payloadBuf = allocb(length, 0);
	memcpy(payloadBuf->b_wptr, payload, length);
	payloadBuf->b_wptr += length;

	os_qdata_t *fu = naluHeaderBuf;
