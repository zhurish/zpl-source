...
void rfc3984_pack(Rfc3984Context *ctx, os_queue_t *naluq, os_queue_t *rtpq, uint32_t ts) {
	ctx->packer.pack(naluq, rtpq, ts);
}

void rfc3984_unpack_out_of_band_sps_pps(Rfc3984Context *ctx, os_qdata_t *sps, os_qdata_t *pps) {
	ctx->unpacker.setOutOfBandSpsPps(sps, pps);
}

unsigned int rfc3984_unpack2(Rfc3984Context *ctx, os_qdata_t *im, os_queue_t *naluq) {
	os_queue_t q;
	...
	if (status & Rfc3984FrameAvailable) {
		...
		while(os_qdata_t *m = os_queue_get(&q)) {
			os_queue_add(naluq, m);
		}
	}
	return status;
}
