...
typedef struct msgb
{
    ...
    OrtpAddress net_addr; /*source address of incoming packet, or dest address of outgoing packet, used only by simulator and modifiers*/
	uint8_t ttl_or_hl;
} mblk_t;
...
ORTP_PUBLIC void qinit(rtp_queue_t *q);

ORTP_PUBLIC void putq(rtp_queue_t *q, mblk_t *m);

ORTP_PUBLIC mblk_t * getq(rtp_queue_t *q);

ORTP_PUBLIC void insq(rtp_queue_t *q,mblk_t *emp, mblk_t *mp);

ORTP_PUBLIC void remq(rtp_queue_t *q, mblk_t *mp);

ORTP_PUBLIC mblk_t * peekq(rtp_queue_t *q);

/* remove and free all messages in the q */
#define FLUSHALL 0
ORTP_PUBLIC void flushq(rtp_queue_t *q, int how);
