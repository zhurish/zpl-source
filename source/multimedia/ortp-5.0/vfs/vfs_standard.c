 ...
 * by pFile, sets the methods ortp_io_methods_t to the bcio structure
 ...
 * @return         		ORTP_VFS_ERROR if an error occurs, ORTP_VFS_OK otherwise.
 */
static  int bcOpen(ortp_vfs_t *pVfs, ortp_vfs_file_t *pFile, const char *fName, int openFlags);


/* User data for the standard vfs */
typedef struct ortp_vfs_standard_t ortp_vfs_standard_t;
struct ortp_vfs_standard_t {
	int fd;                         /* File descriptor */
};

ortp_vfs_t bcStandardVfs = {
	"ortp_vfs",		/* vfsName */
	bcOpen,			/*xOpen */
};
 ...
 * @return       	ORTP_VFS_OK if successful, ORTP_VFS_ERROR otherwise.
 */
static int bcClose(ortp_vfs_file_t *pFile) {
	int ret;
	if (pFile==NULL || pFile->pUserData==NULL) return ORTP_VFS_ERROR;
	ortp_vfs_standard_t *ctx = (ortp_vfs_standard_t *)pFile->pUserData;
	ret = close(ctx->fd);
	if (!ret) {
		ret = ORTP_VFS_OK;
	} else {
 ...
 */
static int bcSync(ortp_vfs_file_t *pFile) {
	if (pFile==NULL || pFile->pUserData==NULL) return ORTP_VFS_ERROR;
	ortp_vfs_standard_t *ctx = (ortp_vfs_standard_t *)pFile->pUserData;
	#if _WIN32
	int ret;
	ret = FlushFileBuffers((HANDLE)_get_osfhandle(ctx->fd));
	return (ret!=0 ? ORTP_VFS_OK : ORTP_VFS_ERROR);
	#else
	int rc = fsync(ctx->fd);
	return (rc==0 ? ORTP_VFS_OK : ORTP_VFS_ERROR);
	#endif
}
 ...
 *                if the error was something else ORTP_VFS_ERROR otherwise
 */
static ssize_t bcRead(ortp_vfs_file_t *pFile, void *buf, size_t count, off_t offset) {
 ...
 */
static ssize_t bcWrite(ortp_vfs_file_t *pFile, const void *buf, size_t count, off_t offset) {
 ...
 */
static int64_t bcFileSize(ortp_vfs_file_t *pFile) {
 ...
 * @return -errno if an error occurred, 0 otherwise.
  */
static int bcTruncate(ortp_vfs_file_t *pFile, int64_t new_size){
...
static const  ortp_io_methods_t bcio = {
	bcClose,		/* pFuncClose */
	bcRead,			/* pFuncRead */
	bcWrite,		/* pFuncWrite */
...
static int bcOpen(ORTP_UNUSED(ortp_vfs_t * pVfs), ortp_vfs_file_t *pFile, const char *fName, int openFlags) {
	...
	ortp_vfs_standard_t *userData = (ortp_vfs_standard_t *)ortp_malloc(sizeof(ortp_vfs_standard_t));
