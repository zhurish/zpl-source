...
vector<string> ortp::Utils::split (const string &str, const string &delimiter) {
	vector<string> out;
...
string ortp::Utils::fold (const string &str) {
	string output = str;
	size_t crlf = 0;
	size_t next_crlf = 0;
	const char *endline = "\r\n";
...
string ortp::Utils::unfold (const string &str) {
	string output = str;
	const char *endline = "\r\n";
...
void ortp::Utils::replace(string& source, const string& from, const string& to, const bool& recursive){
	size_t start_pos = 0;
...
std::string ortp::Utils::getMemoryReportAsString() {
