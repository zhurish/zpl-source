...
extern "C" bool_t ortp_is_matching_regex_log(const char *entry, const char* regex, bool_t show_log){
	...
	try {
		...
		if(show_log)
			ortp_error("Could not compile regex '%s': %s", regex, e.what());
		return FALSE;
	}
#else
	...
	if(res != 0) {
		if(show_log) {
			...
			ortp_error("Could not compile regex '%s': %s", regex, err_msg);
		}
		return FALSE;
	}
...
extern "C" bool_t ortp_is_matching_regex(const char *entry, const char* regex) {
	return ortp_is_matching_regex_log(entry, regex, TRUE);
}
