...
struct RNG::Impl {
	...
	Impl() {
		...
		if (mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, NULL, 0) != 0) {
			throw ORTP_EXCEPTION << "RNG failure at creation: entropy source failure";
		}
	}
	~Impl() {
...
void RNG::randomize(uint8_t *buffer, size_t size) {
	...
	if ( ret != 0) {
		throw ORTP_EXCEPTION << ((ret == MBEDTLS_ERR_CTR_DRBG_REQUEST_TOO_BIG)?"RNG failure: Request too big":"RNG failure: entropy source failure");
	}
}
...
std::vector<uint8_t> HMAC(ORTP_UNUSED(const std::vector<uint8_t> &key), ORTP_UNUSED(const std::vector<uint8_t> &input)) {
...
template <typename hashAlgo>
std::vector<uint8_t> HKDF(ORTP_UNUSED(const std::vector<uint8_t> &salt), ORTP_UNUSED(const std::vector<uint8_t> &ikm), ORTP_UNUSED(const std::vector<uint8_t> &info), ORTP_UNUSED(size_t okmSize)) {
	/* if this template is instanciated the static_assert will fail but will give us an error message */
	static_assert(sizeof(hashAlgo) != sizeof(hashAlgo), "You must specialize HKDF function template");
	return std::vector<uint8_t>(0);
}
template <typename hashAlgo>
std::vector<uint8_t> HKDF(ORTP_UNUSED(const std::vector<uint8_t> &salt), ORTP_UNUSED(const std::vector<uint8_t> &ikm), ORTP_UNUSED(const std::string &info), ORTP_UNUSED(size_t okmSize)) {
	/* if this template is instanciated the static_assert will fail but will give us an error message */
	static_assert(sizeof(hashAlgo) != sizeof(hashAlgo), "You must specialize HKDF function template");
...
template <> std::vector<uint8_t> HKDF<SHA256>(const std::vector<uint8_t> &salt, const std::vector<uint8_t> &ikm, const std::vector<uint8_t> &info, size_t outputSize) {
	...
	if (mbedtls_hkdf(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), salt.data(), salt.size(), ikm.data(), ikm.size(), info.data(), info.size(), okm.data(), outputSize) != 0) {
		throw ORTP_EXCEPTION<<"HKDF-SHA256 error";
	}
	return okm;
};
...
template <> std::vector<uint8_t> HKDF<SHA384>(const std::vector<uint8_t> &salt, const std::vector<uint8_t> &ikm, const std::vector<uint8_t> &info, size_t outputSize) {
	...
	if (mbedtls_hkdf(mbedtls_md_info_from_type(MBEDTLS_MD_SHA384), salt.data(), salt.size(), ikm.data(), ikm.size(), info.data(), info.size(), okm.data(), outputSize) != 0) {
		throw ORTP_EXCEPTION<<"HKDF-SHA384 error";
	}
	return okm;
};
...
template <> std::vector<uint8_t> HKDF<SHA512>(const std::vector<uint8_t> &salt, const std::vector<uint8_t> &ikm, const std::vector<uint8_t> &info, size_t outputSize) {
	...
	if (mbedtls_hkdf(mbedtls_md_info_from_type(MBEDTLS_MD_SHA512), salt.data(), salt.size(), ikm.data(), ikm.size(), info.data(), info.size(), okm.data(), outputSize) != 0) {
		throw ORTP_EXCEPTION<<"HKDF-SHA512 error";
	}
	return okm;
};
...
std::vector<uint8_t> HMAC_KDF(const std::vector<uint8_t> &salt, const std::vector<uint8_t> &ikm, const infoType &info, size_t outputSize) {
	...
	ortp_clean(prk.data(), prk.size());
	ortp_clean(T.data(), T.size());
...
std::vector<uint8_t> AEADEncrypt(ORTP_UNUSED(const std::vector<uint8_t> &key), ORTP_UNUSED(const std::vector<uint8_t> IV), ORTP_UNUSED(const std::vector<uint8_t> &plain), ORTP_UNUSED(const std::vector<uint8_t> &AD),
		ORTP_UNUSED(std::vector<uint8_t> &tag)) {
...
template <typename AEADAlgo>
bool AEADDecrypt(ORTP_UNUSED(const std::vector<uint8_t> &key), ORTP_UNUSED(const std::vector<uint8_t> &IV), ORTP_UNUSED(const std::vector<uint8_t> &cipher), ORTP_UNUSED(const std::vector<uint8_t> &AD),
		ORTP_UNUSED(const std::vector<uint8_t> &tag), ORTP_UNUSED(std::vector<uint8_t> &plain)) {
	/* if this template is instanciated the static_assert will fail but will give us an error message with faulty type */
	static_assert(sizeof(AEADAlgo) != sizeof(AEADAlgo), "You must specialize AEADEncrypt function template");
	return false;
}
...
template <> std::vector<uint8_t> AEADEncrypt<AES256GCM128>(const std::vector<uint8_t> &key, const std::vector<uint8_t> IV, const std::vector<uint8_t> &plain, const std::vector<uint8_t> &AD,
	...
	if (key.size() != AES256GCM128::keySize()) {
		throw ORTP_EXCEPTION<<"AEADEncrypt: Bad input parameter, key is expected to be "<<AES256GCM128::keySize()<<" bytes but "<<key.size()<<" provided";
	}
	...
	if (ret != 0) {
		...
		throw ORTP_EXCEPTION<<"Unable to set key in AES_GCM context : return value "<<ret;
	}
	...
	if (ret != 0) {
		throw ORTP_EXCEPTION<<"Error during AES_GCM encryption : return value "<<ret;
	}
	return cipher;
}
...
template <> bool AEADDecrypt<AES256GCM128>(const std::vector<uint8_t> &key, const std::vector<uint8_t> &IV, const std::vector<uint8_t> &cipher, const std::vector<uint8_t> &AD,
	...
	if (key.size() != AES256GCM128::keySize()) {
		throw ORTP_EXCEPTION<<"AEADDecrypt: Bad input parameter, key is expected to be "<<AES256GCM128::keySize()<<" bytes but "<<key.size()<<" provided";
	}
	if (tag.size() != AES256GCM128::tagSize()) {
		throw ORTP_EXCEPTION<<"AEADDecrypt: Bad input parameter, tag is expected to be "<<AES256GCM128::tagSize()<<" bytes but "<<tag.size()<<" provided";
	}
	...
	throw ORTP_EXCEPTION<<"Error during AES_GCM decryption : return value "<<ret;
}
...
int AES_key_wrap(const std::vector<uint8_t> &plaintext, const std::vector<uint8_t> &key, std::vector<uint8_t> &ciphertext, AesId id){
	...
	uint8_t *R = (uint8_t *) ortp_malloc((m+8) * sizeof(uint8_t));	// An array of 8-bit registers
...
int AES_key_unwrap(const std::vector<uint8_t> &ciphertext, const std::vector<uint8_t> &key, std::vector<uint8_t> &plaintext, AesId id){
	...
	uint8_t input[16];						// Buffer of the AES input
	uint8_t B[16];							// Buffer of the AES output
	uint8_t *R = (uint8_t *) ortp_malloc(r * sizeof(uint8_t));	// An array of 8-bit registers
	...
	if (!(A[0] == 0xA6 && A[1] == 0x59 && A[2] == 0x59 && A[3] == 0xA6)){
		ortp_free(R);
		return ORTP_ERROR_UNSPECIFIED_ERROR;
	}
...
struct ortp_rng_context_struct {
	std::unique_ptr<ortp::RNG> m_rng; // encapsulate the RNG in a unique_ptr
};

ortp_rng_context_t *ortp_rng_context_new(void) {
	ortp_rng_context_t *context = new ortp_rng_context_struct();
	context->m_rng = std::unique_ptr<ortp::RNG>(new ortp::RNG());
	return context;
}
