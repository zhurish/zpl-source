...
#include "utils.h"
#include <ortp/crypto.h>

#ifdef ZPL_MBEDTLS_MODULE
 ...
 */
void ortp_clean(void *buffer, size_t size) {
	volatile uint8_t *p = buffer;
	while(size--) *p++ = 0;
}

ortp_type_implementation_t ortp_ssl_get_implementation_type(void) {
	return ORTP_MBEDTLS;
}

void ortp_strerror(int32_t error_code, char *buffer, size_t buffer_length) {
	if (error_code>0) {
...
#ifdef ZPL_MBEDTLS_MODULE
int32_t ortp_base64_encode(unsigned char *output, size_t *output_length, const unsigned char *input, size_t input_length) {
	size_t byte_written = 0;
	int ret = mbedtls_base64_encode(output, *output_length, &byte_written, input, input_length);
	*output_length = byte_written;
	if (ret == MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) {
		return ORTP_ERROR_OUTPUT_BUFFER_TOO_SMALL;
	}
	return ret;
}

int32_t ortp_base64_decode(unsigned char *output, size_t *output_length, const unsigned char *input, size_t input_length) {
	size_t byte_written = 0;
	...
	*output_length = byte_written;
	...
	if (ret == MBEDTLS_ERR_BASE64_INVALID_CHARACTER) {
		return ORTP_ERROR_INVALID_BASE64_INPUT;
	}
...
/*** signing key ***/
ortp_signing_key_t *ortp_signing_key_new(void) {
	mbedtls_pk_context *key = ortp_malloc0(sizeof(mbedtls_pk_context));
	mbedtls_pk_init(key);
	return (ortp_signing_key_t *)key;
}

void ortp_signing_key_free(ortp_signing_key_t *key) {
	mbedtls_pk_free((mbedtls_pk_context *)key);
	ortp_free(key);
}

char *ortp_signing_key_get_pem(ortp_signing_key_t *key) {
	char *pem_key;
	if (key == NULL) return NULL;
	pem_key = (char *)ortp_malloc0(4096);
	mbedtls_pk_write_key_pem( (mbedtls_pk_context *)key, (unsigned char *)pem_key, 4096);
	return pem_key;
}

int32_t ortp_signing_key_parse(ortp_signing_key_t *key, const char *buffer, size_t buffer_length, const unsigned char *password, size_t password_length) {
	int err;

	err=mbedtls_pk_parse_key((mbedtls_pk_context *)key, (const unsigned char *)buffer, buffer_length, password, password_length, NULL, NULL);
	
	if (err<0) {
		char tmp[128];
		mbedtls_strerror(err,tmp,sizeof(tmp));
		ortp_error("cannot parse public key because [%s]",tmp);
		return ORTP_ERROR_UNABLE_TO_PARSE_KEY;
	}
	return 0;
}

int32_t ortp_signing_key_parse_file(ortp_signing_key_t *key, const char *path, const char *password) {
	int err;
	
	err=mbedtls_pk_parse_keyfile((mbedtls_pk_context *)key, path, password, NULL, NULL);
...
/*** Certificate ***/
char *ortp_x509_certificates_chain_get_pem(const ortp_x509_certificate_t *cert) {
	char *pem_certificate = NULL;
	size_t olen=0;

	pem_certificate = (char *)ortp_malloc0(4096);
...
ortp_x509_certificate_t *ortp_x509_certificate_new(void) {
	mbedtls_x509_crt *cert = ortp_malloc0(sizeof(mbedtls_x509_crt));
	mbedtls_x509_crt_init(cert);
	return (ortp_x509_certificate_t *)cert;
}

void ortp_x509_certificate_free(ortp_x509_certificate_t *cert) {
	mbedtls_x509_crt_free((mbedtls_x509_crt *)cert);
	ortp_free(cert);
}

int32_t ortp_x509_certificate_get_info_string(char *buf, size_t size, const char *prefix, const ortp_x509_certificate_t *cert) {
	return mbedtls_x509_crt_info(buf, size, prefix, (mbedtls_x509_crt *)cert);
}

int32_t ortp_x509_certificate_parse_file(ortp_x509_certificate_t *cert, const char *path) {
	return mbedtls_x509_crt_parse_file((mbedtls_x509_crt *)cert, path);
}

int32_t ortp_x509_certificate_parse_path(ortp_x509_certificate_t *cert, const char *path) {
	return mbedtls_x509_crt_parse_path((mbedtls_x509_crt *)cert, path);
}

int32_t ortp_x509_certificate_parse(ortp_x509_certificate_t *cert, const char *buffer, size_t buffer_length) {
	return mbedtls_x509_crt_parse((mbedtls_x509_crt *)cert, (const unsigned char *)buffer, buffer_length);
}

int32_t ortp_x509_certificate_get_der_length(ortp_x509_certificate_t *cert) {
	if (cert!=NULL) {
...
int32_t ortp_x509_certificate_get_der(ortp_x509_certificate_t *cert, unsigned char *buffer, size_t buffer_length) {
	if (cert==NULL) {
		return ORTP_ERROR_INVALID_CERTIFICATE;
	}
...
int32_t ortp_x509_certificate_get_subject_dn(const ortp_x509_certificate_t *cert, char *dn, size_t dn_length) {
	if (cert==NULL) {
		return ORTP_ERROR_INVALID_CERTIFICATE;
	}
...
ortp_list_t *ortp_x509_certificate_get_subjects(const ortp_x509_certificate_t *cert){
	ortp_list_t *ret = NULL;


	if (cert != NULL) {
		mbedtls_x509_crt *mbedtls_cert = (mbedtls_x509_crt *)cert; // ortp_x509_certificate_t is just a cast of mbedtls_x509_crt
		...
		if( mbedtls_cert->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME ) {
			...
			while (cur != NULL) {
				ret = ortp_list_append(ret, ortp_strndup((const char*)cur->buf.p, (int)cur->buf.len));
				cur = cur->next;
			}
		}
		...
		while (subject != NULL) { // Certificate should hold only one CN, but be permissive and parse several if they are in the certificate
			if( MBEDTLS_OID_CMP( MBEDTLS_OID_AT_CN, &subject->oid ) == 0 ) { // subject holds all the distinguished name in asn1 format, get the CN only
				ret = ortp_list_append(ret, ortp_strndup((const char*)subject->val.p, (int)subject->val.len));
			}
...
int32_t ortp_x509_certificate_generate_selfsigned(const char *subject, ortp_x509_certificate_t *certificate, ortp_signing_key_t *pkey, char * pem, size_t pem_length) {
	...
	{
		ortp_error("Certificate generation can't init ctr_drbg: [-0x%x]", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}
	...
	if ( (ret = mbedtls_pk_setup( (mbedtls_pk_context *)pkey, mbedtls_pk_info_from_type( MBEDTLS_PK_RSA ) )) != 0) {
		ortp_error("Certificate generation can't init pk_ctx: [-0x%x]", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}

	if ( ( ret = mbedtls_rsa_gen_key( mbedtls_pk_rsa( *(mbedtls_pk_context *)pkey ), mbedtls_ctr_drbg_random, &ctr_drbg, 3072, 65537 ) ) != 0) {
		ortp_error("Certificate generation can't generate rsa key: [-0x%x]", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}
	...
	if ( (ret = mbedtls_mpi_read_string( &serial, 10, "1" ) ) != 0 ) {
		ortp_error("Certificate generation can't read serial mpi: [-0x%x]", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}
	...
	if ( (ret = mbedtls_x509write_crt_set_subject_name( &crt, formatted_subject) ) != 0) {
		ortp_error("Certificate generation can't set subject name: [-0x%x]", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}

	if ( (ret = mbedtls_x509write_crt_set_issuer_name( &crt, formatted_subject) ) != 0) {
		ortp_error("Certificate generation can't set issuer name: -%x", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}

	if ( (ret = mbedtls_x509write_crt_set_serial( &crt, &serial ) ) != 0) {
		ortp_error("Certificate generation can't set serial: -%x", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}
	mbedtls_mpi_free(&serial);

	if ( (ret = mbedtls_x509write_crt_set_validity( &crt, "20010101000000", "20300101000000" ) ) != 0) {
		ortp_error("Certificate generation can't set validity: -%x", -ret);
		return ORTP_ERROR_CERTIFICATE_GENERATION_FAIL;
	}
	...
	if ( (ret = mbedtls_x509write_crt_pem( &crt, (unsigned char *)file_buffer+file_buffer_len, 4096, mbedtls_ctr_drbg_random, &ctr_drbg ) ) != 0) {
		ortp_error("Certificate generation can't write crt pem: -%x", -ret);
		return ORTP_ERROR_CERTIFICATE_WRITE_PEM;
	}
	...
	if (pem != NULL) {
		if (strlen(file_buffer)+1>pem_length) {
			ortp_error("Certificate generation can't copy the certificate to pem buffer: too short [%ld] but need [%ld] bytes", (long)pem_length, (long)strlen(file_buffer));
			return ORTP_ERROR_OUTPUT_BUFFER_TOO_SMALL;
		}
	...
	if ( (ret = mbedtls_x509_crt_parse((mbedtls_x509_crt *)certificate, (unsigned char *)file_buffer, strlen(file_buffer)+1) ) != 0) {
		ortp_error("Certificate generation can't parse crt pem: -%x", -ret);
		return ORTP_ERROR_CERTIFICATE_PARSE_PEM;
	}
...
int32_t ortp_x509_certificate_get_signature_hash_function(const ortp_x509_certificate_t *certificate, ortp_md_type_t *hash_algorithm) {

	mbedtls_x509_crt *crt;
	if (certificate == NULL) return ORTP_ERROR_INVALID_CERTIFICATE;

	crt = (mbedtls_x509_crt *)certificate;

	switch (crt->sig_md) {
		case MBEDTLS_MD_SHA1:
			*hash_algorithm = ORTP_MD_SHA1;
		break;

		case MBEDTLS_MD_SHA224:
			*hash_algorithm = ORTP_MD_SHA224;
		break;

		case MBEDTLS_MD_SHA256:
			*hash_algorithm = ORTP_MD_SHA256;
		break;

		case MBEDTLS_MD_SHA384:
			*hash_algorithm = ORTP_MD_SHA384;
		break;

		case MBEDTLS_MD_SHA512:
			*hash_algorithm = ORTP_MD_SHA512;
		break;

		default:
			*hash_algorithm = ORTP_MD_UNDEFINED;
			return ORTP_ERROR_UNSUPPORTED_HASH_FUNCTION;
		break;
	}
...
int32_t ortp_x509_certificate_get_fingerprint(const ortp_x509_certificate_t *certificate, char *fingerprint, size_t fingerprint_length, ortp_md_type_t hash_algorithm) {
...
#define ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH 256
int32_t ortp_x509_certificate_flags_to_string(char *buffer, size_t buffer_size, uint32_t flags) {
	size_t i=0;
	char outputString[ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH];

	if (flags & MBEDTLS_X509_BADCERT_EXPIRED)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "expired ");
	if (flags & MBEDTLS_X509_BADCERT_REVOKED)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "revoked ");
	if (flags & MBEDTLS_X509_BADCERT_CN_MISMATCH)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "CN-mismatch ");
	if (flags & MBEDTLS_X509_BADCERT_NOT_TRUSTED)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "not-trusted ");
	if (flags & MBEDTLS_X509_BADCERT_MISSING)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "missing ");
	if (flags & MBEDTLS_X509_BADCERT_SKIP_VERIFY)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "skip-verify ");
	if (flags & MBEDTLS_X509_BADCERT_OTHER)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "other-reason ");
	if (flags & MBEDTLS_X509_BADCERT_FUTURE)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "future-validity ");
	if (flags & MBEDTLS_X509_BADCERT_KEY_USAGE)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "keyUsage-mismatch");
	if (flags & MBEDTLS_X509_BADCERT_EXT_KEY_USAGE)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "extendedKeyUsage-mismatch ");
	if (flags & MBEDTLS_X509_BADCERT_NS_CERT_TYPE)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "nsCertType-mismatch ");
	if (flags & MBEDTLS_X509_BADCERT_BAD_MD)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "unacceptable-hash ");
	if (flags & MBEDTLS_X509_BADCERT_BAD_PK)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "unacceptable-PK-alg ");
	if (flags & MBEDTLS_X509_BADCERT_BAD_KEY)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "unacceptable-key ");

	if (flags & MBEDTLS_X509_BADCRL_NOT_TRUSTED)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-not-trusted ");
	if (flags & MBEDTLS_X509_BADCRL_EXPIRED)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-expired ");

	if (flags & MBEDTLS_X509_BADCRL_FUTURE)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-future ");
	if (flags & MBEDTLS_X509_BADCRL_BAD_MD)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-unacceptable-hash ");
	if (flags & MBEDTLS_X509_BADCRL_BAD_PK)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-unacceptable-PK-alg ");
	if (flags & MBEDTLS_X509_BADCRL_BAD_KEY)
		i+=snprintf(outputString+i, ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH-i, "crl-unacceptable-key ");
...
int32_t ortp_x509_certificate_set_flag(uint32_t *flags, uint32_t flags_to_set) {
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_EXPIRED)
		*flags |= MBEDTLS_X509_BADCERT_EXPIRED;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_REVOKED)
		*flags |= MBEDTLS_X509_BADCERT_REVOKED;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_CN_MISMATCH)
		*flags |= MBEDTLS_X509_BADCERT_CN_MISMATCH;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_NOT_TRUSTED)
		*flags |= MBEDTLS_X509_BADCERT_NOT_TRUSTED;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_MISSING)
		*flags |= MBEDTLS_X509_BADCERT_MISSING;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_SKIP_VERIFY)
		*flags |= MBEDTLS_X509_BADCERT_SKIP_VERIFY;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_OTHER)
		*flags |= MBEDTLS_X509_BADCERT_OTHER;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_FUTURE)
		*flags |= MBEDTLS_X509_BADCERT_FUTURE;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_KEY_USAGE)
		*flags |= MBEDTLS_X509_BADCERT_KEY_USAGE;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_EXT_KEY_USAGE)
		*flags |= MBEDTLS_X509_BADCERT_EXT_KEY_USAGE;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_NS_CERT_TYPE)
		*flags |= MBEDTLS_X509_BADCERT_NS_CERT_TYPE;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_MD)
		*flags |= MBEDTLS_X509_BADCERT_BAD_MD;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_PK)
		*flags |= MBEDTLS_X509_BADCERT_BAD_PK;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_KEY)
		*flags |= MBEDTLS_X509_BADCERT_BAD_KEY;

	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_NOT_TRUSTED)
		*flags |= MBEDTLS_X509_BADCRL_NOT_TRUSTED;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_EXPIRED)
		*flags |= MBEDTLS_X509_BADCRL_EXPIRED;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_FUTURE)
		*flags |= MBEDTLS_X509_BADCRL_FUTURE;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_MD)
		*flags |= MBEDTLS_X509_BADCRL_BAD_MD;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_PK)
		*flags |= MBEDTLS_X509_BADCRL_BAD_PK;
	if (flags_to_set & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_KEY)
		*flags |= MBEDTLS_X509_BADCRL_BAD_KEY;

	return 0;
}

uint32_t ortp_x509_certificate_remap_flag(uint32_t flags) {
	uint32_t ret = 0;
	if (flags & MBEDTLS_X509_BADCERT_EXPIRED)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_EXPIRED;
	if (flags & MBEDTLS_X509_BADCERT_REVOKED)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_REVOKED;
	if (flags & MBEDTLS_X509_BADCERT_CN_MISMATCH)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_CN_MISMATCH;
	if (flags & MBEDTLS_X509_BADCERT_NOT_TRUSTED)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_NOT_TRUSTED;
	if (flags & MBEDTLS_X509_BADCERT_MISSING)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_MISSING;
	if (flags & MBEDTLS_X509_BADCERT_SKIP_VERIFY)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_SKIP_VERIFY;
	if (flags & MBEDTLS_X509_BADCERT_FUTURE)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_FUTURE;
	if (flags & MBEDTLS_X509_BADCERT_OTHER)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_OTHER;
	if (flags & MBEDTLS_X509_BADCERT_KEY_USAGE)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_KEY_USAGE;
	if (flags & MBEDTLS_X509_BADCERT_EXT_KEY_USAGE)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_EXT_KEY_USAGE;
	if (flags & MBEDTLS_X509_BADCERT_NS_CERT_TYPE)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_NS_CERT_TYPE;
	if (flags & MBEDTLS_X509_BADCERT_BAD_MD)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_MD;
	if (flags & MBEDTLS_X509_BADCERT_BAD_PK)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_PK;
	if (flags & MBEDTLS_X509_BADCERT_BAD_KEY)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_KEY;

	if (flags & MBEDTLS_X509_BADCRL_NOT_TRUSTED)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_NOT_TRUSTED;
	if (flags & MBEDTLS_X509_BADCRL_EXPIRED)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_EXPIRED;
	if (flags & MBEDTLS_X509_BADCRL_FUTURE)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_FUTURE;
	if (flags & MBEDTLS_X509_BADCRL_BAD_MD)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_MD;
	if (flags & MBEDTLS_X509_BADCRL_BAD_PK)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_PK;
	if (flags & MBEDTLS_X509_BADCRL_BAD_KEY)
		ret |= ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_KEY;

	return ret;
}

int32_t ortp_x509_certificate_unset_flag(uint32_t *flags, uint32_t flags_to_unset) {
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_EXPIRED)
		*flags &= ~MBEDTLS_X509_BADCERT_EXPIRED;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_REVOKED)
		*flags &= ~MBEDTLS_X509_BADCERT_REVOKED;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_CN_MISMATCH)
		*flags &= ~MBEDTLS_X509_BADCERT_CN_MISMATCH;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_NOT_TRUSTED)
		*flags &= ~MBEDTLS_X509_BADCERT_NOT_TRUSTED;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_MISSING)
		*flags &= ~MBEDTLS_X509_BADCERT_MISSING;

	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_SKIP_VERIFY)
		*flags &= ~MBEDTLS_X509_BADCERT_SKIP_VERIFY;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_OTHER)
		*flags &= ~MBEDTLS_X509_BADCERT_OTHER;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_FUTURE)
		*flags &= ~MBEDTLS_X509_BADCERT_FUTURE;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_KEY_USAGE)
		*flags &= ~MBEDTLS_X509_BADCERT_KEY_USAGE;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_EXT_KEY_USAGE)
		*flags &= ~MBEDTLS_X509_BADCERT_EXT_KEY_USAGE;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_NS_CERT_TYPE)
		*flags &= ~MBEDTLS_X509_BADCERT_NS_CERT_TYPE;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_MD)
		*flags &= ~MBEDTLS_X509_BADCERT_BAD_MD;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_PK)
		*flags &= ~MBEDTLS_X509_BADCERT_BAD_PK;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCERT_BAD_KEY)
		*flags &= ~MBEDTLS_X509_BADCERT_BAD_KEY;


	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_NOT_TRUSTED)
		*flags &= ~MBEDTLS_X509_BADCRL_NOT_TRUSTED;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_EXPIRED)
		*flags &= ~MBEDTLS_X509_BADCRL_EXPIRED;

	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_FUTURE)
		*flags &= ~MBEDTLS_X509_BADCRL_FUTURE;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_MD)
		*flags &= ~MBEDTLS_X509_BADCRL_BAD_MD;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_PK)
		*flags &= ~MBEDTLS_X509_BADCRL_BAD_PK;
	if (flags_to_unset & ORTP_CERTIFICATE_VERIFY_BADCRL_BAD_KEY)
		*flags &= ~MBEDTLS_X509_BADCRL_BAD_KEY;
...
ortp_DHMContext_t *ortp_CreateDHMContext(uint8_t DHMAlgo, uint8_t secretLength)
{
	mbedtls_dhm_context *mbedtlsDhmContext;

	/* create the context */
	ortp_DHMContext_t *context = (ortp_DHMContext_t *)malloc(sizeof(ortp_DHMContext_t));
	memset (context, 0, sizeof(ortp_DHMContext_t));
	...
	switch (DHMAlgo) {
		case ORTP_DHM_2048:
			...
			char p[] = MBEDTLS_DHM_RFC3526_MODP_2048_P_BIN;
			char pg[] = MBEDTLS_DHM_RFC3526_MODP_2048_G_BIN;
			if ((mbedtls_mpi_read_string(&(mbedtlsDhmContext->P), 16, p) != 0) ||
			(mbedtls_mpi_read_string(&(mbedtlsDhmContext->G), 16, pg) != 0)) {
				return NULL;
			}
			...
			break;
		case ORTP_DHM_3072:
			char pa[] = MBEDTLS_DHM_RFC3526_MODP_3072_P_BIN;
			char pga[] = MBEDTLS_DHM_RFC3526_MODP_3072_G_BIN;
			/* set P and G in the mbedtls context */
			if ((mbedtls_mpi_read_string(&(mbedtlsDhmContext->P), 16, pa) != 0) ||
			(mbedtls_mpi_read_string(&(mbedtlsDhmContext->G), 16, pga) != 0)) {
				return NULL;
			}
...
void ortp_DHMCreatePublic(ortp_DHMContext_t *context, int (*rngFunction)(void *, uint8_t *, size_t), void *rngContext) {
...
void ortp_DHMComputeSecret(ortp_DHMContext_t *context, int (*rngFunction)(void *, uint8_t *, size_t), void *rngContext) {
	size_t keyLength;
	...
	ortp_clean(sharedSecretBuffer, 384); /* purge secret from temporary buffer */
}

/* clean DHM context */
void ortp_DestroyDHMContext(ortp_DHMContext_t *context) {
	if (context!= NULL) {
		/* key and secret must be erased from memory and not just freed */
		if (context->secret != NULL) {
			ortp_clean(context->secret, context->secretLength);
			free(context->secret);
		}
		free(context->self);
		if (context->key != NULL) {
			ortp_clean(context->key, context->primeLength);
			free(context->key);
		}
		free(context->peer);
...
static int ortp_ssl_sendrecv_callback_return_remap(int32_t ret_code) {
	switch (ret_code) {
		case ORTP_ERROR_NET_WANT_READ:
			return MBEDTLS_ERR_SSL_WANT_READ;
		case ORTP_ERROR_NET_WANT_WRITE:
			return MBEDTLS_ERR_SSL_WANT_WRITE;
		case ORTP_ERROR_NET_CONN_RESET:
 ...
 */
const mbedtls_x509_crt_profile ortp_x509_crt_profile_default =
{
...
/** context **/
struct ortp_ssl_context_struct {
	mbedtls_ssl_context ssl_ctx;
	int(*callback_cli_cert_function)(void *, ortp_ssl_context_t *, const ortp_list_t *); /**< pointer to the callback called to update client certificate during handshake
...
ortp_ssl_context_t *ortp_ssl_context_new(void) {
	ortp_ssl_context_t *ssl_ctx = ortp_malloc0(sizeof(ortp_ssl_context_t));
...
void ortp_ssl_context_free(ortp_ssl_context_t *ssl_ctx) {
	mbedtls_ssl_free(&(ssl_ctx->ssl_ctx));
	ortp_free(ssl_ctx);
}

int32_t ortp_ssl_close_notify(ortp_ssl_context_t *ssl_ctx) {
	return mbedtls_ssl_close_notify(&(ssl_ctx->ssl_ctx));
}

int32_t ortp_ssl_session_reset(ortp_ssl_context_t *ssl_ctx) {
	return mbedtls_ssl_session_reset(&(ssl_ctx->ssl_ctx));
}

int32_t ortp_ssl_write(ortp_ssl_context_t *ssl_ctx, const unsigned char *buf, size_t buf_length) {
	int ret = mbedtls_ssl_write(&(ssl_ctx->ssl_ctx), buf, buf_length);
	/* remap some output code */
	if (ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
		ret = ORTP_ERROR_NET_WANT_WRITE;
	}
	return ret;
}

int32_t ortp_ssl_read(ortp_ssl_context_t *ssl_ctx, unsigned char *buf, size_t buf_length) {
	int ret = mbedtls_ssl_read(&(ssl_ctx->ssl_ctx), buf, buf_length);
	/* remap some output code */
	if (ret == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
		ret = ORTP_ERROR_SSL_PEER_CLOSE_NOTIFY;
	}
	if (ret == MBEDTLS_ERR_SSL_WANT_READ) {
		ret = ORTP_ERROR_NET_WANT_READ;
	}
	return ret;
}

int32_t ortp_ssl_handshake(ortp_ssl_context_t *ssl_ctx) {
	...
	while( ssl_ctx->ssl_ctx.state != MBEDTLS_SSL_HANDSHAKE_OVER )
	{
		...
		if (ssl_ctx->callback_cli_cert_function != NULL) { /* check we have a callback function */
			...
			if (ssl_ctx->ssl_ctx.state == MBEDTLS_SSL_CLIENT_CERTIFICATE/* && ssl_ctx->ssl_ctx.client_auth > 0*/) {
				...
				ortp_list_t *names = ortp_x509_certificate_get_subjects((const ortp_x509_certificate_t *)ssl_ctx->ssl_ctx.session_negotiate->peer_cert);

				if (ssl_ctx->callback_cli_cert_function(ssl_ctx->callback_cli_cert_data, ssl_ctx, names)!=0) {
					ortp_list_free_with_data(names, ortp_free);
					...
					return ORTP_ERROR_NO_CLIENT_CERTIFICATE;
				}
				ortp_list_free_with_data(names, ortp_free);
			}
		}
...
int32_t ortp_ssl_set_hs_own_cert(ortp_ssl_context_t *ssl_ctx, ortp_x509_certificate_t *cert, ortp_signing_key_t *key) {
...
int ortp_ssl_send_callback(void *data, const unsigned char *buffer, size_t buffer_length) {
	int ret = 0;
	/* data is the ssl_context which contains the actual callback and data */
	ortp_ssl_context_t *ssl_ctx = (ortp_ssl_context_t *)data;

	ret = ssl_ctx->callback_send_function(ssl_ctx->callback_sendrecv_data, buffer, buffer_length);

	return ortp_ssl_sendrecv_callback_return_remap(ret);
}

int ortp_ssl_recv_callback(void *data, unsigned char *buffer, size_t buffer_length) {
	int ret = 0;
	/* data is the ssl_context which contains the actual callback and data */
	ortp_ssl_context_t *ssl_ctx = (ortp_ssl_context_t *)data;
...
void ortp_ssl_set_io_callbacks(ortp_ssl_context_t *ssl_ctx, void *callback_data,
		int(*callback_send_function)(void *, const unsigned char *, size_t), /* callbacks args are: callback data, data buffer to be send, size of data buffer */
		int(*callback_recv_function)(void *, unsigned char *, size_t)){ /* args: callback data, data buffer to be read, size of data buffer */
	...
	mbedtls_ssl_set_bio(&(ssl_ctx->ssl_ctx), ssl_ctx, ortp_ssl_send_callback, ortp_ssl_recv_callback, NULL);
}

const ortp_x509_certificate_t *ortp_ssl_get_peer_certificate(ortp_ssl_context_t *ssl_ctx) {
	return (const ortp_x509_certificate_t *)mbedtls_ssl_get_peer_cert(&(ssl_ctx->ssl_ctx));
}

const char *ortp_ssl_get_ciphersuite(ortp_ssl_context_t *ssl_ctx){
	return mbedtls_ssl_get_ciphersuite(&(ssl_ctx->ssl_ctx));
}

int ortp_ssl_get_ciphersuite_id(const char *ciphersuite){
	return mbedtls_ssl_get_ciphersuite_id(ciphersuite);
}

const char *ortp_ssl_get_version(ortp_ssl_context_t *ssl_ctx){
	return mbedtls_ssl_get_version(&(ssl_ctx->ssl_ctx));
}

int32_t ortp_ssl_set_hostname(ortp_ssl_context_t *ssl_ctx, const char *hostname){
	return mbedtls_ssl_set_hostname(&(ssl_ctx->ssl_ctx), hostname);
}

/** DTLS SRTP functions **/
#ifdef HAVE_DTLS_SRTP
uint8_t ortp_dtls_srtp_supported(void) {
	return 1;
}

void ortp_ssl_set_mtu(ortp_ssl_context_t *ssl_ctx, uint16_t mtu) {
...
static ortp_dtls_srtp_profile_t ortp_srtp_profile_mbedtls2bctoolbox(mbedtls_ssl_srtp_profile mbedtls_profile) {
	switch (mbedtls_profile) {
		case MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_80:
			return ORTP_SRTP_AES128_CM_HMAC_SHA1_80;
		case MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_32:
			return ORTP_SRTP_AES128_CM_HMAC_SHA1_32;
		case MBEDTLS_TLS_SRTP_NULL_HMAC_SHA1_80:
			return ORTP_SRTP_NULL_HMAC_SHA1_80;
		case MBEDTLS_TLS_SRTP_NULL_HMAC_SHA1_32:
			return ORTP_SRTP_NULL_HMAC_SHA1_32;
		default:
			return ORTP_SRTP_UNDEFINED;
	}
}

static mbedtls_ssl_srtp_profile ortp_srtp_profile_bctoolbox2mbedtls(ortp_dtls_srtp_profile_t ortp_profile) {
	switch (ortp_profile) {
		case ORTP_SRTP_AES128_CM_HMAC_SHA1_80:
			return MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_80;
		case ORTP_SRTP_AES128_CM_HMAC_SHA1_32:
			return MBEDTLS_TLS_SRTP_AES128_CM_HMAC_SHA1_32;
		case ORTP_SRTP_NULL_HMAC_SHA1_80:
			return MBEDTLS_TLS_SRTP_NULL_HMAC_SHA1_80;
		case ORTP_SRTP_NULL_HMAC_SHA1_32:
...
ortp_dtls_srtp_profile_t ortp_ssl_get_dtls_srtp_protection_profile(ortp_ssl_context_t *ssl_ctx) {
	if (ssl_ctx==NULL) {
		return ORTP_ERROR_INVALID_SSL_CONTEXT;
	}

	mbedtls_dtls_srtp_info dtls_srtp_negotiation_result;
	mbedtls_ssl_get_dtls_srtp_negotiation_result(&(ssl_ctx->ssl_ctx), &dtls_srtp_negotiation_result);
	return ortp_srtp_profile_mbedtls2bctoolbox(dtls_srtp_negotiation_result.chosen_dtls_srtp_profile);
};
...
/** config **/
#ifdef HAVE_DTLS_SRTP
typedef struct ortp_dtls_srtp_keys {
...
} ortp_dtls_srtp_keys_t;
#endif /* HAVE_DTLS_SRTP */

struct ortp_ssl_config_struct {
	...
	ortp_dtls_srtp_keys_t dtls_srtp_keys; /**< Key material is stored during the handshake there and used after completion to generate the DTLS-SRTP shared secret */
#endif /* HAVE_DTLS_SRTP */
};

ortp_ssl_config_t *ortp_ssl_config_new(void) {
	ortp_ssl_config_t *ssl_config = ortp_malloc0(sizeof(ortp_ssl_config_t));
	/* allocate and init anyway a ssl_config, it may be then crashed by an externally provided one */
	ssl_config->ssl_config = ortp_malloc0(sizeof(mbedtls_ssl_config));
...
int32_t ortp_ssl_config_set_crypto_library_config(ortp_ssl_config_t *ssl_config, void *internal_config) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
	...
	if (ssl_config->ssl_config != NULL && ssl_config->ssl_config_externally_provided==0) {
		...
		ortp_free(ssl_config->ssl_config);
	}

	/* set the given pointer as the ssl_config context */
	ssl_config->ssl_config = (mbedtls_ssl_config *)internal_config;

	/* set the flag in order to not free the mbedtls config when freing ortp_ssl_config */
	ssl_config->ssl_config_externally_provided = 1;

	return 0;
}

void ortp_ssl_config_free(ortp_ssl_config_t *ssl_config) {
	if (ssl_config == NULL) {
		return;
	}
...
#ifdef HAVE_DTLS_SRTP
	ortp_clean(ssl_config->dtls_srtp_keys.master_secret, sizeof(ssl_config->dtls_srtp_keys.master_secret));
	ortp_clean(ssl_config->dtls_srtp_keys.randoms, sizeof(ssl_config->dtls_srtp_keys.randoms));
#endif /* HAVE_DTLS_SRTP */

	ortp_free(ssl_config);
}

int32_t ortp_ssl_config_defaults(ortp_ssl_config_t *ssl_config, int endpoint, int transport) {
	...
	switch (endpoint) {
		case ORTP_SSL_IS_CLIENT:
			mbedtls_endpoint = MBEDTLS_SSL_IS_CLIENT;
			break;
		case ORTP_SSL_IS_SERVER:
			mbedtls_endpoint = MBEDTLS_SSL_IS_SERVER;
			break;
		default:
			return ORTP_ERROR_INVALID_INPUT_DATA;
	}

	switch (transport) {
		case ORTP_SSL_TRANSPORT_STREAM:
			mbedtls_transport = MBEDTLS_SSL_TRANSPORT_STREAM;
			break;
		case ORTP_SSL_TRANSPORT_DATAGRAM:
	...
	if (transport == ORTP_SSL_TRANSPORT_DATAGRAM) {
	...
	mbedtls_ssl_conf_cert_profile(ssl_config->ssl_config, &ortp_x509_crt_profile_default);

	return ret;
}

int32_t ortp_ssl_config_set_endpoint(ortp_ssl_config_t *ssl_config, int endpoint) {
...
int32_t ortp_ssl_config_set_transport (ortp_ssl_config_t *ssl_config, int transport) {
...
int32_t ortp_ssl_config_set_ciphersuites(ortp_ssl_config_t *ssl_config, const int *ciphersuites) {
	if (ssl_config == NULL) {
	 return ORTP_ERROR_INVALID_SSL_CONFIG;
	 }
	/* remap input arguments */
...
void *ortp_ssl_config_get_private_config(ortp_ssl_config_t *ssl_config) {
	return (void *)ssl_config->ssl_config;
}

int32_t ortp_ssl_config_set_authmode(ortp_ssl_config_t *ssl_config, int authmode) {
...
int32_t ortp_ssl_config_set_rng(ortp_ssl_config_t *ssl_config, int(*rng_function)(void *, unsigned char *, size_t), void *rng_context) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
...
int32_t ortp_ssl_config_set_callback_verify(ortp_ssl_config_t *ssl_config, int(*callback_function)(void *, ortp_x509_certificate_t *, int, uint32_t *), void *callback_data) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
...
int32_t ortp_ssl_config_set_callback_cli_cert(ortp_ssl_config_t *ssl_config, int(*callback_function)(void *, ortp_ssl_context_t *, const ortp_list_t *), void *callback_data) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
...
int32_t ortp_ssl_config_set_ca_chain(ortp_ssl_config_t *ssl_config, ortp_x509_certificate_t *ca_chain) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
...
int32_t ortp_ssl_config_set_own_cert(ortp_ssl_config_t *ssl_config, ortp_x509_certificate_t *cert, ortp_signing_key_t *key) {
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
...
static int ortp_ssl_dtls_srtp_key_derivation(void *key_ctx, const unsigned char *ms,
		ORTP_UNUSED(const unsigned char * kb), ORTP_UNUSED(size_t maclen), ORTP_UNUSED(size_t keylen), ORTP_UNUSED(size_t ivlen), // these params are useless for our purpose
		const unsigned char client_random[32], const unsigned char server_random[32],
		mbedtls_tls_prf_types tls_prf_type ) {

	ortp_dtls_srtp_keys_t *keys = (ortp_dtls_srtp_keys_t *)key_ctx;
...
int32_t ortp_ssl_get_dtls_srtp_key_material(ortp_ssl_config_t *ssl_config, uint8_t *output, size_t *output_length) {
	int ret = 0;
	if (ssl_config==NULL) {
		return ORTP_ERROR_INVALID_SSL_CONTEXT;
	}
...
int32_t ortp_ssl_config_set_dtls_srtp_protection_profiles(ortp_ssl_config_t *ssl_config, const ortp_dtls_srtp_profile_t *profiles, size_t profiles_number) {
	size_t i;
	if (ssl_config == NULL) {
		return ORTP_ERROR_INVALID_SSL_CONFIG;
	}
	...
	for (i=0; i<profiles_number && i<MBEDTLS_TLS_SRTP_MAX_PROFILE_LIST_LENGTH; i++) { /* MBEDTLS_TLS_SRTP_MAX_PROFILE_LIST_LENGTH profiles defined max */
		ssl_config->dtls_srtp_mbedtls_profiles[i] = ortp_srtp_profile_bctoolbox2mbedtls(profiles[i]);
	}
	...
	mbedtls_ssl_conf_export_keys_ext_cb(ssl_config->ssl_config, ortp_ssl_dtls_srtp_key_derivation, &(ssl_config->dtls_srtp_keys));
...
#else /* HAVE_DTLS_SRTP */
int32_t ortp_ssl_get_dtls_srtp_key_material(ortp_ssl_config_t *ssl_ctx, uint8_t *output, size_t *output_length) {
	*output_length = 0;
	return ORTP_ERROR_UNAVAILABLE_FUNCTION;
}
...
int32_t ortp_ssl_context_setup(ortp_ssl_context_t *ssl_ctx, ortp_ssl_config_t *ssl_config) {
	int ret;
 ...
 *
 */
void ortp_hmacSha512(const uint8_t *key,
		size_t keyLength,
		const uint8_t *input,
		size_t inputLength,
		uint8_t hmacLength,
 ...
 *
 */
void ortp_hmacSha384(const uint8_t *key,
		size_t keyLength,
		const uint8_t *input,
		size_t inputLength,
		uint8_t hmacLength,
 ...
 *
 */
void ortp_hmacSha256(const uint8_t *key,
		size_t keyLength,
		const uint8_t *input,
		size_t inputLength,
		uint8_t hmacLength,
 ...
 *
 */
void ortp_sha512(const uint8_t *input,
		size_t inputLength,
		uint8_t hashLength,
		uint8_t *output)
{
 ...
 *
 */
void ortp_sha384(const uint8_t *input,
		size_t inputLength,
		uint8_t hashLength,
		uint8_t *output)
{
 ...
 *
 */
void ortp_sha256(const uint8_t *input,
		size_t inputLength,
		uint8_t hashLength,
		uint8_t *output)
{
 ...
 *
 */
void ortp_hmacSha1(const uint8_t *key,
		size_t keyLength,
		const uint8_t *input,
		size_t inputLength,
		uint8_t hmacLength,
 ...
 *
 */
void ortp_md5(const uint8_t *input,
		size_t inputLength,
		uint8_t output[16])
{
 ...
 */
int32_t ortp_aes_gcm_encrypt_and_tag(const uint8_t *key, size_t keyLength,
 ...
 *
 * @return 0 on succes, ORTP_ERROR_AUTHENTICATION_FAILED if tag doesn't match or mbedtls error code
 */
int32_t ortp_aes_gcm_decrypt_and_auth(const uint8_t *key, size_t keyLength,
	...
	if (ret == MBEDTLS_ERR_GCM_AUTH_FAILED) {
		return ORTP_ERROR_AUTHENTICATION_FAILED;
	}
 ...
 * @param[in]	initializationVector		Buffer holding the initialisation vector
 * @param[in]	initializationVectorLength	Initialisation vector length in bytes
 * @param[in]	mode						Operation mode : ORTP_GCM_ENCRYPT or ORTP_GCM_DECRYPT
 *
 * @return 0 on success, crypto library error code otherwise
 */
ortp_aes_gcm_context_t *ortp_aes_gcm_context_new(const uint8_t *key, size_t keyLength,
		const uint8_t *authenticatedData, size_t authenticatedDataLength,
		const uint8_t *initializationVector, size_t initializationVectorLength,
		uint8_t mode) {
	...
	if (mode == ORTP_GCM_ENCRYPT) {
		mbedtls_mode = MBEDTLS_GCM_ENCRYPT;
	} else if ( mode == ORTP_GCM_DECRYPT) {
		mbedtls_mode = MBEDTLS_GCM_DECRYPT;
	} else {
		return NULL;
	}

	ctx = ortp_malloc0(sizeof(mbedtls_gcm_context));
	...
	//zhurish ret = mbedtls_gcm_starts(ctx, mbedtls_mode, initializationVector, initializationVectorLength, authenticatedData, authenticatedDataLength);
	if (ret != 0) {
		ortp_free(ctx);
		return NULL;
	}

	return (ortp_aes_gcm_context_t *)ctx;
}

/**
 * @Brief AES-GCM encrypt or decrypt a chunk of data
 *
 * @param[in/out]	context			a context already initialized using ortp_aes_gcm_context_new
 ...
 * @return 0 on success, crypto library error code otherwise
 */
int32_t ortp_aes_gcm_process_chunk(ortp_aes_gcm_context_t *context,
		const uint8_t *input, size_t inputLength,
		uint8_t *output) {
		size_t output_size = 0;
        size_t output_length = 0;
		return mbedtls_gcm_update((mbedtls_gcm_context *)context,
                       input,  inputLength,
                       output, output_size,
                       &output_length);
 ...
 * @return 0 on success, crypto library error code otherwise
 */
int32_t ortp_aes_gcm_finish(ortp_aes_gcm_context_t *context,
		uint8_t *tag, size_t tagLength) {
	int ret;

	//zhurish ret = mbedtls_gcm_finish((mbedtls_gcm_context *)context, tag, tagLength);
 ...
 *
 */
void ortp_aes128CfbEncrypt(const uint8_t *key,
		const uint8_t *IV,
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes128CfbDecrypt(const uint8_t *key,
		const uint8_t *IV,
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes256CfbEncrypt(const uint8_t *key,
		const uint8_t *IV,
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes256CfbDecrypt(const uint8_t *key,
		const uint8_t *IV,
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
...
struct ortp_rng_context_struct {
	int entropy;
	int ctr_drbg;
};

ortp_rng_context_t *ortp_rng_context_new(void) {
	return NULL;
}

int32_t ortp_rng_get(ortp_rng_context_t *context, unsigned char*output, size_t output_length) {
	return 0;
}

void ortp_rng_context_free(ortp_rng_context_t *context) {
}
...
ortp_list_t *ortp_x509_certificate_get_subjects(const ortp_x509_certificate_t *cert){
	ortp_error("ortp_x509_certificate_get_subjects(): not implemented for polarssl.");
	return NULL;
}
...
#define ORTP_MAX_CERTIFICATE_FLAGS_STRING_LENGTH 128
int32_t ortp_x509_certificate_flags_to_string(char *buffer, size_t buffer_size, uint32_t flags) {
...
struct ortp_ssl_context_struct {
	void* ssl_ctx;
	char *cn;
	int(*callback_cli_cert_function)(void *, ortp_ssl_context_t *, unsigned char *, size_t); /**< pointer to the callback called to update client certificate during handshake
...
static ortp_dtls_srtp_profile_t ortp_srtp_profile_polarssl2bctoolbox(enum DTLS_SRTP_protection_profiles polarssl_profile) {

			return ORTP_SRTP_UNDEFINED;
}

static enum DTLS_SRTP_protection_profiles ortp_srtp_profile_bctoolbox2polarssl(ortp_dtls_srtp_profile_t ortp_profile) {
...
int32_t ortp_ssl_get_dtls_srtp_key_material(ortp_ssl_context_t *ssl_ctx, char *output, size_t *output_length) {
...
/** config **/
struct ortp_ssl_config_struct {
	int8_t endpoint; /**< ORTP_SSL_IS_CLIENT or ORTP_SSL_IS_SERVER */
	int8_t authmode; /**< ORTP_SSL_VERIFY_NONE, ORTP_SSL_VERIFY_OPTIONAL, ORTP_SSL_VERIFY_REQUIRED */
	int8_t transport; /**< ORTP_SSL_TRANSPORT_STREAM(TLS) or ORTP_SSL_TRANSPORT_DATAGRAM(DTLS) */
	...
	int(*callback_verify_function)(void *, void *, int, int *); /**< pointer to the verify callback function */
 ...
 * @param[out]	output						Buffer holding the output, shall be at least the length of cipherText buffer
 *
 * @return 0 on succes, ORTP_ERROR_AUTHENTICATION_FAILED if tag doesn't match or polarssl error code
 */
int32_t ortp_aes_gcm_decrypt_and_auth(const uint8_t *key, size_t keyLength,
		const uint8_t *cipherText, size_t cipherTextLength,
		const uint8_t *authenticatedData, size_t authenticatedDataLength,
 ...
 *
 */
void ortp_aes128CfbEncrypt(const uint8_t key[16],
		const uint8_t IV[16],
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes128CfbDecrypt(const uint8_t key[16],
		const uint8_t IV[16],
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes256CfbEncrypt(const uint8_t key[32],
		const uint8_t IV[16],
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
 ...
 *
 */
void ortp_aes256CfbDecrypt(const uint8_t key[32],
		const uint8_t IV[16],
		const uint8_t *input,
		size_t inputLength,
		uint8_t *output)
