...
static int32_t ozrtp_sendDataZRTP (ZrtpContext* ctx, const uint8_t* data, const int32_t length ){
	...
	*buffer8 = 0x10;
	...
	// Send packet
	ssize_t bytesSent = ipstack_sendto(sockfd, (void*)buffer8, newlength,0,destaddr,destlen);
...
static int ozrtp_generic_sendto(stream_type stream, RtpTransport *t, mblk_t *m, int flags, const struct sockaddr *to, socklen_t tolen){
	...
	if (userData->srtpSend == NULL || !zrtp_inState(zrtpContext, SecureState)) {
		...
		msgpullup(m,-1);
		size=msgdsize(m);
		return ipstack_sendto(socket,(void*)m->b_rptr,size,flags,to,tolen);
	}
	slen=msgdsize(m);
	// Protect with srtp
	...
	if (err==err_status_ok){
		return ipstack_sendto(socket,(void*)m->b_rptr,slen,flags,to,tolen);
	} else {
