...
float ortp_video_bandwidth_estimator_get_estimated_available_bandwidth(OrtpVideoBandwidthEstimator *vbe) {
    ortp_list_t *bitrate_sorted = NULL;
    ortp_list_t *elem;
    float *result = NULL;
    int index = (int)(vbe->trust_percentage * vbe->packets_size_max / 100);
    for(elem = vbe->packets; elem != NULL; elem = ortp_list_next(elem)) {
        OrtpVideoBandwidthEstimatorPacket *packet = (OrtpVideoBandwidthEstimatorPacket *)ortp_list_get_data(elem);
        bitrate_sorted = ortp_list_insert_sorted(bitrate_sorted, &packet->bitrate, (ortp_compare_func)compare_float);
    }
    result = (float *)ortp_list_nth_data(bitrate_sorted, index);
    ortp_list_free(bitrate_sorted);
    return (float)*result;
}
