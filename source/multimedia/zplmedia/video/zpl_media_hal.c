...
{
    int createflag = 0;
    zpl_media_video_vpss_channel_t *video_vpss = NULL;
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    zpl_media_video_vpssgrp_t *video_vpssgrp = NULL;
#endif

#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_media_video_vpssgrp_lookup(vpss_group) == NULL)
    {
        if(zpl_media_video_vpssgrp_create(vpss_group)!= OK)
        {
    ...
    video_vpssgrp = zpl_media_video_vpssgrp_lookup(vpss_group);
    if (video_vpssgrp == NULL)
    {
        zpl_media_video_vpssgrp_destroy(vpss_group);
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
    ...
    zpl_media_video_vpssgrp_bindcount_set(video_vpssgrp, zpl_true);
    zpl_media_debugmsg_debug("channel(%d/%d) VPSSGRP Flags (0x%x)", chn->channel, chn->channel_index, video_vpssgrp->res_flag);

#endif
    createflag = 0;
    if (zpl_media_video_vpss_channel_lookup(vpss_channel) == NULL)
    {
        if(zpl_media_video_vpss_channel_new(vpss_channel)!= OK)
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
    ...
    video_vpss = zpl_media_video_vpss_channel_lookup(vpss_channel);
    if (video_vpss == NULL)
    {
        if (zpl_media_video_vpssgrp_bindcount_get(video_vpssgrp) == 0)
            zpl_media_video_vpssgrp_destroy(vpss_group);
        if (zpl_media_video_vpss_channel_bindcount_get(video_vpss) == 0)    
            zpl_media_video_vpss_channel_delete(vpss_channel);
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
    ...
    zpl_media_video_vpss_channel_bindcount_set(video_vpss, zpl_true);
...
{
    int createflag = 0;
    zpl_meida_video_input_channel_t *video_input = NULL;
    zpl_meida_video_input_pipe_t *video_input_pipe = NULL;
    if (zpl_meida_video_input_channel_lookup(input_chn) == NULL)
    {
        if (zpl_meida_video_input_channel_create(input_chn) != OK)
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
    ...
    video_input = zpl_meida_video_input_channel_lookup(input_chn);

    if (video_input == NULL)
    {
        zpl_media_debugmsg_error("can not get input hal for channel(%d)", input_chn);
        if (zpl_meida_video_input_channel_bindcount_get(video_input) == 0)
            zpl_meida_video_input_channel_destroy( input_chn);
        return ERROR;
    }
    zpl_meida_video_input_channel_bindcount_set(video_input, zpl_true);
    ...
    createflag = 0;
    if (zpl_meida_video_input_pipe_lookup(input_dev, input_pipe) == NULL)
    {
        if (zpl_meida_video_input_pipe_create(input_dev, input_pipe) != OK)
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
    ...
    video_input_pipe = zpl_meida_video_input_pipe_lookup(input_dev, input_pipe);
    ...
    if (video_input_pipe == NULL)
    {
        ...
        if (zpl_meida_video_input_channel_bindcount_get(video_input) == 0)
            zpl_meida_video_input_channel_destroy(input_chn);
        if (zpl_meida_video_input_pipe_bindcount_get(video_input_pipe) == 0)
        zpl_meida_video_input_pipe_destroy( input_dev, input_pipe);
        return ERROR;
    }
    zpl_meida_video_input_pipe_bindcount_set(video_input_pipe, zpl_true);
...
int zpl_media_hal_request_IDR(zpl_media_channel_t *chn)
{
    ...
    if (venc_channel >= 0)
    {
        zpl_media_video_encode_t *video_encode = zpl_media_video_encode_lookup(venc_channel);
        if(video_encode)
...
zpl_media_hal_sys_init();
        if(zpl_media_video_encode_new(venc_channel) != OK)
        {
            if(ZPL_MEDIA_DEBUG(CHANNEL, ERROR))
            {
        ...
        video_encode = chn->media_param.video_media.halparam = zpl_media_video_encode_lookup(venc_channel);
        if (chn->media_param.video_media.halparam == NULL)
        {
            zpl_media_debugmsg_error("can not lookup video_encode channel(%d)", venc_channel);
            zpl_media_video_encode_delete(venc_channel);
            return ERROR;
        }
        if (zpl_media_video_encode_frame_queue_set(venc_channel, buffer_queue) != OK)
        {
...
int zpl_media_hal_destroy(zpl_media_channel_t *chn)
{
    ...
    if (zpl_media_video_vpss_channel_bindcount_get(video_vpss) == 1)
    {
        zpl_media_video_vpss_channel_delete(video_vpss->vpss_channel);
    }
    else
    {
        zpl_media_video_vpss_channel_bindcount_set(video_vpss, zpl_false);
    }
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_media_video_vpssgrp_bindcount_get(video_vpssgrp) == 1)
    {
        zpl_media_video_vpssgrp_destroy(video_vpssgrp->vpss_group);
    }
    else
    {
        zpl_media_video_vpssgrp_bindcount_set(video_vpssgrp, zpl_false);
    }
#endif
    if (zpl_meida_video_input_channel_bindcount_get(video_input) == 1)
    {
        zpl_meida_video_input_channel_destroy(video_input->input_chn);
    }
    else
    {
        zpl_meida_video_input_channel_bindcount_set(video_vpss, zpl_false);
    }

    if (zpl_meida_video_input_pipe_bindcount_get(video_input_pipe) == 1)
    {
        zpl_meida_video_input_pipe_destroy(video_input_pipe->input_pipe, video_input_pipe->input_dev);
    }
    else
    {
        zpl_meida_video_input_pipe_bindcount_set(video_input_pipe, zpl_false);
    }
    zpl_media_video_encode_delete(video_encode->venc_channel);
    return OK;
}
...
int zpl_media_hal_active(zpl_media_channel_t *chn)
{
    ...
    if (zpl_meida_video_input_pipe_hal_create(video_input_pipe) != OK)
    {
        return ERROR;
    } 
    if (zpl_meida_video_input_channel_hal_create(video_input) != OK)
    {
        return ERROR;
    } 
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_meida_video_vpssgrp_hal_create(video_vpssgrp) != OK)
    {
        return ERROR;
    }
#endif
    if (zpl_meida_video_vpss_channel_hal_create(video_vpss) != OK)
    {
        return ERROR;
    }
    if (zpl_media_video_encode_hal_create(video_encode) != OK)
    {
        return ERROR;
    }
...
int zpl_media_hal_start(zpl_media_channel_t *chn)
{
    ...
    if (zpl_meida_video_input_pipe_start(video_input_pipe) != OK)
    {
        return ERROR;
    }
    if (zpl_meida_video_input_channel_start(video_input) != OK)
    {
        return ERROR;
    }
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_media_video_vpssgrp_start(video_vpssgrp) != OK)
    {
        return ERROR;
    }
#endif
    if (zpl_media_video_vpss_channel_start(video_vpss) != OK)
    {
        return ERROR;
    }
    if (zpl_media_video_encode_start(video_encode) != OK)
    {
        return ERROR;
    }
    if (VIDHAL_RES_FLAG_CHECK(video_vpssgrp->res_flag, DSTBIND) && !video_vpssgrp->hwbind)
    {
        ...
        {
            zpl_meida_video_input_pipe_read_stop(video_input_pipe);
            zpl_meida_video_input_channel_read_stop(video_input);
        }
    ...
    if (VIDHAL_RES_FLAG_CHECK(video_encode->res_flag, DSTBIND) && !video_encode->hwbind)
    {
        ...
        if (zpl_syshal_vpss_bind_venc(video_vpssgrp->vpss_group, video_vpss->vpss_channel, video_encode->venc_channel) == OK)
            zpl_media_video_vpss_channel_read_stop(video_vpss);
...
int zpl_media_hal_stop(zpl_media_channel_t *chn)
{
    ...
    if (zpl_media_video_encode_stop(video_encode) != OK)
    {
        return ERROR;
    }
    if (zpl_media_video_vpss_channel_stop(video_vpss) != OK)
    {
        return ERROR;
    }
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_media_video_vpssgrp_stop(video_vpssgrp) != OK)
    {
        return ERROR;
    }
#endif
    if (zpl_meida_video_input_channel_stop(video_input) != OK)
    {
        return ERROR;
    }
    if (zpl_meida_video_input_pipe_stop(video_input_pipe) != OK)
    {
        return ERROR;
    }    
    return OK;
...
int zpl_media_hal_inactive(zpl_media_channel_t *chn)
{
    ...
    if (zpl_media_video_encode_hal_destroy(video_encode) != OK)
    {
        return ERROR;
    }

    if (zpl_meida_video_vpss_channel_hal_destroy(video_vpss) != OK)
    {
        return ERROR;
    }
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    if (zpl_meida_video_vpssgrp_hal_destroy(video_vpssgrp) != OK)
    {
        return ERROR;
    }
#endif
    if (zpl_meida_video_input_channel_hal_destroy(video_input) != OK)
    {
        return ERROR;
    }
    if (zpl_meida_video_input_pipe_hal_destroy(video_input_pipe) != OK)
    {
        return ERROR;
    }
    return OK;
        ...
        if(VIDHAL_RES_FLAG_CHECK(video_encode->res_flag, DSTBIND) && video_encode->hwbind)
        {
            ...
            if(zpl_syshal_vpss_unbind_venc(video_vpssgrp->vpss_group, video_vpss->vpss_channel, video_encode->venc_channel) == OK)
            {
                if(video_vpss->t_master)
                    zpl_media_video_vpss_channel_read_start(video_vpss->t_master, video_vpss);
            }
        ...
        if(VIDHAL_RES_FLAG_CHECK(video_vpssgrp->res_flag, DSTBIND) && video_vpssgrp->hwbind)
        {
            ...
            if(zpl_syshal_input_unbind_vpss(video_input_pipe->input_pipe, video_input->input_chn, video_vpssgrp->vpss_group) == OK)
            {
                if(video_input_pipe->t_master)
                    zpl_meida_video_input_pipe_read_start(video_input_pipe->t_master, video_input_pipe);
                if(video_input->t_master)
                    zpl_meida_video_input_channel_read_start(video_input->t_master, video_input); 
            }
...
int zpl_media_hal_read_stop(ZPL_MEDIA_NODE_E module, zpl_media_channel_t *chn)
{
    ...
    ret = zpl_meida_video_input_pipe_read_stop(video_input_pipe);
    break;
    case ZPL_MEDIA_NODE_INPUT:
    ret = zpl_meida_video_input_channel_read_stop(video_input);
    break;
    //case ZPL_MEDIA_NODE_PROCESS:  
    //ret = zpl_media_video_vpss_channel_read_stop(video_vpss);
    //break;  
    case ZPL_MEDIA_NODE_PROCESS: 
    ret = zpl_media_video_vpss_channel_read_stop(video_vpss);
    break;
    case ZPL_MEDIA_NODE_ENCODE:
    ret = zpl_media_video_encode_read_stop(video_encode);
    break;
    default:
    break;
    }
...
int zpl_media_hal_read_start(ZPL_MEDIA_NODE_E module, zpl_void *master, zpl_media_channel_t *chn)
{
    ...
    //if(!VIDHAL_RES_FLAG_CHECK(video_input_pipe->res_flag, SRCBIND))
        ret = zpl_meida_video_input_pipe_read_start(master, video_input_pipe);
    break;
    case ZPL_MEDIA_NODE_INPUT:
    if(video_input->hwbind == zpl_false)
    //if(!VIDHAL_RES_FLAG_CHECK(video_input->res_flag, SRCBIND))
        ret = zpl_meida_video_input_channel_read_start(master, video_input);
    break;
    //case ZPL_MEDIA_HARDADAP_VPSSGRP:  
    //ret = zpl_media_video_vpss_channel_read_start(master, video_vpss);
    //break;  
    case ZPL_MEDIA_NODE_PROCESS: 
    ...
    //if(video_encode->hwbind == zpl_false)
        ret = zpl_media_video_encode_read_start(master, video_encode);
    break;
    default:
    break;
    }
