...
static int zpl_media_channel_hwdestroy(zpl_media_channel_t *chn)
{
    if (chn)
    {
        ...
        if(chn->channel_type == ZPL_MEDIA_CHANNEL_FILE)
        {
            ...
            {
                zpl_media_file_destroy(chn->media_param.video_media.halparam);
                zpl_media_channel_extradata_delete(&chn->media_param.video_media.extradata);
            }
            if(chn->media_param.audio_media.enable && chn->media_param.audio_media.halparam)
            {
                zpl_media_file_destroy(chn->media_param.audio_media.halparam);
            }
        }
        else
        {
							 ...
							 ZPL_MEDIA_CHANNEL_INDEX_E channel_index, zpl_char *filename)
{
    ...
    if (chn)
    {
		...
		else
		{
            ...
            {
                chn->media_param.video_media.enable = zpl_true;
                chn->media_param.video_media.halparam = media_file;
                zpl_media_file_extradata(media_file, &chn->media_param.video_media.extradata);
            }
            else if(!media_file->b_video && media_file->b_audio)
            {
                chn->media_param.audio_media.enable = zpl_true;
                chn->media_param.audio_media.halparam = media_file;
            }
            else if(media_file->b_video && media_file->b_audio)
            {
                chn->media_param.video_media.enable = zpl_true;
            ...
            if(chn->media_param.video_media.enable)
            {
                zpl_media_file_codecdata(media_file, zpl_true, &chn->media_param.video_media.codec);
            }
            if(chn->media_param.audio_media.enable)
            {
                zpl_media_file_codecdata(media_file, zpl_false, &chn->media_param.audio_media.codec);
            }
		}
		 ...
		 chn != NULL; chn = (zpl_media_channel_t *)lstNext(&node))
	{
        ...
        if(filename)
        {
            if (chn && chn->channel_type == ZPL_MEDIA_CHANNEL_FILE)
            {
                if(chn->media_param.video_media.enable)
                {
                    media_file = chn->media_param.video_media.halparam;
                }
                else if(chn->media_param.audio_media.enable)// && chn->media_param.video_media.halparam
                {
                    media_file = chn->media_param.audio_media.halparam;
                }
                if(media_file)
                {
                    if(filename[0] == '/')
...
int zpl_media_channel_update_interval(zpl_int32 channel, ZPL_MEDIA_CHANNEL_INDEX_E channel_index, zpl_char *filename, int interval)
{
    ...
    if (chn)
	{
        ...
        {
            if(chn->media_param.video_media.enable)
                ret = zpl_media_file_interval(chn->media_param.video_media.halparam, interval);
            else if(chn->media_param.audio_media.enable)
                ret = zpl_media_file_interval(chn->media_param.audio_media.halparam, interval);
        }
		if (ZPL_MEDIA_DEBUG(CHANNEL, EVENT))
		{
...
int zpl_media_channel_start(zpl_int32 channel, ZPL_MEDIA_CHANNEL_INDEX_E channel_index, zpl_char *filename)
{
    ...
    if (chn)
	{
        ...
        {
            if(chn->media_param.video_media.enable)
                ret = zpl_media_file_play_start(chn->media_param.video_media.halparam, zpl_true);
            else if(chn->media_param.audio_media.enable)
                ret = zpl_media_file_play_start(chn->media_param.audio_media.halparam, zpl_true);
        }
		if (ZPL_MEDIA_DEBUG(CHANNEL, EVENT))
		{
...
int zpl_media_channel_stop(zpl_int32 channel, ZPL_MEDIA_CHANNEL_INDEX_E channel_index, zpl_char *filename)
{
    ...
    if (chn)
	{
        ...
        {
            if(chn->media_param.video_media.enable)
                ret = zpl_media_file_play_start(chn->media_param.video_media.halparam, zpl_false);
            else if(chn->media_param.audio_media.enable)
                ret = zpl_media_file_play_start(chn->media_param.audio_media.halparam, zpl_false);
        }
		if (ZPL_MEDIA_DEBUG(CHANNEL, EVENT))
		{
...
int zpl_media_channel_show(void *pvoid)
{
	...
	for (node = lstFirst(media_channel_list); node != NULL; node = lstNext(node))
	{
		...
		{
			if(chn->media_param.video_media.halparam)
			{
				zpl_media_file_t *fdesc = (zpl_media_file_t*)chn->media_param.video_media.halparam;
				...
				*/
				vty_out(vty, "%-4d  %-4d %-6d %-8d %dx%d%s", chn->channel, chn->channel_index, chn->channel_type, 
					chn->media_param.video_media.halparam?((zpl_media_file_t*)chn->media_param.video_media.halparam)->file_size:-1, 
