...
typedef struct 
{
...
#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
    zpl_video_vpssgrp_t *vpssgrp;   //绑定的分组
#else
    zpl_void            *video_input;   //绑定的输入
#endif
    ...
    zpl_uint32          dbg_recv_count;    
#endif
}zpl_video_vpss_channel_t;

int zpl_video_vpss_init(void);
int zpl_video_vpss_exit(void);

#ifdef ZPL_VIDEO_VPSSGRP_ENABLE
int zpl_video_vpssgrp_init(void);
int zpl_video_vpssgrp_exit(void);
...
int zpl_video_vpss_channel_count(zpl_int32 vpssgrp);
...
#endif
int zpl_video_vpss_channel_encode_set(zpl_int32 vpss_channel, void *encode);
