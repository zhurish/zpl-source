...
/* functions used by both sockets & files. */
#if 0
#define ipstack_ioctl(fd,request,argp)             ioctl((int)fd,request,argp)
#define ipstack_close(fd)                          close((int)fd)
#define ipstack_write(fd,buf,nbytes)               write((int)fd,(const void *)buf,(size_t)nbytes)
#define ipstack_writev(fd,iov,iovlen)              writev((int)fd,iov,iovlen)
#define ipstack_read(fd,buf,nbytes)                read((int)fd,buf,(size_t)nbytes)
#define ipstack_select(nfds,rf,wf,ef,t)            select(nfds,rf,wf,NULL,t)
#endif
 ...
 */
#if 0
#define ipstack_socket(domain,type,protocol)       socket(domain,type,protocol)
#define ipstack_shutdown(fd,how)                   shutdown((int)fd,how)
#define ipstack_bind(fd,addr,addrlen)              bind((int)fd,(const struct sockaddr *)addr,(socklen_t)addrlen)
#define ipstack_connect(fd,addr,addrlen)           connect((int)fd,(const struct sockaddr *)addr,(socklen_t)addrlen)
#define ipstack_accept(fd,addr,addrlenp)           accept((int)fd,(struct sockaddr *)addr,(socklen_t *)addrlenp)
#define ipstack_listen(fd,backlog)                 listen((int)fd,backlog)
#define ipstack_closesocket(fd)                    closesocket((int)fd)


#define ipstack_send(fd,msg,len,flags)             send((int)fd,(const void *)msg,(size_t)len,flags)
#define ipstack_sendto(fd,msg,len,flags,to,tolen)  sendto((int)fd,(const void *)msg,(size_t)len,flags,(const struct sockaddr *)to,(socklen_t)tolen)
#define ipstack_sendmsg(fd,msg,flags)              sendmsg((int)fd,(const struct Ip_msghdr *)msg,flags)

#define ipstack_recv(fd,buf,len,flags)             recv((int)fd,(void *)buf,(size_t)len,flags)
#define ipstack_recvfrom(fd,buf,len,flags,from,fp) recvfrom((int)fd,(void *)buf,(size_t)len,flags,(struct sockaddr *)from,(socklen_t *)fp)
#define ipstack_recvmsg(fd,msg,flags)              recvmsg((int)fd,(struct Ip_msghdr *)msg,flags)

#define ipstack_getsockopt(fd,level,optname,optval,optlenp)  getsockopt((int)fd,level,optname,optval,(socklen_t *)optlenp)
#define ipstack_setsockopt(fd,level,optname,optval,optlen)   setsockopt((int)fd,level,optname,(const void *)optval,(socklen_t)optlen)

#define ipstack_getsockname(fd,name,namelenp)         getsockname((int)fd,(struct sockaddr *)name,(socklen_t *)namelenp)
#define ipstack_getpeername(fd,name,namelenp)         getpeername((int)fd,(struct sockaddr *)name,(socklen_t *)namelenp)
#endif
